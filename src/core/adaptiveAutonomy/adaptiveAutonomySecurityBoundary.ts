...
export type AdaptiveAutonomyCheckpoint =
  | 'session_entry'
  | 'pre_health_evaluation'
  | 'pre_recovery'
  | 'pre_adaptation'
  | 'pre_delegation'
  | 'post_delegation'
...
export class AdaptiveAutonomySecurityBoundary {
   ...
   */
  public assertStopInactive(checkpoint: AdaptiveAutonomyCheckpoint, tenantId?: string, sessionId?: string): void {
    if (this.isEmergencyStopActive()) {
      throw new AdaptiveAutonomyEmergencyStopError(
        `EMERGENCY_STOP active at checkpoint '${checkpoint}' — immediate halt enforced`,
        tenantId,
        sessionId
      );
    }
    if (this.isUserStopActive()) {
      throw new AdaptiveAutonomyUserStopError(
        `USER_STOP active at checkpoint '${checkpoint}' — immediate halt enforced`,
        tenantId,
        sessionId
      );
    }
   ...
   */
  public assertTenantIsolation(requestedTenant: string, expectedTenant: string, sessionId?: string): void {
    if (!requestedTenant || !expectedTenant || requestedTenant.trim() !== expectedTenant.trim()) {
      throw new AdaptiveAutonomyTenantIsolationError(
   ...
   */
  public assertSessionIsolation(requestedSession: string, activeSession: string, tenantId?: string): void {
    if (!requestedSession || !activeSession || requestedSession.trim() !== activeSession.trim()) {
      throw new AdaptiveAutonomySessionIsolationError(
  ...
  public assertLeaseValidity(
    ...
    if (lease.tenantId !== expectedTenant) {
      throw new AdaptiveAutonomyLeaseError(
  ...
  public assertScopeBound(
    proposedOperations: readonly string[],
    ...
    for (const op of proposedOperations) {
      if (!authorizedSet.has(op)) {
        throw new AdaptiveAutonomyAuthorizationError(
