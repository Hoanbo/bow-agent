...
export class GovernedRecoveryManager {
  ...
  public getAttempts(): readonly RecoveryAttempt[] {
    return [...this.attempts];
  }

  public getCheckpoints(): readonly RecoveryCheckpoint[] {
   ...
   */
  public isRecoverable(failureClass: string): failureClass is RecoverableFailureClass {
    return this.policy.allowedFailureClasses.includes(failureClass as RecoverableFailureClass);
  }
   ...
   */
  public createCheckpoint(cycleNumber: number, generationNumber: number, stateSnapshot: Record<string, unknown>): RecoveryCheckpoint {
  ...
  public async executeRecovery(
    ...
    if (!this.isRecoverable(failureClass)) {
      throw new AdaptiveAutonomyRecoveryError(
        `Failure class '${failureClass}' is non-recoverable under active policy`,
        this.tenantId,
        this.sessionId
      );
    }
    ...
    if (currentAttemptCount > MAX_RECOVERY_ATTEMPTS) {
      throw new AdaptiveAutonomyRecoveryError(
        `Recovery attempt ${currentAttemptCount} exceeds hard ceiling of ${MAX_RECOVERY_ATTEMPTS} attempts`,
        this.tenantId,
        this.sessionId
      );
    }
