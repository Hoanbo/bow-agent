...
export interface CycleExecutionContext {
  ...
  readonly stepExecutor?: (cycle: number) => Promise<{ success: boolean; result?: unknown; error?: string; failureClass?: string }>;
}

export class GovernedAdaptiveAutonomyOrchestrator {
  private readonly validator: AdaptiveAutonomyValidator;
  private readonly healthEvaluator: OperationalHealthEvaluator;
  private readonly securityBoundary: AdaptiveAutonomySecurityBoundary;
  private readonly persistenceBridge: AdaptiveAutonomyAuditPersistenceBridge;
  ...
  public initializeSession(params: {
    ...
    const sessionPayload = {
      ...
      supervisionState: {
        state: 'AUTHORIZED' as AdaptiveAutonomyState,
        currentHealth: 'HEALTHY' as OperationalHealthState,
        activeGeneration: 1,
        currentCycle: 0,
        waitingForHumanReview: false,
      },
      budget: budgetManager.getSnapshot(),
      incidents: [] as readonly OperationalIncident[],
      healthEvaluations: [] as readonly HealthEvaluation[],
      recoveryAttempts: [] as readonly RecoveryAttempt[],
      adaptations: [] as readonly AdaptationDecision[],
      generations: [
        {
          generationIndex: 1,
          startedAt: now,
      ...
      ] as readonly RecoveryGeneration[],
    ...
    const session: AdaptiveAutonomySession = {
      ...sessionPayload,
      sessionHash,
    };
   ...
   */
  public async runSupervisedCycles(
    session: AdaptiveAutonomySession,
    cyclesToRun: number,
    executor?: (cycle: number) => Promise<{ success: boolean; result?: unknown; error?: string; failureClass?: string }>
  ): Promise<AdaptiveAutonomyResult> {
    let currentState: AdaptiveAutonomyState = session.supervisionState.state;
    let currentHealth: OperationalHealthState = session.supervisionState.currentHealth;
    ...
    const continuityManager = new ContinuityIntegrityManager(
      session.tenantId,
      session.sessionId,
      session.continuityRecord.snapshots
    );

    const recoveryManager = new GovernedRecoveryManager(
      session.tenantId,
      session.sessionId,
    ...
    const incidents: OperationalIncident[] = [...session.incidents];
    const evaluations: HealthEvaluation[] = [...session.healthEvaluations];
    ...
    for (let i = 0; i < boundCycles; i++) {
      ...
      let stepResult: { success: boolean; result?: unknown; error?: string; failureClass?: string } = { success: true };
      if (executor) {
        try {
        ...
        if (isRecoverable) {
          ...
          try {
            ...
            await recoveryManager.executeRecovery(
              incidentId,
              failureClass,
              { recoveryAction: 'RETRY_STEP_WITH_BACKOFF', targetParameters: {} },
              {
                cycleNumber: currentCycle,
    ...
    const updatedSession: AdaptiveAutonomySession = {
  ...
  public resumeSession(
    ...
    if (session.supervisionState.state !== 'SUSPENDED' && session.supervisionState.state !== 'AWAITING_HUMAN_REVIEW') {
      throw new AdaptiveAutonomyValidationError(
        `Session is in state '${session.supervisionState.state}', which cannot be resumed`,
        session.tenantId,
        session.sessionId
      );
    }
    ...
    const resumedSessionPayload = {
      ...
      supervisionState: {
        ...
        state: 'RESUMABLE' as AdaptiveAutonomyState,
        waitingForHumanReview: false,
      },
      updatedAt: Date.now(),
      version: session.version + 1,
    ...
    const resumedSession: AdaptiveAutonomySession = {
