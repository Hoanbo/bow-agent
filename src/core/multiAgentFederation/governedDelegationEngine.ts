...
export class GovernedDelegationEngine {
  ...
  private readonly activeDelegations: Map<string, GovernedDelegation> = new Map();
   ...
   */
  public createDelegation(params: CreateDelegationParams): GovernedDelegation {
    // 1. Checkpoint: PRE_DELEGATION_CREATION
    this.securityBoundary.assertStopInactive(
      'PRE_DELEGATION_CREATION',
      params.parentAgent.tenantId,
      params.missionId
    );

    // 2. Multi-tenant and session isolation
    this.securityBoundary.assertTenantIsolation(
      params.parentAgent.tenantId,
      params.delegateAgent.tenantId
    );
    this.securityBoundary.assertSessionIsolation(
      params.parentAgent.sessionId,
      params.delegateAgent.sessionId
    );
    ...
    if (this.activeDelegations.size >= MAX_DELEGATIONS_PER_FEDERATION) {
      throw new MultiAgentFederationBudgetError(
        `Reached MAX_DELEGATIONS_PER_FEDERATION (${MAX_DELEGATIONS_PER_FEDERATION})`,
        params.parentAgent.tenantId,
        undefined,
        params.parentAgent.agentId
      );
    }
    ...
    if (currentDepth > MAX_DELEGATION_DEPTH) {
      throw new MultiAgentFederationDelegationError(
        `Delegation depth ${currentDepth} exceeds MAX_DELEGATION_DEPTH (${MAX_DELEGATION_DEPTH})`,
        params.parentAgent.tenantId,
        undefined,
        params.delegateAgent.agentId
      );
    }

    // 5. Scope containment (delegate.scope subset of parent.scope)
    this.securityBoundary.assertScopeContainment(
      params.parentAgent.authorizationBinding.scope,
      params.scope,
      params.parentAgent.tenantId,
      params.delegationId
    );
    ...
    if (params.expiresAt > params.parentAgent.authorizationBinding.expiresAt) {
      throw new MultiAgentFederationDelegationError(
        `Delegation expiration (${params.expiresAt}) exceeds parent authorization expiration (${params.parentAgent.authorizationBinding.expiresAt})`,
        params.parentAgent.tenantId,
        undefined,
        params.delegateAgent.agentId
      );
    ...
    this.securityBoundary.assertValidLeaseBinding(
      params.parentAgent.leaseBinding,
      params.expiresAt,
      Date.now(),
      params.parentAgent.tenantId
    );
    ...
    const delegationBase: Omit<GovernedDelegation, 'provenanceHash'> = {
    ...
    const delegation: GovernedDelegation = {
      ...delegationBase,
      provenanceHash,
    };
    ...
    if (unresolvable.length > 0) {
      throw new MultiAgentFederationDelegationError(
        `Cannot create delegation due to unresolvable conflict: ${unresolvable[0].description}`,
        delegation.tenantId,
        undefined,
  ...
  public getDelegation(delegationId: string): GovernedDelegation | undefined {
    return this.activeDelegations.get(delegationId);
  }

  public getActiveDelegations(): readonly GovernedDelegation[] {
   ...
   */
  public updateDelegationStatus(
    delegationId: string,
    newStatus: GovernedDelegation['status']
  ): GovernedDelegation {
    ...
    const updatedBase: Omit<GovernedDelegation, 'provenanceHash'> = {
      ...d,
      status: newStatus,
    };
    const provenanceHash = computeDelegationBindingHash(updatedBase);
    const updated: GovernedDelegation = {
      ...updatedBase,
      provenanceHash,
    };
