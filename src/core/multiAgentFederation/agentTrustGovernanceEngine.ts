...
export class AgentTrustGovernanceEngine {
   ...
   */
  public evaluateTrust(input: TrustEvaluationInput): AgentTrustProfile {
    ...
    const historicalSuccessRate =
      input.historicalFailures === 0 ? 1.0 : Math.max(0, 1.0 - input.historicalFailures * 0.2);

    const isTrustedForHighRisk =
      identityScore >= 0.9 && governanceComplianceScore >= 0.8 && input.delegationDepth <= 2;
   ...
   */
  public assertTrustSufficiency(
    agent: GovernedAgent,
    requiredRiskTier: string,
    delegationDepth: number
  ): void {
    if (delegationDepth > 5) {
      throw new MultiAgentFederationTrustError(
        `Delegation depth ${delegationDepth} exceeds maximum allowable depth 5`,
        agent.tenantId,
        undefined,
        agent.agentId
      );
    ...
    if (agent.trustProfile.governanceComplianceScore < 0.5) {
      throw new MultiAgentFederationTrustError(
        `Agent '${agent.agentId}' governance compliance score (${agent.trustProfile.governanceComplianceScore}) is below acceptable threshold 0.5`,
        agent.tenantId,
        undefined,
        agent.agentId
      );
    ...
    if ((requiredRiskTier === 'HIGH' || requiredRiskTier === 'CRITICAL') && !agent.trustProfile.isTrustedForHighRisk) {
      throw new MultiAgentFederationTrustError(
        `Agent '${agent.agentId}' is not trusted for ${requiredRiskTier} risk tier delegations`,
        agent.tenantId,
        undefined,
        agent.agentId
      );
