...
import {
  ...
  type DeliberationContradiction,
...
import {
  globalSymbolicConstraintEngine,
  type SymbolicConstraintEngine,
} from './symbolicConstraintEngine.js';
...
export class DeliberationSearchEngine {
   ...
   */
  public searchAndRank(
    hypotheses: readonly DeliberationHypothesis[],
    ...
    if (hypotheses.length > DELIBERATION_BOUNDS.MAX_HYPOTHESES_PER_SESSION) {
      throw new DeliberationCapacityError(
        ...
        'hypotheses in search evaluation'
      );
    }

    const maxDepth = Math.min(
      options?.maxDepth ?? DELIBERATION_BOUNDS.MAX_INFERENCE_DEPTH,
      DELIBERATION_BOUNDS.MAX_INFERENCE_DEPTH
    );
    const convergenceThreshold = options?.convergenceThreshold ?? 0.65;

    const evaluated: DeliberationHypothesis[] = [];
    const allContradictions: DeliberationContradiction[] = [];
    ...
    for (const rawHypo of hypotheses) {
      ...
      const evalResult = this.constraintEngine.evaluateHypothesis(
        rawHypo,
        constraints,
        evidenceBindings
      );
      ...
      const supporting = evidenceBindings.filter((e) =>
        rawHypo.supportingEvidenceIds.includes(e.evidenceId)
      );
      const refuting = evidenceBindings.filter((e) =>
        rawHypo.refutingEvidenceIds.includes(e.evidenceId)
      );
      ...
      let combinedConfidence =
        CANONICAL_DELIBERATION_WEIGHTS.symbolicValidityWeight * validityScore +
      ...
      let nextStatus: HypothesisStatus = rawHypo.status;
