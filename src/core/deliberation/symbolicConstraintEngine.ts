...
export interface SymbolicConstraintEvaluationResult {
  ...
  readonly validityScore: number; // [0.0, 1.0]
...
export class SymbolicConstraintEngine {
  ...
  public createConstraint(input: {
    ...
    const validPolarities: SymbolicConstraintPolarity[] = ['MUST', 'MUST_NOT', 'PREFER'];
    ...
    if (forbiddenPatterns.some((pattern) => lower.includes(pattern))) {
      throw new DeliberationValidationError(
        'Dangerous execution tokens detected in constraint predicate',
        ['dynamic_code_execution_prohibited']
      );
    }
  ...
  public evaluateHypothesis(
    ...
    const contradictions: DeliberationContradiction[] = [];
    ...
    if (p.startsWith('requires:')) {
      ...
      if (req === 'verified_evidence' || req === 'verified') {
        const supporting = evidenceBindings.filter((e) =>
          hypothesis.supportingEvidenceIds.includes(e.evidenceId)
        );
