  ...
  public detectAndAnalyze(
    ...
    if (hypotheses.length > DELIBERATION_BOUNDS.MAX_HYPOTHESES_PER_SESSION) {
      throw new DeliberationCapacityError(
        ...
        'hypotheses in contradiction analysis'
      );
    }
    ...
    // Bounded check
    const boundedContradictions = contradictions.slice(
      0,
      DELIBERATION_BOUNDS.MAX_CONTRADICTIONS_PER_SESSION
    );
  ...
  private detectConstraintContradictions(
    ...
    for (let i = 0; i < active.length; i++) {
      for (let j = i + 1; j < active.length; j++) {
        ...
        if (
          (c1.polarity === 'MUST' && c2.polarity === 'MUST_NOT' && c1.predicate === c2.predicate) ||
  ...
  private detectLogicalContradictions(
    ...
    for (let i = 0; i < hypotheses.length; i++) {
      for (let j = i + 1; j < hypotheses.length; j++) {
        ...
        if (
          (t1.includes('enable') && t2.includes('disable') && t1.replace('enable', '') === t2.replace('disable', '')) ||
