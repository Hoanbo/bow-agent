...
export function createNetworkHeartbeatSignal(params: {
  ...
  const connectionId = validateNetworkIdentifier(
    'networkConnectionId',
  ...
  const signal: NetworkHeartbeatSignal = {
    heartbeatId: `hb_${fp}`,
    networkConnectionId: connectionId,
    sequence: params.sequence,
    timestamp: ts,
 ...
 */
export function evaluateNetworkHeartbeat(
  signal: Readonly<NetworkHeartbeatSignal>,
  ack: Readonly<NetworkHeartbeatAck>,
  maxAllowedRttMs = 15000,
): Readonly<HeartbeatEvaluationResult> {
