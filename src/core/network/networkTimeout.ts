...
export type NetworkTimeoutType =
  | 'CONNECT_TIMEOUT'
  | 'FRAME_TIMEOUT'
  | 'HEARTBEAT_TIMEOUT'
  | 'RESPONSE_TIMEOUT'
...
export const DEFAULT_NETWORK_TIMEOUT_CONFIG: Readonly<NetworkTimeoutConfig> = Object.freeze({
...
export function evaluateNetworkTimeout(params: {
  ...
  const cfg: NetworkTimeoutConfig = {
    ...DEFAULT_NETWORK_TIMEOUT_CONFIG,
    ...params.config,
  };

  let allowedMs = cfg.frameTimeoutMs;
  let failureCode: NetworkFailureCode = 'FRAME_TIMEOUT';
