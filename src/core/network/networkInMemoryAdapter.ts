...
export class NetworkInMemoryAdapter implements NetworkAdapter {
  public readonly adapterId: string;
  public readonly adapterType: NetworkAdapterType = 'IN_MEMORY';

  private state: NetworkAdapterState = 'READY';
  private readonly codec: NetworkCodec = new NetworkCodec();
  private readonly maxQueueDepth: number;
  private readonly connections: Map<string, Readonly<NetworkConnectionSnapshot>> = new Map();
  private readonly frameQueues: Map<string, NetworkFrame[]> = new Map();
  ...
  public getState(): NetworkAdapterState {
    return this.state;
  }
  ...
  public async close(connectionId: string, reason?: string): Promise<void> {
  ...
  public getHealth(): 'HEALTHY' | 'DEGRADED' | 'UNRESPONSIVE' {
    if (this.state === 'FAILED') return 'UNRESPONSIVE';
    if (this.state === 'PAUSED') return 'DEGRADED';
    return 'HEALTHY';
  }

  public getMetrics(): Readonly<NetworkAdapterMetrics> {
    return deepFreeze({
