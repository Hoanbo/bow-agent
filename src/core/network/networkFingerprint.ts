 ...
 */
export function computeNetworkScopeKey(scope: ScopedNetworkIdentity): string {
...
export function computeNetworkFrameFingerprint(params: {
  ...
  return fnv1aHex(
    `NET_FRAME::${params.networkConnectionId}::${params.sequence}::${params.direction}::${params.messageType}::${params.payloadChecksum}`,
  );
}
 ...
 */
export function computePayloadChecksum(payload: Readonly<Record<string, unknown>>): string {
...
export function computeNetworkHeartbeatFingerprint(params: {
  ...
  return fnv1aHex(
    `NET_HB::${params.networkConnectionId}::${params.sequence}::${params.timestamp}`,
  );
}
...
export function computeNetworkAuditFingerprint(params: {
  ...
  return fnv1aHex(
    `NET_AUDIT::${params.eventType}::${params.networkConnectionId ?? 'NONE'}::${params.sequence ?? 0}::${params.outcome}::${params.timestamp}`,
  );
}
...
export function computeNetworkFailureFingerprint(params: {
  ...
  return fnv1aHex(
    `NET_FAIL::${params.code}::${params.networkConnectionId ?? 'NONE'}::${params.message}::${params.timestamp}`,
  );
}
