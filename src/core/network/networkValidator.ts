...
export const WINDOWS_RESERVED_DEVICE_NAMES: ReadonlySet<string> = Object.freeze(
  new Set<string>([
    'CON',
    'PRN',
    'AUX',
...
export const PROTOTYPE_POLLUTION_KEYS: ReadonlySet<string> = Object.freeze(
 ...
 */
export function validateNetworkIdentifier(name: string, value: string): string {
  ...
  if (value.length > MAX_NETWORK_IDENTIFIER_LENGTH) {
    throw new Error(
      `[NETWORK_SECURITY_ERROR] Identifier "${name}" exceeds max length of ${MAX_NETWORK_IDENTIFIER_LENGTH} characters.`,
    );
  }
  ...
  if (PROTOTYPE_POLLUTION_KEYS.has(value.toLowerCase())) {
    throw new Error(
      `[NETWORK_SECURITY_ERROR] Identifier "${name}" attempts prototype pollution ("${value}").`,
    );
  }

  if (value.includes('..') || value.includes('/') || value.includes('\\')) {
    throw new Error(
      `[NETWORK_SECURITY_ERROR] Identifier "${name}" contains path traversal elements.`,
    );
  }
  ...
  if (WINDOWS_RESERVED_DEVICE_NAMES.has(upper)) {
    throw new Error(
      `[NETWORK_SECURITY_ERROR] Identifier "${name}" matches Windows reserved device name "${value}".`,
    );
  }
 ...
 */
export function validateScopedNetworkIdentity(
  scope: ScopedNetworkIdentity,
): Readonly<ScopedNetworkIdentity & { scopeKey: string }> {
  ...
  const cleanScope: ScopedNetworkIdentity = {
    userId,
    sessionId,
    brainId,
    surfaceId,
 ...
 */
export function validateNetworkPayloadBounds(
  payload: unknown,
  currentDepth = 0,
): void {
  if (currentDepth > MAX_NETWORK_NESTING_DEPTH) {
    throw new Error(
      `[NETWORK_SECURITY_ERROR] Payload nesting depth exceeds maximum of ${MAX_NETWORK_NESTING_DEPTH} levels.`,
    );
  }
  ...
  if (serialized && serialized.length > MAX_NETWORK_PAYLOAD_SIZE_BYTES) {
    throw new Error(
      `[NETWORK_SECURITY_ERROR] Payload size (${serialized.length} bytes) exceeds limit of ${MAX_NETWORK_PAYLOAD_SIZE_BYTES} bytes.`,
    );
  }
  ...
  for (const key of Object.keys(payload as Record<string, unknown>)) {
    if (PROTOTYPE_POLLUTION_KEYS.has(key.toLowerCase())) {
      throw new Error(
        `[NETWORK_SECURITY_ERROR] Payload key "${key}" violates prototype pollution boundary.`,
      );
    }
 ...
 */
export function redactNetworkSecrets(text: string): string {
