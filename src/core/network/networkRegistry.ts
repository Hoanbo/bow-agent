...
export class NetworkRegistry {
  ...
  private readonly connections: Map<string, Readonly<NetworkConnectionSnapshot>> = new Map();
  private readonly connectionsByScopeKey: Map<string, string> = new Map();
  private readonly surfaceToConnection: Map<string, string> = new Map();

  /**
   * EN: Registers an adapter instance.
   * VI: Đăng ký một thực thể adapter.
   */
  public registerAdapter(adapter: NetworkAdapter): void {
   ...
   */
  public getAdapter(adapterId: string): NetworkAdapter | undefined {
   ...
   */
  public registerConnection(snapshot: Readonly<NetworkConnectionSnapshot>): void {
    ...
    if (this.connections.has(snapshot.networkConnectionId)) {
      ...
      if (existing.scopeKey !== validScope.scopeKey) {
        throw new Error(
          `[NETWORK_REGISTRY_ERROR] Cross-scope collision detected for connection "${snapshot.networkConnectionId}".`,
        );
      }
    }
  ...
  public getConnection(
    connectionId: string,
    requestingScope?: ScopedNetworkIdentity,
  ): Readonly<NetworkConnectionSnapshot> | undefined {
    ...
    if (requestingScope) {
      ...
      if (conn.scopeKey !== validScope.scopeKey) {
        throw new Error(
          `[NETWORK_REGISTRY_SECURITY_VIOLATION] Cross-scope access denied. Connection belongs to "${conn.scopeKey}", requested by "${validScope.scopeKey}".`,
        );
      }
    }
   ...
   */
  public getConnectionBySurface(
    surfaceId: string,
    requestingUserId?: string,
    requestingBrainId?: string,
  ): Readonly<NetworkConnectionSnapshot> | undefined {
    ...
    if (requestingUserId && conn.scope.userId !== requestingUserId) {
      throw new Error(
        `[NETWORK_REGISTRY_SECURITY_VIOLATION] User isolation: surface "${surfaceId}" is bound to user "${conn.scope.userId}", requested by "${requestingUserId}".`,
      );
    }

    if (requestingBrainId && conn.scope.brainId !== requestingBrainId) {
      throw new Error(
        `[NETWORK_REGISTRY_SECURITY_VIOLATION] Brain isolation: surface "${surfaceId}" is bound to brain "${conn.scope.brainId}", requested by "${requestingBrainId}".`,
      );
    }
   ...
   */
  public listAdapters(): readonly NetworkAdapter[] {
