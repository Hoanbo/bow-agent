 ...
 */
export function createNetworkConnectionSnapshot(
  ...
  const snapshotFp = fnv1aHex(
    `${connectionId}::${validScope.scopeKey}::${params.adapterType}::${state}::${initialSeq}`,
  );

  const snapshot: NetworkConnectionSnapshot = {
 ...
 */
export function updateNetworkConnectionSnapshot(
  current: Readonly<NetworkConnectionSnapshot>,
  updates: {
    readonly state?: NetworkConnectionState;
    readonly lastSentSequence?: number;
    readonly lastReceivedSequence?: number;
  ...
  const snapshotFp = fnv1aHex(
    `${current.networkConnectionId}::${current.scopeKey}::${current.adapterType}::${nextState}::${sentSeq}::${recvSeq}::${reconnectCount}`,
  );

  const updated: NetworkConnectionSnapshot = {
