...
export class Phase14ExitBoundaryGate {
  private readonly _authority?: MasterHumanAuthority;

  constructor(options?: Phase14BoundaryGateOptions) {
    this._authority = options?.authority;
  }

  private isUserStopActive(): boolean {
    if (this._authority) {
  ...
  private getUserStopReason(): string {
    if (this._authority) {
  ...
  private emitAbortedAudit(checkpoint: string, tenantId: string, reason: string): void {
    try {
   ...
   */
  public validateIdentifier(id: string, paramName: string): void {
   ...
   */
  public assertCheckpoint1_AuditInit(tenantId: string): void {
   ...
   */
  public assertCheckpoint2_EvidenceCollection(tenantId: string): void {
    this.validateIdentifier(tenantId, 'tenantId');
    if (this.isUserStopActive()) {
      const reason = this.getUserStopReason();
   ...
   */
  public assertCheckpoint3_EvidenceReconciliation(tenantId: string): void {
    this.validateIdentifier(tenantId, 'tenantId');
    if (this.isUserStopActive()) {
      const reason = this.getUserStopReason();
   ...
   */
  public assertCheckpoint4_CriteriaEvaluation(criterionId: Phase14ExitCriterionId, tenantId: string): void {
    this.validateIdentifier(tenantId, 'tenantId');
    if (this.isUserStopActive()) {
      const reason = this.getUserStopReason();
      this.emitAbortedAudit(`CP4_EVALUATION_${criterionId}`, tenantId, reason);
      throw new Phase14AuditAbortedError(
        `Preempted at Checkpoint 4 (Criterion ${criterionId} Evaluation): ${reason}`
      );
    }
  }
   ...
   */
  public assertCheckpoint5_CertificateConstruction(auditId: string, tenantId: string): void {
   ...
   */
  public assertCheckpoint6_CertificateSealing(auditId: string, tenantId: string): void {
    this.validateIdentifier(auditId, 'auditId');
    this.validateIdentifier(tenantId, 'tenantId');
    if (this.isUserStopActive()) {
      const reason = this.getUserStopReason();
   ...
   */
  public assertCheckpoint7_CertificateExport(auditId: string, tenantId: string): void {
    this.validateIdentifier(auditId, 'auditId');
    this.validateIdentifier(tenantId, 'tenantId');
    if (this.isUserStopActive()) {
      const reason = this.getUserStopReason();
