...
export interface ReconciliationInput {
  ...
  readonly independentEvidenceOverrides?: Partial<
    Record<
      Phase14ExitCriterionId,
      { status: Phase14CriterionAuditStatus; score: number; notes: string }
    >
  >;
}
...
export class Phase14EvidenceReconciler {
  ...
  public reconcile(input: ReconciliationInput): ReconciliationOutcome {
    this._gate.assertCheckpoint3_EvidenceReconciliation(input.tenantId);

    const results: Phase14CriterionAuditResult[] = [];
    const contradictions: Phase14ContradictionFinding[] = [];
    const nowIso = new Date().toISOString();

    const allCriterionIds: Phase14ExitCriterionId[] = [
      'CRIT-1.4-01',
      'CRIT-1.4-02',
      'CRIT-1.4-03',
      'CRIT-1.4-04',
    ...
    const evidenceBySource = new Map<string, Phase14EvidenceItem>();
    ...
    for (const critId of allCriterionIds) {
      ...
      let status: Phase14CriterionAuditStatus = 'PASS';
      let score = 1.0;
        ...
        switch (critId) {
          ...
          case 'CRIT-1.4-02': { // Zero Tool Execution Without PDP Clearance
            if (auditEvidence) {
              ...
              const unauth = (auditEvidence.data.unauthorizedToolsCount as number) || 0;
              if (unauth > 0) {
                status = 'FAIL';
                score = 0.0;
          ...
          case 'CRIT-1.4-05': { // Real Tool Execution Isolation
            if (sanityEvidence) {
              ...
              const effective = sanityEvidence.data.sanitizationEffective as boolean;
              if (!effective) {
                status = 'FAIL';
                score = 0.0;
          ...
          case 'CRIT-1.4-11': { // Full Regression Integrity
            if (regEvidence) {
              ...
              const satisfied = regEvidence.data.baselineSatisfied as boolean;
              const registered = (regEvidence.data.totalRegisteredSuites as number) || 0;
          ...
          case 'CRIT-1.4-12': { // Protected Workspace Untouched C:\BOW\shopofbow
            if (fsEvidence) {
              ...
              const exists = fsEvidence.data.exists as boolean;
              if (exists) {
                status = 'FAIL';
                score = 0.0;
      ...
      if (input.readinessReport) {
        const reportEval = input.readinessReport.criteriaEvaluations.find(
          e => e.criterionId === critId
        );
        if (reportEval) {
