...
export class ProductionAgentLoopFacade {
  private readonly composer: AgentLoopSubsystemComposer;
  private readonly gate: AgentLoopExecutionGate;
   ...
   */
  private validateRequest(request: unknown): asserts request is AgentLoopRequest {
    if (!request || typeof request !== 'object') {
      throw new AgentLoopValidationError('AgentLoopRequest must be a non-null object');
    }

    const req = request as AgentLoopRequest;
   ...
   */
  private recordAudit(
    eventType: AgentLoopAuditEventType,
    tenantId: string,
   ...
   */
  public async executeLoop(request: AgentLoopRequest): Promise<AgentLoopResult> {
   ...
   */
  public async runLoop(request: AgentLoopRequest): Promise<AgentLoopResult> {
    ...
    const stepExecutions: AgentLoopStepExecution[] = [];
    const completedStepIds: string[] = [];

    let finalState: AgentLoopState = 'IDLE';
    let outcomeStatus: AgentLoopOutcomeStatus = 'FAILED';
    let failureReason: string | undefined = undefined;
    let pendingApprovalStepId: string | undefined = undefined;
    ...
    try {
      ...
      if (task.tenantId !== request.tenantId) {
        throw new AgentLoopSecurityViolationError(
          `Tenant isolation mismatch: request tenant '${request.tenantId}' does not match task tenant '${task.tenantId}'`
        );
      }
      if (task.taskId !== request.taskId) {
        throw new AgentLoopSecurityViolationError(
          `Task binding mismatch: request task '${request.taskId}' does not match retrieved task '${task.taskId}'`
        );
      }
      ...
      while (hasMoreSteps) {
        ...
        let handoff: AuthorizedActionHandoff;
        try {
          ...
          handoff = proposalOutcome as AuthorizedActionHandoff;
