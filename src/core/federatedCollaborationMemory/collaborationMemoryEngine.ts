...
export class CollaborationMemoryEngine {
  ...
  private readonly entries: Map<string, CollaborationMemoryEntry> = new Map();
   ...
   */
  public sanitizeContent(content: string): void {
    ...
    for (const marker of cotMarkers) {
      if (content.includes(marker)) {
        throw new FederatedCollaborationMemoryValidationError(
          `Deliberation or CoT marker '${marker}' forbidden in collaboration memory`
        );
      }
    }
    ...
    for (const pattern of injectionPatterns) {
      if (pattern.test(content)) {
        throw new FederatedCollaborationMemoryValidationError(
          `Prompt injection pattern forbidden in collaboration memory: "${pattern.source}"`
        );
      }
    }
    ...
    for (const sp of secretPatterns) {
      if (sp.test(content)) {
        throw new FederatedCollaborationMemoryValidationError(
          'Credential, private key, or secret pattern detected in collaboration memory'
        );
      }
    }
  }
   ...
   */
  public writeMemory(params: WriteMemoryParams): CollaborationMemoryEntry {
    ...
    const fedEntries = Array.from(this.entries.values()).filter(
      (e) => e.tenantId === params.tenantId && e.federationId === params.federationId && e.expiresAt > Date.now()
    );
    if (!this.entries.has(params.memoryId) && fedEntries.length >= MAX_MEMORY_ENTRIES_PER_FEDERATION) {
      throw new FederatedCollaborationMemoryBudgetError(
        `Federation reached MAX_MEMORY_ENTRIES_PER_FEDERATION limit (${MAX_MEMORY_ENTRIES_PER_FEDERATION})`,
        params.tenantId,
        params.federationId
      );
    }
    ...
    if (!this.entries.has(params.memoryId) && agentEntries.length >= MAX_MEMORY_ENTRIES_PER_AGENT) {
      throw new FederatedCollaborationMemoryBudgetError(
        `Agent '${params.agentId}' reached MAX_MEMORY_ENTRIES_PER_AGENT limit (${MAX_MEMORY_ENTRIES_PER_AGENT})`,
        params.tenantId,
        params.federationId
      );
    }
    ...
    if (existing) {
      ...
      if (existing.tenantId !== params.tenantId) {
        throw new FederatedCollaborationMemoryTenantIsolationError(
          `Cross-tenant overwrite attempt: '${existing.tenantId}' != '${params.tenantId}'`,
          params.tenantId
        );
      }
      if (existing.sessionId !== params.sessionId) {
        throw new FederatedCollaborationMemorySessionIsolationError(
          `Cross-session overwrite attempt: '${existing.sessionId}' != '${params.sessionId}'`,
          params.tenantId
        );
      }
    ...
    const now = Date.now();
    const base: Omit<CollaborationMemoryEntry, 'provenanceHash'> = {
    ...
    const entry: CollaborationMemoryEntry = {
      ...base,
      provenanceHash,
    };
  ...
  public readMemory(
    ...
    if (entry.tenantId !== tenantId) {
      throw new FederatedCollaborationMemoryTenantIsolationError(
        `Cross-tenant read rejected: entry tenant '${entry.tenantId}' != caller '${tenantId}'`,
        tenantId
      );
    }
    ...
    if (entry.sessionId !== sessionId) {
      throw new FederatedCollaborationMemorySessionIsolationError(
        `Cross-session read rejected: entry session '${entry.sessionId}' != caller '${sessionId}'`,
        tenantId
      );
    }
    ...
    if (entry.federationId !== federationId) {
      throw new FederatedCollaborationMemoryValidationError(
        `Cross-federation read rejected: entry federation '${entry.federationId}' != caller '${federationId}'`
      );
    }
   ...
   */
  public queryActiveMemories(tenantId: string, sessionId: string, federationId: string): readonly CollaborationMemoryEntry[] {
    const now = Date.now();
