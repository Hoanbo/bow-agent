...
export interface ConsensusSession {
  ...
  readonly quorumThreshold: number; // e.g. 0.5 for simple majority, 0.67 for supermajority
...
export class FederatedConsensusEngine {
  ...
  private readonly sessions: Map<string, ConsensusSession> = new Map();
  private readonly results: Map<string, ConsensusResult> = new Map();
   ...
   */
  public createConsensusSession(
    consensusId: string,
    proposalParams: CreateProposalParams,
    participatingAgentIds: readonly string[],
    authorizationBinding: CollaborationAuthorizationBinding,
    leaseBinding?: CollaborationLeaseBinding,
    quorumThreshold = 0.5
  ): ConsensusSession {
    ...
    const activeSessions = Array.from(this.sessions.values()).filter(
      (s) =>
        s.sessionId === proposalParams.sessionId &&
        s.status !== 'COMPLETED' &&
        s.status !== 'FAILED' &&
    ...
    if (activeSessions.length >= MAX_ACTIVE_CONSENSUS_SESSIONS) {
      throw new FederatedCollaborationMemoryBudgetError(
        `Session reached MAX_ACTIVE_CONSENSUS_SESSIONS limit (${MAX_ACTIVE_CONSENSUS_SESSIONS})`,
        proposalParams.tenantId,
        proposalParams.contextId
      );
    }
    ...
    if (participatingAgentIds.length === 0 || participatingAgentIds.length > MAX_CONSENSUS_PARTICIPANTS) {
      throw new FederatedCollaborationMemoryBudgetError(
        `Participant count must be between 1 and ${MAX_CONSENSUS_PARTICIPANTS}`,
        proposalParams.tenantId,
        proposalParams.contextId
      );
    }
    ...
    if (!participatingAgentIds.includes(proposalParams.proposingAgentId)) {
      throw new FederatedCollaborationMemoryValidationError(
        `Proposing agent '${proposalParams.proposingAgentId}' is not among authorized participants`
      );
    }
    ...
    const propBase: Omit<ConsensusProposal, 'provenanceHash'> = {
   ...
   */
  public castVote(consensusId: string, voteParams: RecordVoteParams): ConsensusVote {
    ...
    if (session.status !== 'READY' && session.status !== 'VOTING' && session.status !== 'PROPOSING') {
      throw new FederatedCollaborationMemoryValidationError(
        `Cannot cast vote in session status '${session.status}'`
      );
    }
    ...
    if (!session.participatingAgentIds.includes(voteParams.agentId)) {
      throw new FederatedCollaborationMemoryValidationError(
        `Agent '${voteParams.agentId}' is not an authorized participant in consensus '${consensusId}'`
      );
    }
    ...
    const now = Date.now();
    const signatureHash = computeSha256(
      `vote:${session.consensusId}:${voteParams.agentId}:${voteParams.decision}:${now}`
    );
   ...
   */
  public finalizeConsensus(consensusId: string): ConsensusResult {
    const session = this.sessions.get(consensusId);
    if (!session) {
      throw new FederatedCollaborationMemoryValidationError(`Consensus session '${consensusId}' not found`);
    }
    ...
    if (session.roundsConsumed >= MAX_CONSENSUS_ROUNDS) {
      session.status = 'FAILED';
      throw new FederatedCollaborationMemoryBudgetError(
        `Consensus exceeded MAX_CONSENSUS_ROUNDS (${MAX_CONSENSUS_ROUNDS})`,
        session.tenantId,
        session.contextId
      );
    }
    ...
    let finalStatus: ConsensusStatus;
    ...
    const resBase: Omit<ConsensusResult, 'provenanceHash'> = {
   ...
   */
  public getConsensusResult(consensusId: string): ConsensusResult | undefined {
   ...
   */
  public getSession(consensusId: string): ConsensusSession | undefined {
