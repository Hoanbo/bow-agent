...
export interface LexicalCandidate {
  ...
  readonly lexicalScore: number; // Raw or normalized score >= 0.0
}
...
export class HybridMemoryRetrievalEngine {
  private readonly defaultLexicalWeight = 0.4;
  private readonly defaultSemanticWeight = 0.6;
  ...
  public async search(
    ...
    lexicalRetriever: (queryText: string, tenantId: string) => Promise<readonly LexicalCandidate[]>,
    signal?: AbortSignal
  ): Promise<{
    ...
    let lexicalCandidates: readonly LexicalCandidate[] = [];
    try {
    ...
    let semanticCandidates: ReturnType<typeof index.search> = [];
    ...
    const normalizedLexicalMap = new Map<string, { candidate: LexicalCandidate; score: number }>();
