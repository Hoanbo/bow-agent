...
// ============================================================================
// 1. DETERMINISTIC LOCAL EMBEDDING PROVIDER (NODE-NATIVE, ZERO DEPENDENCY)
// ============================================================================
 ...
 */
export class DeterministicLocalEmbeddingProvider implements LocalEmbeddingProvider {
  public readonly providerId = 'deterministic-local-embedder';
  ...
  public async embed(text: string, signal?: AbortSignal): Promise<EmbeddingVectorDescriptor> {
    if (signal?.aborted) {
    ...
    const descriptor: EmbeddingVectorDescriptor = Object.freeze({
  ...
  public async healthCheck(signal?: AbortSignal): Promise<EmbeddingProviderHealth> {
   ...
   */
  private generateDeterministicVector(text: string): readonly number[] {
...
// ============================================================================
// 2. OLLAMA LOCAL EMBEDDING PROVIDER (CONNECTS TO LOCAL HOST ONLY)
// ============================================================================
...
export class OllamaEmbeddingProvider implements LocalEmbeddingProvider {
  public readonly providerId = 'ollama-local-embedder';
  public readonly baseUrl: string;
  public readonly modelId: string;
  public readonly modelVersion = 'ollama-native';
  public dimensions = 768; // Dynamically adapted upon first valid embedding
    ...
    try {
      ...
      if (!isLoopback) {
        throw new SemanticMemorySecurityError(
          `Security violation: OllamaEmbeddingProvider must only connect to local loopback daemon. External host '${host}' is strictly prohibited.`,
          { host, url: rawUrl }
        );
      }
    } catch (err: any) {
      if (err instanceof SemanticMemorySecurityError) throw err;
