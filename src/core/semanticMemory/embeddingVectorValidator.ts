...
export class EmbeddingVectorValidator {
   ...
   */
  public static validateRawVector(values: readonly number[], expectedDimension?: number): void {
    ...
    if (values.length < MIN_VECTOR_DIMENSIONS || values.length > MAX_VECTOR_DIMENSIONS) {
      throw new EmbeddingValidationError(
        `Vector length ${values.length} violates bounds [${MIN_VECTOR_DIMENSIONS}, ${MAX_VECTOR_DIMENSIONS}]`,
    ...
    if (expectedDimension !== undefined && values.length !== expectedDimension) {
      throw new EmbeddingValidationError(
        `Vector dimension mismatch: expected ${expectedDimension} but found ${values.length}`,
        ['DIMENSION_MISMATCH']
      );
    }
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      if (typeof v !== 'number' || !Number.isFinite(v) || Number.isNaN(v)) {
        throw new EmbeddingValidationError(
          `Vector contains invalid numeric element at index ${i}: ${v}`,
          ['NON_FINITE_ELEMENT']
        );
      }
    }
   ...
   */
  public static validateDescriptor(descriptor: EmbeddingVectorDescriptor): void {
   ...
   */
  public static assertCompatibleVectorSpaces(
    a: EmbeddingVectorDescriptor,
    b: EmbeddingVectorDescriptor
  ): void {
