 ...
 */
export function resolveUserPartition(userId: string, baseDir: string): UserPartition {
  ...
  if (ANONYMOUS_IDENTIFIERS.includes(trimmed.toLowerCase())) {
    throw new DurablePersistenceSecurityError(
      'Anonymous or unresolved user cannot access durable memory partition'
    );
  }
  ...
  if (
    ...
    throw new DurablePersistenceSecurityError(
      `Path traversal or illegal separator detected in userId: "${userId}"`
    );
  }
  ...
  if (WINDOWS_RESERVED_NAMES.test(trimmed)) {
    throw new DurablePersistenceSecurityError(
      `Windows reserved device name detected in userId: "${userId}"`
    );
  }
  ...
  if (relative.startsWith('..') || path.isAbsolute(relative) || relative.includes(path.sep + '..')) {
    throw new DurablePersistenceSecurityError(
      `Security violation: resolved partition path escapes baseDir`
    );
  }
