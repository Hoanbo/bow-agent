...
export class DurableJsonStore<T> {
  ...
  public readonly allowedBaseDir?: string;
  private readonly validator: (data: unknown) => ValidationResult<T>;
  private readonly defaultFactory: () => T;
  private readonly maxRetries: number;
  private readonly quarantineCorrupted: boolean;
  ...
  constructor(options: DurableJsonStoreOptions<T>) {
    ...
    if (options.allowedBaseDir) {
      ...
      if (relative.startsWith('..') || path.isAbsolute(relative)) {
        throw new DurablePersistenceSecurityError(
          `Path traversal blocked: target file path is outside allowedBaseDir`
        );
      }
    }
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (this.allowedBaseDir) {
          ...
          if (relative.startsWith('..') || path.isAbsolute(relative)) {
            throw new DurablePersistenceSecurityError(
              'Symbolic link points outside allowed base directory'
            );
          }
        }
      }
    } catch (err: any) {
      if (err instanceof DurablePersistenceSecurityError) throw err;
   ...
   */
  private quarantineFile(): string | null {
    if (!this.quarantineCorrupted) return null;
    try {
  ...
  public read(): T {
    ...
    if (!fs.existsSync(this.filePath)) {
      ...
      if (!validation.success || validation.data === undefined) {
        throw new DurablePersistenceSchemaError(
          'Default factory produced invalid data schema',
          validation.errors
        );
      }
    ...
    try {
      ...
      throw new DurablePersistenceCorruptionError(
        `Durable JSON file is malformed and cannot be parsed${qMsg}: ${err.message}`,
        err
      );
    }
    ...
    if (!validation.success || validation.data === undefined) {
      ...
      const qMsg = qPath ? ` (Quarantined to ${path.basename(qPath)})` : '';
      throw new DurablePersistenceSchemaError(
        `Durable JSON data failed runtime schema validation${qMsg}`,
        validation.errors
      );
    }
  ...
  public write(data: T): void {
    ...
    if (!validation.success || validation.data === undefined) {
      throw new DurablePersistenceSchemaError(
        'Cannot persist data: payload failed runtime schema validation',
        validation.errors
      );
    }
    ...
    if (!success) {
      ...
      throw new DurablePersistenceError(
        `Atomic replace failed after ${this.maxRetries} retries: ${lastError?.message || 'unknown error'}`,
        lastError
      );
    }
  }
   ...
   */
  public update(mutator: (current: T) => T): T {
