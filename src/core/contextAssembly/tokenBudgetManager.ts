...
export class TokenBudgetManager {
  ...
  constructor(options?: ContextBudgetOptions) {
    ...
    if (promptLimit > HARD_MAX_PROMPT_TOKENS) {
      throw new ContextBudgetExceededError(
        promptLimit,
        HARD_MAX_PROMPT_TOKENS,
        `Requested maxPromptTokens (${promptLimit}) cannot exceed hard maximum of ${HARD_MAX_PROMPT_TOKENS}`
      );
    }
    if (promptLimit <= 0) {
  ...
  public getBudget(): NormalizedBudget {
    return {
   ...
   */
  public estimateFragmentsTotalTokens(fragments: readonly ContextFragment[]): number {
   ...
   */
  public validateTier1Budget(fragments: readonly ContextFragment[]): void {
    ...
    if (tier1Tokens > this.maxPromptTokens) {
      throw new ContextBudgetExceededError(
        ...
        tier1Tokens,
        { reason: 'Tier 1 Critical safety and task metadata exceeds total prompt budget' }
      );
    }
  }
   ...
   */
  public exceedsBudget(fragments: readonly ContextFragment[]): boolean {
