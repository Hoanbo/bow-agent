...
export class ContextSourceResolver {
   ...
   */
  public async resolveSources(request: ContextAssemblyRequest): Promise<{
    task?: AgentTask;
  ...
  private resolveTask(
    ...
    try {
      ...
      if (err.name === 'TaskNotFoundError') {
        throw new ContextSourceUnavailableError(
          `AgentTask:${taskId}`,
          `Task '${taskId}' not found in tenant '${tenantId}'`
        );
      }
      throw err;
    }
    ...
    if (!task.taskId || !task.state || typeof task.version !== 'number') {
      throw new ContextSourceCorruptedError(
        `AgentTask:${taskId}`,
        'Task missing essential schema properties'
      );
    }
  ...
  private resolveTier1Fragments(
    ...
    const systemSafety =
      request.promptInputs.systemPrompt?.trim() ||
    ...
    const policyConstraints =
      request.promptInputs.policyConstraints?.trim() ||
