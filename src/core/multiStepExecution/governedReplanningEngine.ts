...
export class GovernedReplanningEngine {
   ...
   */
  public createReplanningRequest(params: CreateReplanningRequestParams): ReplanningRequest {
    const totalReplans = params.totalPreviousReplans ?? 0;
    if (totalReplans >= MAX_REPLANS_PER_SESSION) {
      throw new MultiStepExecutionReplanningError(
        `Replanning limit exceeded: session already underwent ${totalReplans} replans (max ${MAX_REPLANS_PER_SESSION})`
      );
    }
    ...
    const sealedRequest: ReplanningRequest = Object.freeze({
      ...rawRequest,
      provenanceHash,
    });
