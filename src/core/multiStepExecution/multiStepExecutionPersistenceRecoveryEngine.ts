...
export class MultiStepExecutionPersistenceRecoveryEngine {
   ...
   */
  public loadSessionDocument(tenantId: string, sessionId: string): MultiStepExecutionSessionDocument {
    ...
    if (fs.existsSync(canonicalPath)) {
      try {
        ...
        const doc: MultiStepExecutionSessionDocument = JSON.parse(raw);
    ...
    throw new MultiStepExecutionPersistenceError(
      `No valid session document found for tenant "${tenantId}" and session "${sessionId}"`
    );
  }
  ...
  public saveSessionDocument(
    doc: MultiStepExecutionSessionDocument,
    ...
    if (fs.existsSync(canonicalPath)) {
      try {
        ...
        const existingDoc: MultiStepExecutionSessionDocument = JSON.parse(existingRaw);
        if (expectedVersion !== undefined && existingDoc.sessionVersion !== expectedVersion) {
          throw new MultiStepExecutionConcurrencyError(
            `OCC CAS conflict saving session "${session.sessionId}": expected version ${expectedVersion}, but found ${existingDoc.sessionVersion}`
          );
        }
      } catch (err: any) {
   ...
   */
  public initializeSession(session: MultiStepExecutionSession): MultiStepExecutionSessionDocument {
    this.securityBoundary.assertUserStop('pre_persistence');
    ...
    const sealedDoc: MultiStepExecutionSessionDocument = Object.freeze({
      ...rawDoc,
      provenanceHash,
    });
