...
export class ExecutionStepScheduler {
   ...
   */
  public static evaluateStepReadiness(generation: MultiStepExecutionGeneration): SchedulerEvaluationResult {
    ...
    if (allStepList.length > MAX_EXECUTION_STEPS) {
      throw new MultiStepExecutionValidationError(
        `Step count exceeds MAX_EXECUTION_STEPS limit of ${MAX_EXECUTION_STEPS}`
      );
    }

    const readySteps: MultiStepExecutionStepState[] = [];
    const blockedSteps: MultiStepExecutionStepState[] = [];
    const completedSteps: MultiStepExecutionStepState[] = [];
    const failedSteps: MultiStepExecutionStepState[] = [];
    const invalidatedSteps: MultiStepExecutionStepState[] = [];

    // Track statuses of all steps
    const stepStatusMap = new Map<string, StepExecutionStatus>();
   ...
   */
  public static computeCascadingInvalidations(
    failedStepId: string,
    steps: Readonly<Record<string, MultiStepExecutionStepState>>
  ): string[] {
    ...
    const queue: string[] = [failedStepId];
   ...
   */
  public static getTopologicalOrder(generation: MultiStepExecutionGeneration): string[] {
