...
export class ExecutionRequestValidator {
   ...
   */
  public static validateRequest(request: unknown): asserts request is ExecutionRequest {
    ...
    if (Buffer.byteLength(serialized, 'utf8') > MAX_EXECUTION_PAYLOAD_BYTES) {
      throw new ExecutionValidationError(
        `Execution request exceeds maximum allowed size of ${MAX_EXECUTION_PAYLOAD_BYTES} bytes`
      );
    }
   ...
   */
  public static validateOperation(operation: unknown): asserts operation is ExecutionOperation {
    ...
    const candidate = operation as Partial<ExecutionOperation>;
   ...
   */
  public static validateLease(lease: unknown): asserts lease is ExecutionLease {
    ...
    const candidate = lease as Partial<ExecutionLease>;
   ...
   */
  public static validateAuthorization(auth: unknown): asserts auth is ExecutionAuthorizationEnvelope {
    ...
    const candidate = auth as Partial<ExecutionAuthorizationEnvelope>;
   ...
   */
  public static assertNoPrototypePollution(target: unknown): void {
   ...
   */
  public static assertNoCoTArtifacts(target: unknown): void {
    if (!target) return;
   ...
   */
  public static assertNoPromptInjection(target: unknown): void {
    if (!target) return;
