 ...
 */
export class SafeInspectionAdapter implements GovernedExecutionAdapter {
  public readonly adapterId = 'safe_inspection_adapter';
  public readonly supportedKinds: readonly ExecutionOperationKind[] = ['INSPECT_ELEMENT', 'READ_STATE', 'VERIFY_ASSERTION'];

  public async execute(context: ExecutionContext): Promise<Readonly<Record<string, unknown>>> {
 ...
 */
export class DeterministicActuationAdapter implements GovernedExecutionAdapter {
  public readonly adapterId = 'deterministic_actuation_adapter';
  public readonly supportedKinds: readonly ExecutionOperationKind[] = ['SIMULATE_INTERACTION', 'EXECUTE_GOVERNED_ACTION', 'CUSTOM_REGISTERED'];
...
export class ExecutionBoundaryGate {
  private readonly adapters = new Map<ExecutionOperationKind, GovernedExecutionAdapter>();
   ...
   */
  public registerAdapter(adapter: GovernedExecutionAdapter): void {
   ...
   */
  public async executeThroughBoundary(
    ...
    if (!adapter) {
      throw new ExecutionAdapterError(
        `No registered execution adapter found for operation kind "${request.operation.kind}"`
      );
    }
    ...
    try {
      ...
      throw new ExecutionAdapterError(
        `Adapter "${adapter.adapterId}" failed during execution: ${err?.message || 'Unknown error'}`,
        { error: String(err) }
      );
    } finally {
      if (timeoutHandle) {
