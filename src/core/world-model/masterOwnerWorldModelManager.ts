...
export class MasterOwnerWorldModelManager {
  ...
  private _currentSnapshot: MasterOwnerWorldModelSnapshot;
  private readonly _stalenessThresholdMs: number;
  ...
  private _createInitialSnapshot(): MasterOwnerWorldModelSnapshot {
    const now = Date.now();
    ...
    const initialProjects: Record<string, ProjectModel> = {
      personal_core: {
    ...
    for (const [projId, desc] of Object.entries(REGISTERED_PROJECTS)) {
      if (projId !== 'personal_core') {
        const rel: 'PERSONAL' | 'OPTIONAL_SURFACE' | 'FUTURE_APPLICATION' =
          desc.relationship === 'OPTIONAL_SURFACE' ? 'OPTIONAL_SURFACE' : 'FUTURE_APPLICATION';
  ...
  public getSnapshot(): MasterOwnerWorldModelSnapshot {
  ...
  public registerProject(project: ProjectModel): void {
    this._currentSnapshot = {
      ...this._currentSnapshot,
      projects: {
  ...
  public getProjects(): Record<string, ProjectModel> {
    return { ...this._currentSnapshot.projects };
  }

  public refreshObservations(): void {
    const now = Date.now();
    const host = globalHostDiscovery.discoverHost();
    const caps = globalCapabilityDiscoveryBridge
      .discoverAllCapabilities(host)
  ...
  public addVerifiedOutcome(outcome: VerifiedOutcomeRecord): void {
  ...
  public setContradictions(contradictions: WorldModelContradiction[]): void {
    this._currentSnapshot = {
      ...this._currentSnapshot,
  ...
  public rehydrate(): boolean {
    try {
      ...
      const parsed: MasterOwnerWorldModelSnapshot = JSON.parse(raw);
