...
export class CapabilityGroundedReasoningEngine {
   ...
   */
  public evaluateCapabilityStage(
    capabilityId: string,
    host: HostEnvironment,
    isDiscovered: boolean,
    isAvailableOnHost: boolean,
    isGovernedPermitted: boolean,
   ...
   */
  public evaluatePlanFeasibility(
    ...
    const requiredAuthorizations: string[] = [];
    ...
    for (const reqCap of spec.requiredCapabilities) {
      const isAvailable =
        availableCapIds.includes(reqCap) ||
        availableCapIds.includes(`cap_${reqCap}`) ||
        availableCapIds.some((id: string) => id.endsWith(reqCap));
