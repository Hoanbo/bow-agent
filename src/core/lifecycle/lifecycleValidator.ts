 ...
 */
export function redactLifecycleSecrets(text: string): string {
 ...
 */
export function containsLifecycleSecret(val: unknown): boolean {
 ...
 */
export function hasLifecyclePrototypePollution(target: unknown): boolean {
  ...
  for (const [k, v] of Object.entries(target)) {
    ...
    if (v && typeof v === 'object' && hasLifecyclePrototypePollution(v)) return true;
  }
 ...
 */
export function validateScope(userId: unknown, sessionId: unknown): { valid: boolean; error?: string } {
 ...
 */
export function validateSafeMetadata(metadata: unknown): { valid: boolean; error?: string } {
 ...
 */
export function assertRiskPreservation(
  previousRisk?: PlanRiskLevel | string,
  newRisk?: PlanRiskLevel | string,
): void {
  if (!previousRisk || !newRisk) return;
  ...
  if (prevScore > 0 && newScore > 0 && newScore < prevScore) {
    throw new Error(
      `RISK_DOWNGRADE_FORBIDDEN: Cannot downgrade risk from "${previousRisk}" to "${newRisk}" (INV-STATE-07).`,
    );
  }
}
 ...
 */
export function assertGovernancePreservation(
  prevGov?: boolean,
  newGov?: boolean,
  prevAppr?: boolean,
  newAppr?: boolean,
