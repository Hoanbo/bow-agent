...
export const VALID_ACTION_TRANSITIONS: Readonly<Record<ActionLifecycleState, readonly ActionLifecycleState[]>> = {
...
export function isValidActionTransition(from: ActionLifecycleState, to: ActionLifecycleState): boolean {
  if (from === to) return true;
  const allowed = VALID_ACTION_TRANSITIONS[from];
  return allowed ? allowed.includes(to) : false;
}

export function assertValidActionTransition(from: ActionLifecycleState, to: ActionLifecycleState, actionId?: string): void {
  if (!isValidActionTransition(from, to)) {
    throw new Error(
      `INVALID_WORLD_ACTION_TRANSITION: Action ${actionId ?? 'unknown'} cannot transition from "${from}" to "${to}".`
    );
  }
}
