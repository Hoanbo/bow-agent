...
export class WorldActionCommitEngine {
   ...
   */
  public commit(action: WorldAction): WorldAction {
    if (action.lifecycleState !== 'VERIFYING' && action.lifecycleState !== 'EXECUTING') {
      throw new WorldActionError(
        'COMMIT_FAILURE',
        `Cannot commit action in lifecycle state "${action.lifecycleState}". Must be VERIFYING.`,
        action.actionId
      );
    }
    ...
    if (action.verificationState !== 'VERIFIED') {
      throw new WorldActionError(
        'COMMIT_FAILURE',
        `Cannot commit action: verificationState is "${action.verificationState}". Action is not verified.`,
        action.actionId
      );
    }
   ...
   */
  public async rollback(action: WorldAction): Promise<ActionRollbackResult> {
