...
export function validateAndResolvePath(targetPath: string): string {
  ...
  if (!normalizedLower.startsWith(EXECUTION_WORKSPACE)) {
    throw new WorldActionError(
      'SECURITY_VIOLATION',
      `Target "${resolved}" is outside the execution workspace. Access denied.`,
      undefined,
      targetPath
    );
  }
...
export class WorldActionExecutor {
  ...
  public static async executeFsWrite(action: WorldAction): Promise<ActionExecutionResult> {
  ...
  public static async executeFsRead(action: WorldAction): Promise<ActionExecutionResult> {
    const startedAt = Date.now();
    const executionId = generateExecutionId();
    const resolved = validateAndResolvePath(action.target);
  ...
  public static async executeFsAppend(action: WorldAction): Promise<ActionExecutionResult> {
    const startedAt = Date.now();
    const executionId = generateExecutionId();
  ...
  public static async executeFsMkdir(action: WorldAction): Promise<ActionExecutionResult> {
    const startedAt = Date.now();
    const executionId = generateExecutionId();
  ...
  public static async executeFsRename(action: WorldAction): Promise<ActionExecutionResult> {
    const startedAt = Date.now();
    const executionId = generateExecutionId();
  ...
  public static async executeFsCopy(action: WorldAction): Promise<ActionExecutionResult> {
    const startedAt = Date.now();
    const executionId = generateExecutionId();
  ...
  public static async executeFsMove(action: WorldAction): Promise<ActionExecutionResult> {
    // Move is atomic rename if on same volume, or copy + unlink
    return this.executeFsRename(action);
  }

  public static async executeFsDelete(action: WorldAction): Promise<ActionExecutionResult> {
    const startedAt = Date.now();
    const executionId = generateExecutionId();
  ...
  public static async executeProcessList(action: WorldAction): Promise<ActionExecutionResult> {
    const startedAt = Date.now();
    const executionId = generateExecutionId();

    // Enumerate active processes known to host and runtime
    const processes: Array<{ pid: number; memoryUsage: NodeJS.MemoryUsage; uptime: number }> = [
      {
        pid: process.pid,
  ...
  public static async executeProcessInspect(action: WorldAction): Promise<ActionExecutionResult> {
    const startedAt = Date.now();
    const executionId = generateExecutionId();
  ...
  public static async executeProcessExists(action: WorldAction): Promise<ActionExecutionResult> {
  ...
  public static async executeProcessStart(action: WorldAction): Promise<ActionExecutionResult> {
    const startedAt = Date.now();
    const executionId = generateExecutionId();
    ...
    if (!command.endsWith('node.exe') && !command.endsWith('node') && command !== process.execPath) {
      throw new WorldActionError(
        'SECURITY_VIOLATION',
        `Unapproved executable "${command}". Only verified node runtime is allowlisted.`,
        action.actionId
      );
    }
    ...
    child.on('exit', () => {
      if (child.pid) activeGovernedProcesses.delete(child.pid);
    });
  ...
  public static async executeProcessStop(action: WorldAction): Promise<ActionExecutionResult> {
    const startedAt = Date.now();
    const executionId = generateExecutionId();
    ...
    if (targetPid === process.pid) {
      throw new WorldActionError(
        'SECURITY_VIOLATION',
        'Cannot terminate the host Agent process self.',
        action.actionId
      );
    }
    ...
    if (governed) {
      ...
      try {
        ...
        throw new WorldActionError(
          'EXECUTION_FAILURE',
          `Failed to terminate PID ${targetPid}: ${err.message}`,
          action.actionId
        );
      }
    }
  ...
  public static async executeAllowlistedCommand(action: WorldAction): Promise<ActionExecutionResult> {
    const startedAt = Date.now();
    const executionId = generateExecutionId();
    ...
    if (!ALLOWLISTED_BINARIES.has(binaryBase) && !ALLOWLISTED_BINARIES.has(command)) {
      throw new WorldActionError(
        'SECURITY_VIOLATION',
        `Command "${command}" is not in the governed allowlist. Arbitrary execution is strictly forbidden.`,
        action.actionId
      );
    }
  ...
  public static getGovernedProcessCount(): number {
    return activeGovernedProcesses.size;
  }

  public static clearAllGovernedProcesses(): void {
