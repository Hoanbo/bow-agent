 ...
 */
export function validateRemoteIdentifier(name: string, value: unknown): string {
 ...
 */
export function validateRemoteScope(params: ScopedRemoteIdentity): {
 ...
 */
export function containsRemoteSecret(input: string): boolean {
 ...
 */
export function redactRemoteSecrets(input: string): string {
  let result = input;
 ...
 */
export function validateRemoteMetadataDepth(obj: unknown, currentDepth = 1): void {
  ...
  if (obj && typeof obj === 'object') {
    for (const key of Object.keys(obj as Record<string, unknown>)) {
      ...
      validateRemoteMetadataDepth((obj as Record<string, unknown>)[key], currentDepth + 1);
    }
  }
}
 ...
 */
export function validateRemotePayloadBounds(payload: unknown): void {
  if (payload !== undefined && payload !== null) {
    validateRemoteMetadataDepth(payload);
    const serialized = JSON.stringify(payload);
    if (serialized.length > MAX_REMOTE_PAYLOAD_BYTES) {
      throw new Error(
        `[REMOTE_RESOURCE_ERROR] Payload size (${serialized.length} bytes) exceeds maximum limit (${MAX_REMOTE_PAYLOAD_BYTES} bytes).`,
      );
    }
  }
}
 ...
 */
export function assertRemoteRiskPreservation(
  ...
  if (newRank < priorRank) {
    throw new Error(
      `[REMOTE_RISK_DOWNGRADE_ERROR] Monotonic risk violation: cannot downgrade risk from ${priorRisk} to ${newRisk}.`,
    );
  }
}
