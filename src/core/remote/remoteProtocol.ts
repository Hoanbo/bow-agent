...
export const CANONICAL_REMOTE_PROTOCOL: Readonly<RemoteProtocolDescriptor> = deepFreeze({
...
export function negotiateProtocolVersion(requestedVersion: string): string {
  if (!isProtocolVersionSupported(requestedVersion)) {
    throw new Error(
      `[REMOTE_PROTOCOL_MISMATCH] Unsupported protocol version "${requestedVersion}". Supported versions: ${Array.from(SUPPORTED_REMOTE_PROTOCOL_VERSIONS).join(', ')}.`,
    );
  }
