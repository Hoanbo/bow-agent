...
export class ExecutiveRecoveryCoordinator {
   ...
   */
  public assessFailure(task: ExecutiveTask, goalId: GoalId): RecoveryAssessment {
   ...
   */
  public shouldRetry(task: ExecutiveTask): boolean {
    const maxAttempts = task.retryPolicy?.maxAttempts ?? 3;
   ...
   */
  public calculateBackoff(task: ExecutiveTask): number {
    const policy = task.retryPolicy as any;
   ...
   */
  public async prepareRecovery(task: ExecutiveTask): Promise<boolean> {
    if (globalExecutiveCancellation.isUserStopActive) {
      return false;
    }
    ...
    if (task.recoveryPolicy.supervisorSupervised && !globalSupervisorRuntime.isSafeStopActive()) {
      const anomaly = {
        ...
        type: 'WORLDACTION_EXECUTION_FAILURE' as const,
