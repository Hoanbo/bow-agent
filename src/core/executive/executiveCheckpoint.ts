...
export class ExecutiveCheckpointManager {
  private _maxCheckpointAgeMs = 24 * 60 * 60 * 1000; // 24 hours

  /**
   * Creates a tamper-evident checkpoint for a goal and its associated tasks.
   */
  public createCheckpoint(
    goalOrGoalId: ExecutiveGoal | GoalId,
    maybeTasks?: ExecutiveTask[],
    maybeEscalations?: ExecutiveEscalationRecord[]
  ): ExecutiveCheckpoint {
    let goal: ExecutiveGoal;
    let tasks: ExecutiveTask[];
    let escalations: ExecutiveEscalationRecord[];
    ...
    const finalCheckpoint: ExecutiveCheckpoint = {
      ...baseCheckpoint,
   ...
   */
  public restoreFromCheckpoint(checkpoint: ExecutiveCheckpoint): boolean {
  ...
  public validateCheckpoint(
    ...
    try {
      ...
      for (const task of checkpoint.tasks) graph.addTask(task);
      const graphValidation = graph.validateGraph();
      if (!graphValidation.valid) return { valid: false, error: graphValidation.errors.join('; ') };
    } catch (error) {
    ...
    if (age > this._maxCheckpointAgeMs) {
      return {
        valid: false,
        error: `Checkpoint is stale (age ${Math.round(age / 1000)}s exceeds limit of ${Math.round(
          this._maxCheckpointAgeMs / 1000
        )}s)`,
      };
    }
