...
import type { PostconditionResult } from './postconditionTypes.js';
...
} from './verificationFingerprint.js';
import {
...
export class VerificationService {
  private readonly historyStore = new Map<string, VerificationResult[]>();
   ...
   */
  public verify(request: VerificationRequest): Readonly<VerificationResult> {
    ...
    if (hasVerificationPrototypePollution(request)) {
      const failureFingerprint = computeVerificationFailureFingerprint(
        userId,
        sessionId,
        'SECURITY_FAILURE',
        'Prototype pollution detected in verification request',
      );
    ...
    const combinedEvidence: VerificationEvidence[] = [
      ...derivedEvidence,
    ...
    const postconditionResults: PostconditionResult[] = [];
    let passedCount = 0;
    let failedCount = 0;
    let unknownCount = 0;
    ...
    for (const postcondition of postconditions) {
      ...
      if (result.status === 'PASSED') {
        ...
        failedCount++;
        if (result.required) requiredFailed = true;
        if (result.priority === 'CRITICAL') criticalFailed = true;
    ...
    let recommendation: VerificationRecommendation = 'NONE';
    ...
    // CASE 5: All required postconditions passed
      ...
      if (rawConfidence < 0.5) rawConfidence = 0.85;
    }
    // Default fallback
    else {
      status = 'UNKNOWN';
    ...
    const fingerprint = computeVerificationFingerprint(
      userId,
      sessionId,
      toolName,
      status,
   ...
   */
  public getHistory(userId: string, sessionId: string): readonly VerificationResult[] {
    validateVerificationScope(userId, sessionId);
    const partitionKey = this.getPartitionKey(userId, sessionId);
