 ...
 */
export function redactVerificationSecrets(text: string): string {
 ...
 */
export function containsVerificationSecret(val: unknown): boolean {
 ...
 */
export function hasVerificationPrototypePollution(target: unknown): boolean {
  ...
  for (const [k, v] of Object.entries(target)) {
    ...
    if (v && typeof v === 'object' && hasVerificationPrototypePollution(v)) return true;
  }
...
export function validateSafePath(path: string): boolean {
  ...
  for (const seg of segments) {
    ...
    if (WINDOWS_RESERVED_DEVICE_NAMES.has(trimmed)) return false;
    if (PROTOTYPE_POLLUTION_REGEX.test(seg)) return false;
  }
 ...
 */
export function validateVerificationScope(userId: string, sessionId: string): void {
 ...
 */
export function assertVerificationRiskPreservation(
  ...
  if (currentRank < priorRank) {
    throw new Error(
      `RISK_DOWNGRADE_FORBIDDEN: Cannot downgrade verification risk from ${priorRisk} to ${currentRisk}`,
    );
  }
}
