...
export class PolicyActiveRuntimeSyncEngine {
  ...
  private readonly activeSnapshots: Map<string, RuntimePolicySnapshot> = new Map();
   ...
   */
  public syncTenantActivePolicy(tenantPartition: string): ActiveRuntimeSyncResult {
    this.assertUserStopInactive();

    const startTime = Date.now();
    const syncId = createActiveRuntimeSyncId(
      `arsync_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`
    );
    ...
    const freshness = this.freshnessValidator.validateFreshness(
      activeState,
      cleanTenant,
      currentSnapshot?.policyVersion
    );

    if (!freshness.isValid) {
      const state: ActiveRuntimeSyncState =
   ...
   */
  public getCachedSnapshot(tenantPartition: string): RuntimePolicySnapshot | null {
    this.assertUserStopInactive();
   ...
   */
  private createRuntimeSnapshot(state: ActivePolicyState): RuntimePolicySnapshot {
    const snapshotId = createRuntimePolicySnapshotId(
      `rpsnap_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`
    );

    const modifications = state.activeModifications ?? {};
    const toolClassifications: Record<string, ActionClassification> = {
      ...(modifications.toolClassifications || modifications.toolOverrides || {}),
    };
    const guardrailParameters: Record<string, any> = {
