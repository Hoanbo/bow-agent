...
export class PolicyActiveRuntimeProvenanceEngine {
  ...
  private readonly tenantChains: Map<string, ActiveRuntimeProvenanceRecord[]> = new Map();
  ...
  public appendRecord(params: {
    ...
    const provenanceId = createActiveRuntimeProvenanceId(
      `arprov_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`
    );
    ...
    const recordHash = crypto
      .createHash('sha256')
      .update(
        `${provenanceId}:${cleanTenant}:${params.activePolicyStateId}:${params.snapshotId ?? ''}:${params.enforcementId ?? ''}:${params.eventType}:${timestamp}:${previousHash}:${payloadHash}`
      )
      .digest('hex');

    const record: ActiveRuntimeProvenanceRecord = Object.freeze({
      provenanceId,
  ...
  public verifyChainIntegrity(tenantPartition: string): boolean {
    ...
    for (let i = 0; i < chain.length; i++) {
      ...
      const recomputedHash = crypto
        .createHash('sha256')
        .update(
          `${rec.provenanceId}:${rec.tenantPartition}:${rec.activePolicyStateId}:${rec.snapshotId ?? ''}:${rec.enforcementId ?? ''}:${rec.eventType}:${rec.timestamp}:${rec.previousHash}:${rec.payloadHash}`
        )
        .digest('hex');
  ...
  public getChain(tenantPartition: string): readonly ActiveRuntimeProvenanceRecord[] {
