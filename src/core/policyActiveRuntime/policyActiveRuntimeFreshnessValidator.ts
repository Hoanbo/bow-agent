...
export class PolicyActiveRuntimeFreshnessValidator {
   ...
   */
  public validateFreshness(
    activeState: ActivePolicyState | null | undefined,
    ...
    if (issues.length > 0) {
      ...
      const status: RuntimePolicyFreshnessStatus = isMismatch
        ? 'MISMATCH'
        : isStale
        ? 'STALE'
        : 'CORRUPTED';
   ...
   */
  private isVersionStale(incoming: string, known: string): boolean {
