// src/core/recovery/recoveryTypes.ts
// BOWCON V4.0 — MILESTONE 1.3.16: BRAIN RECOVERY & CRASH CONSISTENCY TYPES
//
// EN:
// Authoritative type definitions for Brain Recovery & Crash Consistency Engine.
// Establishes immutable contracts for crash conditions, interrupted operation classifications,
// recovery decisions, journal records, and reconstructed states.
//
// VI:
// Các định nghĩa kiểu dữ liệu có thẩm quyền cho Động cơ Phục hồi Não bộ & Tính Nhất quán Sự cố.
// Thiết lập các hợp đồng bất biến cho điều kiện sự cố, phân loại thao tác bị gián đoạn,
// quyết định phục hồi, bản ghi nhật ký và trạng thái được tái thiết lập.
 ...
 */
export type CrashConsistencyCondition =
  | 'NO_ACTIVE_OPERATION'
  | 'INTERRUPTED_BEFORE_EXECUTION'
  | 'INTERRUPTED_DURING_EXECUTION'
  | 'INTERRUPTED_DURING_VERIFICATION'
  | 'INTERRUPTED_DURING_COMMIT'
  | 'COMMIT_CONFIRMED_BEFORE_CRASH'
  | 'UNKNOWN_DURABLE_STATE'
  | 'CONFLICTING_DURABLE_STATE';

/**
 * EN: Classification of an interrupted operation.
 * VI: Phân loại thao tác bị gián đoạn.
 */
export type InterruptedOperationClassification =
  | 'NO_OPERATION'
  | 'SAFE_TO_RESUME'
  | 'RESUME_REQUIRES_GOVERNANCE'
  | 'RESUME_REQUIRES_VERIFICATION'
  | 'RESUME_BLOCKED'
 ...
 */
export type ApprovalRecoveryStatus =
  | 'APPROVAL_UNKNOWN'
  | 'APPROVAL_PENDING'
  | 'APPROVAL_VALID_BUT_NOT_CONSUMED'
  | 'APPROVAL_ALREADY_CONSUMED'
  | 'APPROVAL_INVALID'
  | 'APPROVAL_EXPIRED_OR_UNAVAILABLE';
...
export type RecoveryFailureCategory =
  ...
  | 'RECOVERY_SEQUENCE_FAILURE'
  | 'RECOVERY_COMMIT_CONFLICT'
  | 'RECOVERY_VERIFICATION_CONFLICT'
  | 'RECOVERY_GOVERNANCE_CONFLICT'
  | 'RECOVERY_RISK_MISMATCH'
  | 'RECOVERY_APPROVAL_CONFLICT'
