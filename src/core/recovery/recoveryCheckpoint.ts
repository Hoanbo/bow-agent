...
import type { LifecycleCheckpoint, LifecycleState } from '../lifecycle/lifecycleTypes.js';
...
const DURABLE_CLEAN_STATES: ReadonlySet<LifecycleState> = Object.freeze(
 ...
 */
export function resolveLastKnownGoodState(
  userId: string,
  sessionId: string,
  checkpoints?: readonly LifecycleCheckpoint[],
  commitRecord?: { commitId: string; status: string; isPartial?: boolean },
  verificationRecord?: { verificationId: string; status: string; taskSucceeded: boolean },
  ...
  let selectedCheckpoint: LifecycleCheckpoint | null = null;
  ...
  const fingerprint = computeLastKnownGoodStateFingerprint(
    userId,
    sessionId,
    selectedCheckpoint.checkpointId,
    selectedCheckpoint.sequence,
    selectedCheckpoint.state,
  );

  const lkgs: LastKnownGoodState = {
