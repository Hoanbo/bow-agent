// src/core/recovery/recoveryValidator.ts
// BOWCON V4.0 — MILESTONE 1.3.16: RECOVERY SECURITY & INVARIANT VALIDATOR
//
// EN:
// Authoritative security and invariant validator for Brain Recovery & Crash Consistency.
// Enforces multi-tenant session isolation, secret scrubbing, prototype pollution defense,
// null byte defense, path traversal defense, Windows reserved device checks, and monotonic risk preservation.
//
// VI:
// Bộ xác thực an ninh và bất biến có thẩm quyền cho Phục hồi Não bộ & Tính Nhất quán Sự cố.
// Thực thi cô lập phiên multi-tenant, tẩy sạch bí mật, phòng thủ prototype pollution,
// null byte, duyệt đường dẫn (path traversal), tên thiết bị Windows dè dặt, và bảo toàn rủi ro đơn điệu.
 ...
 */
export function redactRecoverySecrets(text: string): string {
 ...
 */
export function containsRecoverySecret(val: unknown): boolean {
 ...
 */
export function hasRecoveryPrototypePollution(target: unknown): boolean {
  ...
  for (const [k, v] of Object.entries(target)) {
    ...
    if (v && typeof v === 'object' && hasRecoveryPrototypePollution(v)) return true;
  }
 ...
 */
export function validateRecoveryScope(userId: string, sessionId: string): void {
  ...
  if (userId.includes('\0') || sessionId.includes('\0')) {
    throw new Error('NULL_BYTE_DETECTED: Null bytes in recovery scope identity are strictly forbidden');
  }

  if (PROTOTYPE_POLLUTION_REGEX.test(userId) || PROTOTYPE_POLLUTION_REGEX.test(sessionId)) {
    throw new Error('PROTOTYPE_POLLUTION: Recovery scope identity contains disallowed prototype pollution keys');
  }
  ...
  if (WINDOWS_RESERVED_DEVICE_NAMES.has(userUpper) || WINDOWS_RESERVED_DEVICE_NAMES.has(sessionUpper)) {
    throw new Error('RESERVED_DEVICE_NAME: Recovery scope identity contains reserved Windows device name');
  }

  if (userId.includes('..') || sessionId.includes('..')) {
    throw new Error('PATH_TRAVERSAL: Recovery scope identity contains disallowed path traversal characters');
  }

  if (userId.includes('/') || userId.includes('\\') || sessionId.includes('/') || sessionId.includes('\\')) {
    throw new Error('PATH_TRAVERSAL: Recovery scope identity cannot contain path separators');
  }
}
 ...
 */
export function assertRecoveryRiskPreservation(
  ...
  if (currentRank < priorRank) {
    throw new Error(
      `RISK_DOWNGRADE_FORBIDDEN: Cannot downgrade recovery risk from ${priorRisk} to ${currentRisk}`,
    );
  }
}
