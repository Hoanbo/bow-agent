...
export class RecoveryService {
  private readonly journals = new Map<string, RecoveryJournal>();
  private readonly lastResults = new Map<string, RecoveryResult>();

  private getPartitionKey(userId: string, sessionId: string): string {
    return `${userId}::${sessionId}`;
  }

  private getOrCreateJournal(userId: string, sessionId: string): RecoveryJournal {
   ...
   */
  public async recover(request: RecoveryRequest): Promise<RecoveryResult> {
    ...
    const recoveryId = computeRecoveryId(
      userId,
      sessionId,
      primaryCheckpoint?.checkpointId,
      request.requestId,
    );

    let currentState: RecoveryState = 'RECOVERY_REQUIRED';
    ...
    try {
      ...
      const consistency = evaluateCrashConsistency(
        request.activeLifecycleState,
        request.checkpoints,
        request.verificationRecord,
        request.commitRecord,
      );
      ...
      const approvalStatus = evaluateApprovalRecoveryStatus(
        request.approvalMetadata,
        request.activeLifecycleState,
      );

      const classification = classifyInterruptedOperation(
        consistency.condition,
        request.activeLifecycleState,
        request.risk || 'LOW',
        approvalStatus,
        request.commitRecord?.isPartial,
      );
      ...
      const decision = determineRecoveryDecision(
        classification,
        approvalStatus,
        request.risk || 'LOW',
      );
      ...
      if (request.risk && lkgs?.metadata?.risk) {
        assertRecoveryRiskPreservation(lkgs.metadata.risk as any, request.risk);
      }
      ...
      const reconstructedFingerprint = computeReconstructedStateFingerprint(
        userId,
        sessionId,
        lifecycleState,
        sequence,
      ...
      const reconstructedState: ReconstructedState = {
        userId,
        sessionId,
      ...
      const failure = createRecoveryFailure(
        ...
        err?.message || 'Unknown internal recovery error',
        userId,
        sessionId,
        false,
        { error: err?.name },
      ...
      const fallbackReconstructedState: ReconstructedState = {
        userId,
        sessionId,
        correlationId: request.correlationId || 'NONE',
        requestId: request.requestId,
      ...
      const failedResult = createRecoveryResult(
        recoveryId,
        userId,
        sessionId,
        currentState,
        'BLOCK_RECOVERY',
        'UNKNOWN_STATE',
        'UNKNOWN_DURABLE_STATE',
        fallbackReconstructedState,
        null,
        journal.getEvents(),
        failure,
      );
   ...
   */
  public getLastRecoveryResult(userId: string, sessionId: string): RecoveryResult | undefined {
   ...
   */
  public getJournal(userId: string, sessionId: string): readonly import('./recoveryTypes.js').RecoveryJournalRecord[] {
   ...
   */
  public resetSessionRecovery(userId: string, sessionId: string): void {
    const key = this.getPartitionKey(userId, sessionId);
