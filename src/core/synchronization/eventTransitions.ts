 ...
 */
export const VALID_EVENT_TRANSITIONS: ReadonlyMap<
  EventLifecycleState,
  ReadonlySet<EventLifecycleState>
> = Object.freeze(
  new Map<EventLifecycleState, ReadonlySet<EventLifecycleState>>([
    [
      'CREATED',
      Object.freeze(
    ...
    ['REJECTED', Object.freeze(new Set<EventLifecycleState>())],
    ['STALE', Object.freeze(new Set<EventLifecycleState>())],
    ['CONFLICTED', Object.freeze(new Set<EventLifecycleState>())],
    ['SUPERSEDED', Object.freeze(new Set<EventLifecycleState>())],
  ]),
);
...
export function validateEventTransition(
  ...
  if (!allowed || !allowed.has(to)) {
    return {
      valid: false,
      reason: `Illegal event state transition from "${from}" to "${to}". Allowed targets: [${
