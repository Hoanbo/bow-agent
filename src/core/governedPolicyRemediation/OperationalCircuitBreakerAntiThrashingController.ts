...
interface MutableBreakerEntry {
  ...
  halfOpenProbesAttempted: number;
...
export class OperationalCircuitBreakerAntiThrashingController {
  ...
  private readonly params: CircuitBreakerParameters;
  // Key: tenantId + "::" + policyDomain + "::" + actionType
  private readonly registry: Map<string, MutableBreakerEntry> = new Map();

  constructor(
    emergencyStopProvider?: EmergencyStopProvider,
    customParams?: Partial<CircuitBreakerParameters>
  ) {
  ...
  private getBreakerKey(tenantId: string, policyDomain: PolicyDomain, actionType: string): string {
    return `${tenantId}::${policyDomain}::${actionType}`;
  }

  private getOrCreateEntry(key: string): MutableBreakerEntry {
  ...
  public recordFailure(
    ...
    if (entry.lockedOut) {
      throw new CircuitBreakerLockoutError(
        `Circuit breaker for '${actionType}' is LOCKED OUT after ${entry.tripCount} consecutive trips; requires Boss intervention`
      );
    }
    ...
    if (entry.state === 'HALF_OPEN') {
      entry.state = 'OPEN';
      entry.tripCount += 1;
      entry.currentCooldownSeconds = Math.min(
        this.params.maxCooldownSeconds,
        Math.round(entry.currentCooldownSeconds * this.params.backoffMultiplier)
      );
  ...
  public attemptProbe(
    ...
    if (status.state === 'OPEN') {
      throw new CircuitBreakerOpenError(
        `Circuit breaker for '${actionType}' is OPEN (cooldown active until ${status.cooldownExpiresAt})`
      );
    }
    if (status.state === 'HALF_OPEN') {
      ...
      if (entry.halfOpenProbesAttempted >= this.params.maxHalfOpenProbes) {
        throw new CircuitBreakerOpenError(
          `Circuit breaker for '${actionType}' is HALF_OPEN and probe limit (${this.params.maxHalfOpenProbes}) exhausted`
        );
      }
  ...
  public resetLockoutByHumanAuthority(
    tenantId: string,
    policyDomain: PolicyDomain,
    actionType: string,
    ...
    if (callerIdentity !== 'BOSS_ROOT_OPERATOR') {
      throw new SecondaryAuthorityRejectedError(
        `Only Sole Human Authority ('BOSS_ROOT_OPERATOR') may reset circuit breaker lockout; received '${callerIdentity}'`
      );
    }
