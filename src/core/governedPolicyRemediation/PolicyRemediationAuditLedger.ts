...
export class PolicyRemediationAuditLedger {
  ...
  private inMemoryLedger: RemediationAuditRecord[] = [];

  constructor(
    emergencyStopProvider?: EmergencyStopProvider,
    baseStorageDir: string = path.join(process.cwd(), 'data', 'partitions_governed_policy_remediation')
  ) {
    this.emergencyStopProvider = emergencyStopProvider;
    this.baseStorageDir = baseStorageDir;
  }

  private assertEmergencyStopInactive(eventType?: RemediationAuditEventType): void {
  ...
  private scrubSecrets(payload: Record<string, unknown>): Record<string, unknown> {
    ...
    for (const [k, v] of Object.entries(payload)) {
      if (typeof v === 'string') {
        ...
        scrubbed[k] = this.scrubSecrets(v as Record<string, unknown>);
      } else {
        scrubbed[k] = v;
      }
    }
  ...
  public async appendEvent(
  ...
  ): Promise<RemediationAuditRecord> {
    ...
    if (fs.existsSync(filePath)) {
      ...
      if (content.length > 0) {
        ...
        if (lastLine) {
          try {
            const lastRecord = JSON.parse(lastLine) as RemediationAuditRecord;
            previousEventHash = lastRecord.eventHash;
            sequenceNumber = lastRecord.sequenceNumber + 1;
          } catch (_err) {
            throw new RemediationAuditLedgerError(
              `Remediation audit ledger corrupted: unable to parse last entry for tenant '${tenantId}'`
            );
          }
        }
      }
    ...
    const recordId = asRemediationAuditRecordId(
      `rec_${tenantId}_${sequenceNumber}_${Date.now()}`
    );

    const cleanPayload = this.scrubSecrets(eventPayload as Record<string, unknown>);
    ...
    const record: RemediationAuditRecord = Object.freeze({
      recordId,
      sequenceNumber,
      timestamp,
      tenantId,
  ...
  public verifyLedgerChain(tenantId: string, policyDomain?: PolicyDomain): {
    verified: boolean;
    recordCount: number;
    tipHash: string;
  } {
    ...
    let records: RemediationAuditRecord[] = [];
    if (fs.existsSync(filePath)) {
      const content = fs.readFileSync(filePath, 'utf8').trim();
      if (content.length > 0) {
        const lines = content.split('\n');
        records = lines.map((line) => JSON.parse(line) as RemediationAuditRecord);
      }
    } else {
    ...
    for (const rec of records) {
      ...
      if (rec.sequenceNumber !== expectedSeq) {
        throw new RemediationAuditLedgerError(
          `Sequence broken: expected ${expectedSeq}, found ${rec.sequenceNumber}`
        );
      }
      if (rec.previousEventHash !== prevHash) {
        throw new RemediationAuditLedgerError(
          `Hash chain broken at seq ${rec.sequenceNumber}: expected prev '${prevHash}', found '${rec.previousEventHash}'`
        );
      }
      ...
      const computed = computeAuditEventHash(
        ...
        rec.eventType,
        rec.eventPayload,
        rec.previousEventHash
      );

      if (computed !== rec.eventHash) {
        throw new RemediationAuditLedgerError(
          `Tampered record at seq ${rec.sequenceNumber}: expected hash '${computed}', found '${rec.eventHash}'`
        );
      }
