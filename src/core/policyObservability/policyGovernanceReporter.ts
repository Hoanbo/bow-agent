...
import {
  type PolicyRuntimeHealthSnapshot,
  type PolicyGovernanceReport,
...
import { PolicyEvidenceCollector, globalPolicyEvidenceCollector } from './policyEvidenceCollector.js';
import {
  type PolicyCandidateId,
  type PolicyRing,
  type PolicyCanaryHealth,
  type PolicyCanaryState,
} from '../policyCanary/policyCanaryTypes.js';
import { PolicyCanaryTelemetryAggregator, globalPolicyCanaryTelemetryAggregator } from '../policyCanary/policyCanaryTelemetryAggregator.js';
import { PolicyCanaryHealthMonitor, globalPolicyCanaryHealthMonitor } from '../policyCanary/policyCanaryHealthMonitor.js';
import { PolicyCanaryCircuitBreaker, globalPolicyCanaryCircuitBreaker } from '../policyCanary/policyCanaryCircuitBreaker.js';
...
export class PolicyGovernanceReporter {
   ...
   */
  public buildHealthSnapshot(input: {
    ...
    const snapshotId = createPolicyObservabilitySnapshotId(
      `snap_${Date.now()}_${crypto.randomBytes(3).toString('hex')}`
    );
    ...
    let healthClassification: PolicyCanaryHealth | 'UNKNOWN' = 'UNKNOWN';
    let healthRecommendation: 'PROMOTE' | 'HOLD' | 'DEMOTE' | 'ROLLBACK' | undefined = undefined;
   ...
   */
  public generateGovernanceReport(input: {
    readonly tenantPartition: string;
    readonly activePolicyVersion: string;
    readonly candidateId?: PolicyCandidateId;
    readonly candidateState?: PolicyCanaryState;
    ...
    const reportId = createPolicyGovernanceReportId(
      `rpt_${Date.now()}_${crypto.randomBytes(3).toString('hex')}`
    );
    ...
    const periodEvidence = allEvidence.filter(
      r => r.timestamp >= input.reportPeriodStart && r.timestamp <= input.reportPeriodEnd
    );

    const recentMismatches: Pick<PolicyMismatchEvidence, 'tenantPartition' | 'toolName' | 'activeAllowed' | 'candidateAllowed' | 'timestamp'>[] = [];
    const recentGuardrails: Pick<PolicyGuardrailEvidence, 'violationType' | 'reason' | 'timestamp'>[] = [];
    const recentCBEvents: Pick<PolicyCircuitBreakerEvidence, 'tripped' | 'tripReason' | 'timestamp'>[] = [];
   ...
   */
  public queryEvidence(filter: PolicyEvidenceQuery): readonly PolicyEvidenceRecord[] {
  ...
  private buildAdvisorySummary(
    snapshot: PolicyRuntimeHealthSnapshot,
