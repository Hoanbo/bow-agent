...
export class PolicyPhaseTransitionProvenanceEngine {
  private readonly chains = new Map<string, PhaseTransitionProvenanceRecord[]>();
   ...
   */
  public recordTransitionEvent(params: {
    ...
    const provenanceId = createPhaseTransitionProvenanceId(
      `prov_trans_${tenantId}_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`
    );
    ...
    const record: PhaseTransitionProvenanceRecord = Object.freeze({
      provenanceId,
      tenantId,
      transitionType,
      targetPhase,
   ...
   */
  public verifyChain(tenantId: string): { readonly valid: boolean; readonly error?: string } {
    this.assertUserStopInactive();
   ...
   */
  public getChain(tenantId: string): readonly PhaseTransitionProvenanceRecord[] {
    this.assertUserStopInactive();
