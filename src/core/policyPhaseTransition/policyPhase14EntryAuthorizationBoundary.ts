...
export class PolicyPhase14EntryAuthorizationBoundary {
   ...
   */
  public authorizePhase14Entry(
    readiness: Phase14EntryReadinessRecord,
    request: Phase14EntryAuthorizationRequest
    ...
    const authorizationId = createPhase14EntryAuthorizationId(
      `auth_14_${request.tenantId}_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`
    );
