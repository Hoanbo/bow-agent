...
export class PdpPolicyHandoffIntakeGateway {
  ...
  private readonly consumedProposalIds = new Set<string>();
  private readonly inFlightHandoffs = new Map<string, number>(); // tenantId -> count
  private readonly validatedIntakeRecords = new Map<string, ValidatedIntakeRecord>();
  ...
  public ingestHandoff(
    ...
    if (callingTenantContext && callingTenantContext !== handoff.tenantId) {
      throw new PolicyTenantIsolationError(
        `TENANT_MISMATCH: Calling tenant '${callingTenantContext}' does not match handoff tenant '${handoff.tenantId}'.`
      );
    }
    ...
    if (handoff.isAuthoritativePolicy !== false) {
      throw new PolicyHandoffSchemaValidationError(
        'AUTHORITY_ELEVATION_REJECTED: Handoff package must declare isAuthoritativePolicy: false.'
      );
    }

    if (handoff.humanApprovalCertified !== true) {
      throw new PolicyHandoffSchemaValidationError(
        'HUMAN_APPROVAL_UNCERTIFIED: Handoff package must have humanApprovalCertified: true.'
      );
    }

    // 5. Replay Defense
    if (this.consumedHandoffIds.has(handoff.handoffId)) {
      throw new PolicyHandoffReplayError(
        `REPLAY_REJECTED: Handoff ID '${handoff.handoffId}' has already been consumed and cannot be re-ingested.`
      );
    }

    if (this.consumedProposalIds.has(handoff.proposalId)) {
      throw new PolicyHandoffReplayError(
        `REPLAY_REJECTED: Proposal ID '${handoff.proposalId}' has already been ingested in this lifecycle epoch.`
      );
    }
    ...
    if (currentInFlight >= MAX_HANDOFFS_IN_FLIGHT) {
      throw new PolicyHandoffSchemaValidationError(
        `CONCURRENCY_LIMIT_EXCEEDED: Tenant '${handoff.tenantId}' has reached the maximum of ${MAX_HANDOFFS_IN_FLIGHT} in-flight handoffs.`
      );
    }
   ...
   */
  public releaseInFlight(tenantId: string): void {
  ...
  public getIntakeRecord(intakeId: string): ValidatedIntakeRecord | undefined {
    return this.validatedIntakeRecords.get(intakeId);
  }

  public isConsumed(handoffId: string): boolean {
    return this.consumedHandoffIds.has(handoffId);
  }

  // --- Private Validation Subroutines ---

  private validateHandoffSchema(handoff: PdpPolicyHandoffPackage): void {
    if (!handoff || typeof handoff !== 'object') {
      throw new PolicyHandoffSchemaValidationError('Malformed handoff package: expected non-null object.');
    }
    ...
    if (now - handoff.packagedAt > MAX_HANDOFF_TTL_MS) {
      throw new PolicyHandoffSchemaValidationError(
        `EXPIRED_HANDOFF: Handoff was packaged ${now - handoff.packagedAt}ms ago, exceeding TTL of ${MAX_HANDOFF_TTL_MS}ms.`
      );
    }
    ...
    if (!handoff.dossierProvenanceHash || !/^[a-f0-9]{64}$/i.test(handoff.dossierProvenanceHash)) {
      throw new PolicyHandoffSchemaValidationError(
        'INVALID_PROVENANCE_HASH: dossierProvenanceHash must be a 64-character hex string.'
      );
    }
    ...
    if (handoff.proposedChanges.length > MAX_DELTAS_PER_PROPOSAL) {
      throw new PolicyHandoffSchemaValidationError(
        `DELTA_LIMIT_EXCEEDED: proposedChanges contains ${handoff.proposedChanges.length} deltas (max ${MAX_DELTAS_PER_PROPOSAL}).`
      );
    }
    ...
    if (serializedSize > MAX_POLICY_SIZE_BYTES) {
      throw new PolicyHandoffSchemaValidationError(
        `PAYLOAD_CEILING_EXCEEDED: Handoff size ${serializedSize} bytes exceeds maximum ${MAX_POLICY_SIZE_BYTES} bytes.`
      );
    }
  ...
  private assertValidTenantId(tenantId: string): void {
    if (!tenantId || typeof tenantId !== 'string' || !/^[a-zA-Z0-9_-]{1,64}$/.test(tenantId)) {
      throw new PolicyTenantIsolationError(
        `INVALID_TENANT_ID: Tenant ID '${tenantId}' must be 1-64 alphanumeric, underscore, or hyphen characters.`
      );
    }
    ...
    if (reserved.includes(upper)) {
      throw new PolicyTenantIsolationError(
        `RESERVED_DEVICE_NAME_BLOCKED: Tenant ID '${tenantId}' is a reserved OS device name.`
      );
    }
  }

  private assertInterlocksInactive(tenantId: string, domain?: string): void {
    if (this.isUserStopActiveFn && this.isUserStopActiveFn(tenantId)) {
      throw new PolicyIngestionInterlockActiveError(
        `USER_STOP_ACTIVE: Operations suspended by USER_STOP supremacy for tenant '${tenantId}'.`
      );
    }
    ...
    try { emergencyStopActive = this.isEmergencyStopActiveFn(domain); }
    catch { throw new PolicyIngestionInterlockActiveError('EMERGENCY_STOP_PROVIDER_UNAVAILABLE: intake fails closed.'); }
    ...
    if (emergencyStopActive) {
      throw new PolicyIngestionInterlockActiveError(
        `EMERGENCY_STOP_ACTIVE: Domain kill switch engaged for domain '${domain || 'global'}'.`
      );
    }
  }
}
