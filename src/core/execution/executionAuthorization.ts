...
import type { CapabilityRisk } from './capabilityTypes.js';
 ...
 */
export function authorizeExecution(params: AuthorizationParams): ExecutionAuthorization {
  const {
    request,
    registry,
    orchestrationStatus,
  ...
  const toolName = request.toolName || (request as any).actionName || '';
  const args = request.args || (request as any).parameters || {};
  const approvalMetadata = params.approvalMetadata || (request as any).approvalMetadata;
  ...
  if (orchestrationStatus) {
    const disallowedStatuses: OrchestrationStatus[] = [
      'BLOCKED',
  ...
  const effectiveRisk: CapabilityRisk = capability.risk;

  // 6. Risk Downgrade Check (INV-2)
  const declaredRisk = (request as any).riskLevel || (request as any).risk;
  ...
  const schemaValidation = registry.validate(toolName, args as Record<string, unknown>);
