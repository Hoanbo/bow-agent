...
export type VerificationGateCheckpoint =
  | 'GATE_1_BEFORE_REQUEST_ACCEPTANCE'
  | 'GATE_2_BEFORE_EVIDENCE_COLLECTION'
  | 'GATE_3_BEFORE_ORACLE_EVALUATION'
  | 'GATE_4_BEFORE_RESULT_EMISSION';
...
export class VerificationExecutionGate {
   ...
   */
  public assertVerificationPermitted(
    checkpoint: VerificationGateCheckpoint,
    ...
    if (this.isUserStopActiveFn()) {
      ...
      throw new VerificationAbortedError(
        `USER_STOP_ACTIVE: Verification aborted at checkpoint "${checkpoint}". Reason: ${reason}`,
        {
          checkpoint,
          reason,
          ...context,
   ...
   */
  public assertCanAcceptRequest(context?: Readonly<Record<string, unknown>>): void {
   ...
   */
  public assertCanCollectEvidence(context?: Readonly<Record<string, unknown>>): void {
   ...
   */
  public assertCanEvaluateOracle(context?: Readonly<Record<string, unknown>>): void {
   ...
   */
  public assertCanEmitResult(context?: Readonly<Record<string, unknown>>): void {
