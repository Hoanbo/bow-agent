...
import {
  ...
  type QualityGateOverallState,
  type QualityGateStageCategory,
  type QualityGateStageEvaluation,
...
export const MANDATORY_QUALITY_GATE_STAGES: readonly QualityGateStageCategory[] = [
  'TYPECHECK',
  'BUILD',
...
export class QualityGateEngine {
   ...
   */
  public static hashGateEvaluation(
    gateId: string,
    overallState: QualityGateOverallState,
    stageHashes: readonly string[]
  ): string {
   ...
   */
  public static mapCategoryToCommandId(category: QualityGateStageCategory): string {
    switch (category) {
      case 'TYPECHECK':
  ...
  public async evaluateGate(
    ...
    const gateId: QualityGateId = createQualityGateId(`qgate_${crypto.randomUUID()}`);
    const evaluatedAt = Date.now();
    const stagesToRun = options?.stagesToRun ?? MANDATORY_QUALITY_GATE_STAGES;

    const stageEvaluations: QualityGateStageEvaluation[] = [];
    ...
    for (const category of stagesToRun) {
      ...
      try {
        ...
        let stageState: QualityGateOverallState;
    ...
    let overallState: QualityGateOverallState = 'PASS';
