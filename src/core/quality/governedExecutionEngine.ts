...
export class GovernedExecutionEngine {
  constructor(private readonly registry: QualityCommandRegistry = new QualityCommandRegistry()) {}

  /**
   * Computes deterministic SHA-256 hash for raw string payload.
   * Tính toán mã băm SHA-256 tất định cho khối dữ liệu chuỗi thô.
   */
  public static hashPayload(payload: string): string {
  ...
  public async executeCommand(
    ...
    let rawResult: GovernedCommandRawResult;
    let timer: NodeJS.Timeout | undefined;
    try {
      const timeoutPromise = new Promise<GovernedCommandRawResult>((_, reject) => {
        timer = setTimeout(() => {
          reject(
            new QualityError(
              QualityErrorCode.TIMEOUT_EXCEEDED,
              `Command "${commandId}" timed out after ${timeoutLimit}ms.`
            )
          );
        }, timeoutLimit);
      });
    ...
    if (sanitizedStdout.length > maxOutput || sanitizedStderr.length > maxOutput) {
      throw new QualityError(
        QualityErrorCode.RESOURCE_LIMIT_EXCEEDED,
        `Command "${commandId}" exceeded maximum allowed output size (${maxOutput} bytes).`
      );
    }
    ...
    const rawEvidenceHash = GovernedExecutionEngine.hashPayload(
      `${commandId}:${rawResult.exitCode}:${stdoutHash}:${stderrHash}:${rawResult.durationMs}`
    );
