...
export class CrossFederationReconciliationEngine {
  ...
  public reconcileProposals(
    ...
    if (proposals.length === 0) {
      const cleanEmpty: Omit<CrossFederationReconciliationResult, 'provenanceHash'> = {
    ...
    const participatingFederationIds = new Set<string>();
    const congruent: string[] = [];
    ...
    for (const proposal of proposals) {
      ...
      if (proposal.tenantId !== tenantId) {
        throw new GovernedCrossFederationTenantIsolationError(
          `Tenant isolation violation in reconciliation: Proposal ${proposal.proposalId} belongs to tenant ${proposal.tenantId}, expected ${tenantId}`,
          tenantId,
          sessionId
        );
      }
      ...
      if (proposal.sessionId !== sessionId) {
        throw new GovernedCrossFederationSessionIsolationError(
          `Session isolation violation in reconciliation: Proposal ${proposal.proposalId} belongs to session ${proposal.sessionId}, expected ${sessionId}`,
          tenantId,
          sessionId
        );
      }
      ...
      if (proposal.missionId !== missionId) {
        throw new GovernedCrossFederationValidationError(
          `Mission mismatch in reconciliation: Proposal ${proposal.proposalId} mission ${proposal.missionId} != ${missionId}`,
          tenantId,
          sessionId
        );
      }
      ...
      if (proposal.objectiveId !== objectiveId) {
        throw new GovernedCrossFederationValidationError(
          `Objective mismatch in reconciliation: Proposal ${proposal.proposalId} objective ${proposal.objectiveId} != ${objectiveId}`,
          tenantId,
          sessionId
        );
      }
      ...
      if (proposal.provenanceHash !== expectedHash) {
        throw new GovernedCrossFederationValidationError(
          `Cryptographic provenance mismatch for proposal ${proposal.proposalId}`,
          tenantId,
          sessionId
        );
      }
    ...
    for (let i = 0; i < proposals.length; i++) {
      ...
      for (let j = 0; j < proposals.length; j++) {
        ...
        if (
          p1.federationId !== p2.federationId &&
    ...
    const status: 'CONGRUENT' | 'MATERIAL_CONTRADICTION' | 'REVIEW_REQUIRED' =
      contradicted.length > 0 ? 'MATERIAL_CONTRADICTION' : 'CONGRUENT';

    const cleanResult: Omit<CrossFederationReconciliationResult, 'provenanceHash'> = {
  ...
  private isContradictory(goalA: string, goalB: string): boolean {
  ...
  private hasActionConflict(actionsA: readonly string[], actionsB: readonly string[]): boolean {
