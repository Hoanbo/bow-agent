...
export class CrossFederationSecurityBoundary {
  ...
  private readonly executedCheckpoints: CrossFederationCheckpoint[] = [];
  ...
  public validateSafePathId(id: string): void {
    ...
    if (id.includes('\0') || id.includes('..') || id.includes('/') || id.includes('\\')) {
      throw new GovernedCrossFederationValidationError(
        `Path traversal or illegal character detected in ID: "${id}"`
      );
    }
    ...
    if (reservedWindows.test(id)) {
      throw new GovernedCrossFederationValidationError(
        `Security violation: Windows reserved device name rejected: "${id}"`
      );
    }
  }
  ...
  public evaluateCheckpoint(
    checkpoint: CrossFederationCheckpoint,
    tenantId?: string,
    sessionId?: string,
    ...
    if (this.emergencyStopActive) {
      throw new GovernedCrossFederationEmergencyStopError(
        `EMERGENCY_STOP is active: All cross-federation operations blocked at checkpoint ${checkpoint}`,
        tenantId,
        sessionId
      );
    }
    ...
    if (this.userStopActive) {
      throw new GovernedCrossFederationUserStopError(
        `USER_STOP is active: Operation halted by operator directive at checkpoint ${checkpoint}`,
        tenantId,
        sessionId
      );
    }
    ...
    if (tenantId && targetTenantId && tenantId !== targetTenantId) {
      throw new GovernedCrossFederationTenantIsolationError(
        `Cross-tenant violation at checkpoint ${checkpoint}: ${tenantId} !== ${targetTenantId}`,
        tenantId,
        sessionId
      );
    }
    ...
    if (sessionId && targetSessionId && sessionId !== targetSessionId) {
      throw new GovernedCrossFederationSessionIsolationError(
        `Cross-session violation at checkpoint ${checkpoint}: ${sessionId} !== ${targetSessionId}`,
        tenantId,
        sessionId
      );
    }
    ...
    if (tenantId) this.validateSafePathId(tenantId);
    if (sessionId) this.validateSafePathId(sessionId);

    this.executedCheckpoints.push(checkpoint);
  }

  public getExecutedCheckpoints(): readonly CrossFederationCheckpoint[] {
