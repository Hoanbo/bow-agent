...
export class PolicyActiveLifecycleProvenanceConsistencyEngine {
  private readonly stagedProvenance?: PolicyStagedActivationProvenanceEngine;
  private readonly runtimeProvenance?: PolicyActiveRuntimeProvenanceEngine;
  private readonly rollbackProvenance?: PolicyActiveRollbackProvenanceEngine;
  private readonly isUserStopActiveFn?: () => boolean;

  constructor(
    options?: PolicyActiveLifecycleReconciliationOptions,
    stagedProvenance?: PolicyStagedActivationProvenanceEngine,
    runtimeProvenance?: PolicyActiveRuntimeProvenanceEngine,
    rollbackProvenance?: PolicyActiveRollbackProvenanceEngine
  ) {
   ...
   */
  public verifyProvenance(tenantPartition: string, candidateDraftId?: string): ProvenanceConsistencyCheckResult {
    ...
    let overallStatus: 'VALID' | 'TAMPER_DETECTED' | 'MISSING' | 'INVALID' = 'VALID';
    ...
    if (this.stagedProvenance) {
      ...
      let recordsCount = 0;
      try {
        if (typeof (this.stagedProvenance as any).verifyChainIntegrity === 'function') {
          stagedValid = (this.stagedProvenance as any).verifyChainIntegrity(tenantPartition);
          const chain = typeof (this.stagedProvenance as any).getChain === 'function' ? (this.stagedProvenance as any).getChain(tenantPartition) : null;
          recordsCount = chain?.length ?? 0;
          if (chain && chain.length > 0) {
            combinedHeadHash = chain[chain.length - 1].recordHash;
          }
        } else if (typeof (this.stagedProvenance as any).verifyChain === 'function') {
          if (candidateDraftId) {
            stagedValid = (this.stagedProvenance as any).verifyChain(tenantPartition, candidateDraftId);
            combinedHeadHash = (this.stagedProvenance as any).getProvenanceHead(tenantPartition, candidateDraftId);
            const tenantChains = (this.stagedProvenance as any).chains?.get(tenantPartition);
            ...
            if (tenantChains && tenantChains.size > 0) {
              ...
              for (const [candId, recs] of tenantChains.entries()) {
                recordsCount += (recs as any).length;
                if (!(this.stagedProvenance as any).verifyChain(tenantPartition, candId)) {
                  stagedValid = false;
                  break;
                }
                combinedHeadHash = (this.stagedProvenance as any).getProvenanceHead(tenantPartition, candId);
              }
            } else {
              stagedValid = true;
            }
