...
import {
  ...
  type CognitiveHealthStatus,
  type CognitiveIntent,
  type CognitiveRiskLevel,
  type CognitiveDecisionType,
  type CognitivePlan,
  type CognitiveDecision,
  type CognitiveConfidence,
  type CognitiveToolCandidate,
  type CognitiveStageTrace,
  type CognitiveInferenceBudget,
...
// ============================================================================
// 1. CLOUD GEMINI COGNITIVE PROVIDER
// ============================================================================
...
export class GeminiCognitiveProvider implements CognitiveProvider {
  public readonly providerType: CognitiveProviderType = 'cloud-gemini';
  public readonly providerName = 'cloud-gemini-provider';
  ...
  private static readonly INPUT_PRICE_PER_TOKEN = 0.00000015;
  private static readonly OUTPUT_PRICE_PER_TOKEN = 0.0000006;
  ...
  public async process(
    ...
    options?: CognitiveExecutionOptions & { readonly budget?: CognitiveInferenceBudget }
  ...
  public async executeInference(
    ...
    const key = this.apiKeyResolver();
    if (!key) {
      throw new CognitiveProviderUnavailableError(
        this.providerType,
    ...
    if (options?.budget && estimatedPromptTokens > options.budget.maxPromptTokens) {
      throw new CognitiveBudgetExceededError(
        'prompt_tokens',
        options.budget.maxPromptTokens,
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401 || res.status === 403) {
          throw new CognitiveProviderUnavailableError(
            this.providerType,
            `Gemini authentication failure: HTTP ${res.status}`
          );
        }
        if (res.status === 429) {
          throw new CognitiveProviderUnavailableError(
        ...
        throw new CognitiveProviderUnavailableError(
          this.providerType,
          `Gemini request failed: HTTP ${res.status} ${res.statusText}`
        );
      }
      ...
      throw new CognitiveProviderUnavailableError(
        this.providerType,
        `Gemini network invocation failure: ${err?.message ?? String(err)}`
      );
    }
    ...
    const estimatedCostUsd =
      promptTokens * GeminiCognitiveProvider.INPUT_PRICE_PER_TOKEN +
  ...
  private parseAndValidateResult(
    ...
    try {
      ...
      throw new CognitiveValidationError(
        `Gemini returned non-JSON output: ${err?.message}`,
        ['UNPARSEABLE_JSON_OUTPUT']
      );
    }

    // Required fields check
    const errors: string[] = [];
    if (!parsed.intent || typeof parsed.intent !== 'string') errors.push('Missing or invalid intent');
    if (!parsed.interpretation || typeof parsed.interpretation !== 'string') errors.push('Missing or invalid interpretation');
    if (!parsed.plan || typeof parsed.plan !== 'object') errors.push('Missing or invalid plan');
    if (!parsed.decision || typeof parsed.decision !== 'object') errors.push('Missing or invalid decision');
    ...
    const plan: CognitivePlan = {
      ...
      steps: Array.isArray(parsed.plan.steps)
        ? parsed.plan.steps.map((s: any, idx: number) => ({
            stepIndex: idx,
      ...
      estimatedRisk: (parsed.plan.estimatedRisk as CognitiveRiskLevel) || 'LOW',
    ...
    const decision: CognitiveDecision = {
      decisionType: (parsed.decision.decisionType as CognitiveDecisionType) || 'PROCEED',
      riskLevel: (parsed.decision.riskLevel as CognitiveRiskLevel) || 'LOW',
    ...
    return {
      ...
      intent: (parsed.intent as CognitiveIntent) || 'ANALYZE',
...
// ============================================================================
// 2. LOCAL OLLAMA COGNITIVE PROVIDER
// ============================================================================
...
export class OllamaCognitiveProvider implements CognitiveProvider {
  public readonly providerType: CognitiveProviderType = 'ollama-local';
  public readonly providerName = 'ollama-local-provider';
  ...
  public async executeInference(
    ...
    try {
      ...
      throw new CognitiveProviderUnavailableError(
        this.providerType,
        `Ollama invocation failure: ${err?.message ?? String(err)}`
      );
    }
  ...
  private parseAndValidateResult(rawText: string, stages: CognitiveStageTrace[]): CognitiveResult {
    const parseStart = Date.now();
    let parsed: any;
    try {
      const cleaned = rawText.replace(/^```json\s*/i, '').replace(/\s*```$/, '').trim();
      ...
      throw new CognitiveValidationError(
        `Ollama returned non-JSON output: ${err?.message}`,
        ['UNPARSEABLE_JSON_OUTPUT']
      );
    }

    const errors: string[] = [];
    if (!parsed.intent || typeof parsed.intent !== 'string') errors.push('Missing intent');
    if (!parsed.interpretation || typeof parsed.interpretation !== 'string') errors.push('Missing interpretation');
    if (!parsed.plan || typeof parsed.plan !== 'object') errors.push('Missing plan');
    if (!parsed.decision || typeof parsed.decision !== 'object') errors.push('Missing decision');
...
export class DeterministicFallbackCognitiveProvider implements CognitiveProvider {
  public readonly providerType: CognitiveProviderType = 'deterministic-fallback';
  public readonly providerName = 'deterministic-fallback-provider';
  public readonly modelName = 'bowcon-rule-engine-v4';
  ...
  public async executeInference(
    context: CognitivePromptContext,
    _options?: CognitiveExecutionOptions
  ): Promise<ProviderExecutionResult> {
    const startedAt = Date.now();
    const requestId = makeCognitiveRequestId();
    const traceId = makeCognitiveTraceId();
