...
export interface LocalOllamaExecutionResult {
  ...
  readonly estimatedCostUsd: number; // Always 0.00
  readonly isLocal: boolean;
}

export class LocalFirstOllamaRuntime {
  public readonly providerName = 'ollama-local-runtime';
   ...
   */
  public async probeHealth(): Promise<CognitiveProviderHealthReport> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new CognitiveProviderUnavailableError(
          'ollama-local',
          `HTTP ${response.status} from Ollama daemon`
        );
      }
      ...
      if (
        err instanceof CognitiveProviderUnavailableError ||
      ...
      throw new CognitiveProviderUnavailableError(
        'ollama-local',
        `Failed to reach Ollama endpoint ${this.baseUrl}: ${err.message}`
      );
    }
  }
    ...
    return {
      cognitiveResult: {
        ...
        toolCandidates: p.toolProposals.map((t) => ({
          toolName: t.toolName,
          toolArgs: t.parameters as Record<string, unknown>,
          intent: p.intent,
   ...
   */
  public classifyFailure(err: any): CognitiveFailureDetail {
    const msg = String(err?.message || err);
    let reason: CognitiveFailureDetail['reason'] = 'UNKNOWN_ERROR';
