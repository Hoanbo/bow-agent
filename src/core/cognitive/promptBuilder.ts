...
import type { CognitivePromptContext, CognitiveTurn } from './cognitiveTypes.js';
...
export class PromptBuilder {
   ...
   */
  public static detectPromptInjection(input: string): { isInjected: boolean; reason?: string } {
    if (!input || typeof input !== 'string') return { isInjected: false };
   ...
   */
  public static neutralizeInjection(input: string): string {
   ...
   */
  public static buildStructuredPrompt(context: CognitivePromptContext): string {
    ...
    if (context.previousTurns && context.previousTurns.length > 0) {
      turnsText = '\n### CONVERSATION HISTORY (RECENT TURNS)\n' +
        context.previousTurns
          ...
          .map((t: CognitiveTurn) => `[${t.role.toUpperCase()}]: ${this.sanitizeText(t.content)}${t.targetEntity ? ` (Entity: ${t.targetEntity})` : ''}`)
          .join('\n') + '\n';
    }
