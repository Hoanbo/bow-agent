...
export class RootCauseHypothesisEngine {
  public static readonly MAX_CONFIDENCE = 0.95;
  public static readonly MIN_CONFIDENCE = 0.05;
  public static readonly DISSENT_PENALTY = 0.40;
  public static readonly CONTRADICTION_PENALTY_PER_ITEM = 0.35;
  ...
  public evaluateHypotheses(
    cluster: CorrelatedEvidenceCluster,
    ...
    // -------------------------------------------------------------------------
    const mutationDrift = driftItems.filter(
      (d) => d.category === 'UNAUTHORIZED_MUTATION' || d.category === 'FILESYSTEM'
    );
    const boundaryViolations = invariantItems.filter(
      (i) => i.category === 'PROTECTED_WORKSPACE' || i.category === 'BOUNDARY' || i.category === 'MANIFEST'
    );
    ...
    // -------------------------------------------------------------------------
    const resourceViolations = invariantItems.filter(
      (i) =>
        i.description.toLowerCase().includes('memory') ||
    ...
    const highLatencyAlerts = alertItems.filter(
      (a) => a.description.toLowerCase().includes('latency') || a.description.toLowerCase().includes('error')
    );
    ...
    // -------------------------------------------------------------------------
    const configDrift = driftItems.filter(
      (d) =>
    ...
    // -------------------------------------------------------------------------
    const canaryDegradations = telemetryItems.filter(
      (t) => t.severity === 'HIGH' && t.description.toLowerCase().includes('consecutive degradations')
    );
    ...
    // -------------------------------------------------------------------------
    const upstreamAlerts = alertItems.filter(
      (a) =>
        a.description.toLowerCase().includes('timeout') ||
  ...
  private buildHypothesis(args: {
    ...
    const contradictionPenalty =
      args.contradictingEvidenceIds.length * RootCauseHypothesisEngine.CONTRADICTION_PENALTY_PER_ITEM;
    ...
    const confidenceScore = Number(
      Math.max(
        RootCauseHypothesisEngine.MIN_CONFIDENCE,
        Math.min(RootCauseHypothesisEngine.MAX_CONFIDENCE, rawConfidence)
      ).toFixed(4)
    );
