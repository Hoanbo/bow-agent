 ...
 */
export function getBodyPsk(): string {
  if (cachedPsk) {
    return cachedPsk;
  }
  ...
  try {
    ...
    if (process.platform === 'win32') {
      console.warn(
        `[BODY-AUTH] Đã lưu PSK vào ${pskFilePath}. Lưu ý: Hệ điều hành Windows không hỗ trợ đầy đủ POSIX 0o600 permission. Cần hạn chế quyền truy cập file này nếu cần thiết.`
      );
    } else {
      try {
 ...
 */
export function validateBodyPsk(providedToken?: string): boolean {
 ...
 */
export function _resetCachedPskForTesting(): void {
  cachedPsk = null;
}
