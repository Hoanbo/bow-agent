...
export interface AggregationBaselineOptions {
  ...
  readonly degradationErrorThreshold?: number; // delta threshold, e.g. +0.02
  readonly degradationLatencyThresholdMs?: number; // delta threshold, e.g. +50ms
}

export class TelemetryAggregationEngine {
  private windows = new Map<string, TelemetryAggregationWindow[]>();
  ...
  public aggregateWindow(
    ...
    let baselineComparison: TelemetryAggregationWindow['baselineComparison'] | undefined;
    if (options && (options.baselineErrorRate !== undefined || options.baselineLatencyP95Ms !== undefined)) {
      ...
      const isDegradedAgainstBaseline =
        errorRateDelta >= errThreshold || latencyDeltaMs >= latThreshold;
    ...
    const window: TelemetryAggregationWindow = {
      windowId,
      sessionId,
      targetId,
      startTime,
   ...
   */
  public getWindows(sessionId: ObservabilitySessionId): readonly TelemetryAggregationWindow[] {
