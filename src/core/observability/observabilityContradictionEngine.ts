...
export class ObservabilityContradictionEngine {
  private contradictions = new Map<string, ObservabilityContradictionRecord[]>();
  ...
  public detectContradictions(
    ...
    if (hasStateConflict) conflictingFields.push('reportedHealthState');
    if (hasScoreConflict) conflictingFields.push('reportedHealthScore');
    if (hasDriftConflict) conflictingFields.push('reportedDriftClassification');

    if (conflictingFields.length === 0) {
      return null;
    }

    const contradictionRecord: ObservabilityContradictionRecord = {
   ...
   */
  public getContradictions(sessionId: ObservabilitySessionId): readonly ObservabilityContradictionRecord[] {
