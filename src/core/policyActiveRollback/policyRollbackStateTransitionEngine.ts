...
export class PolicyRollbackStateTransitionEngine {
  ...
  public commitRollback(params: {
    ...
    this.rollbackStore.saveHistoricalPolicy({
      targetId: `roltgt_${crypto.createHash('sha256').update(currentActive.activePolicyStateId).digest('hex').substring(0, 16)}` as any,
    ...
    const commitRawHash = crypto.createHash('sha256')
      ...
      .digest('hex');
    const commitId = `rolcmt_${commitRawHash.substring(0, 16)}` as RollbackCommitId;
    const newStateId = `actpol_${commitRawHash.substring(0, 20)}` as ActivePolicyStateId;
    ...
    const newActiveState: ActivePolicyState = Object.freeze({
      activePolicyStateId: newStateId,
      activationCommitId: commitId as unknown as ActivationCommitId,
      stagedActivationId: `stg_rol_${commitRawHash.substring(0, 12)}` as StagedActivationId,
      candidateDraftId: `cnd_rol_${commitRawHash.substring(0, 12)}` as CandidateDraftId,
      evolutionPlanId: `pln_rol_${commitRawHash.substring(0, 12)}` as EvolutionPlanId,
      intakeId: `itk_rol_${commitRawHash.substring(0, 12)}` as PolicyEvolutionIntakeId,
      authorizationDecisionId: `auth_rol_${commitRawHash.substring(0, 12)}` as AuthorizationDecisionId,
      activationReadinessId: `rdy_rol_${commitRawHash.substring(0, 12)}` as ActivationReadinessId,
      preflightId: `pfl_rol_${commitRawHash.substring(0, 12)}` as ActivationPreflightId,
  ...
  public commitSunset(params: {
    ...
    const commitRawHash = crypto.createHash('sha256')
      ...
      .digest('hex');
    const commitId = `suncmt_${commitRawHash.substring(0, 16)}` as SunsetCommitId;
    const newStateId = `actpol_${commitRawHash.substring(0, 20)}` as ActivePolicyStateId;
    ...
    const newActiveState: ActivePolicyState = Object.freeze({
      activePolicyStateId: newStateId,
      activationCommitId: commitId as unknown as ActivationCommitId,
      stagedActivationId: `stg_sun_${commitRawHash.substring(0, 12)}` as StagedActivationId,
      candidateDraftId: `cnd_sun_${commitRawHash.substring(0, 12)}` as CandidateDraftId,
      evolutionPlanId: `pln_sun_${commitRawHash.substring(0, 12)}` as EvolutionPlanId,
      intakeId: `itk_sun_${commitRawHash.substring(0, 12)}` as PolicyEvolutionIntakeId,
      authorizationDecisionId: `auth_sun_${commitRawHash.substring(0, 12)}` as AuthorizationDecisionId,
      activationReadinessId: `rdy_sun_${commitRawHash.substring(0, 12)}` as ActivationReadinessId,
      preflightId: `pfl_sun_${commitRawHash.substring(0, 12)}` as ActivationPreflightId,
      tenantPartition: request.tenantPartition,
      previousPolicyVersion: currentActive.activePolicyVersion,
  ...
  public commitRecovery(params: {
    ...
    const commitRawHash = crypto.createHash('sha256')
      ...
      .digest('hex');
    const commitId = `reccmt_${commitRawHash.substring(0, 16)}` as RecoveryCommitId;
    const newStateId = `actpol_${commitRawHash.substring(0, 20)}` as ActivePolicyStateId;
    ...
    const newActiveState: ActivePolicyState = Object.freeze({
      activePolicyStateId: newStateId,
      activationCommitId: commitId as unknown as ActivationCommitId,
      stagedActivationId: `stg_rec_${commitRawHash.substring(0, 12)}` as StagedActivationId,
      candidateDraftId: `cnd_rec_${commitRawHash.substring(0, 12)}` as CandidateDraftId,
      evolutionPlanId: `pln_rec_${commitRawHash.substring(0, 12)}` as EvolutionPlanId,
      intakeId: `itk_rec_${commitRawHash.substring(0, 12)}` as PolicyEvolutionIntakeId,
      authorizationDecisionId: `auth_rec_${commitRawHash.substring(0, 12)}` as AuthorizationDecisionId,
      activationReadinessId: `rdy_rec_${commitRawHash.substring(0, 12)}` as ActivationReadinessId,
      preflightId: `pfl_rec_${commitRawHash.substring(0, 12)}` as ActivationPreflightId,
      tenantPartition: request.tenantPartition,
