...
const AUTHORIZED_GOVERNANCE_ROLES = new Set<HumanAuthorizationRole>([
...
export class PolicyGovernedRollbackBoundary {
  ...
  public assertAuthorizedRole(role: string): asserts role is HumanAuthorizationRole {
    if (!role || !AUTHORIZED_GOVERNANCE_ROLES.has(role as HumanAuthorizationRole)) {
   ...
   */
  public authorizeOperation(params: {
    ...
    const result: GovernedRollbackAuthorization = Object.freeze({
      authorizationId,
      operationType: params.operationType,
      targetRequestId: params.targetRequestId,
