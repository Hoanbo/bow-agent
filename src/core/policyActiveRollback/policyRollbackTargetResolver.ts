...
export class PolicyRollbackTargetResolver {
  ...
  private readonly store: PolicyActiveRollbackStore;
  ...
  public resolveTarget(params: {
    ...
    let candidate: HistoricalPolicyVersion | null = null;
    ...
    for (const action of ROLLBACK_HARD_FORBIDDEN_ACTIONS) {
      ...
      const actionClass = (candidate.policyModifications as any)[action] ??
                          (candidate.policyModifications as any).actionClassifications?.[action];
