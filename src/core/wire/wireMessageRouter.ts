...
export class WireMessageRouter {
   ...
   */
  public route<T = unknown>(
    envelope: WireEnvelope<T>,
    ...
    if (this.seenMessageIds.has(envelope.messageId)) {
      throw new WireTransportError(
        ...
        `Replay attack detected: messageId "${envelope.messageId}" has already been processed.`
      );
    }
      ...
      if (lastSeq !== undefined) {
        if (envelope.sequence <= lastSeq) {
          throw new WireTransportError(
            ...
            `Sequence rewind detected for session "${sessionKey}". Last=${lastSeq}, incoming=${envelope.sequence}. Blocked fail-closed.`
          );
        }
      }
  ...
  public getLastSequence(sessionId: string, deviceId: string): number | undefined {
    return this.lastSeenSequence.get(`${sessionId}::${deviceId}`);
  }

  public reset(sessionId?: string, deviceId?: string): void {
