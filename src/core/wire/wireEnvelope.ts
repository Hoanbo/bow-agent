...
export function assertNoForbiddenContent(payload: unknown, depth = 0): void {
  if (depth > 10) return;
  ...
  if (typeof payload === 'string') {
    ...
    if (
      str.includes('-----BEGIN PRIVATE KEY-----') ||
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (typeof value === 'string') {
      ...
      if (
        strVal.includes('-----BEGIN PRIVATE KEY-----') ||
...
export function createWireEnvelope<T = unknown>(options: {
  messageId: string;
  sequence: number;
  relayId: string;
  brainId: string;
  ...
  const envelope: WireEnvelope<T> = Object.freeze({
...
export function validateWireEnvelope(envelope: WireEnvelope): void {
  ...
  if (
    envelope.scope.deviceId !== envelope.deviceId ||
    ...
    throw new WireTransportError(
      ...
      'WireEnvelope scope mismatch with envelope header identities. Cross-boundary tampering detected.'
    );
  }
