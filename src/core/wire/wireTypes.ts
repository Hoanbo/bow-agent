 ...
 */
export type WireMessageCategory =
  | 'CONTROL'
  | 'REQUEST'
  | 'RESPONSE'
  | 'EVENT'
...
export interface WireFrame {
  ...
  readonly checksum: string; // SHA-256 hex digest of payload
  readonly timestamp: number;
  readonly payload: string; // Serialized opaque data or envelope
}
...
export interface WireHandshakeResponse {
  ...
  readonly challenge?: string; // Ephemeral challenge for zero-trust admission
...
export type WireSecurityEventType =
  ...
  | 'WIRE_HANDSHAKE_REQUESTED'
  | 'WIRE_HANDSHAKE_COMPLETED'
  | 'WIRE_HANDSHAKE_REJECTED'
  | 'WIRE_ADMISSION_REQUESTED'
  | 'WIRE_ADMISSION_GRANTED'
  | 'WIRE_ADMISSION_DENIED'
  | 'WIRE_SESSION_BOUND'
  | 'WIRE_SESSION_RESUMED'
  | 'WIRE_SESSION_RESUME_REJECTED'
  | 'WIRE_FRAME_SENT'
  | 'WIRE_FRAME_RECEIVED'
  | 'WIRE_FRAME_REJECTED'
  ...
  | 'WIRE_SIZE_EXCEEDED'
  | 'WIRE_BACKPRESSURE_ELEVATED'
  | 'WIRE_BACKPRESSURE_OVERFLOW'
  | 'WIRE_RECONNECT_SCHEDULED'
  | 'WIRE_RECONNECT_SUCCEEDED'
  | 'WIRE_RECONNECT_FAILED'
  | 'WIRE_ROAMING_DETECTED'
  | 'WIRE_HEARTBEAT_MISSED'
  | 'WIRE_HEARTBEAT_RESTORED'
  | 'WIRE_ERROR_RECORDED';
