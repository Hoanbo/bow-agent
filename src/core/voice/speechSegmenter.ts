 ...
 */
function classifySegment(text: string): SpeechSegment['segmentType'] {
 ...
 */
export function segmentSpeech(text: string, options: SegmenterOptions = {}): SpeechSegment[] {
  ...
  const segments: SpeechSegment[] = restoredChunks.map((chunkText, index) => {
    ...
    let emphasis: SpeechSegment['emphasis'] = 'none';
