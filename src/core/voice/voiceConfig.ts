...
export const SUPPORTED_AUDIO_FORMATS: readonly AudioOutputFormat[] = [
  'audio/mpeg',
  'audio/wav',
  'audio/pcm',
] as const;
...
export interface VoiceConfig {
  ...
  similarity?: number; // 0.0 to 1.0 (ElevenLabs)
  instructions?: string;
  timeoutMs?: number;
 ...
 */
export function validateVoiceConfig(config: unknown): {
  valid: boolean;
  config?: VoiceConfig;
  errors?: string[];
} {
  ...
  if (raw.maxTextLength !== undefined) {
    if (
      typeof raw.maxTextLength !== 'number' ||
  ...
  if (raw.personalityPreset !== undefined) {
    if (typeof raw.personalityPreset !== 'string' || !VOICE_PERSONALITY_PRESETS[raw.personalityPreset as VoicePersonalityPreset]) {
      errors.push(`personalityPreset must be one of: ${Object.keys(VOICE_PERSONALITY_PRESETS).join(', ')}, got: "${raw.personalityPreset}"`);
    } else {
      clean.personalityPreset = raw.personalityPreset as VoicePersonalityPreset;
    }
  }
  ...
  if (raw.personality !== undefined) {
    if (typeof raw.personality === 'string') {
      const upper = raw.personality.toUpperCase().trim() as VoicePersonalityPreset;
