...
import { processTextForSpeech, processAndPlanSpeech } from './speechTextProcessor.js';
import { AudioAssembler, AudioChunk } from './audioAssembler.js';
...
export class VoiceService {
   ...
   */
  public getCapabilities(providerId?: string): TTSCapabilities | undefined {
   ...
   */
  public validateConfiguration(config: VoiceConfig): { valid: boolean; errors?: string[] } {
   ...
   */
  public async synthesize(request: VoiceSynthesisRequest): Promise<VoiceResult> {
    const startTime = Date.now();
    const config = request.voiceConfig || {};
    const defaultFormat: AudioOutputFormat = config.outputFormat || 'audio/wav';
    ...
    try {
      ...
      const candidateChain: string[] = [primaryProviderId];
        ...
        try {
          ...
          if (negotiation.useSentenceLevel && planned.segments.length > 1) {
            ...
            for (const seg of planned.segments) {
              ...
              const segResponse = await this.executeWithTimeout(
                provider.synthesize({
                  ...request,
                  language,
                  text: segText,
   ...
   */
  public async *synthesizeStream(request: VoiceSynthesisRequest): AsyncIterable<Buffer | Uint8Array> {
    const config = request.voiceConfig || {};
   ...
   */
  private persistAudioToDisk(audioData: Buffer | Uint8Array, format: AudioOutputFormat, outputDir: string): string {
    const resolvedDir = path.resolve(outputDir);

    // Verify containment and create directory
    fs.mkdirSync(resolvedDir, { recursive: true });

    const extMap: Record<AudioOutputFormat, string> = {
      'audio/mpeg': 'mp3',
      'audio/wav': 'wav',
      'audio/pcm': 'pcm',
    };
