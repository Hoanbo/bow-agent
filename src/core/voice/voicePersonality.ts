...
export interface VoicePersonality {
  ...
  pauseIntensity: number; // 0.5 to 2.0
  emphasisIntensity: number; // 0.0 to 1.0
}

export type VoicePersonalityPreset =
  | 'CALM_ASSISTANT'
  | 'SMART_ASSISTANT'
  | 'PROFESSIONAL'
  | 'JARVIS_INSPIRED';

export const VOICE_PERSONALITY_PRESETS: Record<VoicePersonalityPreset, VoicePersonality> = {
  CALM_ASSISTANT: {
    name: 'CALM_ASSISTANT',
    warmth: 0.8,
    confidence: 0.85,
 ...
 */
export function validateVoicePersonality(input: unknown): {
  valid: boolean;
  ...
  function checkRange(field: string, min: number, max: number, defaultVal: number): number {
    const val = raw[field];
...
export function resolveVoicePersonality(
  input?: string | VoicePersonality | VoicePersonalityPreset
): VoicePersonality {
  if (!input) {
  ...
  if (typeof input === 'string') {
    const upper = input.toUpperCase().trim() as VoicePersonalityPreset;
