...
export class ContinuousOperationalAssuranceScorer {
  ...
  public computeAssuranceScore(
    ...
    if (N === 0) {
      const scoreId = `assur_${computeSha256(`${tenantId}:${policyDomain}:quiescent:${currentTimestampMs}`).slice(0, 16)}` as AssuranceScoreId;
    ...
    const matchingViolations = violations.filter((v) =>
      windowEvaluations.some((e) => e.evaluationId === v.evaluationId)
    );
    const hasCriticalViolation = matchingViolations.some((v) => v.severity === 'CRITICAL');

    if (hasCriticalViolation) {
      const scoreId = `assur_${computeSha256(`${tenantId}:${policyDomain}:critical:${currentTimestampMs}`).slice(0, 16)}` as AssuranceScoreId;
      return Object.freeze({
        scoreId,
        tenantId,
        policyDomain,
    ...
    let state: AssuranceState = 'ASSURED_COMPLIANT';
    ...
    const scoreId = `assur_${computeSha256(`${tenantId}:${policyDomain}:${scoreValue}:${currentTimestampMs}`).slice(0, 16)}` as AssuranceScoreId;
