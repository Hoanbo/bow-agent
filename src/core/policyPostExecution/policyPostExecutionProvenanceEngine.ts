...
import {
  type PostExecutionProvenanceId,
  type PostExecutionProvenanceRecord,
...
export class PolicyPostExecutionProvenanceEngine {
  ...
  private readonly chains: Map<string, PostExecutionProvenanceRecord[]> = new Map();
  ...
  public recordTransition(input: {
    ...
    const provenanceId = createPostExecutionProvenanceId(
      `pep_prv_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`
    );
    ...
    const record: PostExecutionProvenanceRecord = Object.freeze({
      provenanceId,
      executionId: input.executionId,
      tenantPartition: input.tenantPartition,
      eventType: input.eventType,
  ...
  public getChain(executionId: ExecutionId): readonly PostExecutionProvenanceRecord[] {
