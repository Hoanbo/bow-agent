...
export class PolicyFeedbackProposalEngine {
  ...
  public generateProposal(
    ...
    effectiveness: PostExecutionEffectivenessResult
  ): PolicyFeedbackProposal {
    this.assertUserStopInactive();
    this.validateTenant(reconciliation.tenantPartition);

    let proposedAction: PolicyFeedbackActionType = 'RETAIN_CURRENT_POLICY';
    let rationale = '';
    ...
    const proposalId = createFeedbackProposalId(
      `fbp_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`
    );
