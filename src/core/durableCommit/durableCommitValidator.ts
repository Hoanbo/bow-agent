...
export class DurableCommitValidator {
   ...
   */
  public defensivelyValidatePayload(payload: unknown, depth = 0): void {
    if (payload === null || payload === undefined) {
      return;
    }

    if (depth > this.maxDepth) {
      throw new CommitSecurityViolationError(
        `Payload exceeds maximum nesting depth limit of ${this.maxDepth}`
      );
    }
    ...
    if (typeof payload === 'object') {
      ...
      for (const key of Object.getOwnPropertyNames(payload)) {
        ...
        this.defensivelyValidatePayload((payload as Record<string, unknown>)[key], depth + 1);
      }
    }
  }
   ...
   */
  public validateRequestEnvelope(request: unknown): asserts request is DurableCommitRequest {
    ...
    try {
      ...
      if (Buffer.byteLength(serialized, 'utf8') > this.maxPayloadBytes) {
        throw new CommitSecurityViolationError(
          `Commit request payload exceeds limit of ${this.maxPayloadBytes} bytes`
        );
      }
    } catch (err: any) {
      if (err instanceof CommitSecurityViolationError) throw err;
    ...
    const req = request as DurableCommitRequest;
   ...
   */
  public validateVerificationAuthority(verification: RealityVerificationResult): void {
    ...
    if (verification.status !== 'VERIFIED') {
      throw new CommitValidationError(
        `Commit authority rejected: RealityVerification status is '${verification.status}', expected 'VERIFIED'`
      );
    }
    ...
    if (!verification.summary || verification.summary.allRequiredPassed !== true) {
      throw new CommitValidationError(
        'Commit authority rejected: RealityVerification summary.allRequiredPassed is false or missing'
      );
    }
  }
  ...
  public validateBindings(
    ...
    context?: DurableCommitContext,
    expectedTaskVersion?: number
  ): void {
    // Tenant Isolation
    if (verification.tenantId.trim() !== authoritativeTask.tenantId.trim()) {
      throw new CrossTenantCommitError(
        `Cross-tenant commit violation: verification tenant '${verification.tenantId}' does not match task tenant '${authoritativeTask.tenantId}'`
      );
    }

    if (context?.tenantId && context.tenantId.trim() !== authoritativeTask.tenantId.trim()) {
      throw new CrossTenantCommitError(
        `Cross-tenant commit violation: context tenant '${context.tenantId}' does not match task tenant '${authoritativeTask.tenantId}'`
      );
    }
    ...
    if (verification.taskId.trim() !== authoritativeTask.taskId.trim()) {
      throw new CommitValidationError(
        `Task mismatch: verification taskId '${verification.taskId}' does not match task '${authoritativeTask.taskId}'`
      );
    }

    // Concurrency / Stale Task Version Check
    if (expectedTaskVersion !== undefined && expectedTaskVersion !== authoritativeTask.version) {
      throw new StaleTaskCommitError(
        `Stale task version: expected version ${expectedTaskVersion} but authoritative task is at version ${authoritativeTask.version}`
      );
    }
   ...
   */
  public validateIdentifierSafety(identifier: string, fieldName: string): void {
    ...
    if (
      ...
      throw new CommitSecurityViolationError(
        `Security violation: ${fieldName} '${trimmed}' contains forbidden traversal sequences or protected paths`
      );
    }
  }
  ...
  public sanitize<T>(data: T): T {
    return this.sanitizer.sanitize(data) as T;
  }
}
