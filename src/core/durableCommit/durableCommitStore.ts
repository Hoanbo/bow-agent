...
export class DurableCommitStore {
   ...
   */
  public computeCommitId(input: CommitIdentityInput): string {
    const raw = [
      input.tenantId.trim(),
      input.taskId.trim(),
      input.stepId.trim(),
   ...
   */
  public getTenantCommitsDir(tenantId: string): string {
   ...
   */
  public getCommitFilePath(tenantId: string, commitId: string): string {
   ...
   */
  public hasCommit(tenantId: string, commitId: string): boolean {
   ...
   */
  public saveCommit(record: DurableCommitRecord): void {
    ...
    if (fs.existsSync(targetFile)) {
      throw new DuplicateCommitError(
        `Commit '${record.commitId}' has already been persisted for tenant '${record.tenantId}'. Replay rejected.`
      );
    }

    const sanitized = this.sanitizer.sanitize(record) as DurableCommitRecord;
   ...
   */
  public getCommit(tenantId: string, commitId: string): DurableCommitRecord | undefined {
    const targetFile = this.getCommitFilePath(tenantId, commitId);
    if (!fs.existsSync(targetFile)) {
      return undefined;
    }
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as DurableCommitRecord;

      if (parsed.tenantId !== tenantId) {
        throw new CrossTenantCommitError(
          `Cross-tenant read violation: file belongs to tenant '${parsed.tenantId}', requested by '${tenantId}'`
        );
      }

      return Object.freeze(parsed);
    } catch (err: any) {
      if (err instanceof CrossTenantCommitError) throw err;
   ...
   */
  public listCommits(tenantId: string): readonly DurableCommitRecord[] {
    const commitsDir = this.getTenantCommitsDir(tenantId);
    if (!fs.existsSync(commitsDir)) {
      return [];
    }
    ...
    const results: DurableCommitRecord[] = [];
