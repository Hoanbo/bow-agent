...
import {
  ...
  type DurableCommitStatus,
  type DurableCommitAuditEventType,
...
import {
  ...
  globalDurableCommitValidator,
} from './durableCommitValidator.js';
import {
  DurableCommitExecutionGate,
  globalDurableCommitExecutionGate,
} from './durableCommitExecutionGate.js';
import {
  DurableCommitStore,
  globalDurableCommitStore,
} from './durableCommitStore.js';
import {
  globalAuditLedger,
  type AuditLedger,
} from '../auditLedger.js';
...
export class DurableCommitRuntime {
  private readonly validator: DurableCommitValidator;
  private readonly gate: DurableCommitExecutionGate;
  private readonly store: DurableCommitStore;
   ...
   */
  public calculateCommitProvenanceHash(params: {
   ...
   */
  private recordAudit(
    eventType: DurableCommitAuditEventType,
    tenantId: string,
    ...
    this.auditLedger.record({
      ...
      resultHash: (metadata.commitProvenanceHash as string) ?? '',
      actor: {
   ...
   */
  public async commitDurable(request: DurableCommitRequest): Promise<DurableCommitResult> {
    ...
    if (this.store.hasCommit(task.tenantId, commitId)) {
      ...
      throw new DuplicateCommitError(
        `Commit '${commitId}' has already been persisted for tenant '${task.tenantId}'. Replay rejected.`,
        { commitId, taskId: task.taskId, tenantId: task.tenantId }
      );
    }

    // 7. Prepare State Payload & Commit Provenance
    const committedAt = new Date().toISOString();
    const committedState: Readonly<Record<string, unknown>> = request.committedStateOverride ?? {
      verificationId: verification.verificationId,
      stepId: verification.stepId,
      executionId: verification.executionId,
    ...
    const commitRecord: DurableCommitRecord = {
      commitId,
      taskId: task.taskId,
      tenantId: task.tenantId,
      stepId: verification.stepId,
    ...
    const result: DurableCommitResult = {
      commitId,
      taskId: task.taskId,
      tenantId: task.tenantId,
      stepId: verification.stepId,
   ...
   */
  public async tryCommitDurable(request: DurableCommitRequest): Promise<DurableCommitResult> {
    try {
      ...
      let status: DurableCommitStatus = 'REJECTED';
      ...
      const failedResult: DurableCommitResult = {
