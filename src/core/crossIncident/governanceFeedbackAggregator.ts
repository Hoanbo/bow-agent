...
interface SupervisoryOutcomeCounts {
  approvals: number;
  denials: number;
  overrides: number;
}
...
export class GovernanceFeedbackAggregator {
  ...
  private readonly outcomeMap = new Map<string, SupervisoryOutcomeCounts>();

  /**
   * Records an observed supervisory interaction outcome.
   * Ghi nhận một kết quả tương tác giám sát đã quan sát.
   */
  public recordSupervisoryOutcome(
    targetScope: string,
    outcome: 'APPROVAL' | 'DENIAL' | 'OVERRIDE'
  ): void {
    ...
    if (outcome === 'APPROVAL') current.approvals++;
    else if (outcome === 'DENIAL') current.denials++;
    else if (outcome === 'OVERRIDE') current.overrides++;
   ...
   */
  public generateAdvisory(targetScope: string, actionClass: string): PolicyRefinementAdvisory {
