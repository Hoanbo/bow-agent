...
export class CrossIncidentProvenanceEngine {
  ...
  public computeProvenanceHash(
    reportId: string,
    clusterHash: string,
    sortedConstituentHashes: readonly string[],
    timestamp: number
  ): string {
    const payload = [
      reportId,
   ...
   */
  public generateProvenance(input: GenerateProvenanceInput): CrossIncidentProvenanceRecord {
    if (!input.reportId) {
  ...
  public verifyProvenance(
    record: CrossIncidentProvenanceRecord,
    actualConstituentHashes: readonly string[]
  ): boolean {
    ...
    const expectedHash = this.computeProvenanceHash(
      record.reportId,
      record.clusterHash,
      sortedActual,
      record.timestamp
