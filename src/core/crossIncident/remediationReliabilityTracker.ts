...
interface StratifiedAccumulator {
  sampleCount: number;
  successfulOutcomes: number;
  rollbackOutcomes: number;
...
export class RemediationReliabilityTracker {
  ...
  private readonly stratifiedData = new Map<string, StratifiedAccumulator>();
   ...
   */
  public serializeKey(key: StratifiedRemediationKey): string {
   ...
   */
  public deserializeKey(serialized: string): StratifiedRemediationKey {
    const parts = serialized.split('::');
    return {
      targetId: parts[0],
      failureCategory: parts[1],
      actionClass: parts[2] as RemediationActionClass,
    };
  }
   ...
   */
  public recordOutcome(record: ArchivedIncidentRecord): void {
    ...
    const key: StratifiedRemediationKey = {
   ...
   */
  public ingestRecords(records: readonly ArchivedIncidentRecord[]): void {
   ...
   */
  public getStratifiedReliability(key: StratifiedRemediationKey): RemediationReliabilityRecord | null {
    const serialized = this.serializeKey(key);
   ...
   */
  public getAllStratifiedRecords(): readonly RemediationReliabilityRecord[] {
    const results: RemediationReliabilityRecord[] = [];
   ...
   */
  public getGlobalEffectivenessScore(): never {
    throw new Error(
      'FORBIDDEN_UNSTRATIFIED_QUERY: Global unstratified effectiveness is strictly forbidden to prevent Simpsons paradox.'
    );
  }
