...
import {
  ...
  type HypothesisOutcomeSource,
  type HypothesisReliabilityScore,
  type HypothesisReliabilityStatus,
...
export class HypothesisReliabilityLedger {
  public static readonly MINIMUM_PUBLICATION_SAMPLE_SIZE = 3;

  // In-memory accumulation table: hypothesisType -> counts
  private readonly ledger = new Map<string, HypothesisOutcomeCounts>();

  /**
   * Records a single hypothesis outcome into the historical ledger.
   * Ghi nhận một kết quả giả thuyết vào sổ cái lịch sử.
   */
  public recordOutcome(hypothesisType: string, outcome: HypothesisOutcomeSource): void {
   ...
   */
  public ingestFromArchivedRecords(records: readonly ArchivedIncidentRecord[]): void {
    for (const record of records) {
      ...
      if (pmr?.hypothesisAccuracy) {
        ...
        let sourceOutcome: HypothesisOutcomeSource;
        if (outcome === 'SUPPORTED') sourceOutcome = 'SUPPORTED';
        else if (outcome === 'PARTIALLY_SUPPORTED') sourceOutcome = 'PARTIALLY_SUPPORTED';
        else if (outcome === 'REFUTED') sourceOutcome = 'REFUTED';
        else sourceOutcome = 'INCONCLUSIVE';
   ...
   */
  public getReliability(hypothesisType: string): HypothesisReliabilityScore | null {
   ...
   */
  public getAllScores(): readonly HypothesisReliabilityScore[] {
    const results: HypothesisReliabilityScore[] = [];
