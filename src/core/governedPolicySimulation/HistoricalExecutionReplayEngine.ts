...
export class HistoricalExecutionReplayEngine {
  ...
  private assertEmergencyStopInactive(): void {
    ...
    try {
      ...
      throw new EmergencyStopActiveError(
        `Emergency stop provider threw error during simulation: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  ...
  public replayCorpus(
    ...
    historicalCorpus: readonly HistoricalObservationItem[]
  ): ReplayExecutionResult {
    ...
    for (const item of historicalCorpus) {
      if (item.tenantId !== cleanTenant || item.policyDomain !== policyDomain) {
        throw new SimulationCrossTenantAccessForbiddenError(
          `Cross-tenant replay violation: item tenant ${item.tenantId} does not match caller ${cleanTenant}`
        );
      }
    }
