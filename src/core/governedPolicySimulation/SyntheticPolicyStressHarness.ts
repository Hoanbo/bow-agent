...
export class SyntheticPolicyStressHarness {
  ...
  private assertEmergencyStopInactive(): void {
    ...
    try {
      ...
      throw new EmergencyStopActiveError(
        `Emergency stop provider threw error during synthetic stress test: ${err instanceof Error ? err.message : String(err)}`
      );
    }
   ...
   */
  public executeStressHarness(
    candidateDeltas: readonly PolicyDelta[],
    config?: Partial<SyntheticStressConfig>
  ): SyntheticStressResult {
    ...
    const boundaryBreakages: string[] = [];
    let passedProbes = 0;
    let failedProbes = 0;
