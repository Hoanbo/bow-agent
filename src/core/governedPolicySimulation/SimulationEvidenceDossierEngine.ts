...
export class SimulationEvidenceDossierEngine {
  ...
  private assertEmergencyStopInactive(): void {
    ...
    try {
      ...
      throw new EmergencyStopActiveError(
        `Emergency stop provider threw error during dossier compilation: ${err instanceof Error ? err.message : String(err)}`
      );
    }
   ...
   */
  public compileSimulationDossier(input: DossierCompilationInput): PolicySimulationEvidenceDossier {
    ...
    let overallVerdict: SimulationVerdict = 'STABLE';
    ...
    const payloadForFingerprint: Record<string, unknown> = {
      dossierId,
    ...
    const dossier: PolicySimulationEvidenceDossier = {
      dossierId,
      sessionId: input.sessionId,
      tenantId: cleanTenant,
      policyDomain: input.policyDomain,
