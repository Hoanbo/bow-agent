...
import {
  ...
  CounterfactualAssuranceProjection,
...
export class CounterfactualAssuranceProjector {
  ...
  private readonly falseRejectionThreshold: number;

  constructor(emergencyStopProvider?: EmergencyStopProvider, falseRejectionThreshold = DEFAULT_FALSE_REJECTION_THRESHOLD) {
  ...
  private assertEmergencyStopInactive(): void {
    ...
    try {
      ...
      throw new EmergencyStopActiveError(
        `Emergency stop provider threw error during assurance projection: ${err instanceof Error ? err.message : String(err)}`
      );
    }
   ...
   */
  public projectAssuranceImpact(
    replayResult: ReplayExecutionResult,
    baselineAssurance: number
  ): CounterfactualAssuranceProjection {
    ...
    const falsePositiveRejectionRate =
      totalValidBefore > 0 ? replayResult.falseRejectionCount / totalValidBefore : 0;
