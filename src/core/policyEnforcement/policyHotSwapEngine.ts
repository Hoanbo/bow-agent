...
export class PolicyHotSwapEngine {
  ...
  private readonly activePolicies = new Map<string, ActivePolicyReference>();

  // In-flight execution counters per tenant partition
  // Bộ đếm thực thi đang diễn ra trên mỗi phân vùng người thuê
  private readonly inFlightExecutions = new Map<string, number>();
   ...
   */
  public getActivePolicy(tenantPartition: string): ActivePolicyReference {
    ...
    const fallbackRef: ActivePolicyReference = {
  ...
  public swapPolicy(params: {
    ...
    if (!this.fallbackProvider.verifyClassificationSafetyFloor(newConfig.actionClassifications)) {
      throw new Error(
        'HOT_SWAP_REJECTED: FORBIDDEN_DOWNGRADE_ATTEMPT — New policy attempts to downgrade a hard-forbidden action.'
      );
    }
    ...
    if (currentRef && !currentRef.isFallback) {
      if (newConfig.activeSince < currentRef.policyConfig.activeSince) {
        throw new Error(
          `HOT_SWAP_REJECTED: Stale policy activation detected. Incoming activeSince (${newConfig.activeSince}) is older than current (${currentRef.policyConfig.activeSince}).`
        );
      }
    }

    // 6. Build new immutable active policy reference
    const activePolicyId = createActivePolicyId(
      `active_${newConfig.versionId.replace(/[^a-zA-Z0-9]/g, '_').slice(0, 24)}_${Date.now()}`
    );

    const newRef: ActivePolicyReference = {
      activePolicyId,
      tenantPartition,
   ...
   */
  public resetToBaseline(tenantPartition: string, reason: string): ActivePolicyReference {
    const baseline = this.fallbackProvider.getBaselineConfiguration();
    const fallbackRef: ActivePolicyReference = {
   ...
   */
  public getRuntimeState(tenantPartition: string): RuntimePolicyState {
