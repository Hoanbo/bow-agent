...
import {
  ...
  type UserPartition,
} from '../persistence/userPartitionResolver.js';
import {
...
export class ActivePolicyResolver {
   ...
   */
  public resolveActivePolicy(actorUserId?: string): ActivePolicyResolutionResult {
   ...
   */
  public verifyChecksum(config: PolicyConfiguration, provenanceSha256?: string): boolean {
    if (!config || !config.checksum) return false;
