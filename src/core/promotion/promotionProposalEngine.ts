  ...
  public createProposal(input: CreateProposalInput): PromotionProposal {
    ...
    if (input.diff.sandboxId !== input.sandbox.id) {
      throw new PromotionError(
        'DIFF_MISMATCH',
        `Diff sandboxId "${input.diff.sandboxId}" does not match sandbox "${input.sandbox.id}".`
      );
    }
    ...
    if (input.diff.baseManifestHash !== input.baseManifest.manifestHash) {
      throw new PromotionError(
        ...
        `Diff base manifest hash "${input.diff.baseManifestHash}" does not match provided base manifest hash "${input.baseManifest.manifestHash}".`
      );
    }
    ...
    if (input.diff.targetManifestHash !== input.currentManifest.manifestHash) {
      throw new PromotionError(
        'MANIFEST_HASH_MISMATCH',
        `Diff target manifest hash "${input.diff.targetManifestHash}" does not match current manifest hash "${input.currentManifest.manifestHash}".`
      );
    }

    const proposalHash = PromotionProposalEngine.hashProposal(
      input.sandbox.id,
      input.targetProjectRoot,
      input.diff.diffHash,
      input.currentManifest.manifestHash,
      input.diff.changes.length
    );
