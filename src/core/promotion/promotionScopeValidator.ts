...
export class PromotionScopeValidator {
  ...
  public static assertNotProtectedWorkspace(targetPath: string): void {
    try {
      ...
      throw new PromotionError(
        'SECURITY_VIOLATION',
        `Target path "${targetPath}" targets protected workspace C:\\BOW\\shopofbow. Promotion denied.`
      );
    }
  }
   ...
   */
  public static assertPathContained(rootDir: string, relativePath: string): string {
    try {
      ...
      throw new PromotionError(
        ...
        `Path traversal detected: "${relativePath}" escapes root "${rootDir}".`
      );
    }
  }
   ...
   */
  public static assertSessionIntegrity(proposal: PromotionProposal, expectedSessionId: string): void {
    if (proposal.sessionId !== expectedSessionId) {
      throw new PromotionError(
        'SESSION_MISMATCH',
        `Proposal session "${proposal.sessionId}" does not match expected session "${expectedSessionId}".`
      );
    }
  }
  ...
  public static validateScope(
    ...
    if (proposal.sessionId !== sandbox.binding.sessionId) {
      throw new PromotionError(
        'CROSS_SESSION_PROMOTION_REJECTED',
        `Proposal session "${proposal.sessionId}" does not match sandbox session "${sandbox.binding.sessionId}".`
      );
    }
    ...
    if (proposal.taskId !== sandbox.binding.taskId) {
      throw new PromotionError(
        'TASK_MISMATCH',
        `Proposal task "${proposal.taskId}" does not match sandbox task "${sandbox.binding.taskId}".`
      );
    }
    ...
    if (proposal.agentId !== sandbox.binding.agentId) {
      throw new PromotionError(
        'AGENT_MISMATCH',
        `Proposal agent "${proposal.agentId}" does not match sandbox agent "${sandbox.binding.agentId}".`
      );
    }
    ...
    if (proposal.deviceId !== sandbox.binding.deviceId) {
      throw new PromotionError(
        'DEVICE_MISMATCH',
        `Proposal device "${proposal.deviceId}" does not match sandbox device "${sandbox.binding.deviceId}".`
      );
    }
    ...
    if (!isTargetAuthorized) {
      throw new PromotionError(
        ...
        `Target project root "${proposal.targetProjectRoot}" is not authorized by sandbox scope.`
      );
    }
    ...
    if (proposal.expiresAt > sandbox.expiresAt) {
      throw new PromotionError(
        'SCOPE_EXCEEDED',
        `Proposal expiration (${new Date(proposal.expiresAt).toISOString()}) exceeds sandbox expiration (${new Date(sandbox.expiresAt).toISOString()}).`
      );
    }
    ...
    if (
      sandbox.scope.maxFileCount !== undefined &&
      proposal.proposedChanges.length > sandbox.scope.maxFileCount
    ) {
      throw new PromotionError(
        'FILE_LIMIT_EXCEEDED',
        `Proposal change count (${proposal.proposedChanges.length}) exceeds sandbox file limit (${sandbox.scope.maxFileCount}).`
      );
    }
    ...
    if (sandbox.scope.allowedFileExtensions && sandbox.scope.allowedFileExtensions.length > 0) {
      ...
      for (const change of proposal.proposedChanges) {
        ...
        if (ext && !allowedExts.has(ext)) {
          throw new PromotionError(
            'FILE_EXTENSION_DISALLOWED',
            `File extension "${ext}" for "${change.relativePath}" is disallowed by sandbox scope.`
          );
        }
      }
    }
    ...
    if (delegation) {
      if (proposal.delegationId !== delegation.delegationId) {
        throw new PromotionError(
          'DELEGATION_MISMATCH',
          `Proposal delegation "${proposal.delegationId}" does not match active delegation "${delegation.delegationId}".`
        );
      }
      if (delegation.status === 'REVOKED') {
        throw new PromotionError(
          'REVOKED_DELEGATION',
          `Parent delegation "${delegation.delegationId}" is revoked.`
        );
      }
      if (Date.now() > delegation.expiresAt) {
        throw new PromotionError(
          'DELEGATION_EXPIRED',
          `Parent delegation "${delegation.delegationId}" has expired.`
        );
      }
    }
    ...
    if (lease) {
      if (proposal.capabilityLeaseId !== lease.leaseId) {
        throw new PromotionError(
          'LEASE_MISMATCH',
          `Proposal lease "${proposal.capabilityLeaseId}" does not match active lease "${lease.leaseId}".`
        );
      }
      if (lease.status === 'REVOKED') {
        throw new PromotionError(
          'REVOKED_CAPABILITY',
          `Capability lease "${lease.leaseId}" is revoked.`
        );
      }
      if (Date.now() > lease.expiresAt) {
        throw new PromotionError(
          'LEASE_EXPIRED',
          `Capability lease "${lease.leaseId}" has expired.`
        );
      }
    }
  }
