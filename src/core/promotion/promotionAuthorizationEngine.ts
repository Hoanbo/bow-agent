  ...
  public issuePromotionToken(input: IssuePromotionTokenInput): AuthorizationToken {
    ...
    if (input.approval.decision !== 'APPROVED') {
      throw new PromotionError(
        ...
        `Cannot issue token for promotion "${input.proposal.promotionId}" with decision "${input.approval.decision}".`
      );
    }
    ...
    if (input.approval.promotionId !== input.proposal.promotionId) {
      throw new PromotionError(
        'APPROVAL_MISMATCH',
        `Approval record promotionId "${input.approval.promotionId}" does not match proposal "${input.proposal.promotionId}".`
      );
    }
