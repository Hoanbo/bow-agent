...
export class PolicyHealthObservationEngine {
  constructor(private readonly auditLedger?: PolicyLifecycleAuditLedger) {}

  /**
   * Deterministically calculate composite health score H in [0.0, 1.0] from raw metrics.
   */
  public calculateHealthScore(metrics: PolicyHealthMetrics): number {
  ...
  public evaluateHealth(params: {
    ...
    let status: PolicyHealthReport['status'] = 'HEALTHY';
    ...
    const rawReport: Omit<PolicyHealthReport, 'reportHash'> = {
      reportId,
    ...
    const frozenReport: PolicyHealthReport = deepFreeze({
      ...rawReport,
      reportHash,
    });
   ...
   */
  public aggregateSampleMetrics(
    samples: readonly DecisionTraceSample[],
    interDomainConflicts = 0
  ): PolicyHealthMetrics {
    ...
    for (const sample of samples) {
      if (sample.matchedRule) matches++;
      if (sample.divergedFromBaseline) divergences++;
