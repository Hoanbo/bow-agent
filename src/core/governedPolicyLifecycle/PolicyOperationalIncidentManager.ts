...
export class PolicyOperationalIncidentManager {
  private readonly incidents = new Map<string, PolicyIncidentRecord>(); // incidentId -> record
  private readonly activeIncidentIndex = new Map<string, Set<string>>(); // tenant:domain -> Set<incidentId>
  ...
  public openIncident(params: {
    ...
    const activeSet = this.activeIncidentIndex.get(scopeKey) || new Set<string>();

    if (activeSet.size >= MAX_ACTIVE_INCIDENTS_PER_DOMAIN) {
      throw new PolicyIncidentManagementError(
        `INCIDENT_CAPACITY_EXCEEDED: Tenant '${params.tenantId}' domain '${params.policyDomain}' has reached maximum active incidents ceiling (${MAX_ACTIVE_INCIDENTS_PER_DOMAIN}).`
      );
    }

    const incidentId = `inc_${params.tenantId}_${params.policyDomain}_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;
    
    let triggeredState: PolicyLifecycleState | undefined = undefined;
    ...
    const rawRecord: Omit<PolicyIncidentRecord, 'incidentHash'> = {
      incidentId,
      tenantId: params.tenantId,
      policyDomain: params.policyDomain,
    ...
    const frozenRecord: PolicyIncidentRecord = deepFreeze({
      ...rawRecord,
      incidentHash,
    });
  ...
  public resolveIncident(params: {
    ...
    if (!params.resolutionJustification || params.resolutionJustification.trim().length < 10) {
      throw new PolicyIncidentManagementError(
        `RESOLUTION_JUSTIFICATION_TOO_SHORT: Meaningful root-cause justification (>= 10 chars) is required to resolve incident.`
      );
    }

    const updatedRaw: Omit<PolicyIncidentRecord, 'incidentHash'> = {
      ...existing,
      status: 'RESOLVED',
      resolvedAt: Date.now(),
    ...
    const frozenUpdated: PolicyIncidentRecord = deepFreeze({
      ...updatedRaw,
      incidentHash,
    });
  ...
  public getIncident(incidentId: string): PolicyIncidentRecord | undefined {
    return this.incidents.get(incidentId);
  }

  public getActiveIncidents(tenantId: string, policyDomain: PolicyDomain): readonly PolicyIncidentRecord[] {
    const scopeKey = `${tenantId}:${policyDomain}`;
    const activeSet = this.activeIncidentIndex.get(scopeKey);
    if (!activeSet || activeSet.size === 0) return [];

    const result: PolicyIncidentRecord[] = [];
