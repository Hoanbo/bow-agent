...
export class PolicyLifecycleStateManager {
  private readonly stateStore = new Map<string, PolicyLifecycleRecord>(); // key: tenant:domain:policyId
  private readonly singleFlightLocks = new Set<string>(); // key: tenant:domain
  private readonly partitionDir: string;

  constructor(
    customStoreDir?: string,
    private readonly auditLedger?: PolicyLifecycleAuditLedger,
   ...
   */
  public transitionState(params: TransitionRequestParams): PolicyLifecycleRecord {
    ...
    if (this.singleFlightLocks.has(lockKey)) {
      throw new PolicyLifecycleInterlockActiveError(
        `CONCURRENT_LIFECYCLE_MUTATION_BLOCKED: A lifecycle transition is already in-flight for tenant '${params.tenantId}' domain '${params.policyDomain}'.`
      );
    }
    ...
    try {
      ...
      const currentState: PolicyLifecycleState = currentStateRecord ? currentStateRecord.state : 'PROPOSED';
      ...
      if (params.expectedVersion !== undefined && params.expectedVersion !== currentVersion) {
        throw new PolicyLifecycleOCCConflictError(
          `OCC_CONFLICT: Expected lifecycleVersion ${params.expectedVersion}, but current is ${currentVersion}. Mutation aborted.`
        );
      }

      // 2. Terminal State Guard
      if (currentStateRecord && TERMINAL_LIFECYCLE_STATES.has(currentStateRecord.state)) {
        throw new PolicyLifecycleTerminalStateError(
          `TERMINAL_STATE_VIOLATION: Policy is in terminal state '${currentStateRecord.state}'. Resurrection is strictly forbidden.`
        );
      }
      ...
      const rawRecord: Omit<PolicyLifecycleRecord, 'recordHash'> = {
      ...
      const frozenRecord: PolicyLifecycleRecord = deepFreeze({
        ...rawRecord,
        recordHash,
      });
  ...
  private assertValidTransition(
    ...
    params: TransitionRequestParams
  ): void {
    if (from === to) {
      throw new InvalidLifecycleTransitionError(
        `NOOP_TRANSITION_REJECTED: State is already '${from}'. Redundant transition is rejected.`
      );
    }

    // Explicit Transition Rules
    const allowedTransitions: Record<PolicyLifecycleState, readonly PolicyLifecycleState[]> = {
      PROPOSED: ['RATIFIED'],
    ...
    if (!legalNextStates.includes(to)) {
      throw new InvalidLifecycleTransitionError(
        `ILLEGAL_LIFECYCLE_JUMP: Transition from '${from}' to '${to}' is prohibited by the authoritative FSM.`
      );
    }
    ...
    if (from === 'SUSPENDED' && to === 'ACTIVE') {
      if (params.trigger === 'AUTOMATIC_SAFETY_INTERLOCK' || params.trigger === 'HEALTH_DRIFT') {
        throw new UnauthorizedLifecycleMutationError(
          `AUTOMATION_REACTIVATION_FORBIDDEN: Suspended policy cannot be reactivated by automated triggers or health metrics. Human authorization is required.`
        );
      }
      if (!params.authorizationRef || !params.authorizationRef.tokenSignature) {
        throw new UnauthorizedLifecycleMutationError(
          `UNAUTHORIZED_REACTIVATION: Cryptographic Human Authority token is required to reinstate a SUSPENDED policy.`
        );
      }
    }
    ...
    if (from === 'DEGRADED' && to === 'ACTIVE') {
      if (!params.authorizationRef || !params.authorizationRef.tokenSignature) {
        throw new UnauthorizedLifecycleMutationError(
          `UNAUTHORIZED_DEGRADATION_OVERRIDE: Cryptographic Human Authority token is required to restore an ACTIVE policy from DEGRADED state.`
        );
      }
    }
    ...
    if (to === 'RETIRED') {
      if (!params.authorizationRef || !params.authorizationRef.tokenSignature) {
        throw new UnauthorizedLifecycleMutationError(
          `UNAUTHORIZED_RETIREMENT: Cryptographic Human Authority token is required to decommission and permanently retire a policy.`
        );
      }
    }
  }
    ...
    for (const pattern of forbiddenPatterns) {
      if (pattern.test(tenantId) || pattern.test(policyDomain)) {
        throw new PolicyLifecycleTenantIsolationError(
          `MALFORMED_TENANT_OR_DOMAIN: Input contains forbidden path traversal, NUL byte, or reserved device name.`
        );
      }
    }
  }
   ...
   */
  private persistRecordAtomically(record: PolicyLifecycleRecord): void {
  ...
  private loadFromDisk(
    ...
    try {
      ...
      const parsed = JSON.parse(content) as PolicyLifecycleRecord;
