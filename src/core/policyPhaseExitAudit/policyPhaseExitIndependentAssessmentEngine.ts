...
export const CANONICAL_AUDIT_CRITERIA: readonly AuditCriterionDefinition[] = Object.freeze([
...
export class PolicyPhaseExitIndependentAssessmentEngine {
  ...
  public conductAssessment(params: {
    ...
    const evaluations: AuditCriterionEvaluation[] = [];
    ...
    for (const def of CANONICAL_AUDIT_CRITERIA) {
      ...
      if (matchedEvidence.length === 0) {
        ...
        if (def.isMandatory) unresolvedRisks.push(`Missing evidence for mandatory criterion: ${def.name}`);
        continue;
      }
      ...
      for (const ev of matchedEvidence) {
        if (!ev.verified) allVerified = false;
    ...
    const readinessStatus: AuditReadinessStatus =
      allMandatoryPassed && unresolvedRisks.length === 0
        ? 'READY_FOR_PHASE_EXIT_REVIEW'
        : 'NOT_READY';

    const coveragePercentage = totalCriteriaCount > 0 ? Math.round((passedCount / totalCriteriaCount) * 100) : 0;

    const auditedMilestones: GovernanceMilestone[] = [
