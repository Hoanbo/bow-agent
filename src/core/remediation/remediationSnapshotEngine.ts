...
import {
  ...
  type RemediationSnapshotItem,
  type RemediationSnapshotId,
...
export class RemediationSnapshotEngine {
  private readonly snapshots = new Map<RemediationSnapshotId, RemediationSnapshot>();
   ...
   */
  public capturePreRemediationSnapshot(options: CaptureSnapshotOptions): RemediationSnapshot {
    ...
    const items: RemediationSnapshotItem[] = [];
    ...
    if (plan.targetPath) {
      try {
        ...
        throw new RemediationSnapshotError(
          'SNAPSHOT_FILESYSTEM_READ_FAILED',
          `Failed to read filesystem target during snapshot: ${err.message}`
        );
      }
    }
    ...
    if (storageDirectory) {
      try {
        ...
        throw new RemediationSnapshotError(
          ...
          `Failed to persist snapshot to storageDirectory: ${err.message}`
        );
      }
    }
   ...
   */
  public getSnapshot(snapshotId: RemediationSnapshotId): RemediationSnapshot | undefined {
   ...
   */
  public computeSnapshotSha256(
    snapshotId: RemediationSnapshotId,
    plan: GovernedRemediationPlan,
    items: readonly RemediationSnapshotItem[]
  ): string {
    const rawTokens = [
      snapshotId,
      plan.planId,
