...
import { RemediationTokenValidator, RemediationTokenValidationError } from './remediationTokenValidator.js';
...
export class RemediationRuntime {
  private isUserStopActiveFlag = false;

  constructor(
    public readonly tokenValidator: RemediationTokenValidator = new RemediationTokenValidator(),
    public readonly snapshotEngine: RemediationSnapshotEngine = new RemediationSnapshotEngine(),
    public readonly executionEngine: RemediationExecutionEngine = new RemediationExecutionEngine(),
    public readonly verificationEngine: PostMitigationVerificationEngine = new PostMitigationVerificationEngine(),
    public readonly rollbackEngine: RemediationRollbackEngine = new RemediationRollbackEngine(),
    public readonly provenanceEngine: RemediationProvenanceEngine = new RemediationProvenanceEngine(),
    public readonly observabilityRuntime: ObservabilityRuntime = new ObservabilityRuntime(),
    public readonly supervisorHumanGate: SupervisorHumanGate = globalSupervisorHumanGate,
    public readonly masterAuthority: MasterHumanAuthority = new MasterHumanAuthority(),
  ...
  public setUserStop(active: boolean): void {
    ...
    this.logAudit(
      active ? 'USER_STOP_ACTIVATED' : 'USER_STOP_CLEARED',
      { id: 'MASTER_OPERATOR', role: 'SUPERVISOR' },
      { id: 'GLOBAL', type: 'EMERGENCY_STOP' },
      { active }
    );
  }
  ...
  private logAudit(
    ...
    policyDecision: 'PERMIT' | 'DENY' = 'PERMIT',
    executionStatus: 'SUCCESS' | 'FAILURE' | 'BLOCKED' = 'SUCCESS'
  ): void {
    ...
    this.auditLedger.record({
      ...
      toolName,
      classification:
        toolName.includes('SECURITY') || toolName.includes('STOP') || toolName.includes('ESCALAT')
          ? 'SAFETY'
          : 'GOVERNANCE',
      argumentsHash,
      policyDecision,
  ...
  public async executeRemediation(
    request: ExecuteGovernedRemediationRequest
  ): Promise<GovernedRemediationPipelineResponse> {
    const {
      plan,
      token,
      operatorId,
    ...
    try {
      ...
      this.logAudit(
        'SECURITY_VIOLATION',
        { id: operatorId || 'UNKNOWN', role: 'SECURITY' },
        { id: plan.planId, type: 'REMEDIATION_PLAN' },
        { targetId: plan.targetId, targetPath: plan.targetPath, error: err.message },
        'DENY',
        'BLOCKED'
      );
      const executionResult: RemediationExecutionResult = {
        executionId: `exec_sec_${dummyExecutionId}` as any,
        planId: plan.planId,
    ...
    if (this.isUserStopActive()) {
      this.logAudit(
        'USER_STOP_ABORT',
        { id: operatorId || 'MASTER_OPERATOR', role: 'OPERATOR' },
        { id: plan.planId, type: 'REMEDIATION_PLAN' },
        { reason: 'Remediation aborted prior to start by active USER_STOP' },
        'DENY',
        'BLOCKED'
      );
      const executionResult: RemediationExecutionResult = {
        executionId: `exec_abort_${dummyExecutionId}` as any,
        planId: plan.planId,
    ...
    let validatedTokenContext: ReturnType<RemediationTokenValidator['validateAndConsume']>;
    try {
      ...
      this.logAudit(
        ...
        { id: validatedTokenContext.operatorId, role: 'MASTER_OPERATOR' },
        { id: validatedTokenContext.tokenId, type: 'AUTHORIZATION_TOKEN' },
        { planId: plan.planId, actionId: plan.actionId }
      );
      ...
      this.logAudit(
        ...
        { id: validatedTokenContext.operatorId, role: 'MASTER_OPERATOR' },
        { id: validatedTokenContext.tokenId, type: 'AUTHORIZATION_TOKEN' },
        { planId: plan.planId, consumedAt: validatedTokenContext.consumedAt }
      );
    } catch (err: any) {
      this.logAudit(
        'UNAUTHORIZED_EXECUTION_ATTEMPT',
        { id: operatorId || 'UNKNOWN', role: 'REMEDIATION' },
        { id: plan.planId, type: 'REMEDIATION_PLAN' },
        { error: err.message },
        'DENY',
        'BLOCKED'
      ...
      const executionResult: RemediationExecutionResult = {
        executionId: `exec_unauth_${dummyExecutionId}` as any,
        planId: plan.planId,
    ...
    try {
      ...
      this.logAudit(
        'SNAPSHOT_CREATED',
        { id: validatedTokenContext.operatorId, role: 'REMEDIATION_ENGINE' },
        { id: snapshot.snapshotId, type: 'REMEDIATION_SNAPSHOT' },
        { planId: plan.planId, snapshotSha256: snapshot.snapshotSha256, itemCount: snapshot.items.length }
      );
    } catch (err: any) {
      this.logAudit(
        'SNAPSHOT_FAILED',
      ...
      const executionResult: RemediationExecutionResult = {
        executionId: `exec_snap_fail_${dummyExecutionId}` as any,
        planId: plan.planId,
    ...
    this.logAudit(
      ...
      { id: validatedTokenContext.operatorId, role: 'REMEDIATION_ENGINE' },
      { id: plan.planId, type: 'REMEDIATION_PLAN' },
      { actionClass: plan.actionClass, targetId: plan.targetId }
    );
    ...
    this.logAudit(
      ...
      { id: validatedTokenContext.operatorId, role: 'REMEDIATION_ENGINE' },
      { id: executionResult.executionId, type: 'REMEDIATION_EXECUTION' },
      {
        planId: plan.planId,
    ...
    this.logAudit(
      ...
      { id: validatedTokenContext.operatorId, role: 'VERIFICATION_ENGINE' },
      { id: executionResult.executionId, type: 'REMEDIATION_EXECUTION' },
      { windowMs: 30000 }
    );
    ...
    let finalLifecycleState: RemediationLifecycleState = executionResult.lifecycleState;
    let rollbackResult: RemediationRollbackResult | undefined;
    ...
    if (verificationResult.passed && !this.isUserStopActive()) {
      ...
      this.logAudit(
        'REMEDIATION_CLOSED',
        { id: validatedTokenContext.operatorId, role: 'SUPERVISOR' },
        { id: plan.planId, type: 'REMEDIATION_PLAN' },
        { outcome: 'SUCCESS' }
      );
    } else {
      ...
      this.logAudit(
        'ROLLBACK_STARTED',
        { id: validatedTokenContext.operatorId, role: 'ROLLBACK_ENGINE' },
        { id: snapshot.snapshotId, type: 'REMEDIATION_SNAPSHOT' },
        { reason }
      );
      ...
      if (rollbackResult.success) {
        ...
        this.logAudit(
          'ROLLBACK_COMPLETED',
          { id: validatedTokenContext.operatorId, role: 'ROLLBACK_ENGINE' },
          { id: rollbackResult.snapshotId, type: 'REMEDIATION_SNAPSHOT' },
          { restoredItemsCount: rollbackResult.restoredItems.length }
        );
      } else {
      ...
      this.logAudit(
        ...
        { id: validatedTokenContext.operatorId, role: 'SUPERVISOR' },
        { id: plan.planId, type: 'REMEDIATION_PLAN' },
        { outcome: finalLifecycleState }
      );
    }

    // 7. Update final execution result DTO
    // 7. Cập nhật DTO kết quả thực thi cuối cùng
    const finalExecutionResult: RemediationExecutionResult = {
      ...executionResult,
