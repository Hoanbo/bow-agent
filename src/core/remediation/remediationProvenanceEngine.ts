...
export class RemediationProvenanceEngine {
   ...
   */
  public sanitizeSecrets(obj: unknown): unknown {
   ...
   */
  public buildProvenanceRecord(options: BuildProvenanceOptions): RemediationProvenanceRecord {
  ...
  public verifyProvenanceIntegrity(
    record: RemediationProvenanceRecord,
    tokenId: string,
    snapshotSha256: string,
    executionSha256: string,
    verification?: { sha256: string; passed: boolean },
    rollback?: { sha256: string; success: boolean }
  ): boolean {
    const tokenDigest = crypto.createHash('sha256').update(tokenId, 'utf8').digest('hex');
