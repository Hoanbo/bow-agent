...
export class KnowledgeContinuityPersistenceBridge {
  ...
  private readonly auditRecords: KnowledgeAuditRecord[] = [];
  private readonly snapshots: Map<string, KnowledgeContinuitySnapshot> = new Map();
  ...
  public emitAudit(
    ...
    const base: Omit<KnowledgeAuditRecord, 'eventHash'> = {
      eventId,
      eventType,
      timestamp: now,
      tenantId,
    ...
    const record: KnowledgeAuditRecord = {
      ...base,
      eventHash,
    };
  ...
  public getAuditChain(): readonly KnowledgeAuditRecord[] {
   ...
   */
  public recordSnapshot(state: KnowledgeState): KnowledgeContinuitySnapshot {
   ...
   */
  public detectDrift(
    snapshot: KnowledgeContinuitySnapshot,
    currentState: KnowledgeState
  ): KnowledgeDriftCategory | undefined {
    if (snapshot.tenantId !== currentState.tenantId) return 'AUTHORIZATION_DRIFT';
    if (snapshot.sessionId !== currentState.sessionId) return 'AUTHORIZATION_DRIFT';
    if (snapshot.federationId !== currentState.federationId) return 'FEDERATION_DRIFT';
    if (snapshot.generation > currentState.generation) return 'GENERATION_DRIFT';
    if (snapshot.stateVersion > currentState.version) return 'KNOWLEDGE_STATE_DRIFT';
    if (snapshot.entryCount > Object.keys(currentState.entries).length) return 'KNOWLEDGE_ENTRY_DRIFT';
    return undefined;
  }
   ...
   */
  public saveState(state: KnowledgeState, expectedVersion?: number): void {
    ...
    if (fs.existsSync(canonicalPath)) {
      try {
        ...
        if (err instanceof GovernedFederatedKnowledgeStateConcurrencyError) throw err;
      }
    }
   ...
   */
  public recoverState(tenantId: string, stateId: string): KnowledgeState {
    ...
    let canonicalObj: KnowledgeState | undefined;
    ...
    throw new GovernedFederatedKnowledgeStatePersistenceError(
      `State '${stateId}' recovery failed: both canonical and backup files corrupted or missing`
    );
  }
}
