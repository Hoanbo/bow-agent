...
export class KnowledgeLineageEngine {
  private readonly lineages: Map<string, KnowledgeLineageRecord> = new Map();

  /**
   * EN: Registers a new immutable lineage record.
   * VI: Đăng ký một bản ghi huyết thống bất biến mới.
   */
  public recordLineage(params: CreateLineageParams): KnowledgeLineageRecord {
    ...
    if (this.lineages.has(params.lineageId)) {
      throw new GovernedFederatedKnowledgeStateValidationError(
        `Lineage record '${params.lineageId}' already exists`
      );
    }
    ...
    for (const parentId of params.parentLineageIds) {
      ...
      if (!parent) {
        throw new GovernedFederatedKnowledgeStateLineageError(
          `Parent lineage '${parentId}' not found in lineage registry`
        );
      }
    ...
    if (currentDepth > MAX_LINEAGE_DEPTH) {
      throw new GovernedFederatedKnowledgeStateLineageError(
        `Lineage exceeded MAX_LINEAGE_DEPTH limit (${MAX_LINEAGE_DEPTH}): depth is ${currentDepth}`
      );
    }

    const now = Date.now();
    const base: Omit<KnowledgeLineageRecord, 'provenanceHash'> = {
   ...
   */
  public verifyLineageIntegrity(lineageId: string): boolean {
   ...
   */
  public getLineage(lineageId: string): KnowledgeLineageRecord | undefined {
