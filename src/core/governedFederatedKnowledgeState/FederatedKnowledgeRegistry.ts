...
export class FederatedKnowledgeRegistry {
  private readonly entries: Map<string, GovernedKnowledgeEntry> = new Map();
   ...
   */
  public validateUntrustedContent(content: string): void {
    ...
    for (const marker of cotMarkers) {
      if (content.includes(marker)) {
        throw new GovernedFederatedKnowledgeStateValidationError(
          `CoT or deliberation marker '${marker}' detected in knowledge content`
        );
      }
    }
    ...
    for (const pattern of injectionPatterns) {
      if (pattern.test(content)) {
        throw new GovernedFederatedKnowledgeStateValidationError(
          `Prompt injection pattern detected in knowledge content: "${pattern.source}"`
        );
      }
    }
    ...
    for (const sp of secretPatterns) {
      if (sp.test(content)) {
        throw new GovernedFederatedKnowledgeStateValidationError(
          'Credential, private key, or bearer token detected in knowledge content'
        );
      }
    }
  }
   ...
   */
  public assertNoPrototypePollution(obj: Record<string, unknown>): void {
    if (
      ...
      throw new GovernedFederatedKnowledgeStateValidationError(
        'Prototype pollution key detected in knowledge data'
      );
    }
  }
   ...
   */
  public registerKnowledgeEntry(params: RegisterKnowledgeEntryParams): GovernedKnowledgeEntry {
    ...
    if (this.entries.has(params.knowledgeId)) {
      throw new GovernedFederatedKnowledgeStateValidationError(
        `Knowledge entry '${params.knowledgeId}' already exists`
      );
    }
    ...
    if (!params.humanOperatorId || !params.missionId || !params.objectiveId || !params.federationId) {
      throw new GovernedFederatedKnowledgeStateValidationError(
        'Human operator ID, mission ID, objective ID, and federation ID are mandatory'
      );
    }
    ...
    const entry: GovernedKnowledgeEntry = {
      ...base,
      provenanceHash,
    };
   ...
   */
  public getEntry(knowledgeId: string): GovernedKnowledgeEntry | undefined {
  ...
  public assertEntryBoundaries(
    ...
    if (!entry) {
      throw new GovernedFederatedKnowledgeStateValidationError(
        `Knowledge entry '${knowledgeId}' not found`
      );
    }
    if (entry.tenantId !== tenantId) {
      throw new GovernedFederatedKnowledgeStateTenantIsolationError(
        `Cross-tenant access rejected: entry tenant '${entry.tenantId}' != caller '${tenantId}'`,
        tenantId
      );
    }
    if (entry.sessionId !== sessionId) {
      throw new GovernedFederatedKnowledgeStateSessionIsolationError(
        `Cross-session access rejected: entry session '${entry.sessionId}' != caller '${sessionId}'`,
        tenantId
      );
    }
    return entry;
   ...
   */
  public listActiveEntries(
    tenantId: string,
    sessionId: string,
    federationId: string
  ): readonly GovernedKnowledgeEntry[] {
    const now = Date.now();
