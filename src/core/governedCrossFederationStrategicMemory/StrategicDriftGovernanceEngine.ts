...
import {
  StrategicDriftCategory,
  StrategicDriftSeverity,
  StrategicDriftSnapshot,
...
export class StrategicDriftGovernanceEngine {
  private readonly tenantConsecutiveFailures = new Map<string, number>();
  ...
  public evaluateDrift(
    ...
    const now = Date.now();
    const rawSnapshot: StrategicDriftSnapshot = {
    ...
    const snapshot: StrategicDriftSnapshot = { ...rawSnapshot, snapshotHash };
    ...
    if (severity === 'CRITICAL') {
      ...
      throw new GovernedStrategicMemoryDriftError(
        `Critical strategic drift detected (${aggregateScore.toFixed(3)} > ${DRIFT_THRESHOLDS.WARNING_MAX}): institutional memory quarantined`
      );
    }
    ...
    if (severity === 'WARNING') {
      ...
      if (failures >= MAX_CONSECUTIVE_DRIFT_FAILURES) {
        throw new GovernedStrategicMemoryDriftError(
          `Consecutive drift warning ceiling reached (${failures} >= ${MAX_CONSECUTIVE_DRIFT_FAILURES}): failing closed`
        );
      }
    } else {
  ...
  private computeCategoryDriftScores(records: StrategicMemoryRecord[]): Record<StrategicDriftCategory, number> {
    const scores: Record<StrategicDriftCategory, number> = {
  ...
  private computeAggregateScore(categoryScores: Record<StrategicDriftCategory, number>): number {
  ...
  private classifySeverity(aggregateScore: number): StrategicDriftSeverity {
