...
import type { DecisionInput } from './decisionTypes.js';
 ...
 */
export function hasSecurityAnomaly(value: unknown): boolean {
  if (typeof value !== 'string') return true;
  if (value.includes('\0')) return true;
  if (PROTOTYPE_POLLUTION_REGEX.test(value)) return true;
  if (containsSecrets(value)) return true;
  return false;
}
 ...
 */
export function containsPrototypePollution(target: unknown): boolean {
 ...
 */
export function validateDecisionInput(input: unknown): { valid: boolean; errors: readonly string[] } {
  ...
  const value = input as Partial<DecisionInput>;
