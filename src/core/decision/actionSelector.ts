 ...
 */
function validateCandidate(action: unknown): { valid: boolean; reason?: string } {
...
export function selectAction(
  ...
  const validActions: CandidateAction[] = [];
  const rejectedList: DecisionCandidate[] = [];
  ...
  const scoredCandidates: DecisionCandidate[] = validActions.map(action =>
    scoreCandidate(action, intent, context)
  );
