...
export class PolicyGovernanceReadinessReportStore {
  ...
  private readonly reports: Map<string, Map<string, ReadinessAssessmentReport>> = new Map();
   ...
   */
  public saveReport(report: ReadinessAssessmentReport): void {
   ...
   */
  public getReport(tenantId: string, reportId: ReadinessReportId): ReadinessAssessmentReport | undefined {
    ...
    try {
      ...
      if (fs.existsSync(filePath)) {
        ...
        const report = JSON.parse(raw) as ReadinessAssessmentReport;
  ...
  public listReports(tenantId: string): readonly ReadinessAssessmentReport[] {
    const list: ReadinessAssessmentReport[] = [];
    try {
      const dir = this.getPartitionDir(tenantId);
      const files = fs.readdirSync(dir).filter(f => f.endsWith('.json') && !f.includes('.tmp_'));
      for (const file of files) {
        const raw = fs.readFileSync(path.join(dir, file), 'utf-8');
        list.push(JSON.parse(raw) as ReadinessAssessmentReport);
      }
    } catch {
