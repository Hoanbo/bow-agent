...
export class PolicyGovernanceReadinessRepositoryInspector {
   ...
   */
  public inspect(): RepositoryInspectionFinding {
    ...
    const recordedComponents: Array<{ num: number; name: string; file: string; status: string }> = [];
    ...
    while ((match = componentRegex.exec(matrixContent)) !== null) {
      ...
      if (status === 'REAL') realCount++;
      else if (status === 'PARTIAL') partialCount++;
      else if (status === 'MOCK') mockCount++;
