...
export const CANONICAL_GOVERNANCE_CHAIN: readonly string[] = Object.freeze([
  'OBSERVATION',
  'EVIDENCE',
  'INVESTIGATION',
  'DECISION',
...
export class PolicyGovernanceReadinessIntegrationInspector {
   ...
   */
  public inspect(): readonly IntegrationPathFinding[] {
    const findings: IntegrationPathFinding[] = [];

    // Path 1: Policy Decision -> Execution -> Post-Execution -> Feedback Review
    findings.push(this.evaluateSubPath(
      'DECISION_TO_FEEDBACK_REVIEW',
      ['DECISION', 'AUTHORIZATION', 'EXECUTION', 'VERIFICATION', 'FEEDBACK_PROPOSAL', 'FEEDBACK_REVIEW'],
      [
    ...
    findings.push(this.evaluateSubPath(
      'FEEDBACK_TO_CANDIDATE_AUTHORIZATION',
      ['FEEDBACK_REVIEW', 'EVOLUTION_INTAKE', 'EVOLUTION_PLAN', 'CANDIDATE_SYNTHESIS', 'CANDIDATE_VALIDATION', 'HUMAN_AUTHORIZATION'],
      [
    ...
    findings.push(this.evaluateSubPath(
      'CANDIDATE_TO_RUNTIME_ENFORCEMENT',
      ['HUMAN_AUTHORIZATION', 'ACTIVATION_READINESS', 'STAGED_POLICY', 'ACTIVATION_PREFLIGHT', 'GOVERNED_ACTIVATION', 'ACTIVE_POLICY', 'RUNTIME_SYNCHRONIZATION', 'PDP_PEP_ENFORCEMENT'],
      [
    ...
    findings.push(this.evaluateSubPath(
      'ACTIVE_POLICY_ROLLBACK_RECOVERY',
      ['ACTIVE_POLICY', 'ROLLBACK_SUNSET_RECOVERY', 'RUNTIME_SYNCHRONIZATION'],
      [
    ...
    findings.push(this.evaluateSubPath(
      ...
      ['ACTIVE_POLICY', 'RUNTIME_SYNCHRONIZATION', 'RECONCILIATION'],
      [
    ...
    findings.push(this.evaluateSubPath(
      'INCIDENT_RESPONSE_TO_RESOLUTION_CLOSURE',
      ['INCIDENT_DETECTION', 'INCIDENT_CONTAINMENT', 'HUMAN_CLEARANCE', 'RECOVERY_AUTHORIZATION', 'RECOVERY_VERIFICATION', 'INCIDENT_RESOLUTION', 'INCIDENT_CLOSURE'],
      [
  ...
  private evaluateSubPath(
    ...
    testFile: string
  ): IntegrationPathFinding {
    let allComponentsExist = true;
    let missingComponent: string | undefined;
