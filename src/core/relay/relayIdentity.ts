 ...
 */
export function generateDeterministicRelayId(seed: string): RelayId {
  ...
  if (
    ...
    throw new RelayIdentityError(
      'Network location (IP, SSID, Wi-Fi) is strictly forbidden as relay identity seed.'
    );
  }
 ...
 */
export function isValidRelayId(id: unknown): id is RelayId {
 ...
 */
export function assertRelayIdentitySeparation(
  relayId: string,
  deviceId: string,
  sessionId: string,
  brainId: string
  ...
  if (relayId === deviceId) {
    throw new RelayIdentityError(
      `INVARIANT_VIOLATION: RELAY_ID cannot equal DEVICE_ID (${relayId})`
    );
  }
  if (relayId === sessionId) {
    throw new RelayIdentityError(
      `INVARIANT_VIOLATION: RELAY_ID cannot equal SESSION_ID (${relayId})`
    );
  }
  if (relayId === brainId) {
    throw new RelayIdentityError(
      `INVARIANT_VIOLATION: RELAY_ID cannot equal BRAIN_ID (${relayId})`
    );
  }
}
