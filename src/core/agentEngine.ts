import type { AgentContext, AgentMessage, AgentAction, PlanItemResult } from './types.js';
...
} from '../knowledge/knowledgeGapDetector.js';
...
import type { ResponseSource } from '../monitoring/analyticsTypes.js';
...
export async function processAgentMessageV2(
  ...
  // --------------------------------------------------------------------------
  const isAdmin = context.role === 'admin' || context.role === 'owner' || (context as any)?.isAdmin === true;
  if (isAdmin) {
