...
import {
  ...
  type ReleaseExecutionContradiction,
...
export class ReleaseExecutionRuntime {
  private requests = new Map<string, ReleaseExecutionRequest>();
  private candidates = new Map<string, ReleaseCandidate>();
  private verifications = new Map<string, ReleaseVerificationRecord>();
  private approvals = new Map<string, ReleaseExecutionApprovalBinding>();
  private authBindings = new Map<string, ReleaseExecutionAuthorizationBinding>();
  private results = new Map<string, ReleaseExecutionResult>();
  private states = new Map<string, ReleaseExecutionState>();

  private isUserStopActive = false;
  private isRevoked = false;

  constructor(
    private readonly authBridge: ReleaseExecutionAuthorizationBridge = new ReleaseExecutionAuthorizationBridge(),
    private readonly reviewBridge: ReleaseExecutionReviewBridge = new ReleaseExecutionReviewBridge(),
    private readonly executionEngine: GovernedReleaseExecutionEngine = new GovernedReleaseExecutionEngine(authBridge),
   ...
   */
  public recordReview(input: Omit<RecordReviewInput, 'request' | 'candidate' | 'verification'> & {
    ...
    if (!request || !candidate || !verification) {
      throw new ReleaseExecutionError(
        ...
        `No active execution request found for executionId "${input.executionId}".`
      );
    }
    ...
    this.logAudit(
      ...
      request.target.projectRoot,
      approval.decision === 'APPROVED' ? 'PERMIT' : 'DENY',
      approval.decision === 'APPROVED' ? 'SUCCESS' : 'BLOCKED',
      {
   ...
   */
  public requestHumanGate(executionId: ReleaseExecutionId): HumanGateRequest {
    ...
    if (!request || !candidate) {
      throw new ReleaseExecutionError(
        'MISSING_REQUIRED_BINDING',
        `No active execution request found for executionId "${executionId}".`
      );
    }
   ...
   */
  public issueAuthorizationToken(executionId: ReleaseExecutionId, ttlMs?: number): {
    ...
    if (!request || !candidate || !approval) {
      throw new ReleaseExecutionError(
        'MISSING_REQUIRED_BINDING',
        `Cannot issue token: missing request, candidate, or approval binding for "${executionId}".`
      );
    }
  ...
  public executeRelease(
    ...
    if (!request || !candidate || !approval) {
      throw new ReleaseExecutionError(
        'MISSING_REQUIRED_BINDING',
        `Missing execution bindings for "${executionId}".`
      );
    }
    ...
    let finalState: ReleaseExecutionState = 'COMPLETED';
    ...
    const evidence = ReleaseExecutionProvenanceEngine.assembleEvidence(
      request,
      mutationOutput.preExecutionManifestHash,
      verificationOutput.postManifest.manifestHash,
      token.tokenId,
      Date.now()
    );
    ...
    this.logAudit(
      request.operatorId,
      finalState === 'COMPLETED' ? 'RELEASE_EXECUTION_COMPLETED' : 'RELEASE_EXECUTION_FAILED',
      request.target.projectRoot,
      finalState === 'COMPLETED' ? 'PERMIT' : 'DENY',
      finalState === 'COMPLETED' ? 'SUCCESS' : 'FAILURE',
      {
        executionId,
        finalState,
        rollbackOccurred,
  ...
  public detectContradictions(
    ...
    if (hasStateConflict || hasHashConflict) {
      ...
      const contradiction: ReleaseExecutionContradiction = {
      ...
      this.logAudit(
        'SYSTEM',
        'RELEASE_CONTRADICTION_DETECTED',
        executionId,
        'DENY',
        'BLOCKED',
        {
   ...
   */
  public getResult(executionId: ReleaseExecutionId): ReleaseExecutionResult | undefined {
   ...
   */
  public getState(executionId: ReleaseExecutionId): ReleaseExecutionState | undefined {
