...
export class NodePolicyAttestationVerifier {
  private readonly keyResolver: NodeAttestationKeyResolver;
  ...
  constructor(
    keyResolverOrOptions: NodeAttestationKeyResolver | NodePolicyAttestationVerifierOptions,
    baseStorageDir?: string,
    registry?: FleetNodeRegistry,
    auditLedger?: PolicyDistributionAuditLedger,
    emergencyStopProvider?: EmergencyStopProvider
  ...
  private getNoncesFilePath(tenantId: string, domain: PolicyDomain): string {
  ...
  private loadNoncesUnderLock(filePath: string): Record<string, number> {
  ...
  private saveNoncesUnderLock(filePath: string, nonces: Record<string, number>): void {
    const dir = path.dirname(filePath);
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir, { recursive: true });
    }
  ...
  public async verifyReceipt(
    ...
    if (receipt.tenantId !== manifest.tenantId || receipt.policyDomain !== manifest.policyDomain) {
      throw new DistributionTenantIsolationError(
        `Cross-tenant or domain mismatch: receipt tenant/domain ${receipt.tenantId}/${receipt.policyDomain} != manifest ${manifest.tenantId}/${manifest.policyDomain}`
      );
    }

    if (
      receipt.federationId !== manifest.federationId ||
    ...
    if (receipt.timestamp < minTimestamp || receipt.timestamp > maxTimestamp) {
      throw new DistributionAttestationVerificationError(
        `Receipt timestamp ${receipt.timestamp} outside valid window [${minTimestamp}, ${maxTimestamp}]`
      );
    }
    ...
    try {
      ...
      if (this.registry) {
        ...
        await this.registry.updateNodeSyncStatus(
          receipt.tenantId,
          receipt.federationId,
          receipt.policyDomain,
          receipt.nodeId,
          nextStatus,
          nowMs,
          receipt.epoch,
          receipt.policyVersion,
          receipt.canonicalPolicyHash
        );
      }
