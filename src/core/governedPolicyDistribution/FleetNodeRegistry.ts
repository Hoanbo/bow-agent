...
export class FleetNodeRegistry {
  ...
  private readonly memoryStore = new Map<string, FleetNodeRecord>();

  constructor(
    baseStorageDirOrOptions?: string | FleetNodeRegistryOptions,
    emergencyStopProvider?: EmergencyStopProvider
  ) {
  ...
  private getNodeKey(tenantId: string, federationId: string, nodeId: string): string {
  ...
  private getNodesFilePath(tenantId: string, domain: PolicyDomain): string {
  ...
  private loadNodesUnderLock(filePath: string): Record<string, FleetNodeRecord> {
  ...
  private saveNodesUnderLock(filePath: string, nodes: Record<string, FleetNodeRecord>): void {
    const dir = path.dirname(filePath);
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir, { recursive: true });
    }
   ...
   */
  public async registerNode(record: FleetNodeRecord, nowMs: number): Promise<FleetNodeRecord> {
    ...
    try {
      ...
      for (const existingKey of Object.keys(nodes)) {
        ...
        if (existing.nodeId === record.nodeId && existing.federationId !== record.federationId) {
          throw new DistributionValidationError(
            `Node ${record.nodeId} already registered under federation ${existing.federationId}; explicit deregistration required`
          );
        }
      }
      ...
      const freshRecord: FleetNodeRecord = {
  ...
  public async recordHeartbeat(
    tenantId: string,
    federationId: string,
    domainOrNodeId: PolicyDomain | FleetNodeId,
    nodeIdOrNowMs: FleetNodeId | number,
    maybeNowMs?: number
    ...
    let domain: PolicyDomain | undefined;
    let nodeId: FleetNodeId;
    let nowMs: number;

    if (typeof nodeIdOrNowMs === 'number') {
      nodeId = domainOrNodeId as FleetNodeId;
      nowMs = nodeIdOrNowMs;
    } else {
      domain = domainOrNodeId as PolicyDomain;
    ...
    let foundDomain: PolicyDomain | undefined = domain || cached?.policyDomain;

    if (!foundDomain) {
      const domains: PolicyDomain[] = ['LEASE', 'CONVERGENCE', 'FEDERATION', 'SECURITY', 'RESOURCE', 'AUDIT'];
    ...
    try {
      ...
      const updatedRecord: FleetNodeRecord = {
        ...existing,
        lastHeartbeatAt: nowMs,
      };
   ...
   */
  public getFleetCohort(
    tenantId: string,
    federationId: string,
    domain: PolicyDomain,
    ring: CanaryRing,
    _nowMs: number
  ): readonly FleetNodeRecord[] {
    assertValidIdentifier(tenantId, 'tenantId');
    assertValidIdentifier(federationId, 'federationId');
    assertValidDomain(domain);

    const filePath = this.getNodesFilePath(tenantId, domain);
    let nodes: Record<string, FleetNodeRecord> = {};
  ...
  public async updateNodeSyncStatus(
    tenantId: string,
    federationId: string,
    domain: PolicyDomain,
    nodeId: FleetNodeId,
  ...
  public async updateNodeQuarantine(
    tenantId: string,
    federationId: string,
    domain: PolicyDomain,
    nodeId: FleetNodeId,
  ...
  public updateNodeQuarantineStatus = this.updateNodeQuarantine.bind(this);
