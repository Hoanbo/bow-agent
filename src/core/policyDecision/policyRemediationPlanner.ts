...
export class PolicyRemediationPlanner {
   ...
   */
  public assertHardForbiddenImmutability(targetAction: string): void {
    const normalized = targetAction.toLowerCase().trim();
    for (const forbidden of HARD_FORBIDDEN_ACTIONS) {
      if (normalized.includes(forbidden)) {
        throw new Error(
          `HARD_FORBIDDEN_REMEDIATION_DENIED: Action '${targetAction}' violates permanent immutable safety floor (${forbidden})`
        );
      }
    }
  }
   ...
   */
  public createRemediationPlan(proposal: PolicyDecisionProposal): PolicyRemediationPlan {
    ...
    let blastRadius: RemediationBlastRadius = 'TENANT_LOCAL';
    let requiredOperatorRole: 'MASTER_HUMAN_OPERATOR' | 'GOVERNED_OPERATOR' = 'GOVERNED_OPERATOR';
    const proposedActions: string[] = [];
    const safetyPreconditions: string[] = [
