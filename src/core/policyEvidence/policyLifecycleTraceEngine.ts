...
import {
  type PolicyLifecycleTraceId,
  type PolicyLifecycleTrace,
  type LifecycleTraceEvent,
  type RingLifecycleMilestone,
...
const CANONICAL_RINGS: readonly PolicyRing[] = [
  'RING_0',
  'RING_1',
  'RING_2',
  'RING_3',
...
export class PolicyLifecycleTraceEngine {
  ...
  public reconstructTrace(
    ...
    const traceEvents: LifecycleTraceEvent[] = [];
    ...
    for (const prov of tenantProvenance) {
      ...
      let status: LifecycleEventStatus = 'OBSERVED';
    ...
    const milestones: RingLifecycleMilestone[] = [];
    const missingRings: PolicyRing[] = [];
    let wasRolledBack = false;
    let rollbackReason: PolicyCanaryFailureReason | undefined;
    let wasCircuitBreakerTripped = false;
    let highestActiveRing: PolicyRing = 'RING_0';
    ...
    for (const ring of CANONICAL_RINGS) {
      ...
      const ringEvidence = candidateRecords.filter(r => {
        if ('currentRing' in r && r.currentRing === ring) return true;
        if ('rolledBackRing' in r && r.rolledBackRing === ring) return true;
        if ('targetRing' in r && r.targetRing === ring) return true;
        return false;
      });
      ...
      if (ringRollback) {
        wasRolledBack = true;
        const rbRecord = candidateRecords.find(r => r.eventType === 'ROLLBACK') as any;
        if (rbRecord) rollbackReason = rbRecord.reason;
      }
      if (ringCB) {
    ...
    const isGloballyActive = Boolean(
      ring4Milestone &&
    ...
    let currentState: PolicyCanaryState | 'UNKNOWN' = 'STAGED';
    if (wasRolledBack) {
