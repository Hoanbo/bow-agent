...
export class LongHorizonExecutionValidator {
    ...
    if (typeof obj === 'string') {
      ...
      for (const marker of PROHIBITED_COT_MARKERS) {
        if (obj.includes(marker)) {
          throw new LongHorizonValidationError(
   ...
   */
  public static validateBudget(budget: LongHorizonAutonomyBudget): void {
    ...
    if (budget.maxGenerations <= 0 || budget.maxGenerations > MAX_LONG_HORIZON_GENERATIONS) {
      throw new LongHorizonValidationError(
        `maxGenerations (${budget.maxGenerations}) must be between 1 and ${MAX_LONG_HORIZON_GENERATIONS}`
      );
    }
    if (budget.maxSteps <= 0 || budget.maxSteps > MAX_LONG_HORIZON_STEPS) {
      throw new LongHorizonValidationError(
        `maxSteps (${budget.maxSteps}) must be between 1 and ${MAX_LONG_HORIZON_STEPS}`
      );
    }
    if (budget.maxReplanningAttempts < 0 || budget.maxReplanningAttempts > MAX_REPLANNING_ATTEMPTS) {
      throw new LongHorizonValidationError(
        `maxReplanningAttempts (${budget.maxReplanningAttempts}) must be between 0 and ${MAX_REPLANNING_ATTEMPTS}`
      );
    }
    if (budget.maxWallClockMs <= 0 || budget.maxWallClockMs > MAX_WALL_CLOCK_MS) {
      throw new LongHorizonValidationError(
        `maxWallClockMs (${budget.maxWallClockMs}) must be between 1 and ${MAX_WALL_CLOCK_MS}`
      );
    }
  }
   ...
   */
  public static validateObjective(objective: GovernedLongHorizonObjective): void {
   ...
   */
  public static validateGeneration(generation: LongHorizonGeneration): void {
    ...
    if (generation.generationNumber >= MAX_LONG_HORIZON_GENERATIONS) {
      throw new LongHorizonValidationError(
        `generationNumber ${generation.generationNumber} exceeds ceiling ${MAX_LONG_HORIZON_GENERATIONS}`
      );
    }
   ...
   */
  public static validateProgressRecord(record: LongHorizonProgressRecord): void {
   ...
   */
  public static validateSession(session: LongHorizonSession): void {
    ...
    if (session.generations.length > session.budget.maxGenerations) {
      throw new LongHorizonValidationError(
        `Session generation count ${session.generations.length} exceeds max allowed ${session.budget.maxGenerations}`
      );
    }
   ...
   */
  public static validateSessionDocument(doc: LongHorizonSessionDocument): void {
  ...
  public validateObjectiveEnvelope(objective: GovernedLongHorizonObjective): void {
    LongHorizonExecutionValidator.validateObjective(objective);
  }

  public validateObjective(objective: GovernedLongHorizonObjective): void {
    LongHorizonExecutionValidator.validateObjective(objective);
  }

  public validateObjectiveScope(objective: GovernedLongHorizonObjective, requestedScope: string[]): void {
    for (const scope of requestedScope) {
      if (!objective.authorizationScope.includes(scope)) {
        throw new LongHorizonSecurityBoundaryError(
          `Requested scope "${scope}" exceeds authorized objective scope [${objective.authorizationScope.join(', ')}]`
        );
      }
    }
  }

  public validatePrototypePollution(obj: unknown): void {
    const checkKeys = (val: unknown, currentPath = 'root'): void => {
      ...
      for (const k of Object.keys(val)) {
        if (DANGEROUS_KEYS.includes(k)) {
          throw new LongHorizonSecurityBoundaryError(
            `Prototype pollution attempt detected with key "${k}" at ${currentPath}`
          );
        }
        checkKeys((val as Record<string, unknown>)[k], `${currentPath}.${k}`);
      }
    };
    checkKeys(obj);
  }

  public validateNoCoT(obj: unknown): void {
    const checkCoT = (val: unknown, currentPath = 'root'): void => {
      if (typeof val === 'string') {
        for (const marker of PROHIBITED_COT_MARKERS) {
          if (val.includes(marker)) {
            throw new LongHorizonSecurityBoundaryError(
              `Prohibited CoT/deliberation marker detected at ${currentPath}: "${marker}"`
            );
          }
        }
        return;
  ...
  public validateObjectiveProvenance(objective: GovernedLongHorizonObjective): void {
    const expected = computeObjectiveProvenanceHash(objective);
    if (objective.provenanceHash !== expected) {
      throw new LongHorizonSecurityBoundaryError(
        `Objective provenance mismatch: expected ${expected}, got ${objective.provenanceHash}`
      );
    }
  }

  public validateReplanningRequest(req: any): void {
