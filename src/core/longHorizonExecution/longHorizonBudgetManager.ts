...
export class LongHorizonBudgetManager {
   ...
   */
  public initializeUsage(): LongHorizonResourceUsage {
   ...
   */
  public assertBudgetWithinLimits(
    budget: LongHorizonAutonomyBudget,
    ...
    if (usage.generationsConsumed >= budget.maxGenerations) {
      throw new LongHorizonBudgetExhaustedError(
        `Generation budget exhausted: ${usage.generationsConsumed} / ${budget.maxGenerations} generations consumed`,
        { budget, usage, reason: 'GENERATIONS_EXHAUSTED' }
      );
    }

    if (usage.stepsConsumed >= budget.maxSteps) {
      throw new LongHorizonBudgetExhaustedError(
        `Step budget exhausted: ${usage.stepsConsumed} / ${budget.maxSteps} steps consumed`,
        { budget, usage, reason: 'STEPS_EXHAUSTED' }
      );
    }

    if (usage.replanningAttempts >= budget.maxReplanningAttempts) {
      throw new LongHorizonBudgetExhaustedError(
        `Replanning budget exhausted: ${usage.replanningAttempts} / ${budget.maxReplanningAttempts} replans consumed`,
        { budget, usage, reason: 'REPLANNING_EXHAUSTED' }
      );
    }

    if (usage.consecutiveFailures >= budget.maxConsecutiveFailures) {
      throw new LongHorizonBudgetExhaustedError(
        `Consecutive failure limit reached: ${usage.consecutiveFailures} / ${budget.maxConsecutiveFailures} failures`,
        { budget, usage, reason: 'CONSECUTIVE_FAILURES_EXCEEDED' }
      );
    }

    if (usage.stagnationCycles >= budget.maxStagnationCycles) {
      throw new LongHorizonBudgetExhaustedError(
        `Stagnation limit reached: ${usage.stagnationCycles} / ${budget.maxStagnationCycles} stagnation cycles`,
        { budget, usage, reason: 'STAGNATION_CYCLES_EXCEEDED' }
      );
    }

    if (elapsedMs >= budget.maxWallClockMs) {
      throw new LongHorizonBudgetExhaustedError(
        `Wall-clock time limit exceeded: ${elapsedMs}ms >= ${budget.maxWallClockMs}ms`,
        { budget, usage, reason: 'WALL_CLOCK_EXCEEDED' }
      );
    }
  }
   ...
   */
  public recordUsage(
    current: LongHorizonResourceUsage,
    delta: Partial<LongHorizonResourceUsage>
  ): LongHorizonResourceUsage {
    const now = Date.now();
    ...
    const updated: LongHorizonResourceUsage = Object.freeze({
  ...
  public initializeBudgetLedger(budget: LongHorizonAutonomyBudget): BudgetLedger {
    LongHorizonExecutionValidator.validateBudget(budget);
    return {
      budget,
      generationsConsumed: 0,
  ...
  public consumeGeneration(ledger: BudgetLedger): void {
    if (ledger.generationsConsumed >= ledger.budget.maxGenerations) {
      throw new LongHorizonBudgetExhaustedError(
        `Generation budget exhausted: ${ledger.generationsConsumed} / ${ledger.budget.maxGenerations}`
      );
    }
    ledger.generationsConsumed++;
  }

  public consumeStep(ledger: BudgetLedger): void {
    if (ledger.stepsConsumed >= ledger.budget.maxSteps) {
      throw new LongHorizonBudgetExhaustedError(
        `Step budget exhausted: ${ledger.stepsConsumed} / ${ledger.budget.maxSteps}`
      );
    }
    ledger.stepsConsumed++;
  }

  public consumeReplan(ledger: BudgetLedger): void {
    if (ledger.replanningAttempts >= ledger.budget.maxReplanningAttempts) {
      throw new LongHorizonBudgetExhaustedError(
        `Replanning budget exhausted: ${ledger.replanningAttempts} / ${ledger.budget.maxReplanningAttempts}`
      );
    }
    ledger.replanningAttempts++;
  }

  public recordFailure(ledger: BudgetLedger): void {
    ledger.executionAttempts++;
    ledger.consecutiveFailures++;
    if (ledger.consecutiveFailures >= ledger.budget.maxConsecutiveFailures) {
      throw new LongHorizonBudgetExhaustedError(
        `Consecutive failures limit exceeded: ${ledger.consecutiveFailures} / ${ledger.budget.maxConsecutiveFailures}`
      );
    }
  }

  public recordSuccess(ledger: BudgetLedger): void {
    ledger.consecutiveFailures = 0;
  }

  public recordStagnation(ledger: BudgetLedger): void {
    ledger.stagnationCycles++;
    if (ledger.stagnationCycles >= ledger.budget.maxStagnationCycles) {
      throw new LongHorizonStagnationError(
        `Stagnation cycle limit reached: ${ledger.stagnationCycles} / ${ledger.budget.maxStagnationCycles}`
      );
    }
  }

  public enforceBudget(ledger: BudgetLedger): void {
    const startMs = Date.parse(ledger.startedAt);
    const elapsedMs = Date.now() - startMs;
    if (elapsedMs >= ledger.budget.maxWallClockMs) {
      throw new LongHorizonBudgetExhaustedError(
        `Wall-clock budget exhausted: ${elapsedMs}ms >= ${ledger.budget.maxWallClockMs}ms`
      );
    }
    if (ledger.generationsConsumed >= ledger.budget.maxGenerations) {
