...
export class LongHorizonPersistenceRecoveryEngine {
   ...
   */
  public loadSessionDocument(tenantId: string, sessionId: string): LongHorizonSessionDocument & { version: number; sessionId: string; tenantId: string } {
    ...
    if (fs.existsSync(canonicalPath)) {
      try {
        ...
        return Object.assign(doc, {
          version: doc.sessionVersion ?? doc.session.sessionVersion ?? (doc.session as any).version ?? 1,
    ...
    throw new LongHorizonPersistenceError(
      `No valid session document found for tenant "${tenantId}" and session "${sessionId}" (both canonical and backup corrupted or absent)`
    );
  }
   ...
   */
  public hasSessionDocument(tenantId: string, sessionId: string): boolean {
    try {
      const sessionDir = this.getSessionDir(tenantId, sessionId);
      return (
        fs.existsSync(path.join(sessionDir, 'long_horizon_session.json')) ||
  ...
  public saveSessionDocument(
    doc: LongHorizonSessionDocument,
    ...
    const checkOCC = (filePath: string): void => {
      if (fs.existsSync(filePath)) {
        try {
          ...
          const incomingVersion = doc.sessionVersion ?? session.sessionVersion ?? (session as any).version ?? 1;

          if (expectedVersion !== undefined && existingVersion !== expectedVersion) {
            throw new LongHorizonConcurrencyError(
              `OCC CAS conflict saving session "${session.sessionId}": expected version ${expectedVersion}, but found ${existingVersion}`
            );
          }
          if (incomingVersion < existingVersion && expectedVersion === undefined) {
            throw new LongHorizonConcurrencyError(
              `OCC CAS conflict saving session "${session.sessionId}": incoming version ${incomingVersion} is less than existing version ${existingVersion}`
            );
          }
        } catch (err: any) {
  ...
  public persistSessionDocument(
    ...
    const tenantId: string = sessionOrDoc.tenantId ?? sessionOrDoc.session?.tenantId;
    const sessionId: string = sessionOrDoc.sessionId ?? sessionOrDoc.session?.sessionId;
    ...
    if (sessionOrDoc.session && sessionOrDoc.sessionVersion !== undefined) {
      this.saveSessionDocument(sessionOrDoc as LongHorizonSessionDocument, expectedVersion);
    } else {
      const session = sessionOrDoc as LongHorizonSession;
      const incomingVer = (session as any).version !== undefined
        ? (session as any).version
      ...
      const doc: LongHorizonSessionDocument = Object.freeze({
        ...rawDoc,
        documentHash,
      });
   ...
   */
  public initializeSession(session: LongHorizonSession): LongHorizonSessionDocument {
    this.securityBoundary.assertUserStop('pre_persistence');
