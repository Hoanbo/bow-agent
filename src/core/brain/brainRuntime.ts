...
import { BrainTask, type BrainTaskInput } from './brainTask.js';
import {
  BrainLoop,
...
// ---------------------------------------------------------------------------
// Runtime Snapshot (immutable observation for AgentLoop)
// ---------------------------------------------------------------------------
...
export class BrainRuntime {
  ...
  private _state: BrainLifecycleState = 'IDLE';
  private readonly _modelProvider: BrainModelProvider;
  private readonly _loop: BrainLoop;
  private readonly _ledger: BrainAuditLedger;
  private readonly _config: Required<BrainRuntimeConfig>;
  private readonly _activeTasks = new Map<string, BrainTask>();
  private readonly _startedAt: number;
  private _metrics: BrainMetrics;
  ...
  // State machine
  // ---------------------------------------------------------------------------
  get state(): BrainLifecycleState { return this._state; }
  get isOperational(): boolean { return !isBrainTerminal(this._state); }

  private _transition(to: BrainLifecycleState): void {
   ...
   */
  public async submitTask(input: BrainTaskInput): Promise<BrainTaskResult> {
    ...
    if (!canBrainAcceptInput(this._state)) {
      if (this._config.maxConcurrentTasks > 1 || this._activeTasks.size === 0) {
        ...
        throw new BrainError(
          ...
          `Brain cannot accept input in state "${this._state}". Wait for current task to complete.`,
          false
        );
      }
    }
  ...
  private _transitionToCompleted(): void {
    const validToCompleted: import('./brainStates.js').BrainLifecycleState[] = [
  ...
  private _transitionToFailed(): void {
  ...
  public getSnapshot(): BrainRuntimeSnapshot {
  ...
  public getAuditEvents(): readonly import('./brainTypes.js').BrainAuditEvent[] {
    return Object.freeze([...this._ledger.events]);
  }

  public getModelProvider(): BrainModelProvider {
...
// Global singleton
// ---------------------------------------------------------------------------
let _globalBrainRuntime: BrainRuntime | undefined;

export function getBrainRuntime(): BrainRuntime {
...
export function setBrainRuntimeForTest(runtime: BrainRuntime): void {
