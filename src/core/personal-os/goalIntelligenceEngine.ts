...
export class GoalIntelligenceEngine {
  private readonly reports: Map<string, GoalIntelligenceReport> = new Map();

  /**
   * Evaluate a long-term goal and generate an intelligence report.
   */
  public evaluateGoal(input: GoalEvaluationInput): GoalIntelligenceReport {
    const risks: string[] = [];
    ...
    const hasPriorityConflict = !!(
      input.competingPriorityGoals && input.competingPriorityGoals.length > 0
    );
    ...
    let healthScore = 1.0;
    if (blockers.length > 0) healthScore -= 0.35;
    if (isStalled) healthScore -= 0.25;
    if (hasPriorityConflict) healthScore -= 0.15;
    if (input.constraints && input.constraints.length > 3) healthScore -= 0.1;
    ...
    const report: GoalIntelligenceReport = {
      goalId: input.goalId,
      title: input.title,
   ...
   */
  public getReport(goalId: string): GoalIntelligenceReport | undefined {
   ...
   */
  public getAllReports(): GoalIntelligenceReport[] {
   ...
   */
  public formatReportText(report: GoalIntelligenceReport): string {
    return [
      '==============================================================================',
