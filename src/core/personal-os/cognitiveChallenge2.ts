...
export class CognitiveChallenge2Engine {
  private readonly challenges: Map<string, CognitiveChallenge2> = new Map();

  /**
   * Evaluate a proposal across 14 distinct dimensions.
   */
  public evaluateProposal(context: Challenge2EvaluationContext): {
    hasConcern: boolean;
    isMandatorySafetyBlock: boolean;
    challenge?: CognitiveChallenge2;
  } {
    const vectors: Record<string, { ok: boolean; notes?: string }> = {};
    const concerns: string[] = [];
    const evidence: string[] = [];
    const counterarguments: string[] = [];
    const alternativePlans: string[] = [];
    ...
    if (
      context.isProtectedWorkspaceTarget ||
    ...
    for (const v of [
      ...
      if (!vectors[v]) vectors[v] = { ok: true };
    }
    ...
    const challenge: CognitiveChallenge2 = {
      challengeId,
   ...
   */
  public formatChallengeText(challenge: CognitiveChallenge2): string {
    return [
      '==============================================================================',
  ...
  public resolveChallenge(
    challengeId: string,
    decision: 'CONFIRMED' | 'OVERRIDDEN' | 'ABANDONED'
  ): CognitiveChallenge2 {
   ...
   */
  public getChallenge(challengeId: string): CognitiveChallenge2 | undefined {
