...
import { WorldActionAuthorizationEngine, globalWorldActionAuth } from '../world-action/worldActionAuthorization.js';
...
export class PolicyRingPromotionEngine {
  private readonly authEngine: WorldActionAuthorizationEngine;
  ...
  public promote(
    ...
    if (request.targetRing !== expectedTransition) {
      throw new Error(
        `INVALID_RING_TRANSITION: Cannot promote from '${candidate.currentRing}' (${candidate.state}) directly to '${request.targetRing}'. Expected next ring: '${expectedTransition}'.`
      );
    }
    ...
    if (!evidence || evidence.health !== 'HEALTHY') {
      throw new Error(
        `HEALTH_EVIDENCE_REJECTED: Cannot promote candidate '${candidate.candidateId}'. Health is '${evidence?.health ?? 'UNKNOWN'}' (must be HEALTHY).`
      );
    }
    if (!evidence.metrics || evidence.metrics.totalEvaluations < 1) {
      throw new Error(
        `INSUFFICIENT_EVIDENCE: Cannot promote candidate '${candidate.candidateId}' without verified evaluation metrics.`
      );
    }
    if (evidence.candidateId !== candidate.candidateId) {
      throw new Error(
        `EVIDENCE_MISMATCH: Health evidence candidateId '${evidence.candidateId}' does not match candidate '${candidate.candidateId}'.`
      );
    }
    ...
    for (const forbidden of CANONICAL_HARD_FORBIDDEN_ACTIONS) {
      ...
      if (cls && cls !== 'FORBIDDEN') {
        throw new Error(
          `HARD_FORBIDDEN_DOWNGRADE: Candidate illegally downgraded '${forbidden}' to '${cls}'. Promotion permanently rejected.`
        );
      }
    }
   ...
   */
  private getExpectedNextRing(state: PolicyCanaryState, currentRing: PolicyRing): PolicyRing {
    if (state === 'STAGED') {
      return 'RING_0';
    }
    switch (currentRing) {
   ...
   */
  private mapRingToLifecycleState(ring: PolicyRing): PolicyCanaryState {
    switch (ring) {
      case 'RING_0':
        return 'SHADOWING';
      case 'RING_1':
