...
import { PolicyRingRouter, globalPolicyRingRouter, type PolicyRoutingDecision } from './policyRingRouter.js';
import { PolicyShadowEvaluator, globalPolicyShadowEvaluator, type ShadowEvaluationInput } from './policyShadowEvaluator.js';
import { PolicyCanaryTelemetryAggregator, globalPolicyCanaryTelemetryAggregator } from './policyCanaryTelemetryAggregator.js';
import { PolicyCanaryHealthMonitor, globalPolicyCanaryHealthMonitor } from './policyCanaryHealthMonitor.js';
import { PolicyCanaryCircuitBreaker, globalPolicyCanaryCircuitBreaker } from './policyCanaryCircuitBreaker.js';
import { PolicyRingPromotionEngine, globalPolicyRingPromotionEngine } from './policyRingPromotionEngine.js';
import { PolicyCanaryRollbackEngine, globalPolicyCanaryRollbackEngine } from './policyCanaryRollbackEngine.js';
import { PolicyCanaryProvenanceEngine, globalPolicyCanaryProvenanceEngine } from './policyCanaryProvenanceEngine.js';
import { PolicyCanaryRecoveryEngine, globalPolicyCanaryRecoveryEngine } from './policyCanaryRecoveryEngine.js';
import { PolicyCanaryFaultInjector, globalPolicyCanaryFaultInjector } from './policyCanaryFaultInjector.js';
...
function validatePolicyCanaryStoreRecord(data: unknown): ValidationResult<PolicyCanaryStoreRecord> {
  if (!data || typeof data !== 'object') {
    return { success: false, errors: ['Expected object for PolicyCanaryStoreRecord'] };
  }
  const record = data as Partial<PolicyCanaryStoreRecord>;
  ...
  return { success: true, data: data as PolicyCanaryStoreRecord };
}
...
export class PolicyCanaryRuntime {
  ...
  private readonly shadowEvaluator: PolicyShadowEvaluator;
  private readonly telemetryAggregator: PolicyCanaryTelemetryAggregator;
  private readonly healthMonitor: PolicyCanaryHealthMonitor;
  private readonly circuitBreaker: PolicyCanaryCircuitBreaker;
  private readonly promotionEngine: PolicyRingPromotionEngine;
  private readonly rollbackEngine: PolicyCanaryRollbackEngine;
  private readonly provenanceEngine: PolicyCanaryProvenanceEngine;
  private readonly recoveryEngine: PolicyCanaryRecoveryEngine;
  private readonly faultInjector: PolicyCanaryFaultInjector;
  ...
  private readonly stores = new Map<string, DurableJsonStore<PolicyCanaryStoreRecord>>();
  // Registered candidates in runtime memory: candidateId -> PolicyCandidatePackage
  private readonly candidatePackages = new Map<string, PolicyCandidatePackage>();
   ...
   */
  private getStoreForTenant(userId: string): {
    ...
    if (!store) {
      ...
      store = new DurableJsonStore<PolicyCanaryStoreRecord>({
        filePath,
   ...
   */
  public stageCandidate(candidate: PolicyCandidatePackage, userId: string = DEFAULT_PRIMARY_USER_ID): void {
  ...
  public assignTenantRing(input: {
    ...
    const assignmentId = createPolicyRingAssignmentId(
      `asgn_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
    );
    const assignedAt = new Date().toISOString();

    const assignment: PolicyRingAssignment = {
      assignmentId,
   ...
   */
  public executeShadowEvaluation(
    ...
    if (
      this.faultInjector.shouldInject('SHADOW_EVALUATOR_THROW', {
      ...
      globalAuditLedger.record({
        ...
        argumentsHash: crypto
          .createHash('sha256')
          .update(this.sanitizer.sanitizeString(
            JSON.stringify({ tenantPartition: input.tenantPartition, candidateId, fault: 'SHADOW_EVALUATOR_THROW' })
          ))
          .digest('hex'),
      });
    ...
    try {
      ...
      globalAuditLedger.record({
        ...
        argumentsHash: crypto
          .createHash('sha256')
          .update(this.sanitizer.sanitizeString(
            JSON.stringify({ tenantPartition: input.tenantPartition, candidateId, error: err?.message })
          ))
          .digest('hex'),
      });
   ...
   */
  public evaluateCandidateHealth(
    candidateId: PolicyCandidateId,
    tenantPartition: string
  ): CanaryHealthReport {
    const candidate = this.candidatePackages.get(candidateId);
    const ring: PolicyRing = candidate ? candidate.currentRing : 'RING_0';
  ...
  public promoteCandidate(
    ...
    if (!chainVerification.valid) {
      throw new Error(
        `PROVENANCE_INTEGRITY_FAILURE: Cannot promote candidate. Provenance chain broken: ${chainVerification.reason}`
      );
    }
    ...
    if (existing) {
      ...
      const updatedAssignments = existing.ringAssignments.map((a) =>
        a.tenantPartition === request.tenantPartition
   ...
   */
  public getCandidate(candidateId: string): PolicyCandidatePackage | undefined {
   ...
   */
  public getRouter(): PolicyRingRouter {
    return this.router;
  }
   ...
   */
  public getCircuitBreaker(): PolicyCanaryCircuitBreaker {
   ...
   */
  public getProvenanceEngine(): PolicyCanaryProvenanceEngine {
   ...
   */
  public getTelemetryAggregator(): PolicyCanaryTelemetryAggregator {
   ...
   */
  public getRecoveryEngine(): PolicyCanaryRecoveryEngine {
   ...
   */
  public getFaultInjector(): PolicyCanaryFaultInjector {
   ...
   */
  public reconcileFromDurableStore(userId: string = DEFAULT_PRIMARY_USER_ID): CanaryRecoveryResult {
    if (this.isUserStopActiveFn && this.isUserStopActiveFn()) {
