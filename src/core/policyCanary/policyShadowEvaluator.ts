...
export class PolicyShadowEvaluator {
   ...
   */
  public evaluateShadow(input: ShadowEvaluationInput): ShadowEvaluationRecord {
    const {
      tenantPartition,
      toolName,
      activeConfig,
      ...
      correlationId,
      actorRole,
      requestedApprovalTimeoutMs,
      retryAttempt,
    } = input;

    const observationId = createPolicyCanaryObservationId(
    ...
    let activeClassification: ActionClassification;
    let activeDecisionAllowed: boolean;
    ...
    let candidateClassification: ActionClassification;
    let candidateDecisionAllowed: boolean;
    ...
    const highImpactEscalation =
      activeClassification !== 'HIGH_IMPACT' && candidateClassification === 'HIGH_IMPACT';
   ...
   */
  private isAllowedClassification(classification: ActionClassification, actorRole?: string): boolean {
