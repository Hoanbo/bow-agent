...
export const DEFAULT_CANARY_HEALTH_THRESHOLDS: Readonly<CanaryHealthThresholds> = Object.freeze({
...
export class PolicyCanaryHealthMonitor {
  private readonly thresholds: CanaryHealthThresholds;
  ...
  public evaluateHealth(input: {
    ...
    const healthId = createPolicyCanaryHealthId(
      `hlth_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
    );
