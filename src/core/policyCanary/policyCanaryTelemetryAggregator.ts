...
interface MutableTelemetryBucket {
  ...
  activeAllows: number;
  activeDenies: number;
  candidateAllows: number;
  candidateDenies: number;
  highImpactEscalationCount: number;
  guardrailViolationCount: number;
  approvalTimeoutCount: number;
  retryExhaustionCount: number;
  concurrencyViolationCount: number;
  totalLeaseLatencyMs: number;
  leaseLatencyCount: number;
  totalCandidateLatencyMs: number;
  candidateLatencyCount: number;
  totalActiveLatencyMs: number;
  activeLatencyCount: number;
...
export class PolicyCanaryTelemetryAggregator {
  ...
  private readonly buckets = new Map<string, MutableTelemetryBucket>();
  // Recent observations for inspection / auditing
  private readonly observationHistory = new Map<string, ShadowEvaluationRecord[]>();

  private getBucketKey(tenantPartition: string, candidateId: string): string {
    return `${tenantPartition}:${candidateId}`;
  }

  private getOrCreateBucket(tenantPartition: string, candidateId: string): MutableTelemetryBucket {
   ...
   */
  public recordShadowEvaluation(record: ShadowEvaluationRecord, candidateId: PolicyCandidateId): void {
   ...
   */
  public recordViolation(tenantPartition: string, candidateId: PolicyCandidateId, type: 'GUARDRAIL' | 'CONCURRENCY' | 'RETRY' | 'TIMEOUT'): void {
    const bucket = this.getOrCreateBucket(tenantPartition, candidateId);
    if (type === 'GUARDRAIL') bucket.guardrailViolationCount++;
    if (type === 'CONCURRENCY') bucket.concurrencyViolationCount++;
    if (type === 'RETRY') bucket.retryExhaustionCount++;
    if (type === 'TIMEOUT') bucket.approvalTimeoutCount++;
  }
   ...
   */
  public recordPolicyDrift(tenantPartition: string, candidateId: PolicyCandidateId): void {
    const bucket = this.getOrCreateBucket(tenantPartition, candidateId);
   ...
   */
  public recordChecksumFailure(tenantPartition: string, candidateId: PolicyCandidateId): void {
    const bucket = this.getOrCreateBucket(tenantPartition, candidateId);
   ...
   */
  public recordSafetyRegression(tenantPartition: string, candidateId: PolicyCandidateId): void {
    const bucket = this.getOrCreateBucket(tenantPartition, candidateId);
    bucket.safetyRegressionCount++;
  }
   ...
   */
  public getMetrics(tenantPartition: string, candidateId: PolicyCandidateId): CanaryTelemetryMetrics {
    const bucket = this.getOrCreateBucket(tenantPartition, candidateId);
   ...
   */
  public getRecentObservations(tenantPartition: string, candidateId: PolicyCandidateId): readonly ShadowEvaluationRecord[] {
    const key = this.getBucketKey(tenantPartition, candidateId);
   ...
   */
  public resetMetrics(tenantPartition: string, candidateId: PolicyCandidateId): void {
    const key = this.getBucketKey(tenantPartition, candidateId);
