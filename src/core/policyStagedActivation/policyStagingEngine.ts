...
export class PolicyStagingEngine {
  ...
  private readonly stagedCache: Map<string, Map<string, StagedPolicy>> = new Map();
  ...
  private getTenantStagedMap(tenantPartition: string): Map<string, StagedPolicy> {
  ...
  public stageCandidate(params: {
    ...
    const stagedPolicy: StagedPolicy = Object.freeze({
      stagedActivationId,
      candidateDraftId: params.request.candidateDraftId,
   ...
   */
  public getStagedPolicy(tenantPartition: string, candidateDraftId: string): StagedPolicy | null {
    this.assertUserStopInactive();
    this.validateTenant(tenantPartition);
    return this.getTenantStagedMap(tenantPartition).get(candidateDraftId as any) ?? null;
  }
}
