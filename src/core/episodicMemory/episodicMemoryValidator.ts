...
export class EpisodicMemoryValidator {
   ...
   */
  public validateRequestEnvelope(request: unknown): asserts request is EpisodicMemoryRequest {
    ...
    try {
      ...
      if (Buffer.byteLength(serialized, 'utf8') > this.maxPayloadBytes) {
        throw new MemorySecurityViolationError(
          `Memory request payload exceeds limit of ${this.maxPayloadBytes} bytes`
        );
      }
    } catch (err: any) {
      if (err instanceof MemorySecurityViolationError) throw err;
      throw new MemoryValidationError(`Cannot serialize memory request: ${err.message}`);
    }

    this.defensivelyValidatePayload(request);

    const req = request as EpisodicMemoryRequest;
   ...
   */
  public validateCommitAuthority(commitRecord: DurableCommitRecord): void {
    ...
    if (commitRecord.status !== 'COMMITTED') {
      throw new MemoryValidationError(
        `Memory authority rejected: DurableCommitRecord status is '${commitRecord.status}', expected 'COMMITTED'`
      );
    }
  }
  ...
  public validateBindings(
    commitRecord: DurableCommitRecord,
    authoritativeTask: AgentTask,
    context?: EpisodicMemoryContext,
    expectedTaskVersion?: number
  ): void {
    // Tenant Isolation
    if (commitRecord.tenantId.trim() !== authoritativeTask.tenantId.trim()) {
      throw new CrossTenantMemoryError(
        `Cross-tenant memory violation: commit tenant '${commitRecord.tenantId}' does not match task tenant '${authoritativeTask.tenantId}'`
      );
    }

    if (context?.tenantId && context.tenantId.trim() !== authoritativeTask.tenantId.trim()) {
      throw new CrossTenantMemoryError(
        `Cross-tenant memory violation: context tenant '${context.tenantId}' does not match task tenant '${authoritativeTask.tenantId}'`
      );
    }
    ...
    if (commitRecord.taskId.trim() !== authoritativeTask.taskId.trim()) {
      throw new MemoryValidationError(
        `Task mismatch: commit taskId '${commitRecord.taskId}' does not match task '${authoritativeTask.taskId}'`
      );
    }
    ...
    if (expectedTaskVersion !== undefined && expectedTaskVersion !== authoritativeTask.version) {
      throw new MemoryValidationError(
        `Expected task version (${expectedTaskVersion}) does not match authoritative task version ${authoritativeTask.version}`
      );
    }

    if (commitRecord.taskVersion !== authoritativeTask.version) {
      throw new MemoryValidationError(
        `Task version mismatch: commit record taskVersion ${commitRecord.taskVersion} does not match authoritative task version ${authoritativeTask.version}`
      );
    }
   ...
   */
  public validatePathSafety(identifier: string, fieldName: string): void {
