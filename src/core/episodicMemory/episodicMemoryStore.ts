...
export class EpisodicMemoryStore {
   ...
   */
  public computeMemoryId(input: MemoryIdentityInput): string {
    const raw = [
      input.tenantId.trim(),
      input.taskId.trim(),
      input.stepId.trim(),
   ...
   */
  public getTenantMemoryDir(tenantId: string): string {
   ...
   */
  public getMemoryFilePath(tenantId: string, memoryId: string): string {
   ...
   */
  public hasMemory(tenantId: string, memoryId: string): boolean {
   ...
   */
  public saveMemory(record: EpisodicMemoryRecord): void {
    ...
    if (fs.existsSync(targetFile)) {
      throw new DuplicateMemoryError(
        `Memory record '${record.memoryId}' has already been persisted for tenant '${record.tenantId}'. Replay rejected.`
      );
    }

    const sanitized = this.sanitizer.sanitize(record) as EpisodicMemoryRecord;
  ...
  public getMemory(tenantId: string, memoryId: string): EpisodicMemoryRecord | undefined {
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as EpisodicMemoryRecord;

      if (parsed.tenantId !== tenantId) {
        throw new CrossTenantMemoryError(
          `Tenant isolation violation: record belongs to '${parsed.tenantId}' but requested by '${tenantId}'`
        );
      }
