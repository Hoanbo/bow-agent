...
export class TaskDependencyEngine {
  ...
  public evaluateDependencies(
    task: AgentTask,
    taskLookup: (id: TaskId) => AgentTask | undefined
    ...
    for (const dep of task.dependencies) {
      ...
      if (!isSatisfied) {
        ...
        if (dep.requiredArtifactTypes && dep.requiredArtifactTypes.length > 0) {
          const hasRequiredArtifacts = dep.requiredArtifactTypes.every((reqType) =>
            parentArtifacts.some((art) => art.artifactType === reqType)
          );
  ...
  public assertNoCycle(
    ...
    if (result.hasCycle) {
      throw new TaskDependencyError(
        'TASK_DEPENDENCY_CYCLE',
        `Cyclic dependency detected: ${result.cyclePath?.join(' -> ')}`,
