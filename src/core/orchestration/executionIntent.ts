 ...
 */
export function resolveTargetDomain(actionType: string): string {
...
export function createExecutionIntent(
  ...
  customRisk?: PlanRiskLevel,
): ExecutionIntent {
  const effectiveRisk: PlanRiskLevel = customRisk || decision.riskLevel || 'LOW';
