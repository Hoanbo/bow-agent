...
export class GoalPersistenceEngine {
  ...
  public saveGraph(
    ...
    conflicts: readonly GoalConflictDescriptor[] = [],
    activeTenantId?: string
  ): { readonly filePath: string; readonly bytesWritten: number; readonly graphVersion: number } {
    ...
    const finalDoc: GoalGraphDocument = Object.freeze({
      ...docDraft,
      provenanceHash,
    });
   ...
   */
  public loadGraphDocument(
    tenantId: string,
    activeTenantId?: string
  ): GoalGraphDocument | null {
    ...
    const tryLoad = (filePath: string): GoalGraphDocument | null => {
      if (!fs.existsSync(filePath)) return null;
      try {
        const raw = fs.readFileSync(filePath, 'utf8');
        const doc = JSON.parse(raw) as GoalGraphDocument;
    ...
    if (primaryDoc) return { ...primaryDoc, recoveredFromBackup: false };

    // Fallback to .bak snapshot
    const bakDoc = tryLoad(bakPath);
    if (bakDoc) return { ...bakDoc, recoveredFromBackup: true };
