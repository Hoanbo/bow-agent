...
import type { ContextAwarePlan, ContextPlanStep, DecisionContext, PlanActionType, PlanRiskLevel } from './planningTypes.js';

const step = (order: number, type: PlanActionType, description: string, riskLevel: PlanRiskLevel, requiresApproval = false, dependencies: string[] = [], candidateAction?: ContextPlanStep['candidateAction']): ContextPlanStep => Object.freeze({ stepId: `step-${order}-${type}`, order, type, description, dependencies: Object.freeze(dependencies), riskLevel, requiresApproval, candidateAction });
...
export function createContextAwarePlan(context: DecisionContext, createdAt = '1970-01-01T00:00:00.000Z'): ContextAwarePlan {
  ...
  const steps: ContextPlanStep[] = [];
  if (strategy === 'CLARIFY') steps.push(step(1, 'ASK_CLARIFICATION', 'Request structured clarification before any candidate action.', 'LOW'));
  else if (strategy === 'RESPOND') steps.push(step(1, 'RESPOND', 'Respond using the semantic interpretation.', 'LOW'));
  else if (strategy === 'CONTEXT_RESPONSE') { steps.push(step(1, 'RETRIEVE_CONTEXT', 'Use the already resolved scoped conversation context.', 'LOW')); steps.push(step(2, 'RESPOND', 'Respond from resolved context.', 'LOW', false, ['step-1-RETRIEVE_CONTEXT'])); }
  else {
    ...
    if (riskLevel === 'HIGH' || riskLevel === 'CRITICAL') steps.push(step(2, 'REQUEST_APPROVAL', 'Request approval through the existing governed boundary.', riskLevel, true, ['step-1-PREPARE_ACTION'], context.semanticIntent.candidateAction));
