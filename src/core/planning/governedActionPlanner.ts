...
import {
  ...
  type GovernedPlanningResponse,
  type PlanDag,
  type PlanRiskSummary,
  GOVERNED_PLANNER_VERSION,
...
export class GovernedActionPlanner {
   ...
   */
  public async plan(request: GovernedPlanningRequest): Promise<GovernedPlanningResponse> {
    const startTime = Date.now();
    const timestamp =
      request.plannerConfig?.deterministicTimestamp ??
    ...
    let candidateSteps: readonly GovernedCandidateStep[];
    try {
      candidateSteps = PlanStepBuilder.buildSteps(
        request.cognitiveResult,
    ...
    let resolvedSteps: readonly GovernedCandidateStep[];
    let dag: PlanDag;
    try {
      const resolverResult = PlanDependencyResolver.resolve(
        candidateSteps,
        undefined,
        request.plannerConfig
      );
    ...
    const requiresApproval =
      riskSummary.requiresHumanApproval ||
    ...
    const objective = this.sanitizer.sanitize(
      request.cognitiveResult.plan?.summary ??
        request.cognitiveResult.interpretation ??
        'Multi-step candidate plan execution proposal'
    ) as string;

    const candidatePlanDraft: Omit<GovernedCandidatePlan, 'provenanceHash'> = {
      planId,
      requestId: request.requestId,
      taskId: request.taskId,
      tenantId: request.tenantId,
      ...
      assumptions: Object.freeze(
        (request.constraints ?? []).slice(0, 10).map((c) => this.sanitizer.sanitize(c) as string)
      ),
      constraints: Object.freeze(
        (request.constraints ?? []).map((c) => this.sanitizer.sanitize(c) as string)
      ),
      steps: resolvedSteps,
      dag,
      riskSummary,
    ...
    const candidatePlan: GovernedCandidatePlan = Object.freeze({
      ...candidatePlanDraft,
      provenanceHash: provisionalProvenance,
    });

    const validation = GovernedPlanValidator.validateCandidatePlan(
      candidatePlan,
      request.plannerConfig
    );
    ...
    if (!validation.valid) {
      ...
      throw new PlanValidationError(
        `Candidate plan validation failed with ${validation.errors.length} error(s): ${validation.errors.join('; ')}`,
        validation.errors
      );
    }
    ...
    const finalCandidatePlan: GovernedCandidatePlan = Object.freeze({
      ...candidatePlan,
    ...
    return Object.freeze({
      ...
      plannerMetadata: Object.freeze({
        ...
        durationMs: Date.now() - startTime,
        deterministicTimestampUsed: Boolean(
          request.plannerConfig?.deterministicTimestamp ?? this.deterministicTimestamp
        ),
   ...
   */
  public static verifyPlanProvenance(plan: GovernedCandidatePlan): boolean {
    if (!plan || !plan.provenanceHash) return false;
    const computed = GovernedActionPlanner.calculateProvenance(plan);
    return computed === plan.provenanceHash;
  }

  public static calculateProvenance(
    plan: Omit<GovernedCandidatePlan, 'provenanceHash'> & { provenanceHash?: string }
  ): string {
   ...
   */
  private async verifyTaskBoundary(request: GovernedPlanningRequest): Promise<void> {
    ...
    try {
      ...
      throw new CrossTenantPlanError(
        `Task '${request.taskId}' does not exist or is not accessible under tenant '${request.tenantId}'`
      );
    }
    ...
    if (task.tenantId !== request.tenantId) {
      ...
      throw new CrossTenantPlanError(
        `Cross-tenant planning violation: task tenant '${task.tenantId}' does not match request tenant '${request.tenantId}'`
      );
    }
  ...
  private calculateRiskSummary(steps: readonly GovernedCandidateStep[]): PlanRiskSummary {
    const riskCounts: Record<PlanRiskLevel, number> = {
      LOW: 0,
      MEDIUM: 0,
      HIGH: 0,
      CRITICAL: 0,
    ...
    let highestStepRisk: PlanRiskLevel = 'LOW';

    const riskOrder: Record<PlanRiskLevel, number> = {
      LOW: 0,
      MEDIUM: 1,
      HIGH: 2,
      CRITICAL: 3,
  ...
  private emitAudit(
    eventType: PlannerAuditEventType,
    request: GovernedPlanningRequest,
    payload: Record<string, unknown>
  ): void {
    try {
      const sanitizedPayload = this.sanitizer.sanitize(payload) as Record<string, unknown>;
      this.auditLedger.record({
        ...
        argumentsHash: '',
        policyDecision:
          eventType === PlannerAuditEventType.PLANNER_COMPLETED ||
        ...
        executionStatus:
          ...
          eventType === PlannerAuditEventType.PLANNER_STARTED
            ? 'SUCCESS'
            : 'BLOCKED',
        resultHash: (payload.provenanceHash as string) || '',
        actor: {
          userId: 'system',
          role: 'SYSTEM',
