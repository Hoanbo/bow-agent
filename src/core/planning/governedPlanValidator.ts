...
export class GovernedPlanValidator {
   ...
   */
  public static validateRequest(request: GovernedPlanningRequest): void {
    const errors: string[] = [];
    ...
    if (
      typeof request.expectedTaskVersion !== 'number' ||
    ...
    if (errors.length > 0) {
      throw new PlanValidationError(
        `Planning request validation failed: ${errors.join(', ')}`,
        errors
      );
    }
  }
   ...
   */
  public static validateCandidatePlan(
