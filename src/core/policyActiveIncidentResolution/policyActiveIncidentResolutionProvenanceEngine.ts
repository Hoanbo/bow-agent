...
export class PolicyActiveIncidentResolutionProvenanceEngine {
  ...
  private readonly chains: Map<string, ResolutionProvenanceRecord[]> = new Map();
  ...
  private getChainFilePath(tenantPartition: string): string {
  ...
  private loadChainIfEmpty(tenantPartition: string): ResolutionProvenanceRecord[] {
    if (this.chains.has(tenantPartition)) {
      return this.chains.get(tenantPartition)!;
    }

    const records: ResolutionProvenanceRecord[] = [];
    ...
    if (fs.existsSync(filePath)) {
      ...
      for (let i = 0; i < lines.length; i++) {
        try {
          const rec: ResolutionProvenanceRecord = JSON.parse(lines[i]);
  ...
  public appendRecord(input: AppendProvenanceInput): ResolutionProvenanceRecord {
    this.assertUserStopInactive();
    ...
    const {
      ...
      incidentId,
      activePolicyStateId,
      containmentAssessmentId,
      containmentClearanceId,
    ...
    const record: ResolutionProvenanceRecord = Object.freeze({
      provenanceId,
      tenantPartition,
      incidentId,
  ...
  public getRecords(tenantPartition: string): readonly ResolutionProvenanceRecord[] {
    this.assertUserStopInactive();
