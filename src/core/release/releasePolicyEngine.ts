...
export class ReleasePolicyEngine {
  ...
  public static assertNotProtectedWorkspace(targetPath: string): void {
    ...
    if (
      normalized === 'c:\\bow\\shopofbow' ||
      normalized.startsWith('c:\\bow\\shopofbow\\')
    ) {
      throw new ReleaseError(
        ReleaseErrorCode.PROTECTED_WORKSPACE_VIOLATION,
        `Release operation targets protected workspace C:\\BOW\\shopofbow. Execution denied. ` +
   ...
   */
  public static assertNotUserStopped(isUserStopped: boolean, reason?: string): void {
    if (isUserStopped) {
      throw new ReleaseError(
        ReleaseErrorCode.USER_STOP_ACTIVE,
        `Release pipeline rejected: USER_STOP is currently active${reason ? ` (${reason})` : ''}. ` +
  ...
  public static assertNotRevoked(isRevoked: boolean): void {
    if (isRevoked) {
      throw new ReleaseError(
        ReleaseErrorCode.REVOCATION_ACTIVE,
        `Release pipeline rejected: Execution context has been revoked. ` +
   ...
   */
  public static assertCandidateNotExpired(candidate: ReleaseCandidate): void {
    if (Date.now() > candidate.expiresAt) {
      throw new ReleaseError(
        ReleaseErrorCode.CANDIDATE_EXPIRED,
   ...
   */
  public static assertCandidateExecutable(candidate: ReleaseCandidate): void {
    switch (candidate.state) {
      case 'STALE':
        throw new ReleaseError(
          ReleaseErrorCode.CANDIDATE_STALE,
          `Release candidate "${candidate.candidateId}" is STALE and cannot be re-verified. ` +
            `Ứng viên phát hành đang STALE và không thể được xác minh lại.`
        );
      case 'FAILED':
        throw new ReleaseError(
          ReleaseErrorCode.CANDIDATE_FAILED,
          `Release candidate "${candidate.candidateId}" is in FAILED state. ` +
            `Ứng viên phát hành đang ở trạng thái FAILED.`
        );
      case 'REVOKED':
        throw new ReleaseError(
          ReleaseErrorCode.CANDIDATE_REVOKED,
          `Release candidate "${candidate.candidateId}" has been REVOKED. ` +
            `Ứng viên phát hành đã bị REVOKED.`
        );
      case 'BLOCKED':
        throw new ReleaseError(
          ReleaseErrorCode.CANDIDATE_REVOKED,
          `Release candidate "${candidate.candidateId}" is BLOCKED. ` +
            `Ứng viên phát hành đang BLOCKED.`
        );
      case 'EXPIRED':
        throw new ReleaseError(
          ReleaseErrorCode.CANDIDATE_EXPIRED,
          `Release candidate "${candidate.candidateId}" has EXPIRED. ` +
            `Ứng viên phát hành đã EXPIRED.`
        );
      case 'REJECTED':
        throw new ReleaseError(
          ReleaseErrorCode.ACCEPTANCE_CRITERIA_FAILED,
          `Release candidate "${candidate.candidateId}" was REJECTED. ` +
      ...
      default:
        ...
        throw new ReleaseError(
          ReleaseErrorCode.CANDIDATE_FAILED,
          `Release candidate "${candidate.candidateId}" is in an unrecognized state. Rejecting.`
        );
    }
  }
}
