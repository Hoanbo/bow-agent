...
export const DEFAULT_BASELINE_CLASSIFICATIONS: Readonly<Record<string, ActionClassification>> = Object.freeze({
 ...
 */
export function createBaselinePolicyConfiguration(): PolicyConfiguration {
  ...
  const guardrails = {
    ...
    allowAutonomousDegradation: false as const,
  };
 ...
 */
function validatePolicyStoreRecord(data: unknown): ValidationResult<PolicyStoreRecord> {
  if (!data || typeof data !== 'object') {
    return { success: false, errors: ['Expected object for PolicyStoreRecord'] };
  }
  const record = data as Partial<PolicyStoreRecord>;
  ...
  return { success: true, data: data as PolicyStoreRecord };
}

export class PolicySnapshotStore {
  private readonly baseDir: string;
  private readonly sanitizer: DiagnosisSanitizer;
  private readonly stores = new Map<string, DurableJsonStore<PolicyStoreRecord>>();
   ...
   */
  private getStore(userId?: string): { store: DurableJsonStore<PolicyStoreRecord>; partitionKey: string } {
   ...
   */
  public getActiveSnapshot(userId?: string): PolicySnapshot {
    const { store } = this.getStore(userId);
   ...
   */
  public commitSnapshot(newConfig: PolicyConfiguration, userId?: string): PolicySnapshot {
    const { store, partitionKey } = this.getStore(userId);
    const sanitizedConfig = this.sanitizer.sanitize(newConfig) as PolicyConfiguration;
   ...
   */
  public rollbackToSnapshot(targetSnapshotId: PolicySnapshotId, userId?: string): PolicySnapshot {
    const { store } = this.getStore(userId);
    const currentData = store.read();
   ...
   */
  public queryHistoricalSnapshots(
    options?: { limit?: number; cursor?: string },
    userId?: string
  ): { snapshots: readonly PolicySnapshot[]; nextCursor?: string; totalMatching: number } {
    const { store } = this.getStore(userId);
    const currentData = store.read();
