...
export class GovernedPolicyRolloutEngine {
  constructor(
    private readonly snapshotStore: PolicySnapshotStore = new PolicySnapshotStore(),
   ...
   */
  public executeRollout(input: ExecuteRolloutInput): {
    ...
    if (activeConfig.versionId !== proposal.baseVersionId) {
      throw new Error(
        `VERSION_MISMATCH: Active policy version '${activeConfig.versionId}' does not match proposal baseVersion '${proposal.baseVersionId}'. Rollout blocked.`
      );
    }
    ...
    const newClassifications: Record<string, ActionClassification> = {
    ...
    const newConfig: PolicyConfiguration = {
      versionId: newVersionId,
    ...
    const rolloutRecord: PolicyRolloutRecord = {
      rolloutId,
   ...
   */
  public verifyConfigIntegrity(config: PolicyConfiguration): boolean {
    if (!config || !config.versionId || !config.checksum) return false;
    if (!config.actionClassifications || typeof config.actionClassifications !== 'object') return false;

    // Verify hard forbidden barriers exist and remain FORBIDDEN
    if (config.actionClassifications['transfer_funds'] !== 'FORBIDDEN') return false;
    if (config.actionClassifications['delete_database'] !== 'FORBIDDEN') return false;
