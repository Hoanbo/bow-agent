...
export class GuardrailCalibrationEngine {
  ...
  public calibrateGuardrails(
      ...
      if (GuardrailCalibrationEngine.HARD_FORBIDDEN_ACTIONS.includes(actionName)) {
        if (newClass !== 'FORBIDDEN') {
          ...
          rejectionReasons.push(
            `FORBIDDEN_PROTECTION_BREACH: Action '${actionName}' is hardcoded FORBIDDEN and can never be reclassified to '${newClass}'.`
          );
        }
      }
    }
    ...
    if (simulationResult.simulatedSafetyRegressionsCount > 0) {
      ...
      rejectionReasons.push(
        `SAFETY_REGRESSION_DETECTED: Counterfactual simulation projected ${simulationResult.simulatedSafetyRegressionsCount} safety regressions.`
      );
    }
    ...
    if (guardrails.minApprovalTimeoutMs !== undefined && guardrails.minApprovalTimeoutMs < 5000) {
      ...
      rejectionReasons.push(
        `GATE_STRICTNESS_VIOLATION: minApprovalTimeoutMs cannot be reduced below 5000ms (proposed: ${guardrails.minApprovalTimeoutMs}ms).`
      );
    }
    ...
    if (guardrails.maxRetries !== undefined && guardrails.maxRetries > 10) {
      erosionDetected = true;
      rejectionReasons.push(
        `EXCESSIVE_RETRY_EXPANSION: maxRetries cannot exceed 10 (proposed: ${guardrails.maxRetries}).`
      );
    }

    // 4. Calculate safety margin score [0.0, 1.0]
    let safetyMarginScore = 1.0;
    if (violatesForbiddenProtection) safetyMarginScore = 0.0;
    else if (erosionDetected) safetyMarginScore -= 0.4;
    else if (violatesGateStrictness) safetyMarginScore -= 0.3;
