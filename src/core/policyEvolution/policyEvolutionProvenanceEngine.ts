...
export class PolicyEvolutionProvenanceEngine {
   ...
   */
  public generateProvenance(input: GenerateEvolutionProvenanceInput): PolicyEvolutionProvenanceRecord {
    ...
    const simulationHash = crypto.createHash('sha256').update(
      `${simulationResult.simulationId}:${simulationResult.simulatedMttrDeltaMs}:${simulationResult.simulatedSafetyRegressionsCount}`
    ).digest('hex');
    const guardrailHash = crypto.createHash('sha256').update(
      `${guardrailResult.passed}:${guardrailResult.safetyMarginScore}:${guardrailResult.violatesForbiddenProtection}`
    ).digest('hex');

    let reviewHash = 'PENDING_REVIEW';
    if (reviewRecord) {
      reviewHash = crypto.createHash('sha256').update(
        `${reviewRecord.reviewId}:${reviewRecord.decision}:${reviewRecord.reviewedBy}:${reviewRecord.authorizationToken?.tokenId ?? 'NO_TOKEN'}`
      ).digest('hex');
    }

    const provenanceSha256 = this.computeProvenanceHash(
      proposal.proposalId,
      baseVersionHash,
      diffHash,
      simulationHash,
   ...
   */
  public verifyProvenance(
    record: PolicyEvolutionProvenanceRecord,
    ...
    const expectedSimHash = crypto.createHash('sha256').update(
      `${simulationResult.simulationId}:${simulationResult.simulatedMttrDeltaMs}:${simulationResult.simulatedSafetyRegressionsCount}`
    ).digest('hex');
    const expectedGuardHash = crypto.createHash('sha256').update(
      `${guardrailResult.passed}:${guardrailResult.safetyMarginScore}:${guardrailResult.violatesForbiddenProtection}`
    ).digest('hex');

    let expectedReviewHash = 'PENDING_REVIEW';
    if (reviewRecord) {
      expectedReviewHash = crypto.createHash('sha256').update(
        `${reviewRecord.reviewId}:${reviewRecord.decision}:${reviewRecord.reviewedBy}:${reviewRecord.authorizationToken?.tokenId ?? 'NO_TOKEN'}`
      ).digest('hex');
    }

    const recalculated = this.computeProvenanceHash(
      proposal.proposalId,
      expectedBaseVersionHash,
      expectedDiffHash,
      expectedSimHash,
      expectedGuardHash,
      expectedReviewHash,
