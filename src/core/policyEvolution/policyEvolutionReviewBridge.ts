...
export class PolicyEvolutionReviewBridge {
  constructor(private readonly masterAuthority: MasterHumanAuthority = new MasterHumanAuthority()) {}
  ...
  public stageForReview(
    ...
    targetPolicyVersionId: EvolutionVersionId
  ): StagedReviewPackage {
    ...
    if (!guardrailResult || !guardrailResult.passed) {
      throw new Error(
        `STAGING_BLOCKED: Guardrails failed calibration (${guardrailResult?.rejectionReasons?.join(', ') || 'Failed'}). Cannot present unsafe proposal to human gate.`
      );
    }
   ...
   */
  public processReview(input: SubmitReviewInput): PolicyReviewRecord {
    ...
    if (
      normalizedReviewer.includes('AGENT') ||
      ...
      throw new Error(
        `ANTI_SELF_APPROVAL_VIOLATION: Autonomous agent persona '${reviewerId}' cannot approve policy evolution proposals. Explicit human review is mandatory.`
      );
    }
    ...
    if (effectiveRole !== 'MASTER_HUMAN_OPERATOR' && effectiveRole !== 'SUPERVISOR' && effectiveRole !== 'OWNER') {
      throw new Error(
        `UNAUTHORIZED_REVIEWER_ROLE: Reviewer role '${effectiveRole}' lacks authority to approve policy evolution.`
      );
    }
