...
import { recognizeDevice, type DevicePresentation, type RecognitionResult } from './pairingRecognition.js';
...
export class PairingRuntime {
  ...
  private readonly trustRegistry: PairingTrustRegistry;
  private readonly auditLedger: PairingAuditLedger;
  private readonly replayDetector: PairingReplayDetector;

  constructor(
    registry?: PairingRegistry,
    auditLedger?: PairingAuditLedger,
    replayDetector?: PairingReplayDetector
  ) {
  ...
  public getRegistry(): PairingRegistry {
    return this.registry;
  }

  public getTrustRegistry(): PairingTrustRegistry {
    return this.trustRegistry;
  }

  public getAuditLedger(): PairingAuditLedger {
    return this.auditLedger;
  }

  public getReplayDetector(): PairingReplayDetector {
   ...
   */
  public requestPairing(req: PairingRequest): PairingResponse {
    ...
    if (existing && !existing.revoked && existing.pairingState === 'TRUSTED') {
      throw new PairingError(
        ...
        `Device ${req.deviceId} is already paired and trusted in this scope`
      );
    }
  ...
  public confirmPairing(
    ...
    if (!record) {
      throw new PairingError(
        'PAIRING_NOT_FOUND',
        `No pairing record found for device ${deviceId} in specified scope`
      );
    }
   ...
   */
  public recognizeDevice(presentation: DevicePresentation): RecognitionResult {
  ...
  public revokeDevice(
    ...
    revokedBy: string,
    reason: string
  ): { readonly revokedPairing: PairingRecord; readonly revokedTrust?: TrustRecord } {
    const record = this.registry.getPairingRecord(deviceId, scopeString);
    if (!record) {
      throw new PairingError(
        'PAIRING_NOT_FOUND',
        `Cannot revoke unknown device ${deviceId} in scope ${scopeString}`
      );
    }
  ...
  public initiateRePair(
    ...
    if (!record) {
      throw new PairingError(
        'PAIRING_NOT_FOUND',
        `Cannot re-pair unknown device ${deviceId} in scope ${scopeString}`
      );
    }
