...
export type PairingReplayClassification =
  | 'VALID_NEW'
...
export class PairingReplayDetector {
  ...
  private readonly seenFingerprints = new Map<string, string>(); // nonce -> pairingFingerprint
   ...
   */
  public recordSeen(req: PairingRequest): void {
    const deviceKey = `${req.scope.brainId}::${req.deviceId}`;
    const nonceKey = `${deviceKey}::${req.nonce}`;
   ...
   */
  public assertNotReplay(req: PairingRequest, existingRecord?: PairingRecord): void {
    ...
    switch (classification) {
      ...
      case 'STALE_REPLAY':
        throw new Error(
          `[PAIRING_REPLAY] Stale sequence number ${req.sequence} for device ${req.deviceId}`
        );
      case 'MUTATED_REPLAY':
        throw new Error(
          `[PAIRING_MUTATED_REPLAY] Mutated replay payload detected for nonce ${req.nonce}`
        );
      case 'CROSS_SCOPE_REPLAY':
        throw new Error(
          `[PAIRING_SCOPE_MISMATCH] Cross-scope replay attempt detected for device ${req.deviceId}`
        );
      case 'CROSS_DEVICE_REPLAY':
        throw new Error(
          `[PAIRING_DEVICE_MISMATCH] Cross-device replay attempt detected: ${req.deviceId}`
        );
      case 'REVOKED_REPLAY':
        throw new Error(
          `[PAIRING_REVOKED] Replay attempt from revoked device ${req.deviceId}; re-pairing required`
        );
      default:
