...
export type ToolExecutionGateCheckpoint =
  | 'GATE_1_BEFORE_REQUEST_ACCEPTANCE'
  | 'GATE_2_BEFORE_ADAPTER_RESOLUTION'
  | 'GATE_3_BEFORE_ADAPTER_INVOCATION'
...
export class ToolExecutionGate {
   ...
   */
  public assertExecutionPermitted(
    checkpoint: ToolExecutionGateCheckpoint,
    ...
    if (this.isUserStopActiveFn()) {
      ...
      throw new ToolExecutionAbortedError(
        `USER_STOP_ACTIVE: Execution aborted at checkpoint "${checkpoint}". Reason: ${reason}`,
        {
          checkpoint,
          reason,
          ...context,
   ...
   */
  public assertCanResolveAdapter(context?: Readonly<Record<string, unknown>>): void {
   ...
   */
  public assertCanInvokeAdapter(context?: Readonly<Record<string, unknown>>): void {
