...
export class ToolAdapterRegistry {
   ...
   */
  public register(adapter: ToolAdapter): void {
    ...
    if (!adapter.domain || !['shop', 'desktop', 'robot', 'dynamic_code'].includes(adapter.domain)) {
      throw new ToolSecurityViolationError(
        `INVALID_DOMAIN: Tool adapter "${normalizedName}" must declare a valid domain (shop, desktop, robot, dynamic_code).`
      );
    }

    if (typeof adapter.execute !== 'function') {
      throw new ToolSecurityViolationError(
        `INVALID_EXECUTE: Tool adapter "${normalizedName}" must provide an executable function.`
      );
    }

    if (this.adapters.has(normalizedName)) {
      throw new ToolSecurityViolationError(
        `CONFLICTING_REGISTRATION: Tool adapter "${normalizedName}" is already registered. Conflicting re-registration is prohibited.`
      );
    }
   ...
   */
  public resolveAdapter(toolName: string): ToolAdapter {
    ...
    if (!adapter) {
      throw new ToolAdapterNotFoundError(
        `TOOL_NOT_FOUND: Tool adapter "${normalized}" is not registered in the ToolAdapterRegistry.`
      );
    }

    if (this.disabledTools.has(normalized)) {
      throw new ToolAdapterDisabledError(
        `TOOL_DISABLED: Tool adapter "${normalized}" is currently disabled in the ToolAdapterRegistry.`
      );
    }
   ...
   * Disables an adapter.
   */
  public disable(toolName: string): void {
    const normalized = toolName.trim();
   ...
   * Enables an adapter.
   */
  public enable(toolName: string): void {
    const normalized = toolName.trim();
    if (!this.adapters.has(normalized)) {
   ...
   */
  public getAll(): readonly ToolAdapter[] {
   ...
   */
  public resolveDomain(toolName: string): ToolAdapterDomain {
    const normalized = toolName.trim();
    const adapter = this.adapters.get(normalized);
    if (adapter) {
      return adapter.domain;
    }
    if (normalized.startsWith('desktop_')) return 'desktop';
    if (normalized.startsWith('robot_')) return 'robot';
    if (normalized.includes('skill') || normalized.includes('code')) return 'dynamic_code';
    return 'shop';
  }
