...
export class GroundedPlanHumanGate {
  private readonly masterAuthority: MasterHumanAuthority;
   ...
   */
  public requiresConfirmation(binding: GroundedPlanTaskBinding): boolean {
    if (binding.requiresHumanConfirmation) return true;
    if (binding.riskLevel === 'CRITICAL' || binding.riskLevel === 'HIGH') return true;

    for (const step of binding.stepBindings) {
      if (step.requiresApproval || step.isQuarantinedText) return true;
      if (step.riskLevel === 'CRITICAL' || step.riskLevel === 'HIGH') return true;
    }
   ...
   */
  public createConfirmationRequest(binding: GroundedPlanTaskBinding): HumanConfirmationRequest {
    ...
    const sensitiveActionsSummary: string[] = [];
    for (const step of binding.stepBindings) {
      if (step.requiresApproval || step.riskLevel === 'HIGH' || step.riskLevel === 'CRITICAL') {
        sensitiveActionsSummary.push(
          `[Step ${step.stepIndex}] ${step.taskStepOptions.capabilityId}:${step.taskStepOptions.actionName} - Risk: ${step.riskLevel}`
        );
      }
    }
  ...
  public issueConfirmationToken(
    ...
    if (!this.masterAuthority.isMasterOperator(operatorId)) {
      throw new GroundedPlanTaskHumanConfirmationError(
        `Operator "${operatorId}" is NOT authorized as Master Human Authority. Self-approval / unauthorized issuance forbidden.`
      );
    }
  ...
  public validateAndConsumeConfirmation(
    ...
    if (this.consumedTokens.has(confirmation.token)) {
      throw new GroundedPlanTaskHumanConfirmationError(
        `Human confirmation token "${confirmation.token}" has already been consumed (anti-replay violation)`
      );
    }
    ...
    if (confirmation.bindingId !== binding.bindingId) {
      throw new GroundedPlanTaskHumanConfirmationError(
        `Confirmation bindingId mismatch: expected "${binding.bindingId}", got "${confirmation.bindingId}"`
      );
    }
    if (confirmation.tenantId !== binding.tenantId) {
      throw new GroundedPlanTaskHumanConfirmationError(
        `Confirmation tenantId mismatch: expected "${binding.tenantId}", got "${confirmation.tenantId}"`
      );
    }
    if (confirmation.sessionId !== binding.sessionId) {
      throw new GroundedPlanTaskHumanConfirmationError(
        `Confirmation sessionId mismatch: expected "${binding.sessionId}", got "${confirmation.sessionId}"`
      );
    }
    if (confirmation.planId !== binding.sourcePlanId) {
      throw new GroundedPlanTaskHumanConfirmationError(
        `Confirmation planId mismatch: expected "${binding.sourcePlanId}", got "${confirmation.planId}"`
      );
    }
    if (confirmation.planProvenanceHash !== binding.sourcePlanProvenanceHash) {
      throw new GroundedPlanTaskHumanConfirmationError(
        'Confirmation plan provenance hash mismatch; underlying plan has mutated since review'
      );
    }
    ...
    if (!this.masterAuthority.isMasterOperator(confirmation.operatorId)) {
      throw new GroundedPlanTaskHumanConfirmationError(
        `Operator "${confirmation.operatorId}" is not authentic Master Human Authority`
      );
    }
    ...
    if (confirmation.signatureHash !== expectedSig) {
      throw new GroundedPlanTaskHumanConfirmationError(
        'Cryptographic signature tampering detected on HumanConfirmationRecord'
      );
    }
   ...
   */
  public resetConsumedTokens(): void {
