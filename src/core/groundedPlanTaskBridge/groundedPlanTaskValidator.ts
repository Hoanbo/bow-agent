...
export class GroundedPlanTaskValidator {
   ...
   */
  public static validateSourcePlan(plan: unknown): asserts plan is GroundedActionPlan {
    const errors: string[] = [];
   ...
   */
  public static validateBinding(binding: unknown): asserts binding is GroundedPlanTaskBinding {
    const errors: string[] = [];
   ...
   */
  public static validateStepBinding(step: unknown, errors: string[]): asserts step is GroundedPlanTaskStepBinding {
   ...
   */
  public static validateHumanConfirmation(rec: unknown): asserts rec is HumanConfirmationRecord {
    const errors: string[] = [];
   ...
   */
  public static assertNoPrototypePollution(obj: unknown, errors: string[], path: string, depth = 0): void {
    if (depth > 12) return; // bounded depth recursion
    ...
    for (const key of keys) {
      ...
      this.assertNoPrototypePollution((obj as Record<string, unknown>)[key], errors, `${path}.${key}`, depth + 1);
    }
  }
   ...
   */
  public static assertNoForbiddenCoT(obj: unknown, errors: string[], path: string, depth = 0): void {
    if (depth > 12) return;
    if (!obj) return;
