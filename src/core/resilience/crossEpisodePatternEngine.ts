...
import {
  ...
  CognitiveEpisode,
  CrossEpisodePattern,
  CrossEpisodePatternType,
  PatternProvenance,
  PatternStatus,
} from './cognitiveResilienceTypes.js';

export class CrossEpisodePatternEngine {
  private readonly _patterns: Map<string, CrossEpisodePattern> = new Map();
  private readonly _contradictoryEvidenceMap: Map<string, string[]> = new Map();

  /**
   * Mines patterns across an array of cognitive episodes.
   * Enforces minimum evidence threshold (>= 2 observations).
   */
  public minePatterns(episodes: CognitiveEpisode[]): CrossEpisodePattern[] {
    if (!episodes || episodes.length === 0) return [];

    const discoveredPatterns: CrossEpisodePattern[] = [];

    // 1. Recurring Execution Failures
    const failureGroups = new Map<string, { episodeIds: string[]; timestamps: number[]; details: string[] }>();
    ...
    const verificationFailures = new Map<string, { episodeIds: string[]; timestamps: number[] }>();
    for (const ep of episodes) {
    ...
    const goalStalls = new Map<string, { episodeIds: string[]; timestamps: number[] }>();
    for (const ep of episodes) {
  ...
  public registerPattern(params: {
    ...
    if (count < 2 || params.supportingEpisodeIds.length < 2) {
      throw new Error(
        `EPISTEMIC_VIOLATION: Cannot create cross-episode pattern from single observation (count=${count}). Minimum 2 observations required.`
      );
    }
   ...
   */
  public addContradictoryEvidence(patternId: string, contradictoryEpisodeId: string, reason: string): void {
    ...
    const updated: CrossEpisodePattern = {
      ...pattern,
   ...
   */
  public affirmByOwner(patternId: string, ownerId: string): CrossEpisodePattern {
    const pattern = this._patterns.get(patternId);
    if (!pattern) throw new Error(`Pattern '${patternId}' not found.`);
  ...
  public getPattern(patternId: string): CrossEpisodePattern | undefined {
    return this._patterns.get(patternId);
  }

  public getAllPatterns(): CrossEpisodePattern[] {
    return Array.from(this._patterns.values());
  }

  public getActivePatterns(): CrossEpisodePattern[] {
  ...
  private _registerOrUpdatePattern(params: {
    patternType: CrossEpisodePatternType;
    description: string;
    supportingEpisodeIds: string[];
    observationCount: number;
    ...
    const record: CrossEpisodePattern = {
      patternId,
      patternType: params.patternType,
      description: params.description,
