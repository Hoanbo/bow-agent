...
export class SelfReflectiveEngine {
   ...
   */
  public reflect(input: ReflectionInput): ReflectionRecord {
    const reasoningQuality = this._assessReasoningQuality(
      input.reasoningWasEvidenceBacked,
  ...
  private _assessReasoningQuality(
    ...
    if (!evidenceBacked && assumptions.length > 2) return 'REASONING_ASSUMPTION_BASED';
    if (missing.length > 2) return 'REASONING_INCOMPLETE';
    if (evidenceBacked && assumptions.length === 0 && missing.length === 0) return 'REASONING_SOUND';
    if (!evidenceBacked) return 'REASONING_ASSUMPTION_BASED';
    if (missing.length > 0) return 'REASONING_INCOMPLETE';
    return 'REASONING_SOUND';
  }

  private _assessDecisionQuality(matched: boolean | null | undefined): ReflectionVerdict {
    if (matched === null || matched === undefined) return 'UNKNOWN';
    if (matched === true) return 'RECOMMENDATION_CORRECT';
    if (matched === false) return 'RECOMMENDATION_INCORRECT';
    return 'UNKNOWN';
  }

  private _assessPredictionQuality(outcome?: 'VERIFIED_CORRECT' | 'VERIFIED_INCORRECT' | 'UNVERIFIED'): ReflectionVerdict {
    if (!outcome || outcome === 'UNVERIFIED') return 'PREDICTION_UNVERIFIED';
    if (outcome === 'VERIFIED_CORRECT') return 'PREDICTION_VERIFIED';
    return 'PREDICTION_REFUTED';
  }

  private _generateInsights(
    input: ReflectionInput,
    reasoning: ReflectionVerdict,
    decision: ReflectionVerdict,
    prediction: ReflectionVerdict
  ): string[] {
  ...
  private _generateOpenQuestions(input: ReflectionInput): string[] {
  ...
  public getLatestReflection(): ReflectionRecord | undefined {
    return this._reflections[this._reflections.length - 1];
  }

  public getReflectionsForEpisode(episodeId: string): ReflectionRecord[] {
