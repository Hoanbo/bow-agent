...
export class CognitiveResilienceRuntime {
  private _healthState: ResilienceHealthState = 'HEALTHY';
  private _isStopped = false;
  private _stopReason = '';

  private readonly _failures: Map<string, ResilienceFailureRecord> = new Map();
  private readonly _proposals: Map<string, ResilienceRecoveryProposal> = new Map();
  private readonly _attempts: Map<string, ResilienceRecoveryAttempt[]> = new Map();
  private readonly _verifications: Map<string, ResilienceVerificationResult> = new Map();
  private readonly _resolvedFailures: Set<string> = new Set();
  private readonly _stateStore?: DurableResilienceStateStore;
  private _createdAt: number = Date.now();

  constructor(options?: { stateStore?: DurableResilienceStateStore }) {
  ...
  public getStateStore(): DurableResilienceStateStore | undefined {
    return this._stateStore;
  }

  // INVARIANT: USER_STOP > EVERYTHING_AUTONOMOUS
  public emergencyStop(reason = 'Master Owner USER_STOP invoked'): void {
    ...
    for (const attempts of this._attempts.values()) {
      for (const att of attempts) {
        if (att.status === 'IN_PROGRESS') {
          (att as any).status = 'BLOCKED_BY_STOP';
          (att as any).error = `Halted by USER_STOP: ${reason}`;
        }
      }
    }
    if (this._stateStore) {
  ...
  public resetStop(operatorId: string): void {
  ...
  public getHealthState(): ResilienceHealthState {
  ...
  public detectFailure(params: {
    ...
    const finalClass: FailureClass = isRecurring ? 'REPEATED_RECOVERY_FAILURE' : inferredClass;

    const record: ResilienceFailureRecord = {
  ...
  private _inferFailureClass(description: string, evidence: string[]): FailureClass {
    const combined = [description, ...evidence].join(' ').toLowerCase();
    if (combined.includes('stale') || combined.includes('expired') || combined.includes('outdated')) return 'STALE_STATE';
    if (combined.includes('unavailable') || combined.includes('not found') || combined.includes('missing capability')) return 'CAPABILITY_UNAVAILABLE';
    if (combined.includes('interrupt') || combined.includes('partial') && combined.includes('execution')) return 'PARTIAL_EXECUTION';
    if (combined.includes('corrupt') || combined.includes('integrity') || combined.includes('hash mismatch')) return 'STATE_CORRUPTED';
    if (combined.includes('contradict')) return 'CONTRADICTED';
    if (combined.includes('telemetry') || combined.includes('unmeasured') || combined.includes('unknown host')) return 'TELEMETRY_DEGRADED';
    if (combined.includes('verification') || combined.includes('unverified')) return 'VERIFICATION_INCOMPLETE';
    if (combined.includes('execution failed') || combined.includes('error')) return 'EXECUTION_FAILURE';
    return 'TRANSIENT';
  }
  ...
  // ---------------------------------------------------------------------------
  public proposeRecovery(failureId: string): ResilienceRecoveryProposal {
    if (this._isStopped) {
    ...
    const { recoveryClass, steps, requiresOwnerApproval, isReversible } =
      this._classifyRecovery(failure);

    const proposal: ResilienceRecoveryProposal = {
  ...
  private _classifyRecovery(failure: ResilienceFailureRecord): {
    recoveryClass: ResilienceRecoveryClass;
    steps: string[];
  ...
  public executeRecovery(
    ...
    if (!proposal) throw new Error(`Recovery proposal '${proposalId}' not found.`);
    ...
    const attempt: ResilienceRecoveryAttempt = {
  ...
  public verifyRecovery(
    ...
    passed: boolean,
    evidence: string,
    method: ResilienceVerificationResult['method'] = 'DIRECT_OBSERVATION'
  ): ResilienceVerificationResult {
    const verification: ResilienceVerificationResult = {
    ...
    for (const attempts of this._attempts.values()) {
      ...
      if (att) {
        (att as any).status = passed ? 'SUCCEEDED' : 'FAILED';
        (att as any).completedAt = Date.now();
        (att as any).verificationPassed = passed;
  ...
  // ---------------------------------------------------------------------------
  public generateRecoveryLessons(failureId: string): string[] {
    const failure = this._failures.get(failureId);
    if (!failure) return [];
  ...
  // Accessors
  // ---------------------------------------------------------------------------
  public getAllFailures(): ResilienceFailureRecord[] {
    return Array.from(this._failures.values());
  }

  public getActiveFailures(): ResilienceFailureRecord[] {
    return Array.from(this._failures.values()).filter((f) => !this._resolvedFailures.has(f.failureId));
  }

  public getAllProposals(): ResilienceRecoveryProposal[] {
    return Array.from(this._proposals.values());
  }

  public getAttemptsForProposal(proposalId: string): ResilienceRecoveryAttempt[] {
    return this._attempts.get(proposalId) ?? [];
  }

  public getVerificationForAttempt(attemptId: string): ResilienceVerificationResult | undefined {
    return this._verifications.get(attemptId);
  }

  public isFailureResolved(failureId: string): boolean {
  ...
  public saveDurableState(): DurableResilienceStateRecord | undefined {
    if (!this._stateStore) return undefined;

    const attemptsRecord: Record<string, ResilienceRecoveryAttempt[]> = {};
    for (const [key, val] of this._attempts.entries()) {
      attemptsRecord[key] = [...val];
    }

    const verificationsRecord: Record<string, ResilienceVerificationResult> = {};
    ...
    for (const atts of this._attempts.values()) {
      for (const a of atts) {
        if (a.status === 'SUCCEEDED') totalRecoveries++;
        if (a.status === 'FAILED') failedRecoveries++;
      }
    }
  ...
  public restoreDurableState(): { wasReconstructed: boolean; wasCorrupted: boolean } | undefined {
    if (!this._stateStore) return undefined;
