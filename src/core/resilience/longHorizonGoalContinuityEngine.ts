...
export class LongHorizonGoalContinuityEngine {
  private readonly _goals: Map<string, LongHorizonGoalRecord> = new Map();
  private readonly _storagePath?: string;
  private readonly _stallThresholdMs: number;

  constructor(options?: { storageDir?: string; stallThresholdMs?: number }) {
  ...
  public registerGoal(params: {
    ...
    const record: LongHorizonGoalRecord = {
      goalId,
   ...
   */
  public recordProgress(goalId: string, progressPercent: number, outcomeDetails?: string): LongHorizonGoalRecord {
    ...
    const updated: LongHorizonGoalRecord = {
      ...goal,
   ...
   */
  public evaluateStallStatus(goalId: string, currentTime = Date.now()): LongHorizonGoalRecord {
    const goal = this._getGoal(goalId);
   ...
   */
  public recordInterruption(goalId: string, reason: string): LongHorizonGoalRecord {
    const goal = this._getGoal(goalId);
    const now = Date.now();
   ...
   */
  public resumeInterruptedGoal(goalId: string): LongHorizonGoalRecord {
    const goal = this._getGoal(goalId);
    const now = Date.now();
   ...
   */
  public linkEpisode(goalId: string, episodeId: string): void {
    const goal = this._getGoal(goalId);
   ...
   */
  public updateFeasibility(
    goalId: string,
    feasibility: 'FEASIBLE' | 'FEASIBLE_WITH_CAUTION' | 'PLAN_BLOCKED' | 'UNKNOWN',
    missingGap?: string
  ): LongHorizonGoalRecord {
    const goal = this._getGoal(goalId);
    const updated: LongHorizonGoalRecord = {
   ...
   */
  public setOwnerIntentSemantic(params: {
    goalId: string;
    operatorId: string;
    ...
    if (!isMasterOwner(params.operatorId)) {
      throw new Error(
        `AUTHORITY_DENIED: Only Master Owner (${MASTER_OWNER_ID}) may affirm owner-semantic goal state (${params.semantic}).`
      );
    }
    ...
    let newState = goal.state;
    if (params.semantic === 'ABANDONED') newState = 'ABANDONED';
    if (params.semantic === 'COMPLETED_BY_EXTERNAL_ACTION') newState = 'COMPLETED';
    if (params.semantic === 'REPRIORITIZED') newState = 'ACTIVE';
  ...
  public getGoal(goalId: string): LongHorizonGoalRecord | undefined {
    return this._goals.get(goalId);
  }

  public getAllGoals(): LongHorizonGoalRecord[] {
    return Array.from(this._goals.values());
  }

  public getActiveGoals(): LongHorizonGoalRecord[] {
  ...
  private _getGoal(goalId: string): LongHorizonGoalRecord {
