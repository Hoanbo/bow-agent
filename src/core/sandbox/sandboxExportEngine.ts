...
export class SandboxExportEngine {
  private exports = new Map<string, SandboxExport>();
   ...
   */
  public exportChanges(input: ExecuteExportInput): SandboxExport {
    ...
    if (review.decision !== 'APPROVED') {
      throw new SandboxError(
        'EXPORT_NOT_APPROVED',
        `Cannot export unapproved sandbox modifications. Current decision: "${review.decision}".`
      );
    }
    ...
    const isAllowedRoot = sandbox.scope.allowedProjectRoots.some(
      (root) => path.resolve(root).toLowerCase() === normalizedTarget
    );

    if (!isAllowedRoot) {
      throw new SandboxError(
        'UNAUTHORIZED_PROJECT_ROOT',
        `Target destination "${targetProjectRoot}" is not in sandbox allowedProjectRoots.`
      );
    }
    ...
    if (review.sessionId !== sandbox.binding.sessionId) {
      throw new SandboxError(
        ...
        'Review session does not match sandbox session.'
      );
    }
   ...
   */
  public getExport(exportId: string): SandboxExport | undefined {
