...
export class SandboxReviewEngine {
  private reviews = new Map<string, SandboxReviewRecord>();
  ...
  public reviewSandbox(input: SandboxReviewInput): SandboxReviewRecord {
    ...
    if (globalMasterHumanAuthority.isUserStopActive || sandbox.isStopped) {
      throw new SandboxError(
        'USER_STOP_ACTIVE',
        'Cannot perform sandbox review while USER_STOP is active.'
      );
    }
    ...
    if (sandbox.binding.agentId === reviewerId) {
      throw new SandboxError(
        ...
        `Agent "${reviewerId}" cannot review or approve its own sandbox "${sandbox.id}".`
      );
    }
    ...
    if (manifest.sandboxId !== sandbox.id || diff.sandboxId !== sandbox.id) {
      throw new SandboxError(
        'POLICY_VIOLATION',
        'Manifest or diff does not match the target sandbox identity.'
      );
    }
    ...
    const record: SandboxReviewRecord = {
      reviewId,
  ...
  public async requestHumanGateApproval(
    ...
    diff: SandboxDiff,
    actionName: string = 'sandbox_worktree_export'
  ): Promise<boolean> {
    if (globalMasterHumanAuthority.isUserStopActive || sandbox.isStopped) {
    ...
    const gateReq = this.humanGate.createRequest(diagnosis as any, plan as any, {
   ...
   */
  public getReview(reviewId: string): SandboxReviewRecord | undefined {
