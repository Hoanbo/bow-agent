...
export class GovernedSandboxManager {
  private sandboxes = new Map<SandboxId, SandboxDescriptor>();
  private _isUserStopped = false;
  private _userStopReason = '';

  constructor(private readonly baseStorageDir: string = path.resolve(process.cwd(), 'data', 'sandboxes')) {}
  ...
  private assertNotStopped(opName: string): void {
    if (this._isUserStopped) {
      throw new SandboxError(
        'USER_STOP_ACTIVE',
        `Operation "${opName}" rejected: USER_STOP is active (${this._userStopReason || 'Emergency Stop'}).`
      );
    }
  }
  ...
  public requestUserStop(reason: string): void {
    ...
    for (const [id, sb] of this.sandboxes.entries()) {
      if (sb.state !== 'REVOKED' && sb.state !== 'EXPIRED' && sb.state !== 'EXPORTED') {
        const frozen: SandboxDescriptor = {
          ...sb,
          state: 'INTERRUPTED',
  ...
  public resetUserStop(): void {
    ...
    for (const [id, sb] of this.sandboxes.entries()) {
      if (sb.isStopped) {
        const restored: SandboxDescriptor = {
          ...sb,
  ...
  public createSandbox(input: CreateSandboxInput): SandboxDescriptor {
    ...
    if (normalizedAgent === 'master_owner' || normalizedAgent === 'owner') {
      throw new SandboxError(
        'SECURITY_VIOLATION',
        `Agent identity cannot impersonate Master Owner: "${input.agentId}"`
      );
    }
   ...
   */
  public getSandbox(id: SandboxId): SandboxDescriptor | undefined {
   ...
   */
  public listSandboxes(sessionId?: string): readonly SandboxDescriptor[] {
  ...
  public transitionState(
    ...
    const terminalStates: SandboxState[] = ['REVOKED', 'EXPIRED', 'DISCARDED'];
    if (terminalStates.includes(sb.state) && nextState === 'ACTIVE') {
      throw new SandboxError(
        'POLICY_VIOLATION',
        `Cannot reactivate sandbox "${sandboxId}" from terminal state "${sb.state}".`
      );
    }

    const updated: SandboxDescriptor = {
      ...sb,
      state: nextState,
   ...
   */
  public revokeSandbox(sandboxId: SandboxId, reason: string): SandboxDescriptor {
    const sb = this.sandboxes.get(sandboxId);
    if (!sb) {
      throw new SandboxError('POLICY_VIOLATION', `Sandbox "${sandboxId}" not found.`);
    }

    const revoked: SandboxDescriptor = {
      ...sb,
      state: 'REVOKED',
   ...
   */
  public revokeByDelegation(delegationId: string, reason: string): number {
    let count = 0;
    for (const [id, sb] of this.sandboxes.entries()) {
