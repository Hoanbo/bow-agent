 ...
 */
export function computeEnvelopeChecksum(envelope: Omit<VaultEnvelope, 'integrity'>): string {
 ...
 */
export function validateVaultEntryIntegrity(entry: VaultEntry): {
 ...
 */
export function validateDeviceCredentialRecordIntegrity(
 ...
 */
export function verifyVaultEnvelopeIntegrity(
  rawOrParsed: string | VaultEnvelope,
  options?: IntegrityCheckOptions
): VaultIntegrityRecord {
  const now = Date.now();
  ...
  if (typeof rawOrParsed === 'string') {
    try {
      envelope = JSON.parse(rawOrParsed) as VaultEnvelope;
    } catch (err) {
      return {
        checkedAt: now,
        valid: false,
        ...
        failureReason: `Payload truncation or invalid JSON format: ${(err as Error).message}`,
      };
    }
  } else {
