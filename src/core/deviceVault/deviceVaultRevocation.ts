 ...
 */
export function revokeDeviceCredentialRecord(
  record: DeviceCredentialRecord,
  reason: string,
  timestamp?: number
): RevokeDeviceRecordResult {
  const now = timestamp ?? Date.now();

  const updatedEntries: VaultEntry[] = record.entries.map((entry) =>
    updateVaultEntryStatus(entry, 'REVOKED', now)
  );
