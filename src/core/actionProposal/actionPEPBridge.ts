...
import type { EnforcementContext, EnforcementDecision } from '../policyEnforcement/policyEnforcementTypes.js';
import {
  type ActionProposal,
...
export class ActionPEPBridge {
   ...
   */
  public async enforceProposal(params: {
    ...
    if (!pdpDecision.allowed || pdpDecision.action !== 'PERMIT') {
      throw new ProposalPEPDenyError(
        `PEP_ENFORCEMENT_BLOCKED: Cannot enforce proposal "${proposal.proposalId}" that was not permitted by PDP (Status: ${pdpDecision.action}).`
      );
    }
    ...
    try {
      ...
      throw new ProposalPolicyUnavailableError(
        `PEP_EXECUTION_FAILED: Exception during PEP enforcement: ${err.message}`,
        { proposalId: proposal.proposalId, error: err.message }
      );
    }

    if (!decision || typeof decision.allowed !== 'boolean') {
      throw new ProposalPolicyUnavailableError(
        'MALFORMED_PEP_RESPONSE: GovernedPolicyEnforcementPoint returned an invalid or empty decision envelope.'
      );
    }
    ...
    if (!decision.allowed) {
      throw new ProposalPEPDenyError(
        `PEP_ENFORCEMENT_DENIED: Action "${proposal.actionType}" was rejected by PEP. Reason: ${decision.reason}`,
        {
