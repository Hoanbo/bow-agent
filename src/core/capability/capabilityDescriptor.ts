...
export function validateCapabilityParameters(descriptor: CapabilityDescriptor, params: Record<string, any>): void {
    ...
    for (const reqField of required) {
      if (params[reqField] === undefined) {
        throw new CapabilityError(
          'RECOVERABLE',
          `Missing required parameter "${reqField}" for capability "${descriptor.capabilityId}".`,
