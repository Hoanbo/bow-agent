...
export function mapRiskToPermission(risk: ActionRiskLevel): PermissionLevel {
  switch (risk) {
    case 'OBSERVE':
      return 'OBSERVE';
    case 'LOW':
...
export function requiresAuthorizationToken(level: PermissionLevel): boolean {
  return level !== 'OBSERVE' && level !== 'LOW_RISK';
}

export function requiresExplicitConfirmation(level: PermissionLevel): boolean {
