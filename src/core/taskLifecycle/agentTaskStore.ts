...
import {
  type AgentTask,
  type ProvenanceCalculationInput,
...
export class AgentTaskStore {
   ...
   */
  public getTenantTasksDir(tenantId: string): string {
   ...
   */
  public getTaskFilePath(tenantId: string, taskId: string): string {
   ...
   */
  public calculateProvenanceHash(input: ProvenanceCalculationInput): string {
    const canonicalStr = [
      input.taskId,
      input.tenantId,
      input.state,
   ...
   */
  public verifyTaskProvenance(task: AgentTask): boolean {
  ...
  public saveTask(task: AgentTask): void {
    ...
    const sanitized = this.sanitizer.sanitize(task) as AgentTask;
   ...
   */
  public getTask(tenantId: string, taskId: string): AgentTask | undefined {
  ...
  public rehydrate(
    tenantId: string,
    options?: { readonly isUserStopActive?: () => boolean }
  ): RehydrationResult {
    const tasksDir = this.getTenantTasksDir(tenantId);
    const files = fs.readdirSync(tasksDir).filter((f) => f.endsWith('.json') && !f.includes('.tmp'));

    const rehydrated: AgentTask[] = [];
