...
export class CognitiveStateRecoveryEngine {
  private readonly persistence: CognitiveStatePersistenceEngine;
   ...
   */
  public recover(tenantId: string, sessionId: string, activeTenantId?: string): CognitiveStateRecoveryResult {
    ...
    if (fs.existsSync(backupPath)) {
      try {
        ...
        throw new CognitiveStateIntegrityError(
          `Both canonical and backup state partitions for ${tenantId}::${sessionId} are corrupted or tampered`,
          { canonicalPath, backupPath, reason: backupErr.message }
        );
      }
    }
   ...
   */
  public rehydrateStore(
    store: WorkingRegisterStore,
    tenantId: string,
    sessionId: string,
    activeTenantId?: string
  ): CognitiveStateRecoveryResult {
   ...
   */
  private loadAndValidateFile(filePath: string, expectedTenant: string, expectedSession: string): CognitiveStateDocument {
    ...
    if (parsed.schemaVersion !== COGNITIVE_STATE_SCHEMA_VERSION) {
      throw new CognitiveStateRecoveryError(
        `Unsupported schemaVersion ${parsed.schemaVersion}; expected ${COGNITIVE_STATE_SCHEMA_VERSION}`
      );
    }
    ...
    if (parsed.sessionId !== expectedSession) {
      throw new CognitiveStateRecoveryError(
        `Session ID mismatch: file has '${parsed.sessionId}', expected '${expectedSession}'`
      );
    }
    ...
    const doc = parsed as CognitiveStateDocument;
    ...
    if (doc.provenance.stateHash !== expectedHash) {
      throw new CognitiveStateIntegrityError(
        `Tampered state detected in ${filePath}: claimed hash '${doc.provenance.stateHash}', calculated '${expectedHash}'`,
        { claimedHash: doc.provenance.stateHash, expectedHash }
      );
    }
