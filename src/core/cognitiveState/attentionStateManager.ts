...
export class AttentionStateManager {
  ...
  public get currentState(): AttentionState {
    return this.state;
  }
   ...
   */
  public setPrimaryTarget(target: AttentionTargetDescriptor | null): AttentionState {
    if (target) {
   ...
   */
  public setSecondaryTargets(targets: readonly AttentionTargetDescriptor[]): AttentionState {
    if (targets.length > MAX_SECONDARY_ATTENTION_TARGETS) {
      throw new CognitiveStateValidationError(
        `Secondary attention targets count (${targets.length}) exceeds maximum limit of ${MAX_SECONDARY_ATTENTION_TARGETS}`,
        ['SECONDARY_TARGETS_OVERFLOW']
      );
    }
   ...
   */
  public interrupt(
    newTarget: AttentionTargetDescriptor,
    interruptedBy: string,
    activePriority: number
  ): AttentionState {
    this.validateTarget(newTarget);

    const frame: AttentionFrame = Object.freeze({
   ...
   */
  public resume(): { readonly resumed: boolean; readonly state: AttentionState; readonly restoredFrame?: AttentionFrame } {
   ...
   */
  public static calculateSalience(
    factors: {
      readonly recency: number;           // 0.0 (old) to 1.0 (now)
      readonly userEmphasis: number;      // 0.0 (none) to 1.0 (highest)
      readonly risk: number;              // 0.0 (safe) to 1.0 (critical)
      readonly constraintOverlap: number; // 0.0 (disjoint) to 1.0 (exact match)
    },
    ...
    const raw =
      Math.max(0, Math.min(1, factors.recency)) * wr +
  ...
  private validateTarget(target: AttentionTargetDescriptor): void {
