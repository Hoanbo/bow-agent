...
export function validateServiceRequest(envelope: unknown): BrainServiceRequestEnvelope {
  if (!envelope || typeof envelope !== 'object') {
    throw new BrainServiceError(
      'BRAIN_SERVICE_INVALID_REQUEST',
      'Request envelope must be a non-null object'
    );
  }

  const req = envelope as Partial<BrainServiceRequestEnvelope>;

  if (typeof req.requestId !== 'string' || req.requestId.trim().length === 0) {
    throw new BrainServiceError(
      'BRAIN_SERVICE_INVALID_REQUEST',
      'Missing or invalid "requestId"'
    );
  }
  ...
  if (!req.deviceContext || typeof req.deviceContext !== 'object') {
    throw new BrainServiceError(
      'BRAIN_SERVICE_INVALID_REQUEST',
      'Missing or invalid "deviceContext"'
    );
  }
  ...
  if (typeof req.deviceContext.deviceId !== 'string' || req.deviceContext.deviceId.trim().length === 0) {
    throw new BrainServiceError(
      'BRAIN_SERVICE_INVALID_REQUEST',
      'deviceContext missing valid "deviceId"'
    );
  }
  ...
  if (!req.input || typeof req.input !== 'object') {
    throw new BrainServiceError(
      'BRAIN_SERVICE_INVALID_REQUEST',
      'Missing or invalid "input" in request envelope'
    );
  }
  ...
  if (typeof req.input.userText !== 'string' || req.input.userText.trim().length === 0) {
    throw new BrainServiceError(
      'BRAIN_SERVICE_INVALID_REQUEST',
      'Input must contain non-empty "userText" string field'
    );
  }

  // Check for malicious null bytes in all string properties
  const scanNull = (val: string, label: string) => {
    if (val.includes('\0')) {
      throw new BrainServiceError(
        'BRAIN_SERVICE_INVALID_REQUEST',
        `Null byte detected in request field "${label}"`
      );
    }
  };
  ...
  return req as BrainServiceRequestEnvelope;
}
