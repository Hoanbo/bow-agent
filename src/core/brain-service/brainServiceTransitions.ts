...
export const VALID_SERVICE_TRANSITIONS: Readonly<Record<BrainServiceLifecycleState, readonly BrainServiceLifecycleState[]>> = Object.freeze({
...
export function isValidServiceTransition(
  from: BrainServiceLifecycleState,
  to: BrainServiceLifecycleState
): boolean {
...
export function assertValidServiceTransition(
  from: BrainServiceLifecycleState,
  to: BrainServiceLifecycleState
): void {
  if (!isValidServiceTransition(from, to)) {
    throw new Error(
      `[BrainService] Illegal state transition: "${from}" -> "${to}". Transition not permitted.`
    );
  }
}
