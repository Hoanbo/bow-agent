...
export type IntentType =
  | 'INFORMATION_REQUEST' | 'EXPLANATION_REQUEST' | 'SEARCH_REQUEST' | 'CREATE_REQUEST'
  | 'UPDATE_REQUEST' | 'DELETE_REQUEST' | 'EXECUTE_REQUEST' | 'CANCEL_REQUEST'
  | 'APPROVE_REQUEST' | 'REVOKE_REQUEST' | 'STATUS_REQUEST' | 'COMPARISON_REQUEST'
  | 'CONFIGURATION_REQUEST' | 'UNKNOWN';
...
export interface IntentInterpretationInput {
  ...
  sessionId: string;
  userText: unknown;
  context?: { recentTurns?: readonly { id: string; sender: string; content: string }[] };
  workingMemory?: readonly { id: string; sender: string; content: string }[];
}
