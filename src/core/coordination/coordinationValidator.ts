 ...
 */
export function redactCoordinationSecrets(text: string): string {
 ...
 */
export function containsCoordinationSecret(val: unknown): boolean {
 ...
 */
export function hasCoordinationPrototypePollution(target: unknown): boolean {
  ...
  for (const [k, v] of Object.entries(target)) {
    ...
    if (v && typeof v === 'object' && hasCoordinationPrototypePollution(v)) return true;
  }
 ...
 */
export function validateCoordinationScope(userId: string, sessionId: string, brainId?: string): void {
 ...
 */
export function assertCoordinationRiskPreservation(
  ...
  if (currentRank < priorRank) {
    throw new Error(
      `RISK_DOWNGRADE_FORBIDDEN: Cannot downgrade coordination risk from ${priorRisk} to ${currentRisk}`,
    );
  }
}
 ...
 */
export function assertCoordinationSequenceMonotonicity(
  currentSequence: number,
  incomingSequence: number,
): void {
  if (incomingSequence <= currentSequence) {
    throw new Error(
      `STALE_COORDINATION_SEQUENCE: Incoming sequence ${incomingSequence} must be strictly greater than current ${currentSequence}`,
    );
  }
}
