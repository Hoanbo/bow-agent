...
export function validateHandoff(
  ...
  currentContext: ContinuityContext,
...
export function acceptHandoff(
  request: HandoffRequest,
  currentContext: ContinuityContext,
  updatedActiveSurfaces: readonly ActiveSurface[],
): HandoffResult {
  const newSequence = request.sequence;
  const activeIds = updatedActiveSurfaces.map(s => s.surface.surfaceId);

  const continuityFingerprint = computeContinuityFingerprint(
    request.brainId,
    request.userId,
    request.sessionId,
    newSequence,
  ...
  const updatedContext: ContinuityContext = deepFreeze({
  ...
  const handoffFingerprint = computeHandoffFingerprint(
    request.handoffId,
    request.brainId,
    request.sourceSurfaceId,
    request.targetSurfaceId,
    newSequence,
  );
