...
export class PolicyFeedbackReviewQueue {
  ...
  private readonly tenantEntries: Map<string, Map<string, FeedbackReviewQueueEntry>> = new Map();
  // Proposal lookup for anti-duplicate: tenantPartition -> Map<proposalId, reviewId>
  private readonly proposalLookup: Map<string, Map<string, string>> = new Map();
  ...
  private loadTenantStateIfEmpty(tenantPartition: string): void {
    ...
    const map = new Map<string, FeedbackReviewQueueEntry>();
    ...
    if (fs.existsSync(queueFile)) {
      try {
        ...
        const parsed: FeedbackReviewQueueEntry[] = JSON.parse(raw);
   ...
   */
  public enqueue(entry: FeedbackReviewQueueEntry): FeedbackReviewQueueEntry {
   ...
   */
  public getEntry(tenantPartition: string, reviewId: FeedbackReviewId): FeedbackReviewQueueEntry | undefined {
    this.assertUserStopInactive();
   ...
   */
  public getEntryByProposalId(tenantPartition: string, proposalId: string): FeedbackReviewQueueEntry | undefined {
    this.assertUserStopInactive();
    this.loadTenantStateIfEmpty(tenantPartition);
   ...
   */
  public listEntries(tenantPartition: string, options?: ListQueueOptions): ListQueueResult {
    this.assertUserStopInactive();
    this.loadTenantStateIfEmpty(tenantPartition);
  ...
  public updateEntryState(
    ...
    newState: FeedbackReviewLifecycleState
  ): FeedbackReviewQueueEntry {
    this.assertUserStopInactive();
    this.loadTenantStateIfEmpty(tenantPartition);
    ...
    const terminalStates: FeedbackReviewLifecycleState[] = [
      'ACCEPTED',
      'REJECTED',
      'CANCELLED',
      'EXPIRED',
