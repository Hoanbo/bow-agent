...
export class DeploymentReportEngine {
   ...
   */
  public computeReportHash(report: Omit<DeploymentVerificationReport, 'reportHash'>): string {
   ...
   */
  public compileReport(input: CompileDeploymentReportInput): DeploymentVerificationReport {
    const reportId = createDeploymentReportId(`report_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`);
    const generatedAt = Date.now();

    const partialReport: Omit<DeploymentVerificationReport, 'reportHash'> = {
      reportId,
