...
export class DeploymentContradictionEngine {
  private contradictions = new Map<string, DeploymentContradictionRecord[]>();
  ...
  public detectContradictions(
    ...
    if (hasStatusConflict) conflictingFields.push('reportedStatus');
    if (hasScoreConflict) conflictingFields.push('reportedHealthScore');

    if (conflictingFields.length === 0) {
      return null;
    }

    const contradictionRecord: DeploymentContradictionRecord = {
   ...
   */
  public getContradictions(deploymentId: DeploymentId): readonly DeploymentContradictionRecord[] {
