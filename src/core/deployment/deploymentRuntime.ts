...
import {
  ...
  type DeploymentContradictionRecord,
...
export class DeploymentRuntime {
  private requests = new Map<string, DeploymentRequest>();
  private candidates = new Map<string, DeploymentCandidate>();
  private states = new Map<string, DeploymentState>();
  private approvals = new Map<string, DeploymentApprovalBinding>();
  private authorizations = new Map<string, DeploymentAuthorizationBinding>();
  private activeTokens = new Map<string, AuthorizationToken>();
  private highestRings = new Map<string, RolloutRingLevel>();
  private canaryRecords = new Map<string, CanaryVerificationRecord[]>();
  private backups = new Map<string, DeploymentBackupEntry[]>();
  private preManifestHashes = new Map<string, string>();
  private postManifestHashes = new Map<string, string>();
  private deployedFiles = new Map<string, string[]>();
  private rollbackRecords = new Map<string, DeploymentRollbackRecord>();
  private contradictionRecords = new Map<string, DeploymentContradictionRecord>();

  private isUserStopActive = false;
  private isRevoked = false;

  constructor(
    private readonly policyEngine: DeploymentPolicyEngine = new DeploymentPolicyEngine(),
    private readonly ringEngine: DeploymentRingEngine = new DeploymentRingEngine(),
    private readonly canaryEngine: CanaryVerificationEngine = new CanaryVerificationEngine(),
    private readonly sloEngine: SloPolicyEngine = new SloPolicyEngine(),
    private readonly circuitBreaker: DeploymentCircuitBreaker = new DeploymentCircuitBreaker(),
    private readonly executionEngine: DeploymentExecutionEngine = new DeploymentExecutionEngine(),
    private readonly rollbackEngine: DeploymentRollbackEngine = new DeploymentRollbackEngine(),
    private readonly provenanceEngine: DeploymentProvenanceEngine = new DeploymentProvenanceEngine(),
    private readonly contradictionEngine: DeploymentContradictionEngine = new DeploymentContradictionEngine(),
    private readonly reportEngine: DeploymentReportEngine = new DeploymentReportEngine(),
    ...
    for (const [deploymentId, state] of this.states.entries()) {
      if (state !== 'COMPLETED' && state !== 'ROLLED_BACK' && state !== 'FAILED') {
        ...
        this.logAudit(
          'USER_STOP_TRIGGERED',
          'EMERGENCY_HALT',
          { id: 'MASTER_OWNER', role: 'OWNER' },
          { id: deploymentId, type: 'DEPLOYMENT' },
          { reason }
        );
      }
    }
  ...
  public getState(deploymentId: DeploymentId): DeploymentState {
  ...
  public requestDeployment(
  ...
  ): { readonly deploymentId: DeploymentId; readonly state: DeploymentState } {
    ...
    this.logAudit(
      ...
      'VALIDATE_DEPLOYMENT',
      { id: request.operatorId, role: 'AGENT' },
      { id: request.deploymentId, type: 'DEPLOYMENT' },
      {
   ...
   */
  public requestHumanReview(deploymentId: DeploymentId): HumanGateRequest {
   ...
   */
  public recordOwnerReview(options: {
    ...
    if (options.reviewerId === request.operatorId) {
      throw new DeploymentError(
        ...
        `Operator "${options.reviewerId}" cannot review or approve its own deployment request.`
      );
    }

    const isOwner = options.reviewerType === 'MASTER_OWNER' && isMasterOwner(options.reviewerId);

    const approval: DeploymentApprovalBinding = {
    ...
    if (options.decision === 'REJECTED') {
      ...
      this.logAudit(
        'DEPLOYMENT_REJECTED',
        'HUMAN_DECISION',
        { id: options.reviewerId, role: options.reviewerType },
        { id: options.deploymentId, type: 'DEPLOYMENT' },
        { rationale: options.rationale }
      );
    } else {
      ...
      this.logAudit(
        ...
        { id: options.reviewerId, role: options.reviewerType },
        { id: options.deploymentId, type: 'DEPLOYMENT' },
        { isOwnerApproval: isOwner, rationale: options.rationale }
      );
    }
   ...
   */
  public bindAuthorizationToken(
    deploymentId: DeploymentId,
    token: AuthorizationToken,
    targetRing: RolloutRingLevel
  ): DeploymentAuthorizationBinding {
    const request = this.requests.get(deploymentId);
    if (!request) {
      throw new DeploymentError('INVALID_STATE_TRANSITION', `Deployment "${deploymentId}" not found.`);
    }
    ...
    const binding: DeploymentAuthorizationBinding = {
      deploymentId,
    ...
    this.logAudit(
      'DEPLOYMENT_TOKEN_BOUND',
      'AUTHORIZATION_BINDING',
      { id: request.operatorId, role: 'AGENT' },
      { id: deploymentId, type: 'DEPLOYMENT' },
      { tokenId: token.tokenId, tokenHash, targetRing }
    );
   ...
   */
  public executeRollout(deploymentId: DeploymentId, options: ExecuteRolloutOptions): void {
    const request = this.requests.get(deploymentId);
    const candidate = this.candidates.get(deploymentId);
    ...
    if (!request || !candidate || !token) {
      throw new DeploymentError(
        ...
        `Missing valid request, candidate, or execution token for deployment "${deploymentId}".`
      );
    }
    ...
    if (this.circuitBreaker.isOpen(deploymentId)) {
      throw new DeploymentError(
        ...
        `Cannot execute rollout: Circuit breaker is OPEN for deployment "${deploymentId}".`
      );
    }
   ...
   */
  public recordCanaryObservations(
    deploymentId: DeploymentId,
    ...
    const record = this.canaryEngine.verifyCanary(
      deploymentId,
      ringLevel,
      observations,
      request.sloPolicy,
    ...
    if (record.isPassing) {
      ...
      this.logAudit(
        'CANARY_VERIFICATION_PASSED',
        'EVALUATE_CANARY',
        { id: request.operatorId, role: 'AGENT' },
        { id: deploymentId, type: 'DEPLOYMENT' },
        { ringLevel, evidenceHash: record.evidenceHash }
      );
    } else {
      ...
      this.logAudit(
        ...
        { id: request.operatorId, role: 'AGENT' },
        { id: deploymentId, type: 'DEPLOYMENT' },
        { ringLevel, failureReasons: record.failureReasons }
      );
   ...
   */
  public executeSafeRollback(deploymentId: DeploymentId, reason: string): DeploymentRollbackRecord {
    const request = this.requests.get(deploymentId);
    const candidate = this.candidates.get(deploymentId);
   ...
   */
  public reportAgentAssertions(
    deploymentId: DeploymentId,
    ringLevel: RolloutRingLevel,
    assertions: readonly DeploymentAgentAssertion[]
  ): DeploymentContradictionRecord | null {
    ...
    if (contradiction) {
      ...
      this.logAudit(
        'DEPLOYMENT_CONTRADICTION_DETECTED',
        'ESCALATE_HUMAN_GATE',
        { id: assertions[0].agentId, role: 'AGENT' },
        { id: deploymentId, type: 'DEPLOYMENT' },
        {
          ringLevel,
   ...
   */
  public finalizeDeployment(deploymentId: DeploymentId): DeploymentResult {
    const request = this.requests.get(deploymentId);
    const candidate = this.candidates.get(deploymentId);
