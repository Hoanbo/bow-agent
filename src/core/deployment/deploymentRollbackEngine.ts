...
export class DeploymentRollbackEngine {
   ...
   */
  public executeRollback(input: ExecuteDeploymentRollbackInput): DeploymentRollbackRecord {
    ...
    if (!fs.existsSync(targetRoot)) {
      throw new DeploymentError(
        ...
        `Cannot rollback: Target directory does not exist: "${targetRoot}".`
      );
    }
    ...
    if (!isVerified) {
      throw new DeploymentError(
        'ROLLBACK_VERIFICATION_FAILED',
        `Post-rollback manifest hash "${postRollbackManifest.manifestHash}" does not match expected pre-deployment hash "${input.expectedPreManifestHash}".`
      );
    }
