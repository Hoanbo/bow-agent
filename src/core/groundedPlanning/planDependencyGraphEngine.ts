...
export class PlanDependencyGraphEngine {
   ...
   */
  public static validateAndSort(steps: readonly GroundedActionStep[]): GroundedActionStep[] {
    if (steps.length === 0) return [];

    const stepMap = new Map<string, GroundedActionStep>();
      ...
      for (const depId of step.dependsOnStepIds) {
        ...
        if (!stepMap.has(depId)) {
          throw new GroundedPlanValidationError(
            `Step '${step.stepId}' references non-existent dependency '${depId}'`
          );
        }
      }
    }
    ...
    const checkCycles = (currentId: string, path: string[], depth: number): void => {
      if (depth > GROUNDED_PLAN_BOUNDS.MAX_DEPENDENCY_DEPTH) {
        throw new GroundedPlanValidationError(
          `Dependency chain depth exceeded maximum permitted limit of ${GROUNDED_PLAN_BOUNDS.MAX_DEPENDENCY_DEPTH} at step '${currentId}'`
        );
      }
    ...
    const available: GroundedActionStep[] = [];
    for (const step of steps) {
    ...
    const sorted: GroundedActionStep[] = [];
