...
const VALID_INTENT_TYPES = new Set<GroundedPlanIntentType>([
  'NAVIGATE',
  'INSPECT',
  'INPUT_TEXT',
  'SELECT_ELEMENT',
...
const VALID_RISK_LEVELS = new Set<GroundedPlanRiskLevel>([
  'LOW',
  'MEDIUM',
  'HIGH',
  'CRITICAL',
...
const VALID_STATUSES = new Set<GroundedPlanStatus>([
  'DRAFT',
  'SYNTHESIZED',
  'VALIDATED',
  'SUBMITTED_TO_PDP',
...
export class GroundedPlanValidator {
   ...
   */
  public static validatePlan(plan: unknown): asserts plan is GroundedActionPlan {
    ...
    if (typeof p.planId !== 'string' || !p.planId.trim()) errors.push('planId must be non-empty string');
    if (typeof p.schemaVersion !== 'string' || !p.schemaVersion.trim()) errors.push('schemaVersion must be non-empty string');
    if (typeof p.tenantId !== 'string' || !p.tenantId.trim()) errors.push('tenantId must be non-empty string');
    if (typeof p.sessionId !== 'string' || !p.sessionId.trim()) errors.push('sessionId must be non-empty string');
    if (typeof p.goalId !== 'string' || !p.goalId.trim()) errors.push('goalId must be non-empty string');
    ...
    if (typeof p.status !== 'string' || !VALID_STATUSES.has(p.status as GroundedPlanStatus)) {
      errors.push(`Invalid plan status: '${p.status}'`);
    }
    if (typeof p.overallRiskLevel !== 'string' || !VALID_RISK_LEVELS.has(p.overallRiskLevel as GroundedPlanRiskLevel)) {
      errors.push(`Invalid overallRiskLevel: '${p.overallRiskLevel}'`);
    }

    // 4. Timestamps & Version (Dấu thời gian và phiên bản)
    if (typeof p.createdAt !== 'string' || isNaN(Date.parse(p.createdAt))) errors.push('createdAt must be valid ISO timestamp');
    if (typeof p.updatedAt !== 'string' || isNaN(Date.parse(p.updatedAt))) errors.push('updatedAt must be valid ISO timestamp');
   ...
   */
  public static validateStep(step: unknown, expectedIndex?: number): asserts step is GroundedActionStep {
    ...
    if (typeof s.stepId !== 'string' || !s.stepId.trim()) errors.push('stepId must be non-empty string');
    ...
    if (typeof s.intentType !== 'string' || !VALID_INTENT_TYPES.has(s.intentType as GroundedPlanIntentType)) {
    ...
    if (typeof s.goalId !== 'string' || !s.goalId.trim()) errors.push('goalId must be non-empty string');

    if (typeof s.riskLevel !== 'string' || !VALID_RISK_LEVELS.has(s.riskLevel as GroundedPlanRiskLevel)) {
      errors.push(`Invalid riskLevel: '${s.riskLevel}'`);
    }

    // Numeric confidence [0.0, 1.0] (Độ tin cậy dạng số)
    if (
      typeof s.stepConfidence !== 'number' ||
   ...
   */
  public static validateSynthesisRequest(req: unknown): asserts req is GroundedPlanSynthesisRequest {
    ...
    if (typeof r.tenantId !== 'string' || !r.tenantId.trim()) errors.push('tenantId must be non-empty string');
    if (typeof r.sessionId !== 'string' || !r.sessionId.trim()) errors.push('sessionId must be non-empty string');
    if (!r.goal || typeof r.goal !== 'object') errors.push('goal object is required');

    if (r.intentType !== undefined) {
      if (typeof r.intentType !== 'string' || !VALID_INTENT_TYPES.has(r.intentType as GroundedPlanIntentType)) {
    ...
    if (r.minConfidenceThreshold !== undefined) {
      if (
        typeof r.minConfidenceThreshold !== 'number' ||
   ...
   */
  public static assertNoPrototypePollutionOrCoT(obj: unknown, context: string, depth = 0): void {
    ...
    for (const key of Object.keys(rec)) {
      if (FORBIDDEN_OBJECT_KEYS.includes(key)) {
        throw new GroundedPlanSecurityError(
          `Prototype pollution key '${key}' detected in ${context}`,
          { key, context }
        );
      }
      ...
      if (typeof val === 'string') {
        ...
        for (const forbiddenTag of FORBIDDEN_COT_TAGS) {
          if (valLower.includes(forbiddenTag)) {
            throw new GroundedPlanSecurityError(
              `Forbidden Chain-of-Thought (CoT) marker '${forbiddenTag}' detected in ${context} field '${key}'`,
              { forbiddenTag, key, context }
            );
          }
        }
