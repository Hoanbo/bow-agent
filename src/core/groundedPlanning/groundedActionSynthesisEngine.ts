...
export class GroundedActionSynthesisEngine {
   ...
   */
  public synthesizePlan(
    request: GroundedPlanSynthesisRequest,
    options?: { additionalSteps?: readonly Omit<GroundedActionStep, 'stepId' | 'stepIndex' | 'stepHash'>[] }
  ): GroundedActionPlan {
    ...
    const rawSteps: GroundedActionStep[] = [];
    const intentType: GroundedPlanIntentType = request.intentType ?? 'INSPECT';
    ...
    const stepRisk0: GroundedPlanRiskLevel = PlanRiskAssessmentEngine.classifyStepRisk(intentType, sanitizedPayload, isQuarantined);

    const step0Draft: Omit<GroundedActionStep, 'stepHash'> = {
      stepId: stepId0,
      stepIndex: 0,
      intentType,
    ...
    if (options?.additionalSteps) {
      options.additionalSteps.forEach((extra, idx) => {
        ...
        const sDraft: Omit<GroundedActionStep, 'stepHash'> = {
          stepId: sId,
          stepIndex: stepIdx,
    ...
    const planDraft: Omit<GroundedActionPlan, 'provenanceHash'> = {
      planId,
    ...
    const finalPlan: GroundedActionPlan = Object.freeze({
      ...planDraft,
      provenanceHash,
    });
