...
export class GroundedPlanPersistenceRecoveryEngine {
  ...
  public saveSessionDocument(
    doc: GroundedPlanSessionDocument,
    expectedVersion?: number
  ): { writtenPath: string; bytesWritten: number } {
    ...
    if (fs.existsSync(canonicalFile)) {
      try {
        ...
        const existingDoc = JSON.parse(existingRaw) as GroundedPlanSessionDocument;
        ...
        if (err instanceof GroundedPlanConcurrencyError) throw err;
    ...
    try {
      ...
      throw new GroundedPlanPersistenceError(
        `Failed to persist grounded plan session document: ${err instanceof Error ? err.message : String(err)}`
      );
    }
   ...
   */
  public recoverSessionDocument(
    tenantId: string,
    sessionId: string
  ): { document: GroundedPlanSessionDocument; recoveredFromBackup: boolean } {
    // 1. Synchronous USER_STOP Preemption
    if (this.userStopProvider()) {
    ...
    if (fs.existsSync(canonicalFile)) {
      try {
        ...
        const doc = JSON.parse(raw) as GroundedPlanSessionDocument;
        ...
        if (fs.existsSync(backupFile)) {
          try {
            ...
            const bakDoc = JSON.parse(bakRaw) as GroundedPlanSessionDocument;
            this.verifyDocumentIntegrity(bakDoc);
            return { document: bakDoc, recoveredFromBackup: true };
          } catch (bakErr) {
            throw new GroundedPlanIntegrityError(
              `Both canonical and backup files are corrupt for tenant '${tenantId}' session '${sessionId}'`
            );
          }
        }
        throw new GroundedPlanIntegrityError(
          `Canonical plan file corrupt and no valid backup available for session '${sessionId}'`
        );
      }
    }
    ...
    throw new GroundedPlanPersistenceError(
      `No persistent session document found for tenant '${tenantId}' session '${sessionId}'`
    );
  }

  private verifyDocumentIntegrity(doc: GroundedPlanSessionDocument): void {
    const expectedHash = computePlanSessionDocumentHash(doc);
    if (doc.provenanceHash !== expectedHash) {
      throw new GroundedPlanIntegrityError(
        `Session document provenance hash mismatch: stored '${doc.provenanceHash}', computed '${expectedHash}'`
      );
    }
  }
