...
import {
  ...
  type IncidentResilienceProvenanceRecord,
...
export class IncidentResilienceRuntime {
  ...
  constructor(
    public readonly closureEngine: IncidentClosureEngine = new IncidentClosureEngine(),
    public readonly effectivenessEngine: RemediationEffectivenessEngine = new RemediationEffectivenessEngine(),
    public readonly accuracyScorer: HypothesisAccuracyScorer = new HypothesisAccuracyScorer(),
    public readonly oscillationDetector: AntiOscillationDetector = new AntiOscillationDetector(),
    public readonly baselineEngine: BaselineReconciliationEngine = new BaselineReconciliationEngine(),
    public readonly postMortemSynthesizer: IncidentPostMortemSynthesizer = new IncidentPostMortemSynthesizer(),
    public readonly resilienceProvenanceEngine: IncidentResilienceProvenanceEngine = new IncidentResilienceProvenanceEngine(),
  ...
  public setUserStop(active: boolean): void {
    ...
    this.logAudit(
      active ? 'USER_STOP_ACTIVATED' : 'USER_STOP_DEACTIVATED',
      { id: 'OPERATOR', role: 'MASTER_HUMAN_OPERATOR' },
      { id: 'RESILIENCE_RUNTIME', type: 'EMERGENCY_CONTROL' },
      { active, timestamp: Date.now() },
      active ? 'DENY' : 'PERMIT',
      active ? 'BLOCKED' : 'SUCCESS'
    );
  }
   ...
   */
  public executeResiliencePipeline(
    request: ExecuteResiliencePipelineRequest
  ): GovernedResiliencePipelineResponse {
    const {
      incidentId,
      targetId,
      targetPath,
    ...
    this.logAudit(
      'OSCILLATION_EVALUATED',
      { id: 'ANTI_OSCILLATION_DETECTOR', role: 'RESILIENCE_ENGINE' },
      { id: oscillationPattern.patternId, type: 'OSCILLATION_PATTERN' },
      {
        targetId,
    ...
    this.logAudit(
      ...
      { id: 'REMEDIATION_EFFECTIVENESS_ENGINE', role: 'RESILIENCE_ENGINE' },
      { id: incidentId, type: 'INCIDENT' },
      {
    ...
    let hypothesisAccuracy: HypothesisAccuracyRecord | undefined;
    if (primaryHypothesis) {
      ...
      this.logAudit(
        'HYPOTHESIS_CALIBRATED',
        { id: 'HYPOTHESIS_ACCURACY_SCORER', role: 'RESILIENCE_ENGINE' },
        { id: primaryHypothesis.hypothesisId, type: 'HYPOTHESIS' },
        {
    ...
    this.logAudit(
      'BASELINE_RECONCILED',
      { id: 'BASELINE_RECONCILIATION_ENGINE', role: 'RESILIENCE_ENGINE' },
      { id: baselineReconciliation.reconciliationId, type: 'BASELINE_RECONCILIATION' },
      {
        targetId,
    ...
    this.logAudit(
      'INCIDENT_CLOSED',
      { id: 'INCIDENT_CLOSURE_ENGINE', role: 'RESILIENCE_ENGINE' },
      { id: closureRecord.closureId, type: 'INCIDENT_CLOSURE' },
      {
        incidentId,
      ...
      closureRecord.status === 'CLOSED_RESOLVED' ? 'PERMIT' : 'DENY',
      closureRecord.status === 'CLOSED_RESOLVED' ? 'SUCCESS' : 'BLOCKED'
    );
    ...
    this.logAudit(
      'POST_MORTEM_SYNTHESIZED',
      { id: 'INCIDENT_POST_MORTEM_SYNTHESIZER', role: 'RESILIENCE_ENGINE' },
      { id: postMortemReport.reportId, type: 'POST_MORTEM_REPORT' },
      {
        incidentId,
    ...
    this.logAudit(
      'RESILIENCE_PROVENANCE_SEALED',
      { id: 'INCIDENT_RESILIENCE_PROVENANCE_ENGINE', role: 'RESILIENCE_ENGINE' },
      { id: resilienceProvenance.resilienceProvenanceHash, type: 'PROVENANCE_RECORD' },
      {
        incidentId,
  ...
  private logAudit(
    ...
    this.auditLedger.record({
      ...
      toolName,
      classification:
        toolName.includes('VIOLATION') || toolName.includes('USER_STOP') || toolName.includes('ESCALAT')
          ? 'SAFETY'
          : 'OBSERVATION',
      argumentsHash,
      policyDecision,
