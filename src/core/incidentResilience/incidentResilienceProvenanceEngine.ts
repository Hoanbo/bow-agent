...
import type {
  ...
  PostMortemReport,
  IncidentResilienceProvenanceRecord,
...
export class IncidentResilienceProvenanceEngine {
   ...
   */
  public buildProvenance(options: BuildResilienceProvenanceOptions): IncidentResilienceProvenanceRecord {
    const timestamp = Date.now();
    const upstreamRemediationHash =
      options.upstreamRemediationHash ??
   ...
   */
  public verifyProvenance(record: IncidentResilienceProvenanceRecord): boolean {
    const rawCanonical = JSON.stringify({
