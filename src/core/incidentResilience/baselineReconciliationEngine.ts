...
import {
  ...
  type BaselineReconciliationStatus,
...
export class BaselineReconciliationEngine {
   ...
   */
  public reconcileBaseline(input: ReconcileBaselineInput): BaselineReconciliationRecord {
    const {
      targetId,
      verificationResult,
      rollbackResult,
    ...
    if (isUserStopActive) {
      return this.buildRecord(
        ...
        reconciledAt,
        undefined,
        'Reconciliation aborted: USER_STOP signal is active.'
      );
    }
    ...
    if (hasSecurityViolation) {
      return this.buildRecord(
        ...
        reconciledAt,
        undefined,
        'Reconciliation rejected: Target has active security violation.'
      );
    }
    ...
    if (rollbackResult && rollbackResult.success) {
      return this.buildRecord(
        ...
        reconciledAt,
        undefined,
        'Reconciliation skipped: Target state was rolled back to pre-remediation snapshot.'
      );
    }
    ...
    if (!isVerified || !verificationResult) {
      return this.buildRecord(
        ...
        reconciledAt,
        undefined,
        'Reconciliation skipped: Post-mitigation verification did not pass.'
      );
    }
    ...
    if (hasContradictoryEvidence) {
      return this.buildRecord(
        ...
        reconciledAt,
        undefined,
        'Reconciliation rejected: Multi-agent contradiction detected.'
      );
    }
    ...
    if (verificationResult.newInvariantViolationsCount > 0) {
      return this.buildRecord(
        ...
        reconciledAt,
        undefined,
        `Reconciliation rejected: ${verificationResult.newInvariantViolationsCount} new invariant violations detected.`
      );
    }
    ...
    const reconciledBaseline: AggregationBaselineOptions = {
    ...
    return this.buildRecord(
      ...
      reconciledAt,
      reconciledBaseline,
      `Nominal baseline safely reconciled to post-mitigation telemetry (errorRate: ${reconciledBaseline.baselineErrorRate}, latencyP95: ${reconciledBaseline.baselineLatencyP95Ms}ms).`
    );
  }

  private buildRecord(
    reconciliationId: ReturnType<typeof createReconciliationId>,
    targetId: string,
    status: BaselineReconciliationStatus,
    reconciledAt: number,
    reconciledBaseline?: AggregationBaselineOptions,
    expectedMutationNote?: string
