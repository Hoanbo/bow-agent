...
const VALID_MISSION_TRANSITIONS: Record<MissionState, ReadonlySet<MissionState>> = {
...
export class MissionCoordinationValidator {
   ...
   */
  public validateAuthorizationEnvelope(envelope: MissionAuthorizationEnvelope): void {
   ...
   */
  public validateDependencyGraph(graph: MissionDependencyGraph, maxDepth = MAX_OBJECTIVE_DEPENDENCY_DEPTH): void {
    ...
    const checkCycleAndDepth = (node: string, depth: number): void => {
      if (depth > maxDepth) {
        throw new MissionCoordinationDependencyError(
          `Dependency depth ${depth} exceeds maximum allowable depth of ${maxDepth}`
        );
      }
   ...
   */
  public validateBudget(budget: MissionBudgetSnapshot): void {
   ...
   */
  public assertValidStateTransition(fromState: MissionState, toState: MissionState): void {
