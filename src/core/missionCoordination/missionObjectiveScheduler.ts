...
export class MissionObjectiveScheduler {
  private readonly objectives: Map<string, MissionObjectiveBinding> = new Map();
  private readonly dependencies: Map<string, Set<string>> = new Map(); // objectiveId -> set of upstream dependency objectiveIds
  private readonly dependents: Map<string, Set<string>> = new Map();   // objectiveId -> set of downstream dependent objectiveIds
  ...
  public getObjectives(): readonly MissionObjectiveBinding[] {
    return Array.from(this.objectives.values());
  }

  public getObjective(objectiveId: string): MissionObjectiveBinding | undefined {
   ...
   */
  public registerObjective(binding: MissionObjectiveBinding): void {
   ...
   */
  public validateGraphCycleAndDepth(maxDepth = MAX_OBJECTIVE_DEPENDENCY_DEPTH): void {
    const visited = new Set<string>();
    const recStack = new Set<string>();

    const checkDepthAndCycles = (node: string, currentDepth: number): void => {
      if (currentDepth > maxDepth) {
        throw new MissionCoordinationDependencyError(
          `Dependency depth ${currentDepth} exceeds maximum limit of ${maxDepth} at node '${node}'`
        );
      }
   ...
   */
  public evaluateReadiness(): { ready: string[]; blocked: string[]; active: string[]; completed: string[] } {
    const ready: string[] = [];
   ...
   */
  public updateObjectiveState(objectiveId: string, newState: MissionObjectiveState, summary?: string): MissionObjectiveBinding {
    ...
    const updated: MissionObjectiveBinding = {
      ...existing,
      state: newState,
   ...
   */
  public cascadeBlockDependents(failedObjectiveId: string): string[] {
  ...
  public getDependencyGraph(): MissionDependencyGraph {
