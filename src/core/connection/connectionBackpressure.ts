...
export function evaluateBackpressure(
  queueLength: number,
  maxCapacity = DEFAULT_MAX_QUEUE_CAPACITY
): BackpressureLevel {
 ...
 */
export function canAdmitMessage(level: BackpressureLevel, isCritical = false): boolean {
  if (level === 'OVERFLOW') {
 ...
 */
export function assertQueueCapacity(
  ...
  if (!canAdmitMessage(level, isCritical)) {
    throw new Error(
      `[CONNECTION_BACKPRESSURE_OVERFLOW] Queue capacity exceeded (${currentQueueLength}/${maxCapacity}); non-critical message rejected`
    );
  }
}
