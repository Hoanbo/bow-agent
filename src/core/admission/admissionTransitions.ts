...
export const ADMISSION_STATE_TRANSITION_MATRIX: Record<AdmissionState, readonly AdmissionState[]> = {
...
export function isValidAdmissionTransition(from: AdmissionState, to: AdmissionState): boolean {
  const allowed = ADMISSION_STATE_TRANSITION_MATRIX[from];
  return Boolean(allowed && allowed.includes(to));
}

export function assertValidAdmissionTransition(from: AdmissionState, to: AdmissionState): void {
