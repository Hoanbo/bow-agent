...
import { AdmissionTrustProvider, InMemoryAdmissionTrustProvider, evaluateAdmissionTrust } from './admissionTrust.js';
...
export class ZeroTrustAdmissionRuntime {
  private readonly trustProvider: AdmissionTrustProvider;
  private readonly keyStore: DeviceKeyStore;
  private readonly challengeTracker: AdmissionChallengeTracker;
  private readonly replayTracker: AdmissionReplayTracker;
  private readonly sessionCoordinator: AdmissionSessionCoordinator;
  private readonly revocationRegistry: AdmissionRevocationRegistry;
  private readonly auditLedger: AdmissionAuditLedger;
  private readonly registry: AdmissionRegistry;

  private totalEvaluated: number = 0;
  private totalAdmitted: number = 0;
  private totalRejected: number = 0;
  private roamingTransitionsCount: number = 0;
  ...
  public getTrustProvider(): AdmissionTrustProvider {
  ...
  public getChallengeTracker(): AdmissionChallengeTracker {
    return this.challengeTracker;
  }

  public getReplayTracker(): AdmissionReplayTracker {
    return this.replayTracker;
  }

  public getSessionCoordinator(): AdmissionSessionCoordinator {
    return this.sessionCoordinator;
  }

  public getRevocationRegistry(): AdmissionRevocationRegistry {
    return this.revocationRegistry;
  }

  public getAuditLedger(): AdmissionAuditLedger {
    return this.auditLedger;
  }

  public getRegistry(): AdmissionRegistry {
    return this.registry;
  }

  public async admit(request: any, now: number = Date.now()): Promise<AdmissionDecision> {
    const normalizedRequest: AdmissionRequest = {
   ...
   */
  public evaluateAdmission(request: AdmissionRequest, now: number = Date.now()): AdmissionDecision {
    this.totalEvaluated++;
    ...
    try {
      ...
      this.totalRejected++;
      return createRejectDecision(request, 'MALFORMED_REQUEST', (err as Error).message, now);
    }
    ...
    if (!trustRes.trusted || !trustRes.record) {
      ...
      return createRejectDecision(
        request,
        (trustRes.failureCode === 'ADMISSION_DEVICE_NOT_FOUND' ? 'DEVICE_NOT_TRUSTED' : 'UNKNOWN_DEVICE') as AdmissionRejectionReason,
        trustRes.failureReason || 'Trust verification failed',
        now
      );
    }
    ...
    try {
      ...
      return createRejectDecision(request, 'REPLAY_DETECTED', (err as Error).message, now);
    }
    ...
    try {
      if (request.sessionId) {
        const sessionBinding = this.sessionCoordinator.resumeSession(
          request.sessionId,
          request.deviceId,
          request.scope,
          request.network,
        ...
        const sessionBinding = this.sessionCoordinator.establishSession(
          request.deviceId,
          request.scope,
          request.network,
          undefined,
      ...
      this.totalRejected++;
      return createRejectDecision(request, 'SESSION_TERMINATED', (err as Error).message, now);
    }
  ...
  public getSnapshot(): ZeroTrustAdmissionSnapshot {
