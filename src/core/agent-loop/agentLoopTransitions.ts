...
export const VALID_LOOP_TRANSITIONS: Readonly<Record<AgentLoopState, readonly AgentLoopState[]>> = {
...
export function assertValidLoopTransition(
  from: AgentLoopState,
  to: AgentLoopState,
  context?: string
): void {
  if (!isValidLoopTransition(from, to)) {
    ...
    throw new Error(
      `INVALID_LOOP_TRANSITION: Cannot transition AgentLoop from "${from}" to "${to}"${ctx}.`
    );
  }
}
