...
export class AgentLoopRuntime {
  private _state: AgentLoopState = 'BOOTING';
  private _iterationCount = 0;
  private _consecutiveFailures = 0;
  private _startTime = Date.now();
  private _timer?: NodeJS.Timeout;
  private _pendingPlan?: { plan: AgentLoopPlan; decision: AgentLoopDecision };

  public get state(): AgentLoopState {
    return this._state;
  }

  public get pendingPlan(): { plan: AgentLoopPlan; decision: AgentLoopDecision } | undefined {
    return this._pendingPlan;
  }

  private transitionTo(nextState: AgentLoopState, context?: string): void {
  ...
  public async tick(options?: {
    ...
    if (decision.targetResource) {
      const lockAcquired = globalAgentLoopScheduler.acquireResourceLock(
    ...
    if (!isSuccess) {
      ...
      if (!options?.isDryRun) {
        ...
        const recRes = await globalAgentLoopRecovery.attemptRecovery(
          plan,
          execRes.error || verifRes.failureReason || 'Verification failure',
          this._consecutiveFailures
        );
        ...
        if (recRes.recovered) {
          ...
          const esc = globalAgentLoopEscalation.escalate(
            plan,
            this._consecutiveFailures,
            recRes.reason
          );
  ...
  public async authorizeAndExecute(authorizationToken: AuthorizationToken): Promise<AgentLoopIteration> {
