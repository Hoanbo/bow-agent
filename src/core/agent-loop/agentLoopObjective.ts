...
export class AgentLoopObjectiveManager {
  private readonly objectives = new Map<string, AgentLoopObjective>();
  private activeObjectiveId?: string;
  ...
  public createObjective(params: {
    ...
    const objective: AgentLoopObjective = {
      objectiveId,
      title: params.title,
    ...
    const PRIORITY_SCORES: Record<ObjectivePriority, number> = {
      CRITICAL: 4,
      HIGH: 3,
      MEDIUM: 2,
      LOW: 1,
  ...
  public getObjective(objectiveId: string): AgentLoopObjective | undefined {
    return this.objectives.get(objectiveId);
  }

  public getActiveObjective(): AgentLoopObjective | undefined {
    if (!this.activeObjectiveId) return undefined;
    return this.objectives.get(this.activeObjectiveId);
  }

  public setActiveObjective(objectiveId: string): AgentLoopObjective {
  ...
  public updateStatus(objectiveId: string, status: ObjectiveStatus): AgentLoopObjective {
    const existing = this.objectives.get(objectiveId);
    if (!existing) {
      throw new Error(`Objective "${objectiveId}" not found.`);
    }

    const updated: AgentLoopObjective = {
      ...existing,
      status,
      updatedTimestamp: Date.now(),
      completedTimestamp:
  ...
  public listObjectives(): AgentLoopObjective[] {
