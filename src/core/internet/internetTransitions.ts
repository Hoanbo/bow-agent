 ...
 */
export const INTERNET_TRANSITIONS: Readonly<Record<InternetEdgeState, readonly InternetEdgeState[]>> = Object.freeze({
  INTERNET_OFFLINE: [
    'INTERNET_RESOLVING',
    'INTERNET_CLOSED',
  ],
 ...
 */
export function isValidInternetTransition(
  from: InternetEdgeState,
  to: InternetEdgeState
): boolean {
 ...
 */
export function assertValidInternetTransition(
  from: InternetEdgeState,
  to: InternetEdgeState,
  context?: string
): void {
  if (!isValidInternetTransition(from, to)) {
    throw new InternetEdgeError(
      ...
      `Illegal Internet Edge state transition "${from}" → "${to}"${context ? ` [${context}]` : ''}. Transition blocked fail-closed.`
    );
  }
}
