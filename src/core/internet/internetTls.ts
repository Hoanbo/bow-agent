...
import {
  ...
  type InternetTlsHandshakeResult,
...
export const INTERNET_ALLOWED_CIPHER_SUITES: readonly InternetCipherSuite[] = Object.freeze([
...
export const INTERNET_REJECTED_TLS_VERSIONS: readonly string[] = Object.freeze([
  'TLSv1.0',
  'TLSv1.1',
 ...
 */
export function validateTlsSession(
  params: TlsSessionParameters,
  opts: { requireMtls?: boolean; allowTls12?: boolean } = {}
): TlsValidationResult {
  ...
  const isAcceptedVersion =
    ver === 'TLSv1.3' || (allowTls12 && ver === 'TLSv1.2');
  ...
  // Cipher whitelist
  const cipher = params.negotiatedCipher as InternetCipherSuite;
  if (!(INTERNET_ALLOWED_CIPHER_SUITES as readonly string[]).includes(params.negotiatedCipher)) {
    return {
  ...
  return {
    result: 'TLS_ACCEPTED',
    acceptedVersion: ver as InternetTlsVersion,
 ...
 */
export async function withTlsHandshakeTimeout<T>(
  work: Promise<T>,
  overrideMs?: number
): Promise<T> {
  ...
  const race = await Promise.race([
    work.then((v) => ({ ok: true as const, value: v })),
 ...
 */
export function detectTlsDowngrade(
  previousVersion: InternetTlsVersion,
