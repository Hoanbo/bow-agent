...
import {
  ...
  type InternetAdmissionDecision,
  type InternetAdmissionToken,
...
export class InternetAdmissionBridge {
  private readonly _pendingTokens = new Map<string, InternetAdmissionToken>();
  ...
  get pendingCount(): number { return this._pendingTokens.size; }
  ...
  public issueToken(params: {
    ...
    if (this._pendingTokens.size >= this._maxPending) {
      throw new InternetEdgeError(
        ...
        `Admission bridge token limit reached (${this._pendingTokens.size}/${this._maxPending}). Rejecting new token issuance.`
      );
    }

    const token: InternetAdmissionToken = Object.freeze({
  ...
  public consumeToken(
    ...
    let matchToken: InternetAdmissionToken | undefined;
    ...
    this._pendingTokens.delete(matchKey);
    const consumed: InternetAdmissionToken = Object.freeze({
      ...matchToken,
      consumedAt: Date.now(),
    });
  ...
  public getSnapshot(): { pendingCount: number; ttlMs: number; maxPending: number } {
