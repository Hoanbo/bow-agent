...
export class PersonalDecisionSupport {
  private readonly overrideRecords: Map<string, OwnerOverrideRecord> = new Map();
  private isPaused: boolean = false;

  constructor(private readonly challengeEngine: CognitiveChallengeEngine) {}
    ...
    switch (command.type) {
      ...
      case 'OVERRIDE': {
        ...
        if (
          ...
          throw new Error(
            `[IMMUTABLE_INVARIANT_VIOLATION] Master Owner cannot override non-overridable safety/security invariant: ${challenge.concern}`
          );
        }

        if (
          command.payload.targetPath &&
          (command.payload.targetPath.toLowerCase().includes('shopofbow') ||
            command.payload.targetPath.toLowerCase().includes('system32'))
        ) {
          throw new Error(
            `[IMMUTABLE_INVARIANT_VIOLATION] Master Owner cannot override protected workspace isolation or OS directory protection.`
          );
        }
   ...
   */
  public recordOverride(params: Omit<OwnerOverrideRecord, 'overrideId' | 'timestamp'>): OwnerOverrideRecord {
    const overrideId = generatePartnershipId('ovr');
    const record: OwnerOverrideRecord = {
      overrideId,
   ...
   */
  public getAllOverrides(): OwnerOverrideRecord[] {
