...
export class PolicyActiveIncidentResponseRuntime {
  private readonly signalResolver: PolicyActiveIncidentSignalResolver;
  private readonly degradationDetector: PolicyActivePolicyDegradationDetector;
  private readonly classifier: PolicyActiveIncidentClassifier;
  private readonly safetyBoundary: PolicyEmergencySafetyBoundary;
  private readonly escalationEngine: PolicyIncidentEscalationEngine;
  private readonly incidentStore: PolicyActiveIncidentStore;
  private readonly provenanceEngine: PolicyActiveIncidentProvenanceEngine;
  private readonly auditEngine: PolicyActiveIncidentAuditEngine;
  private readonly isUserStopActiveFn?: () => boolean;

  constructor(
    options?: PolicyActiveIncidentResponseOptions,
    signalResolver?: PolicyActiveIncidentSignalResolver,
    degradationDetector?: PolicyActivePolicyDegradationDetector,
    classifier?: PolicyActiveIncidentClassifier,
    safetyBoundary?: PolicyEmergencySafetyBoundary,
    escalationEngine?: PolicyIncidentEscalationEngine,
    incidentStore?: PolicyActiveIncidentStore,
    provenanceEngine?: PolicyActiveIncidentProvenanceEngine,
    auditEngine?: PolicyActiveIncidentAuditEngine
  ) {
  ...
  public evaluateIncidentState(
    ...
    if (boundaryStatus !== 'INACTIVE') {
      this.safetyBoundary.activateSafetyBoundary(
        tenantPartition,
        incidentId,
        boundaryStatus,
    ...
    if (classification.requiresHumanEscalation && !escalationId) {
      const escRecord = this.escalationEngine.escalateIncident(
        tenantPartition,
        incidentId,
        classification.severity,
        `Active policy degradation: ${primaryCat}`,
        classification.rationale
      );
    ...
    const incidentRecord: ActivePolicyIncidentRecord = Object.freeze({
      incidentId,
      tenantPartition,
      fingerprint,
      activePolicyStateId: (resolvedSignals.activePolicyStateId as any) ?? null,
      runtimeSnapshotId: (resolvedSignals.runtimeSnapshotId as any) ?? null,
    ...
    this.provenanceEngine.appendIncidentEvent(
      tenantPartition,
      incidentId,
      classification.severity,
      'INCIDENT_RECORDED',
      { incidentId, severity: classification.severity, primaryCategory: primaryCat, boundaryStatus }
    );
   ...
   */
  public enforceSafetyBoundary(
    tenantPartition: string,
    action: string,
    actionClassification?: string
  ): SafetyBoundaryEnforcementCheck {
    this.assertUserStopInactive(tenantPartition);
   ...
   */
  public getActiveSafetyBoundary(tenantPartition: string): SafetyBoundaryState | null {
    this.assertUserStopInactive(tenantPartition);
   ...
   */
  public resolveIncidentWithHumanReview(
    tenantPartition: string,
    incidentId: string,
    operatorId: string,
    rationale: string
    ...
    const now = new Date().toISOString();
    const resolvedRecord: ActivePolicyIncidentRecord = Object.freeze({
      ...incident,
      state: 'RESOLVED',
    ...
    this.provenanceEngine.appendIncidentEvent(
      ...
      incident.severity,
      'INCIDENT_RESOLVED',
      { resolvedBy: operatorId, rationale, resolvedAt: now }
    );
  ...
  public closeIncident(
    ...
    const now = new Date().toISOString();
    const closedRecord: ActivePolicyIncidentRecord = Object.freeze({
      ...incident,
      state: 'CLOSED',
      updatedAt: now,
    });
    ...
    this.provenanceEngine.appendIncidentEvent(
      ...
      incident.severity,
      'INCIDENT_CLOSED',
      { closedBy: operatorId, rationale, closedAt: now }
    );
   ...
   */
  public getIncidentProvenance(tenantPartition: string): readonly IncidentProvenanceRecord[] {
    this.assertUserStopInactive(tenantPartition);
   ...
   */
  public verifyIncidentProvenance(tenantPartition: string): boolean {
    this.assertUserStopInactive(tenantPartition);
