 ...
 */
export function validateTransportIdentifier(name: string, value: unknown): string {
 ...
 */
export function containsTransportSecret(input: string): boolean {
 ...
 */
export function redactTransportSecrets(input: string): string {
  let result = input;
 ...
 */
export function validateMetadataDepth(obj: unknown, currentDepth = 1): void {
  ...
  if (obj && typeof obj === 'object') {
    for (const key of Object.keys(obj as Record<string, unknown>)) {
      ...
      validateMetadataDepth((obj as Record<string, unknown>)[key], currentDepth + 1);
    }
  }
}
 ...
 */
export function validateMessageResourceBounds(payload: unknown): void {
  if (payload !== undefined && payload !== null) {
    validateMetadataDepth(payload);
    const serialized = JSON.stringify(payload);
    if (serialized.length > MAX_TRANSPORT_MESSAGE_BYTES) {
      throw new Error(
        `[TRANSPORT_RESOURCE_ERROR] Message size (${serialized.length} bytes) exceeds maximum limit (${MAX_TRANSPORT_MESSAGE_BYTES} bytes).`,
      );
    }
  }
}
 ...
 */
export function assertTransportRiskPreservation(
  ...
  if (newRank < priorRank) {
    throw new Error(
      `[TRANSPORT_RISK_DOWNGRADE_ERROR] Monotonic risk violation: cannot downgrade risk from ${priorRisk} to ${newRisk}.`,
    );
  }
}
 ...
 */
export function assertTransportSequenceMonotonicity(
  priorSequence: number,
  newSequence: number,
): void {
  if (
    typeof newSequence !== 'number' ||
    isNaN(newSequence) ||
    !Number.isInteger(newSequence) ||
    newSequence <= 0
  ) {
    throw new Error(
      `[TRANSPORT_SEQUENCE_ERROR] Invalid sequence number: ${newSequence}. Must be positive integer.`,
    );
  }
  if (newSequence < priorSequence) {
    throw new Error(
      `[TRANSPORT_SEQUENCE_ERROR] Monotonic sequence violation: new sequence ${newSequence} is less than prior sequence ${priorSequence}.`,
    );
  }
}
