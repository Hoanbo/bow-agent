...
export type SequenceAnalysisResult =
  | { readonly status: 'NEXT_IN_ORDER'; readonly expectedSequence: number; readonly actualSequence: number }
  | { readonly status: 'DUPLICATE_SEQUENCE'; readonly expectedSequence: number; readonly actualSequence: number }
  | { readonly status: 'SEQUENCE_GAP'; readonly expectedSequence: number; readonly actualSequence: number; readonly missingCount: number }
  | { readonly status: 'STALE_SEQUENCE'; readonly expectedSequence: number; readonly actualSequence: number }
  | { readonly status: 'INVALID_SEQUENCE'; readonly expectedSequence: number; readonly actualSequence: number; readonly reason: string };
 ...
 */
export function analyzeMessageSequence(
  lastAcceptedSequence: number,
  incomingSequence: number,
): Readonly<SequenceAnalysisResult> {
  if (
 ...
 */
export function assertSequenceProgression(
  lastAcceptedSequence: number,
  incomingSequence: number,
): void {
  ...
  if (analysis.status === 'STALE_SEQUENCE') {
    throw new Error(
      `[TRANSPORT_SEQUENCE_ERROR] Stale sequence detected: got ${incomingSequence}, expected ${analysis.expectedSequence}.`,
    );
  }
  if (analysis.status === 'SEQUENCE_GAP') {
    throw new Error(
      `[TRANSPORT_SEQUENCE_GAP_ERROR] Sequence gap detected: got ${incomingSequence}, expected ${analysis.expectedSequence} (${analysis.missingCount} missing).`,
    );
  }
}
