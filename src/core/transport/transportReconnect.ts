...
export function createResumeRequest(params: {
  ...
  if (
    typeof params.lastAckSequence !== 'number' ||
    isNaN(params.lastAckSequence) ||
    !Number.isInteger(params.lastAckSequence) ||
    params.lastAckSequence < 0
  ) {
    throw new Error(
      `[TRANSPORT_RESUME_ERROR] Invalid lastAckSequence: ${params.lastAckSequence}. Must be non-negative integer.`,
    );
  }
...
export function validateResumeRequest(
  request: Readonly<ResumeRequest>,
  sessionContext: {
    readonly connectionId: string;
