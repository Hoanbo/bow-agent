...
export interface SmartHomeCommandOptions {
  ...
  value?: number; // e.g. 25 (celsius) or 80 (brightness %)
}
...
export class SmartHomeService {
  private devices: Map<string, SmartHomeDeviceState> = new Map();
  ...
  public getAllDevices(): SmartHomeDeviceState[] {
    return Array.from(this.devices.values());
  }

  public async executeCommand(options: SmartHomeCommandOptions): Promise<SmartHomeActionResult> {
    ...
    if (!target) {
      if (devKey.includes('ban') || devKey.includes('hoc')) target = this.devices.get('desk_light');
      else if (devKey.includes('dieu_hoa') || devKey.includes('may_lanh')) target = this.devices.get('air_conditioner');
      else if (devKey.includes('den')) target = this.devices.get('main_light');
      else if (devKey.includes('o_cam')) target = this.devices.get('smart_plug');
      else target = this.devices.get('desk_light');
    }
