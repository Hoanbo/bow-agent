 ...
 */
export function cleanupStaleFilesInDir(
  dirPath: string,
  maxAgeMs: number = VOICE_CONFIG.staleAudioMaxAgeMs,
  filterPattern: RegExp = /\.(wav|tmp)$/i
): number {
  let cleanedCount = 0;
  try {
 ...
 */
export function cleanupAudioTempDirectories(
  ...
  for (const dir of additionalDirs) {
    if (dir) searchDirs.add(path.resolve(dir));
  }
