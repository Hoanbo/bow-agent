 ...
 */
export function bootstrapShopOfBowAdapter(): void {
  registerCommerceProvider(shopOfBowProvider);
  setContractsActiveShopAdapter(fallbackShopAdapter as any);
  ...
  setProductIconResolver((name: string) => {
    const low = name.toLowerCase();
    if (low.includes('capcut')) return '🎬';
    if (low.includes('netflix') || low.includes('youtube')) return '🍿';
    if (low.includes('canva') || low.includes('figma')) return '🎨';
    if (low.includes('gpt') || low.includes('ai')) return '🤖';
    return null;
  });
}
