...
// ---------------------------------------------------------------------------
export function sanitizeProductionTelemetryText(raw) {
...
export function detectPiiInText(text) {
    if (!text)
        return false;
...
export function recordProductionMetric(params) {
...
export function getProductionMetrics(windowMinutes = 60) {
    if (windowMinutes <= 0)
        return [...telemetryBuffer];
    const cutoff = Date.now() - windowMinutes * 60 * 1000;
    return telemetryBuffer.filter((m) => new Date(m.timestamp).getTime() >= cutoff);
}
export function clearProductionTelemetryCache() {
    telemetryBuffer = [];
}
// ---------------------------------------------------------------------------
