...
import { getProductionMetrics } from './productionTelemetryService.js';
// ---------------------------------------------------------------------------
...
export function getSloThresholds() {
    return { ...DEFAULT_SLO_THRESHOLDS };
}
// ---------------------------------------------------------------------------
// 2. DETERMINISTIC SLO EVALUATION
// ---------------------------------------------------------------------------
export function evaluateProductionSlo(providedMetrics, thresholds = DEFAULT_SLO_THRESHOLDS) {
    ...
    // Evaluate Error Rate
    let errStatus = 'HEALTHY';
    if (errorRateVal > thresholds.errorRateWarningMax)
        errStatus = 'CRITICAL';
    else if (errorRateVal > thresholds.errorRateHealthyMax)
        errStatus = 'WARNING';
    // Evaluate P95 Latency
    let p95Status = 'HEALTHY';
    if (p95Val > thresholds.p95LatencyWarningMax)
        p95Status = 'CRITICAL';
    else if (p95Val > thresholds.p95LatencyHealthyMax)
        p95Status = 'WARNING';
    // Evaluate P99 Latency
    let p99Status = 'HEALTHY';
    if (p99Val > thresholds.p99LatencyWarningMax)
        p99Status = 'CRITICAL';
    else if (p99Val > thresholds.p99LatencyHealthyMax)
        p99Status = 'WARNING';
    // Evaluate Fallback Rate
    let fallbackStatus = 'HEALTHY';
    if (fallbackRateVal > thresholds.fallbackRateWarningMax)
        fallbackStatus = 'CRITICAL';
    else if (fallbackRateVal > thresholds.fallbackRateHealthyMax)
        fallbackStatus = 'WARNING';
    // Evaluate Knowledge Gap Rate
    let gapStatus = 'HEALTHY';
    if (gapRateVal > thresholds.knowledgeGapRateWarningMax)
        gapStatus = 'CRITICAL';
    else if (gapRateVal > thresholds.knowledgeGapRateHealthyMax)
        gapStatus = 'WARNING';
    // Overall Status
