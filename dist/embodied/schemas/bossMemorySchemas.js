 ...
 */
export function validateBossProject(val, pathPrefix = 'project') {
    const errors = [];
    if (!isObject(val)) {
 ...
 */
export function validateBossHabits(val) {
    const errors = [];
    if (!isObject(val)) {
 ...
 */
export function validateBossProfile(data) {
    const errors = [];
    if (!isObject(data)) {
    ...
    const validatedProjects = data.projects.map((p, idx) => validateBossProject(p, `projects[${idx}]`).data);
    return {
        success: true,
        data: {
            ...
            healthNotes: data.healthNotes.slice(),
 ...
 */
export function validateBossRule(val, pathPrefix = 'rule') {
    const errors = [];
    if (!isObject(val)) {
        return { success: false, errors: [`${pathPrefix} must be an object`] };
    }
 ...
 */
export function validateBossRules(data) {
