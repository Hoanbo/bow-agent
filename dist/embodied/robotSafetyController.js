...
export class RobotSafetyController {
     ...
     */
    triggerEmergencyStop(reason = 'MANUAL_ESTOP_TRIGGERED') {
    ...
    recordFirmwareHeartbeat(metrics) {
        ...
        if (metrics?.batteryLevel !== undefined)
            this.batteryLevel = metrics.batteryLevel;
        if (metrics?.batteryTempC !== undefined)
            this.batteryTempC = metrics.batteryTempC;
     ...
     */
    validateAndClampMotion(requestedPan, requestedTilt) {
    ...
    getStatus(violationOverride) {
        ...
        if (!safetyViolation) {
            if (this.eStopEngaged)
                safetyViolation = 'EMERGENCY_STOP_ENGAGED';
            else if (!batteryTempSafe)
                safetyViolation = `OVERTEMPERATURE_WARNING: ${this.batteryTempC}°C > ${this.MAX_SAFE_TEMP_C}°C`;
            else if (!heartbeatHealthy)
                safetyViolation = `HEARTBEAT_TIMEOUT: ${elapsedHeartbeat}ms > ${this.HEARTBEAT_TIMEOUT_MS}ms`;
        }
        return {
