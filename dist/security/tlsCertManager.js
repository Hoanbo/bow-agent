 ...
 */
export function findOpenSslBinary() {
    const candidates = [
    ...
    throw new Error('OPENSSL_NOT_FOUND: Không tìm thấy OpenSSL CLI trên hệ thống. ' +
 ...
 */
export function calculateCertFingerprint(certPem) {
 ...
 */
function gatherSanEntries(customHosts = []) {
    const dnsSet = new Set(['localhost', 'MSI']);
    const ipSet = new Set(['127.0.0.1', '192.168.0.100']);
 ...
 */
function createOpenSslSanConfig(sanPath, san) {
    let altNames = '';
 ...
 */
export function ensureTlsCertificates(options = {}) {
    ...
    if (needGenerate) {
        ...
        if (!caExists || options.forceRenew) {
            ...
            execSync(`"${openssl}" req -x509 -new -nodes -key "${CA_KEY_PATH}" -sha256 -days 1825 ` +
                `-out "${CA_CRT_PATH}" -subj "/CN=BOWCON-Local-Root-CA/O=BOWCON/OU=Security"`, { stdio: 'pipe' });
        ...
        execSync(`"${openssl}" req -new -key "${SERVER_KEY_PATH}" -out "${SERVER_CSR_PATH}" -config "${SERVER_EXT_PATH}"`, { stdio: 'pipe' });
        // 3. Ký Server Certificate bằng Root CA
        execSync(`"${openssl}" x509 -req -in "${SERVER_CSR_PATH}" -CA "${CA_CRT_PATH}" -CAkey "${CA_KEY_PATH}" ` +
            `-CAcreateserial -out "${SERVER_CRT_PATH}" -days 825 -sha256 -extfile "${SERVER_EXT_PATH}" -extensions v3_req`, { stdio: 'pipe' });
 ...
 */
export function getCaFingerprint() {
