...
export function isPushToTalkEnabled() {
...
export class PushToTalkManager {
    ...
    anomalyThresholdCount = BODY_CONFIG.pttAnomalyThreshold;
    anomalyWindowMs = BODY_CONFIG.pttAnomalyWindowMs;
    constructor() {
        this.defaultTimeoutMs = BODY_CONFIG.pttConfirmationTimeoutMs;
        this.hotkey = process.env.BOW_PTT_HOTKEY || 'Ctrl+Alt+Space';
    }
    setBeepNotifier(notifier) {
        this.beepNotifier = notifier;
    }
    getRecentTimeoutCount() {
        const now = Date.now();
        this.recentTimeoutTimestamps = this.recentTimeoutTimestamps.filter((t) => now - t <= this.anomalyWindowMs);
        return this.recentTimeoutTimestamps.length;
    }
    clearRecentTimeouts() {
        this.recentTimeoutTimestamps = [];
    }
    recordTimeoutOccurrence(commandId) {
        const now = Date.now();
        this.recentTimeoutTimestamps.push(now);
        this.recentTimeoutTimestamps = this.recentTimeoutTimestamps.filter((t) => now - t <= this.anomalyWindowMs);
        if (this.recentTimeoutTimestamps.length >= this.anomalyThresholdCount) {
            console.error(`[PUSH-TO-TALK] 🚨 CẢNH BÁO AN NINH CẤP CAO (ANOMALY DETECTED): ` +
            ...
            if (this.onAnomalyDetected) {
                try {
                    this.onAnomalyDetected(this.recentTimeoutTimestamps.length);
                }
                catch { }
            }
        }
    }
    async emitTimeoutAlertSound() {
        if (this.onBeep) {
            try {
                this.onBeep('timeout');
            }
        ...
        if (process.platform === 'win32') {
            try {
                ...
                execSync(`powershell -NoProfile -NonInteractive -Command "[Console]::Beep(${timeoutFreqHz}, ${timeoutBurstDurationMs}); Start-Sleep -Milliseconds ${timeoutPauseDurationMs}; [Console]::Beep(${timeoutFreqHz}, ${timeoutBurstDurationMs}); Start-Sleep -Milliseconds ${timeoutPauseDurationMs}; [Console]::Beep(${timeoutFreqHz}, ${timeoutBurstDurationMs})"`, { stdio: 'ignore', timeout: 2000 });
            }
            catch { }
        }
        else {
     ...
     */
    async waitForConfirmation(commandId, customTimeoutMs) {
        if (!this.isEnabled()) {
        ...
        return new Promise((resolve, reject) => {
            const timer = setTimeout(async () => {
                ...
                console.warn(`[PUSH-TO-TALK] ⏱️ TIMEOUT: Hết thời hạn (${timeoutMs}ms) mà không nhận được xác nhận vật lý. ` +
            ...
            console.log(`[PUSH-TO-TALK] 🛡️ Đang chờ Chủ nhân nhấn phím vật lý [${this.hotkey}] ` +
     ...
     */
    __testOnly_simulatePhysicalPress(commandId) {
        if (process.env.NODE_ENV !== 'test') {
            throw new Error('TEST_ONLY_METHOD_CALLED_OUTSIDE_TEST_ENV: __testOnly_simulatePhysicalPress ' +
     ...
     */
    simulatePhysicalPress(commandId) {
        throw new Error(`${BodyProtocolErrorCode.TEST_ONLY_METHOD_CALLED_OUTSIDE_TEST_ENV}: simulatePhysicalPress has been locked down and removed for security. ` +
...
export class TestOnlyPushToTalkController {
    manager;
    constructor(manager = globalPushToTalkManager) {
        this.manager = manager;
    }
    simulatePhysicalPress(commandId) {
        if (process.env.NODE_ENV !== 'test') {
            throw new Error(`${BodyProtocolErrorCode.TEST_ONLY_METHOD_CALLED_OUTSIDE_TEST_ENV}: TestOnlyPushToTalkController.simulatePhysicalPress ` +
