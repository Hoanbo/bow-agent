...
import { calculateDecisionFingerprint, assertAdminAuthorized, sanitizeActionText } from './knowledgeActionService.js';
// ---------------------------------------------------------------------------
// 1. IN-MEMORY ALERT STORE & COOLDOWN TRACKER
// ---------------------------------------------------------------------------
let activeAlerts = [];
const alertCooldowns = new Map(); // fingerprint -> expiresAt timestamp
...
// ---------------------------------------------------------------------------
export function generateAlertFingerprint(entityId, alertType, evidence) {
    return calculateDecisionFingerprint(entityId, alertType, evidence);
}
export function isAlertInCooldown(fingerprint) {
...
// ---------------------------------------------------------------------------
export function acknowledgeAlert(alertId, adminUserId) {
    try {
        assertAdminAuthorized(adminUserId);
        const alert = activeAlerts.find((a) => a.id === alertId);
        if (!alert)
            return { success: false, error: 'Alert không tồn tại' };
...
export function snoozeAlert(alertId, adminUserId, snoozeHours = 24) {
    try {
        assertAdminAuthorized(adminUserId);
        const alert = activeAlerts.find((a) => a.id === alertId);
        if (!alert)
...
export function dismissAlert(alertId, adminUserId, reason) {
    try {
        assertAdminAuthorized(adminUserId);
        const alert = activeAlerts.find((a) => a.id === alertId);
        if (!alert)
