 ...
 */
export function aggregateKnowledgeGapEvents(events, timeWindow) {
    let totalEvents = 0;
    let geminiCalls = 0;
    ...
    for (const ev of events) {
        if (timeWindow?.start && ev.createdAt && new Date(ev.createdAt) < timeWindow.start)
            continue;
        if (timeWindow?.end && ev.createdAt && new Date(ev.createdAt) > timeWindow.end)
            continue;
        totalEvents++;
