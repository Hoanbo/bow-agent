...
import { detectKnowledgeDrift } from './knowledgeDriftService.js';
import { runKnowledgeQaSuite } from './knowledgeQaService.js';
import { detectTrafficAnomalies } from './knowledgeAnomalyService.js';
import { evaluateGovernanceAlerts } from './knowledgeAlertService.js';
import { getNegativePolicies } from './negativePolicyService.js';
// ---------------------------------------------------------------------------
// 1. IN-MEMORY CACHE
// ---------------------------------------------------------------------------
let cachedGovernanceSummary = null;
let governanceCacheExpiry = 0;
const GOVERNANCE_CACHE_TTL_MS = 30_000; // 30 seconds
export function clearGovernanceDashboardCache() {
...
export function calculateKnowledgeGovernanceScore(params) {
    ...
    const rawScore = c.knowledgeIntegrity +
        c.faqHealth +
        c.coverage +
        c.regressionSafety +
        c.driftStability +
...
// ---------------------------------------------------------------------------
export function evaluateGovernanceHealthStatus(score, isCapped = false) {
    if (isCapped || score < 30)
        return 'CRITICAL';
    if (score < 50)
        return 'DEGRADED';
    if (score < 70)
...
export function calculateSlaSloMetrics(events = []) {
    const resolutionLatencies = [];
    const faqLookupLatencies = [];
    const policyLookupLatencies = [];
...
// ---------------------------------------------------------------------------
export async function getGovernanceDashboardSummary(providedFaqs, providedPolicies, providedEvents, forceRefresh = false) {
    const now = Date.now();
