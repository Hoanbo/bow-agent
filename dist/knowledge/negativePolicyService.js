 ...
 */
export function clearNegativePolicyCache() {
    cachedPolicies = null;
    cacheTimestamp = 0;
}
/**
...
export async function getNegativePolicies(filters) {
    try {
        ...
        const events = await getActiveShopAdapter().storage.getAgentEvents(undefined, 1000, [
        ...
        if (filters?.search && filters.search.trim().length > 0) {
            ...
            result = result.filter((p) => p.normalizedQuestion.includes(s) ||
...
export async function rejectAndRememberDecision(params) {
    ...
    try {
        ...
        const dup = existing.find((p) => p.status === 'ACTIVE' &&
 ...
 */
export async function matchNegativePolicy(query, providedPolicies) {
    try {
        ...
        for (const policy of activePolicies) {
            ...
            const hasScopeKeyword = scopeVal.length > 1 &&
            ...
            const isSupportInquiry = normQuery.includes('ho tro') ||
 ...
 */
export async function updateNegativePolicy(policyId, patch, adminUserId) {
    if (!adminUserId)
    ...
    try {
        ...
        if (!existing)
            return { success: false, error: 'Không tìm thấy Negative Policy cần sửa.' };
 ...
 */
export async function activateNegativePolicy(policyId, adminUserId) {
    if (!adminUserId)
        throw new Error('UNAUTHORIZED: Admin user ID required');
    try {
        await getActiveShopAdapter().storage.insertAnalyticsEvents([
 ...
 */
export async function deactivateNegativePolicy(policyId, adminUserId) {
    if (!adminUserId)
        throw new Error('UNAUTHORIZED: Admin user ID required');
    try {
        await getActiveShopAdapter().storage.insertAnalyticsEvents([
 ...
 */
export async function detectPolicyConflict(questionOrScope, providedFaqs) {
    try {
 ...
 */
export async function getNegativePolicyAnalytics() {
