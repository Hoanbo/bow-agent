 ...
 */
export async function findSimilarFaqs(question, providedFaqs) {
    try {
...
export async function getKnowledgeGaps(filters) {
    try {
        const rawEvents = await getActiveShopAdapter().storage.getAgentEvents(undefined, 1000, [
        ...
        for (const ev of events) {
            ...
            if (ev.event_type === 'KNOWLEDGE_GAP_REVIEWED' ||
                ...
                if (gapId && !statusMap.has(gapId)) {
                    ...
                    if (ev.event_type === 'KNOWLEDGE_GAP_REVIEWED')
                        status = 'reviewing';
                    if (ev.event_type === 'KNOWLEDGE_GAP_REJECTED')
                        status = 'rejected';
                    if (ev.event_type === 'KNOWLEDGE_GAP_MERGED')
                        status = 'merged';
                    if (ev.event_type === 'KNOWLEDGE_GAP_APPROVED')
                        status = 'approved';
                    statusMap.set(gapId, {
                        status,
        ...
        const reviewableList = deduped.map((d) => {
        ...
        if (filters?.search && filters.search.trim()) {
            ...
            result = result.filter((r) => normalizeText(r.canonicalQuestion).includes(q) ||
        ...
        if (filters?.sortBy === 'priority') {
            ...
            result.sort((a, b) => new Date(b.reviewedAt || b.lastSeenAt).getTime() - new Date(a.reviewedAt || a.lastSeenAt).getTime());
        }
        else {
 ...
 */
export async function markKnowledgeGapReviewing(gapId, adminUserId) {
    if (!adminUserId)
 ...
 */
export async function rejectKnowledgeGap(gapId, reason, adminUserId) {
    if (!adminUserId)
        throw new Error('UNAUTHORIZED: Admin user ID required');
    try {
        await getActiveShopAdapter().storage.insertAnalyticsEvents([
 ...
 */
export async function smartMergeKnowledgeGaps(targetGapId, sourceGapIds, adminUserId, reason) {
    if (!adminUserId)
        throw new Error('UNAUTHORIZED: Admin user ID required');
 ...
 */
export async function mergeKnowledgeGaps(targetId, sourceIds, adminUserId) {
 ...
 */
export async function approveKnowledgeGap(gapId, faqData, adminUserId) {
    if (!adminUserId) {
        throw new Error('UNAUTHORIZED: Admin user ID required');
    }
 ...
 */
export async function calculateFaqQualityAndStaleMetrics(providedFaqs, providedEvents, providedGaps) {
    try {
        let faqs = providedFaqs;
        if (!faqs) {
        ...
        if (!events) {
            events = await getActiveShopAdapter().storage.getAgentEvents(undefined, 1000, ['FAQ_USED', 'KNOWLEDGE_GAP_DETECTED']);
        }
 ...
 */
export async function editFaqWithVersionHistory(faqId, patch, reason, adminUserId) {
    if (!adminUserId)
        throw new Error('UNAUTHORIZED: Admin user ID required');
    ...
    try {
        ...
        if (!updateOk)
            throw new Error('Cập nhật FAQ thất bại');
 ...
 */
export async function getFaqEditHistory(faqId) {
    try {
        const data = await getActiveShopAdapter().storage.getAgentEvents(undefined, 1000, ['FAQ_EDITED']);
