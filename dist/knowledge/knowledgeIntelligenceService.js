...
import { calculateQuestionSimilarity } from './knowledgeReviewService.js';
import { getNegativePolicies } from './negativePolicyService.js';
import { classifyKnowledgeGap, normalizeKnowledgeQuestion } from './knowledgeGapDetector.js';
// In-Memory Intelligence Cache
let cachedIntelligenceSummary = null;
let lastIntelligenceFetchTime = 0;
const INTELLIGENCE_CACHE_TTL_MS = 60 * 1000; // 60s
export function clearKnowledgeIntelligenceCache() {
...
// ----------------------------------------------------------------------------
export function calculateFaqHealthScores(faqs, events = [], gaps = [], conflicts = []) {
    const now = Date.now();
    return faqs.map((faq) => {
        const faqEvents = events.filter((e) => (e.event_type === 'FAQ_USED' || e.event_type === 'FAQ_MATCHED') && e.metadata?.faqId === faq.id);
        const usageCount = faqEvents.length;
        const lastUsedEvent = faqEvents.sort((a, b) => new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime())[0];
        ...
        // Version edits count
        const versionCount = events.filter((e) => (e.event_type === 'FAQ_EDITED' || e.event_type === 'FAQ_VERSION_CREATED') && e.metadata?.faqId === faq.id).length + 1;
        // Associated conflicts count
        const faqConflicts = conflicts.filter((c) => c.entityA.id === faq.id || c.entityB.id === faq.id);
        ...
        const matchSuccessRate = totalHits + totalMissVariants > 0
...
// ----------------------------------------------------------------------------
export function inferQueryDomain(normalizedText) {
...
export function calculateKnowledgeCoverage(faqs, policies = [], events = [], gaps = []) {
    const domains = [
        'PRODUCT',
        'PAYMENT',
        'WALLET',
    ...
    for (const ev of events) {
        const qText = normalizeText((ev.metadata?.query || ev.intent || ''));
        if (!qText)
            continue;
    ...
    const domainCoverages = domains.map((domain) => {
    ...
    const overallCoveragePercentage = totalAllQueries > 0
...
// ----------------------------------------------------------------------------
function normalizeClusterTokens(text) {
...
export function clusterKnowledgeQueries(rawQueries, faqs = [], policies = []) {
    const clusters = [];
    for (const item of rawQueries) {
        ...
        if (matchedCluster) {
            ...
            if (matchingFaqId)
                suggestedAction = 'EXPAND_EXISTING_FAQ';
            else if (matchingPolicyId)
                suggestedAction = 'MONITOR';
            else if (intent === 'KNOWLEDGE_GAP')
                suggestedAction = 'CREATE_FAQ';
...
// ----------------------------------------------------------------------------
export function detectEmergingTopics(_events, gaps = []) {
    const topics = [];
    const now = Date.now();
    ...
    for (const gap of gaps) {
        ...
        if (count >= 3 && daysActive <= 7) {
            ...
            let classification = 'KNOWLEDGE_GAP';
...
// ----------------------------------------------------------------------------
export function analyzeNegativePolicyIntelligence(policies, events = [], conflicts = []) {
    const now = Date.now();
    return policies.map((pol) => {
        const matchEvents = events.filter((e) => e.event_type === 'NEGATIVE_POLICY_MATCHED' && e.metadata?.policyId === pol.id);
        ...
        const lastEvent = matchEvents.sort((a, b) => new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime())[0];
        ...
        // Policy conflicts
        const policyConflicts = conflicts.filter((c) => c.entityA.id === pol.id || c.entityB.id === pol.id);
        const conflictCount = policyConflicts.length;
        let effectivenessGrade = 'HIGH';
        let recommendation;
        if (matchesCount >= 20) {
...
// ----------------------------------------------------------------------------
export function detectKnowledgeConflicts(faqs, policies = []) {
    const conflicts = [];
        ...
        for (const pol of policies.filter((p) => p.status === 'ACTIVE')) {
            ...
            if (sim >= 75 || (hasScopeOverlap && sim >= 50)) {
                conflicts.push({
                    ...
                    recommendedResolution: 'Admin cần xác định xem dịch vụ này thực tế có được hỗ trợ hay không để giữ FAQ hoặc giữ Negative Policy.',
...
// ----------------------------------------------------------------------------
export function generateKnowledgeRecommendations(faqHealths, coverage, emergingTopics, conflicts, policyIntel, regression) {
...
// ----------------------------------------------------------------------------
export function analyzeKnowledgeRegression(faqId, question, beforeSupportedVariants, afterSupportedVariants, sampleQueries = []) {
    const isRegression = afterSupportedVariants < beforeSupportedVariants;
    const dropPercentage = beforeSupportedVariants > 0 && isRegression
...
// ----------------------------------------------------------------------------
export async function getIntelligenceDashboardSummary(forceRefresh = false) {
    const now = Date.now();
    ...
    try {
        ...
        const rawQueries = events
            .filter((e) => e.metadata?.query || e.metadata?.originalQuestion)
            .map((e) => ({
            query: (e.metadata?.query || e.metadata?.originalQuestion),
            userId: e.user_id,
        ...
        // 12. Recommendations
        const recommendations = generateKnowledgeRecommendations(faqHealthList, coverageReport, emergingTopics, conflicts, negativePolicyIntelligence, regressionReport);
        // Aggregate overall KPI scores
        const overallHealthScore = faqHealthList.length > 0
