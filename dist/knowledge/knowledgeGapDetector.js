...
import { normalizeText, isAmbiguousDemandQuery } from '../core/intentResolver.js';
 ...
 */
export function normalizeKnowledgeQuestion(rawText) {
 ...
 */
export function classifyKnowledgeGap(rawText, intent, searchResultsCount = 0, faqResultsCount = 0, hasNegativePolicyMatch = false) {
    ...
    // --------------------------------------------------------------------------
    const isSecuritySensitive = /(?:ignore\s+previous|system\s+prompt|api\s*key|access\s*token|admin\s+password|secret\s+key|fake\s+admin|sk-[a-zA-Z0-9_\-]{8,}|bearer\s+[a-zA-Z0-9._-]{10,}|(?:tao|luu|them|ghi|approve|save).*?faq)/i.test(clean) ||
    ...
    // --------------------------------------------------------------------------
    const isGreeting = /^(?:(?:xin\s+)?ch[aà]o|hello|hi|hey|alo|good\s+morning|good\s+afternoon|c[aả]m\s+[oơ]n|thank\s*you|thanks|reset|b[aắ]t\s+[đd][aà]u\s+l[aạ]i|l[aà]m\s+m[oớ]i)(?:\s+(?:shop|b[aạ]n|ad|admin|bot|em|anh|nh[eé]|nh[aà]))*[\s!.?]*$/i.test(clean) ||
        norm === 'chao' ||
        norm === 'xin chao' ||
        norm === 'chao shop' ||
        norm === 'xin chao shop' ||
    ...
    // --------------------------------------------------------------------------
    const isPriceInquiry = /(?:gia\s+bao\s+nhieu|bao\s+nhieu\s+tien|gia\s+sao|bn\s+tien|gia\s+nhieu|nhieu\s+tien|bao\s+nhieu|gia)\b/i.test(norm) && /(?:thang|nam|goi|slot|youtube|netflix|spotify|canva|6th|12th|1th)\b/i.test(norm);
    const isExplicitTransactional = /^(?:mua|order|dat\s+hang|thanh\s+toan|check\s*out|nap\s+tien|nap\s+vi|topup|kiem\s+tra\s+don|tra\s+cuu\s+don|xem\s+don|doi\s+mat\s+khau|gia\s+bao\s+nhieu|bao\s+nhieu\s+tien|bao\s+hanh\s+don)\b/i.test(norm) ||
        isPriceInquiry ||
        intent === 'BUY' ||
        intent === 'CHECKOUT' ||
        intent === 'ORDER_STATUS' ||
    ...
    // --------------------------------------------------------------------------
    const isUnsupported = /(?:tau\s+vu\s+tru|ten\s+lua|sao\s+hoa|ve\s+may\s+bay|tra\s+sua|dat\s+xe|khach\s+san|tap\s+gym|o\s+to|hack\s+ngan\s+hang)/i.test(norm);
    if (isUnsupported) {
        return 'UNSUPPORTED';
    }
    // --------------------------------------------------------------------------
    ...
    // --------------------------------------------------------------------------
    const isProductDemandQuery = /\b(netflix|spotify|youtube|canva|chatgpt|gemini|cursor|notion|adobe|figma|duolingo|memrise|capcut|meitu|xingtu|proton|office|word|excel|icloud|tv360|youku|kling|elevenlabs|claude|veo|grok|perplexity)\b/i.test(norm) ||
        /\b(?:app|tool|phan\s*mem|cong\s*cu|tai\s*khoan|goi|ban\s+quyen)\s+(?:xem\s+phim|nghe\s+nhac|chinh\s+anh|ve\s+anh|tao\s+video|viet\s+code|hoc\s+tieng\s+anh|vpn|cloud)\b/i.test(norm) ||
    ...
    const isPureProductDemand = isProductDemandQuery &&
        !/(?:ho\s+tro\s+cai|cai\s+dat|ultraview|anydesk|teamviewer|loi\s+dang\s+nhap|quen\s+mat\s+khau|chinh\s+sach|thoi\s+gian\s+lam\s+viec|lien\s+he|kenh\s+ho\s+tro)/i.test(norm);
    ...
    // --------------------------------------------------------------------------
    const hasKnowledgeKeywords = /(?:ho\s+tro\s+cai|cai\s+dat|cai\s+qua|ultraview|ultraviewer|anydesk|teamviewer|thoi\s+gian\s+ho\s+tro|gio\s+lam\s+viec|lien\s+he|kenh\s+ho\s+tro|loi\s+dang\s+nhap|khong\s+dang\s+nhap\s+duoc|bi\s+loi|chinh\s+sach\s+hoan\s+tien|quy\s+dinh|doi\s+tra|huong\s+dan\s+cai)/i.test(norm) ||
 ...
 */
export function isKnowledgeGapCandidate(rawText, intent, searchResultsCount = 0, faqResultsCount = 0) {
 ...
 */
export function inferKnowledgeGapCategory(normalizedText) {
 ...
 */
export function extractKnowledgeGapMetadata(rawText, intent, source = 'DETERMINISTIC', searchResultsCount = 0, faqResultsCount = 0) {
 ...
 */
export function deduplicateKnowledgeGaps(candidates) {
    const map = new Map();
    for (const c of candidates) {
        if (c.classification !== 'KNOWLEDGE_GAP')
            continue;
