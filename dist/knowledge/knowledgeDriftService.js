...
import { calculateKnowledgeCoverage, detectKnowledgeConflicts } from './knowledgeIntelligenceService.js';
import { getNegativePolicies } from './negativePolicyService.js';
// ---------------------------------------------------------------------------
// 1. IN-MEMORY CACHING
// ---------------------------------------------------------------------------
let cachedDriftReport = null;
let driftCacheExpiry = 0;
const DRIFT_CACHE_TTL_MS = 30_000; // 30 seconds
export function clearKnowledgeDriftCache() {
...
export function scoreToDriftStatus(score) {
    if (score < 20)
        return 'STABLE';
    if (score < 40)
        return 'WATCH';
...
// ---------------------------------------------------------------------------
export function detectFaqDrift(faq, recentEvents = [], baselineUsageCount = 10, conflictCount = 0) {
    const reasons = [];
    let penaltyPoints = 0;
    // 1. Usage drop
    ...
    const faqEvents = recentEvents.filter((e) => e.metadata?.faqId === faq.id || (e.event_type === 'FAQ_USED' && e.metadata?.question === faq.question));
...
// ---------------------------------------------------------------------------
export function detectNegativePolicyDrift(policy, recentEvents = [], conflictCount = 0) {
    const reasons = [];
    let penaltyPoints = 0;
    // Usage and match tracking
    const policyHits = recentEvents.filter((e) => e.event_type === 'NEGATIVE_POLICY_MATCHED' && (e.metadata?.policyKey === policy.policyKey || e.metadata?.policyId === policy.id)).length;
...
// ---------------------------------------------------------------------------
export function detectIntentDrift(currentEvents = [], baselineEvents = []) {
    const currentCounts = {};
...
export function detectQueryDistributionDrift(currentEvents = [], baselineEvents = []) {
    ...
    return intentDrifts.map((id, index) => {
        ...
        const driftSeverity = absChange >= 40 ? 'HIGH' : absChange >= 25 ? 'MODERATE' : absChange >= 15 ? 'LOW' : 'NONE';
        return {
...
// ---------------------------------------------------------------------------
export function detectCoverageDrift(currentCoverage, baselineCoverage) {
    ...
    return currentCoverage.domainCoverages.map((curr) => {
        ...
        const driftSeverity = drop >= 30 ? 'CRITICAL' : drop >= 20 ? 'HIGH' : drop >= 10 ? 'MODERATE' : drop > 0 ? 'LOW' : 'NONE';
        return {
            domain: curr.domain,
...
export function detectMatchRateDrift(baselineMatchRate, currentMatchRate) {
...
export function detectConflictDrift(currentConflictCount, baselineConflictCount) {
    const surge = Math.max(0, currentConflictCount - baselineConflictCount);
    const driftSeverity = surge >= 3 ? 'CRITICAL' : surge === 2 ? 'HIGH' : surge === 1 ? 'MODERATE' : 'NONE';
    return { surge, driftSeverity };
}
export function detectResponseBehaviorDrift(events = []) {
    const details = [];
    ...
    const severity = shiftedCount >= 10 ? 'HIGH' : shiftedCount >= 4 ? 'MODERATE' : shiftedCount > 0 ? 'LOW' : 'NONE';
...
export function calculateDriftScore(params) {
    ...
    if (params.faqDrifts.length > 0) {
        const faqAvg = params.faqDrifts.reduce((acc, f) => {
    ...
    if (params.policyDrifts.length > 0) {
        const polAvg = params.policyDrifts.reduce((acc, p) => {
    ...
    if (params.coverageDrifts.length > 0) {
        const covAvg = params.coverageDrifts.reduce((acc, c) => {
    ...
    if (params.queryDrifts.length > 0) {
        const qAvg = params.queryDrifts.reduce((acc, q) => {
...
// ---------------------------------------------------------------------------
export async function detectKnowledgeDrift(providedFaqs, providedPolicies, providedEvents, forceRefresh = false) {
    const now = Date.now();
    ...
    try {
        ...
        const policyDrifts = (policies || []).map((pol) => {
        ...
        const currentCoverage = calculateKnowledgeCoverage(faqs, policies, events || [], []);
