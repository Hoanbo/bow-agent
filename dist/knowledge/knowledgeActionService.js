...
import { insertAnalyticsEvent } from '../monitoring/agentEvents.js';
import { getActiveShopAdapter } from '../contracts/index.js';
import { clearKnowledgeIntelligenceCache } from './knowledgeIntelligenceService.js';
import { clearNegativePolicyCache } from './negativePolicyService.js';
// ---------------------------------------------------------------------------
// In-Memory Action Center Cache
// ---------------------------------------------------------------------------
let cachedActionCenter = null;
let lastActionCacheFetchTime = 0;
const ACTION_CACHE_TTL_MS = 60 * 1000; // 60s
export function clearActionCenterCache() {
...
export function assertAdminAuthorized(adminUserId, _operation) {
...
// 2. PII SANITIZATION
// ---------------------------------------------------------------------------
export function sanitizeActionText(raw) {
...
// ---------------------------------------------------------------------------
export function calculateDecisionFingerprint(entityId, issueType, normalizedEvidence) {
...
export function estimateImpact(params) {
    let score = 0;
    if ((params.usageCount || 0) >= 50)
        score += 3;
    else if ((params.usageCount || 0) >= 10)
        score += 2;
    else if ((params.usageCount || 0) >= 1)
        score += 1;
    if ((params.healthScore || 100) < 50)
        score += 3;
    else if ((params.healthScore || 100) < 75)
        score += 1;
    if ((params.gapCount || 0) >= 10)
        score += 2;
    else if ((params.gapCount || 0) >= 3)
        score += 1;
    if ((params.conflictCount || 0) >= 2)
        score += 2;
    else if ((params.conflictCount || 0) >= 1)
        score += 1;
    if ((params.coveragePercentage || 100) < 60)
        score += 2;
    if (score >= 6)
        return 'HIGH';
    if (score >= 3)
...
// ---------------------------------------------------------------------------
function assessRisk(actionType) {
    const highRisk = [
    ...
    if (highRisk.includes(actionType))
        return 'HIGH';
    if (medRisk.includes(actionType))
        return 'MEDIUM';
    return 'LOW';
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function mapRecommendationToActionType(recType) {
    const mapping = {
...
export function captureAfterSnapshot(before, params) {
    return {
...
// ---------------------------------------------------------------------------
export function calculateActionOutcome(before, after, observationWindow = '7D', isInsufficientData = false, feedbackReason) {
    ...
    if (matchDelta > 15)
        improvementSignal += 3;
    else if (matchDelta > 5)
        improvementSignal += 2;
    else if (matchDelta > 0)
        improvementSignal += 1;
    else if (matchDelta < -10)
        improvementSignal -= 2;
    if (gapDelta > 5)
        improvementSignal += 2;
    else if (gapDelta > 0)
    ...
    const rawGapDelta = after.gapCountAfter !== undefined && before.gapCountBefore !== undefined
...
// ---------------------------------------------------------------------------
export function calculateKnowledgeImprovementScore(input) {
    if (!Array.isArray(input)) {
        ...
        const trend = totalScore >= 75 ? 'IMPROVING' : totalScore < 50 ? 'DEGRADING' : 'STABLE';
        return {
            score: totalScore,
            components: {
    ...
    const actionsWithOutcome = completedActions.filter((a) => a.outcome && !a.outcome.isInsufficientData);
    ...
    const score = Math.min(100, Math.max(0, Math.round(healthComponent + matchComponent + gapComponent + conflictComponent + coverageComponent)));
    const regressedCount = actionsWithOutcome.filter((a) => a.outcome?.effectiveness === 'REGRESSED').length;
    const improvedCount = actionsWithOutcome.filter((a) => a.outcome?.feedbackType === 'ACTION_IMPROVED').length;
    let trend;
    if (regressedCount > improvedCount)
        trend = 'DEGRADING';
    else if (improvedCount > 0 && score > 20)
        trend = 'IMPROVING';
    else
        trend = 'STABLE';
    return {
...
// ---------------------------------------------------------------------------
function buildActionsFromEvents(events) {
    ...
    for (const ev of sorted) {
        ...
        if (type === 'KNOWLEDGE_ACTION_CREATED' && actionId) {
            ...
            const a = actionsMap.get(actionId) || {
                id: actionId,
                type: (ev.action_type || meta.actionType || 'EDIT_FAQ'),
                priority: 'MEDIUM',
...
// ---------------------------------------------------------------------------
function fingerprintExistsInActions(fingerprint, actions) {
    const sevenDaysMs = 7 * 24 * 60 * 60 * 1000;
    for (const action of actions) {
        if (action.decisionFingerprint !== fingerprint)
            continue;
        // Active actions block new creation
        if (['OPEN', 'ACKNOWLEDGED', 'IN_PROGRESS', 'SNOOZED', 'BLOCKED'].includes(action.status))
            return true;
        ...
        if (action.status === 'DISMISSED') {
            ...
            if (Date.now() - updatedMs < sevenDaysMs)
                return true;
        }
    }
    return false;
...
// ---------------------------------------------------------------------------
export async function syncRecommendationsToActions(recommendations, existingActions) {
    const newActions = [];
    for (const rec of recommendations) {
        if (rec.status !== 'OPEN')
            continue;
        const actionType = mapRecommendationToActionType(rec.type);
        const fingerprint = calculateDecisionFingerprint(rec.affectedEntityId || rec.type, rec.type, (rec.evidence || '').slice(0, 100));
        // Skip if fingerprint already tracked
        if (fingerprintExistsInActions(fingerprint, existingActions))
            continue;
        if (fingerprintExistsInActions(fingerprint, newActions))
            continue;
...
// ---------------------------------------------------------------------------
export async function acknowledgeAction(actionId, adminUserId, allActions) {
    try {
...
export async function startAction(actionId, adminUserId, allActions, beforeSnapshot) {
    try {
        assertAdminAuthorized(adminUserId);
        const action = allActions.find((a) => a.id === actionId);
        if (!action)
...
export async function completeAction(actionId, adminUserId, allActions, params) {
    try {
        assertAdminAuthorized(adminUserId);
        const action = allActions.find((a) => a.id === actionId);
        if (!action)
...
export async function dismissAction(actionId, adminUserId, allActions, reason) {
    try {
        assertAdminAuthorized(adminUserId);
        const action = allActions.find((a) => a.id === actionId);
        if (!action)
...
export async function snoozeAction(actionId, adminUserId, allActions, snoozedUntil, reason) {
    try {
        assertAdminAuthorized(adminUserId);
        const action = allActions.find((a) => a.id === actionId);
        if (!action)
...
export async function recordOutcome(actionId, adminUserId, allActions, outcome, afterSnapshot) {
    try {
        assertAdminAuthorized(adminUserId);
        const action = allActions.find((a) => a.id === actionId);
        if (!action)
...
// ---------------------------------------------------------------------------
export async function getActionCenter(recommendations, forceRefresh = false) {
    const now = Date.now();
    if (!forceRefresh && cachedActionCenter && now - lastActionCacheFetchTime < ACTION_CACHE_TTL_MS) {
        return cachedActionCenter;
    }
    try {
        const eventsData = await getActiveShopAdapter().storage.getAgentEvents(undefined, 2000, [
