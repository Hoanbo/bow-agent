 ...
 */
export function isValidAdmissionDeviceId(id) {
 ...
 */
export function assertValidAdmissionDeviceId(id) {
 ...
 */
export function resolveAdmissionDeviceIdentity(input) {
    ...
    if (input.scope && input.scope.deviceId && input.scope.deviceId !== deviceId) {
        throw new Error(`[ADMISSION_SCOPE_MISMATCH] Scope deviceId ${input.scope.deviceId} does not match request deviceId ${deviceId}.`);
    }
    const net = input.networkMetadata || input.network;
    if (net) {
