...
const FORBIDDEN_CAPABILITIES_SET = new Set(FORBIDDEN_ADMISSION_CAPABILITIES);
const ALLOWED_CAPABILITIES_SET = new Set(ALLOWED_DATA_PLANE_CAPABILITIES);
/**
 * Evaluates requested capabilities and fails closed if any forbidden execution capabilities are present.
 */
export function filterAdmissionCapabilities(requested) {
    const allowed = [];
    const rejected = [];
    const forbidden = [];
...
export function isCognitiveEscalationAttempt(requested) {
    const res = filterAdmissionCapabilities(requested);
    return res.forbidden.length > 0;
}
export function assertSafeAdmissionCapabilities(requested) {
    const res = filterAdmissionCapabilities(requested);
