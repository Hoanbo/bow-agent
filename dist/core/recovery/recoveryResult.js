...
import { computeCrashRecoveryFingerprint, computeRecoveryFailureFingerprint, } from './recoveryFingerprint.js';
 ...
 */
export function createRecoveryFailure(category, rawMessage, userId, sessionId, recoverable = false, details) {
 ...
 */
export function createRecoveryResult(recoveryId, userId, sessionId, state, decision, classification, crashCondition, reconstructedState, lastKnownGoodState, journal, failure) {
    const fingerprint = computeCrashRecoveryFingerprint(userId, sessionId, recoveryId, state, decision, classification);
    return deepFreeze({
        recoveryId,
        userId,
        sessionId,
