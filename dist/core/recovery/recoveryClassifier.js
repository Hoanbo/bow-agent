 ...
 */
export function classifyInterruptedOperation(crashCondition, activeLifecycleState, risk = 'LOW', approvalStatus = 'APPROVAL_UNKNOWN', isPartialCommit = false) {
 ...
 */
export function evaluateApprovalRecoveryStatus(approvalMetadata, lifecycleState) {
    if (!approvalMetadata) {
        return 'APPROVAL_UNKNOWN';
    }
    const status = approvalMetadata.status;
    const consumed = approvalMetadata.consumed;
    const expired = approvalMetadata.expired;
    if (expired === true) {
