 ...
 */
export function redactRecoverySecrets(text) {
 ...
 */
export function containsRecoverySecret(val) {
 ...
 */
export function hasRecoveryPrototypePollution(target) {
    ...
    for (const [k, v] of Object.entries(target)) {
        if (PROTOTYPE_POLLUTION_REGEX.test(k))
            return true;
        if (v && typeof v === 'object' && hasRecoveryPrototypePollution(v))
            return true;
    }
    return false;
}
 ...
 */
export function validateRecoveryScope(userId, sessionId) {
 ...
 */
export function assertRecoveryRiskPreservation(priorRisk, currentRisk) {
    const priorRank = RISK_HIERARCHY[priorRisk] ?? 1;
    const currentRank = RISK_HIERARCHY[currentRisk] ?? 1;
    if (currentRank < priorRank) {
        throw new Error(`RISK_DOWNGRADE_FORBIDDEN: Cannot downgrade recovery risk from ${priorRisk} to ${currentRisk}`);
    }
}
