...
export class PolicyActiveLifecycleConsistencyEngine {
    ...
    provenanceEngine;
    isUserStopActiveFn;
    constructor(options, tenantEngine, stateResolver, driftDetector, versionEngine, rollbackEngine, provenanceEngine) {
     ...
     */
    evaluateLifecycleConsistency(tenantPartition, explicitSnapshot) {
        this.assertUserStopInactive();
        const reconciliationId = createLifecycleReconciliationId(`rec_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`);
        const allDrifts = [];
