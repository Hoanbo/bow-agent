...
export class PolicyActiveLifecycleRuntimeDriftDetector {
    ...
    pepBridge;
    isUserStopActiveFn;
    constructor(options, syncEngine, freshnessValidator, pdpBridge, pepBridge) {
     ...
     */
    detectDrift(tenantPartition, activeState, providedSnapshot) {
        this.assertUserStopInactive();
        const drifts = [];
        const boundaryChecks = [];
        const blockingReasons = [];
        // 1. Resolve snapshot: either provided or retrieved from syncEngine cache (read-only)
        const snapshot = providedSnapshot !== undefined
            ? providedSnapshot
        ...
        if (activeState && snapshot) {
            ...
            const freshness = this.freshnessValidator.validateFreshness(activeState, tenantPartition, snapshot.policyVersion);
            if (freshness.status !== 'FRESH') {
                const cat = freshness.status === 'STALE' ? 'STALE_RUNTIME_SNAPSHOT' : 'SUPERSEDED_RUNTIME_SNAPSHOT';
                drifts.push({
