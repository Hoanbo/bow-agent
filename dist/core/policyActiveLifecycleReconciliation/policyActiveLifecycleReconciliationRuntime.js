...
export class PolicyActiveLifecycleReconciliationRuntime {
    ...
    constructor(options, consistencyEngine, auditEngine) {
    ...
    appendProvenanceRecord(tenantPartition, result) {
        ...
        const recordHash = crypto
            .createHash('sha256')
            .update(`${tenantPartition}:${result.reconciliationId}:${result.status}:${timestamp}:${previousHash}:${payloadHash}`)
            .digest('hex');
     ...
     */
    reconcileLifecycle(tenantPartition, options) {
        ...
        const result = this.consistencyEngine.evaluateLifecycleConsistency(tenantPartition, options?.explicitSnapshot);
     ...
     */
    verifyConsistency(tenantPartition) {
        this.assertUserStopInactive(tenantPartition);
     ...
     */
    getReconciliationProvenance(tenantPartition) {
        this.assertUserStopInactive(tenantPartition);
     ...
     */
    verifyReconciliationProvenance(tenantPartition) {
        this.assertUserStopInactive(tenantPartition);
        ...
        for (const record of chain) {
            ...
            const computedHash = crypto
                .createHash('sha256')
                .update(`${record.tenantPartition}:${record.reconciliationId}:${record.status}:${record.timestamp}:${record.previousHash}:${record.payloadHash}`)
                .digest('hex');
