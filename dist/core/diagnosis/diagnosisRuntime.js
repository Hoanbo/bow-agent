...
import { globalEvidenceCorrelationEngine } from './evidenceCorrelationEngine.js';
import { globalRootCauseHypothesisEngine } from './rootCauseHypothesisEngine.js';
import { globalIncidentClassificationEngine } from './incidentClassificationEngine.js';
import { globalDiagnosisContradictionEngine } from './diagnosisContradictionEngine.js';
import { globalDiagnosisProvenanceEngine } from './diagnosisProvenanceEngine.js';
import { globalDecisionSupportSynthesizer } from './decisionSupportSynthesizer.js';
...
export class DiagnosisRuntime {
    ...
    isRevoked = false;
    constructor(correlationEngine = globalEvidenceCorrelationEngine, hypothesisEngine = globalRootCauseHypothesisEngine, classificationEngine = globalIncidentClassificationEngine, contradictionEngine = globalDiagnosisContradictionEngine, provenanceEngine = globalDiagnosisProvenanceEngine, synthesizer = globalDecisionSupportSynthesizer, sanitizer = globalDiagnosisSanitizer, auditLedger = globalAuditLedger, humanGate = globalSupervisorHumanGate) {
    ...
    diagnoseIncident(options) {
        ...
        const contradictionResult = this.contradictionEngine.evaluateAssertions(options.multiAgentAssertions ?? [], options.observabilityContradictions);
     ...
     */
    submitToSupervisorGate(diagnosis) {
        if (this.isUserStopActive) {
        ...
        let legacySeverity = 'MEDIUM';
        if (diagnosis.severity === 'CRITICAL')
            legacySeverity = 'CRITICAL';
        else if (diagnosis.severity === 'HIGH')
            legacySeverity = 'HIGH';
        else if (diagnosis.severity === 'LOW')
            legacySeverity = 'LOW';
        else if (diagnosis.severity === 'INFORMATIONAL')
            legacySeverity = 'INFO';
    ...
    // ---------------------------------------------------------------------------
    logAudit(action, classification, details, policyDecision) {
