...
export class IncidentClassificationEngine {
    ...
    classify(input) {
        ...
        // C. Check CRITICAL conditions
            ...
            const hasProtectedWorkspaceViolation = items.some((i) => i.source === 'INVARIANT' && i.category === 'PROTECTED_WORKSPACE');
            const hasBoundaryViolation = items.some((i) => i.source === 'INVARIANT' && (i.category === 'BOUNDARY' || i.category === 'USER_STOP' || i.category === 'REVOCATION'));
            const hasCriticalDriftMutation = items.some((i) => i.source === 'DRIFT' && i.severity === 'CRITICAL' && i.category === 'UNAUTHORIZED_MUTATION');
            ...
            if (hasProtectedWorkspaceViolation || hasBoundaryViolation || hasCriticalDriftMutation || isHealthCritical) {
                severity = 'CRITICAL';
                if (hasProtectedWorkspaceViolation)
                    rationale.push('CRITICAL: Protected workspace boundary violation detected.');
                if (hasBoundaryViolation)
                    rationale.push('CRITICAL: Core boundary or safety switch invariant violated.');
                if (hasCriticalDriftMutation)
                    rationale.push('CRITICAL: Unauthorized filesystem or state mutation drift detected.');
                if (isHealthCritical)
                    rationale.push('CRITICAL: Observability mesh reported composite CRITICAL health state.');
            }
            // D. Check HIGH conditions
            else {
                const hasCanaryRegression = items.some((i) => i.source === 'TELEMETRY' && i.severity === 'HIGH');
                ...
                if (hasCanaryRegression || hasHighAlert || hasCanaryRollback) {
                    severity = 'HIGH';
                    if (hasCanaryRegression)
                        rationale.push('HIGH: Consecutive canary degradation streak detected.');
                    if (hasHighAlert)
                        rationale.push('HIGH: High-severity advisory alert triggered.');
                    if (hasCanaryRollback)
                        rationale.push('HIGH: Canary verification engine recommended rollback.');
                }
                // E. Check MEDIUM conditions
                    ...
                    if (driftCount > 1 || invariantViolationCount > 0) {
                        severity = 'MEDIUM';
                        if (driftCount > 1)
                            rationale.push(`MEDIUM: Multi-metric drift detected (${driftCount} drift events).`);
                        if (invariantViolationCount > 0)
                            rationale.push(`MEDIUM: Non-boundary invariant violation detected (${invariantViolationCount} violations).`);
                    }
        ...
        const isCoreTarget = input.cluster.targetId.includes('gateway') ||
