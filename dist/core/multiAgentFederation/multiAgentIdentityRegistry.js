...
import { MultiAgentFederationValidationError, MultiAgentFederationTenantIsolationError, MultiAgentFederationSessionIsolationError, computeAgentProvenanceHash, } from './multiAgentFederationTypes.js';
export class MultiAgentIdentityRegistry {
    ...
    validateAgentInput(input) {
        ...
        if (Object.prototype.hasOwnProperty.call(rec, '__proto__') ||
            ...
            throw new MultiAgentFederationValidationError('Prototype pollution keys (__proto__, constructor, prototype) are strictly prohibited');
        }
        ...
        for (const marker of cotMarkers) {
            if (serialized.includes(marker)) {
                throw new MultiAgentFederationValidationError(`Prohibited chain-of-thought/deliberation marker '${marker}' detected in agent input`);
            }
        }
        ...
        for (const pattern of promptInjectionPatterns) {
            if (pattern.test(serialized)) {
                throw new MultiAgentFederationValidationError(`Adversarial prompt injection pattern '${pattern.source}' detected in agent input`);
            }
        }
    ...
    assertValidIdentifier(value, fieldName) {
        if (value.includes('..') || value.includes('/') || value.includes('\\') || value.includes('\0')) {
            throw new MultiAgentFederationValidationError(`Field '${fieldName}' contains invalid path traversal characters`);
        }
        const upper = value.toUpperCase();
        const reserved = ['CON', 'PRN', 'AUX', 'NUL', 'COM1', 'COM2', 'COM3', 'COM4', 'COM5', 'COM6', 'COM7', 'COM8', 'COM9', 'LPT1', 'LPT2', 'LPT3', 'LPT4', 'LPT5', 'LPT6', 'LPT7', 'LPT8', 'LPT9'];
        if (reserved.includes(upper)) {
            throw new MultiAgentFederationValidationError(`Field '${fieldName}' uses reserved Windows device name '${upper}'`);
        }
    }
    /**
    ...
    registerAgent(params) {
        ...
        if (this.agents.has(params.agentId)) {
            throw new MultiAgentFederationValidationError(`Agent with ID '${params.agentId}' is already registered`, params.tenantId, undefined, params.agentId);
        }
        const now = Date.now();
        const agentData = {
     ...
     */
    assertAgentBoundaries(agentId, expectedTenantId, expectedSessionId) {
        const agent = this.getAgent(agentId);
        if (!agent) {
            throw new MultiAgentFederationValidationError(`Agent with ID '${agentId}' not found in registry`, expectedTenantId, undefined, agentId);
        }
        if (agent.tenantId !== expectedTenantId) {
            throw new MultiAgentFederationTenantIsolationError(`Agent '${agentId}' tenant mismatch: expected '${expectedTenantId}', got '${agent.tenantId}'`, expectedTenantId, undefined, agentId);
        }
        if (agent.sessionId !== expectedSessionId) {
            throw new MultiAgentFederationSessionIsolationError(`Agent '${agentId}' session mismatch: expected '${expectedSessionId}', got '${agent.sessionId}'`, expectedTenantId, undefined, agentId);
        }
        if (agent.status === 'REVOKED' || agent.status === 'INVALIDATED' || agent.status === 'HALTED_BY_USER_STOP' || agent.status === 'HALTED_BY_EMERGENCY_STOP') {
            throw new MultiAgentFederationValidationError(`Agent '${agentId}' is in terminal/unusable state '${agent.status}'`, expectedTenantId, undefined, agentId);
        }
        return agent;
    }
    /**
     ...
     */
    updateAgentTrust(agentId, trustProfile) {
        const agent = this.agents.get(agentId);
        if (!agent) {
            throw new MultiAgentFederationValidationError(`Agent '${agentId}' not found`);
        }
