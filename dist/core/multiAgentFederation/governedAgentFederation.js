...
import { MAX_AGENTS_PER_FEDERATION, MAX_ACTIVE_FEDERATIONS, MAX_FEDERATION_COORDINATION_CYCLES, MAX_CONSECUTIVE_FEDERATION_FAILURES, MAX_FEDERATION_DURATION_MS, MultiAgentFederationValidationError, MultiAgentFederationBudgetError, computeSha256, deterministicJsonStringify, } from './multiAgentFederationTypes.js';
...
export class GovernedAgentFederation {
    ...
    createFederation(params) {
        ...
        const activeForSession = Array.from(this.federations.values()).filter((f) => f.sessionId === params.sessionId && f.status !== 'COMPLETED' && f.status !== 'FAILED');
        if (activeForSession.length >= MAX_ACTIVE_FEDERATIONS) {
            throw new MultiAgentFederationBudgetError(`Session reached MAX_ACTIVE_FEDERATIONS limit (${MAX_ACTIVE_FEDERATIONS})`, params.tenantId, params.federationId);
        }
        // 3. Agent count check
        if (params.initialAgentIds.length === 0 || params.initialAgentIds.length > MAX_AGENTS_PER_FEDERATION) {
            throw new MultiAgentFederationBudgetError(`Federation agent count must be between 1 and ${MAX_AGENTS_PER_FEDERATION}`, params.tenantId, params.federationId);
        }
        ...
        if (!params.initialAgentIds.includes(params.leaderAgentId)) {
            throw new MultiAgentFederationValidationError(`Leader agent '${params.leaderAgentId}' must be a participating agent of the federation`, params.tenantId, params.federationId);
        }
        const now = Date.now();
        const fedBase = {
        ...
        const provenanceHash = computeSha256(`federation_group:${deterministicJsonStringify(fedBase)}`);
        const federation = {
            ...fedBase,
            status: 'READY',
            provenanceHash,
     ...
     */
    async coordinateFederation(federationId, cyclesToRun = 1, delegationExecutor) {
        ...
        let currentStatus = federation.status === 'READY' ? 'COORDINATING' : federation.status;
