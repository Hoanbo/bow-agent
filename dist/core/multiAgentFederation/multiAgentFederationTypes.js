...
export class MultiAgentFederationError extends Error {
    ...
    federationId;
    agentId;
    constructor(message, tenantId, federationId, agentId) {
        super(message);
...
export function computeAgentProvenanceHash(agent) {
...
export function computeAgentCapabilityHash(cap) {
    const canonical = deterministicJsonStringify({
...
export function computeDelegationBindingHash(delegation) {
    const canonical = deterministicJsonStringify({
...
export function computeDelegationChainHash(parentHash, delegationHash, depth) {
    return computeSha256(`delegation_chain:${parentHash}:${delegationHash}:depth_${depth}`);
}
export function computeFederationSnapshotHash(snapshot) {
    const canonical = deterministicJsonStringify({
...
export function computeFederationResultHash(result) {
    const canonical = deterministicJsonStringify({
...
export function computeFederationAuditHash(previousHash, eventType, tenantId, timestamp, payload) {
    const canonicalPayload = deterministicJsonStringify(payload);
    return computeSha256(`federation_audit:${previousHash}:${eventType}:${tenantId}:${timestamp}:${canonicalPayload}`);
}
