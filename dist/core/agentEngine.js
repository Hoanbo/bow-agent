import { resolveMultiIntent, detectPluralDiscoveryIntent, extractDuration, matchPlanByDuration, isAmbiguousDemandQuery, normalizeText } from './intentResolver.js';
import { sanitizeQueryText } from '../monitoring/demandAggregator.js';
import { resolveProductQuery } from './productResolver.js';
import { resolveCategoryQuery, getAllCategories } from './categoryResolver.js';
import { getSessionContext, rememberProductContext, rememberRecommendedCandidates, rememberOrderContext, rememberCategoryContext, rememberDeferredContext, clearDeferredContext, clearSessionContext, } from './sessionContext.js';
import { planCheckoutAction, planRenewalAction, planSupportTicketAction, planTicketDetailAction, planCreateTicketAction, planApplyCouponAction, planDepositAction, planOrderDetailAction, planMultipleCheckoutActions, findRelevantWarrantyOrder, } from './actionPlanner.js';
import { formatSingleProductResponse, formatCatalogOverviewResponse, formatCategoryDetailResponse, formatCompactOrdersResponse, } from './responseFormatter.js';
import { searchProducts, getMyOrders, getMyTickets, checkWarrantyPolicy, getActiveCoupons, getMyWalletBalance, getFaqsAndGuides, getSupportChannels, } from './tools.js';
import { agentAnalytics, normalizeUserDemand } from '../monitoring/agentAnalytics.js';
import { processAgentMessageWithGemini, resetGeminiHistory } from '../gemini/geminiClient.js';
...
import { matchNegativePolicy } from '../knowledge/negativePolicyService.js';
...
function findPlanByDuration(plans, requestedDuration) {
 ...
 */
export async function processAgentMessage(userText, context) {
    ...
    Promise.resolve().then(() => {
        try {
            ...
            const classification = classifyKnowledgeGap(userText, finalMessage?.data?.type || null, finalMessage?.data?.candidates?.length || 0, responseSource === 'FAQ' ? 1 : 0, isNegPolicy);
            ...
            if (isNegPolicy || classification === 'SUPPORTED_NEGATIVE_POLICY') {
                agentAnalytics.track({
                    ...
                    userId: context.userId,
                    metadata: {
                        policyId: finalMessage?.data?.policy?.id,
                        query: sanitizeQueryText(userText),
                        normalizedQuery: normalizeText(userText),
                        responseSource,
                        latencyMs,
 ...
 */
export async function processAgentMessageV2(userText, context) {
    ...
    // --------------------------------------------------------------------------
    const isBossChannel = context.channel === 'ROBOT' || context.channel === 'DESKTOP' || context.role === 'owner' || context?.isBoss === true;
    if (isBossChannel) {
        ...
        if (norm.includes('anh thich uong') ||
    ...
    // --------------------------------------------------------------------------
    const isAdminRole = context.role === 'admin' || context.role === 'owner' || context?.isAdmin === true;
    ...
    if (isAdminRole && isAdminSurface) {
        ...
        if (norm.includes('hom nay shop co gi can toi xu ly') ||
            ...
            if (adapter.admin?.getDailySummary) {
                ...
                const highlightsText = summary.summaryHighlights.map((h) => `• ${h}`).join('\n');
                return {
                    id,
                    sender: 'agent',
        ...
        if (norm.includes('kiem tra don') ||
            ...
            if (adapter.admin?.getOrderLookup) {
                ...
                if (order) {
                    ...
                    const timelineText = (order.timeline || []).map((t) => `• \`${t.time.split('T')[1]?.slice(0, 5) || t.time}\`: ${t.event}`).join('\n');
                    return {
                        id,
                        sender: 'agent',
            ...
            if (adapter.admin?.getCustomerLookup) {
                ...
                if (customer) {
                    const recentText = customer.recentOrders
                        .map((o) => `• #${o.orderId}: ${o.productName} (${o.amount.toLocaleString('vi-VN')}đ) - \`${o.status}\``)
                        .join('\n');
                    return {
                        id,
                        sender: 'agent',
        ...
        if (norm.includes('voucher') || norm.includes('ma giam') || norm.includes('khuyen mai') || norm.includes('tao ma')) {
            if (norm.includes('co voucher') || norm.includes('danh sach voucher') || norm.includes('dang hoat dong')) {
                if (adapter.admin?.getActiveVouchers) {
                    ...
                    const vList = res.vouchers.map((v) => `• \`${v.code}\`: Giảm **${v.discountDisplay}** (Đơn từ ${v.minOrderValue.toLocaleString('vi-VN')}đ)`).join('\n');
                    return {
                        id,
                        sender: 'agent',
    ...
    if (intent === 'SMALL_TALK') {
        ...
        const isAffirmative = cleanLower === 'có' ||
            cleanLower === 'có chứ' ||
            cleanLower === 'tiếp tục' ||
            cleanLower === 'mua đi' ||
            cleanLower === 'mua luôn' ||
    ...
    if (intent === 'CATALOG') {
        ...
        const isCapabilityQuery = detectPluralDiscoveryIntent(userText) ||
    ...
    if (intent === 'BUY') {
        ...
        if (productToBuy) {
            ...
            const requestedDuration = resolution.extractedParams.durationFilter ||
    ...
    if (intent === 'WARRANTY') {
        ...
        policy.warrantySteps.forEach((s) => {
            msg += `${s}\n`;
        });
    ...
    if (intent === 'WALLET') {
        ...
        const amountMatch = userText.match(/(?:nạp|nap|topup|top up|\+)\s*(\d+(?:\.\d+)?)\s*(k|nghìn|ngàn|tr|triệu|đ|vnd)?/i) ||
            userText.match(/\b(\d+)\s*(k|nghìn|ngàn|tr|triệu)\b/i) ||
            userText.match(/\b(\d{4,9})\s*(?:đ|vnd)?\b/i);
        let specifiedAmount = undefined;
        if (amountMatch) {
    ...
    if (intent === 'FAQ') {
        ...
        if (negMatch && negMatch.policy) {
            return {
                ...
                timestamp,
                data: { type: 'negative_policy', policy: negMatch.policy },
    ...
    if (product) {
        ...
        const requestedDur = resolution.extractedParams.durationFilter ||
            extractDuration(userText) ||
            multiIntent.deferredContext?.duration;
        if (requestedDur) {
    ...
    if ((resolution.semanticCandidates && resolution.semanticCandidates.length > 0) ||
