...
import { Phase14GovernanceAuditEngine, } from './phase14GovernanceAuditEngine.js';
...
export class Phase14ExitCertificateRuntime {
     ...
     */
    static async executeIndependentAudit(input, gateOptions) {
     ...
     */
    async runIndependentAudit(input) {
        const auditId = input.auditId || `audit_phase14_${input.tenantId}_${Date.now()}`;
        const certificateId = input.certificateId || `cert_phase14_exit_${input.tenantId}_${Date.now()}`;
        ...
        const outcome = await this._auditEngine.executeAudit(input);
        ...
        const executionResult = {
            ...
            error: outcome.auditStatus !== 'EXIT_READY'
                ? {
