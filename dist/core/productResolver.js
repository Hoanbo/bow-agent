 ...
 */
function extractQueryParameters(rawText) {
 ...
 */
export function cleanQueryTokens(rawText) {
 ...
 */
function scoreProductSemantics(product, demandTokens) {
    if (demandTokens.length === 0)
        return 0;
    // Build normalized search corpus from actual DB data
    const corpusParts = [];
    if (product.name)
        corpusParts.push(normalizeString(product.name));
    if (product.tagline)
        corpusParts.push(normalizeString(product.tagline));
    if (product.description)
        corpusParts.push(normalizeString(product.description));
    if (product.categoryName)
        corpusParts.push(normalizeString(product.categoryName));
    ...
    if (totalWeight === 0 || matchedTokenCount === 0)
        return 0;
    ...
    if (totalWeight >= 3) {
        if (!hasFullPhrase && !hasSpecificBigramMatch)
            return 0;
        if (!hasFullPhrase && ratio < 0.5)
            return 0;
    }
    ...
    if (hasSpecificBigramMatch)
        score += 5;
    else if (hasBigramMatch)
        score += 2;
    if (hasFullPhrase)
 ...
 */
function extractDemandTokens(cleanedQuery) {
 ...
 */
export async function resolveProductQuery(rawQuery) {
    ...
    for (const p of allProducts) {
        ...
        else if (normRaw.includes(normName) ||
        ...
        // V2.2: Check fuzzy against full name, individual words in name, and aliases
            ...
            const aliasSim = aliases.length > 0
                ? Math.max(...aliases.map((a) => calculateSimilarity(normClean, a)), ...aliases.flatMap((a) => a.split(' ').filter((w) => w.length >= 4).map((w) => calculateSimilarity(normClean, w))))
                : 0;
