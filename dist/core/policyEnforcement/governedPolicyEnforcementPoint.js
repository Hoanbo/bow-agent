...
import { createEnforcementDecisionId, CANONICAL_HARD_FORBIDDEN_ACTIONS, } from './policyEnforcementTypes.js';
import { globalPolicyHotSwapEngine } from './policyHotSwapEngine.js';
import { globalActivePolicyResolver } from './activePolicyResolver.js';
...
import { globalPolicyCanaryRuntime } from '../policyCanary/policyCanaryRuntime.js';
export class GovernedPolicyEnforcementPoint {
    ...
    enforce(context) {
        ...
        const decisionId = createEnforcementDecisionId(`pep_dec_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`);
        // 1. Enforce absolute USER_STOP supremacy
        const userStopActive = (this.isUserStopActiveFn && this.isUserStopActiveFn()) ||
        ...
        if (this.canaryRuntime) {
            const canaryRoute = this.canaryRuntime.evaluateExecutionRoute(tenantPartition, activeConfig, context.toolName);
            if (canaryRoute.isBaselineFallback && canaryRoute.failClosedReason) {
                return this.fallbackProvider.createFailClosedDecision({
                    toolName: context.toolName,
                    reason: canaryRoute.reason,
                    failClosedReason: canaryRoute.failClosedReason,
                });
            }
            // Ring 0: Shadow Mode
        ...
        if (resolvedClassification === 'HIGH_IMPACT') {
            ...
            if (context.executionToken) {
                const tokenValidation = approvalService.validateAndConsumeToken(context.executionToken, context.toolName, context.args, context.actor?.userId);
     ...
     */
    getActiveActionClassification(toolName, tenantPartition) {
