...
import { createBaselinePolicyConfiguration, } from '../policyEvolution/policySnapshotStore.js';
import { CANONICAL_HARD_FORBIDDEN_ACTIONS, createEnforcementDecisionId, } from './policyEnforcementTypes.js';
export class FailClosedBaselineFallback {
     ...
     */
    assertHardForbiddenImmutable(actionName, candidateClassification) {
        if (this.isHardForbidden(actionName) && candidateClassification !== 'FORBIDDEN') {
            throw new Error(`ILLEGAL_POLICY_DOWNGRADE: Action '${actionName}' is permanently FORBIDDEN and cannot be reclassified to '${candidateClassification}'.`);
        }
    }
    /**
     ...
     */
    verifyClassificationSafetyFloor(classifications) {
    ...
    createFailClosedDecision(params) {
        ...
        const classification = isForbidden ? 'FORBIDDEN' : 'HIGH_IMPACT';
        return {
