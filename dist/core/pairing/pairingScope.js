 ...
 */
export function validatePairingScopeSegment(segment, segmentName) {
 ...
 */
export function createDeviceScope(identity) {
 ...
 */
export function parseDeviceScope(scopeString) {
    ...
    const [userId, sessionId, brainId, surfaceId, transportId, gatewayId, adapterId, connectionId, deviceId,] = parts;
 ...
 */
export function areDeviceScopesEqual(a, b) {
 ...
 */
export function assertDeviceScopeMatches(expected, actual) {
    if (!areDeviceScopesEqual(expected, actual)) {
        throw new Error(`[PAIRING_SCOPE_MISMATCH] Scope mismatch: expected (${createDeviceScope(expected)}) vs actual (${createDeviceScope(actual)})`);
    }
}
 ...
 */
export function scrubPairingSecrets(data) {
    ...
    if (Array.isArray(data)) {
        return data.map((item) => scrubPairingSecrets(item));
    }
    const result = {};
