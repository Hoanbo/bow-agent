 ...
 */
export function canConfirmPairing(record) {
    return (record.pairingState === 'PAIRING_PENDING' &&
        !record.revoked &&
 ...
 */
export function confirmPairing(record, confirmedBy, timestamp) {
    ...
    if (record.pairingState !== 'PAIRING_PENDING') {
        throw new Error(`[PAIRING_NOT_CONFIRMABLE] Record in state '${record.pairingState}' cannot be confirmed (expected 'PAIRING_PENDING')`);
    }
    const now = timestamp ?? Date.now();
    // Transition: PAIRING_PENDING -> PAIRING_CONFIRMED
    const confirmed = updatePairingRecordState(record, 'PAIRING_CONFIRMED', 'NONE', {
        confirmed: true,
        confirmedBy,
        confirmedAt: now,
    }, now);
    // Transition: PAIRING_CONFIRMED -> PAIRED
    const paired = updatePairingRecordState(confirmed, 'PAIRED', 'PAIRED', undefined, now);
    // Transition: PAIRED -> TRUSTED
    const trusted = updatePairingRecordState(paired, 'TRUSTED', 'TRUSTED', undefined, now);
    return trusted;
}
