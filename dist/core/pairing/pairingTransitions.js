 ...
 */
export const PAIRING_TRANSITION_MATRIX = Object.freeze({
 ...
 */
export const TRUST_TRANSITION_MATRIX = Object.freeze({
    NONE: ['PAIRED', 'REVOKED'],
 ...
 */
export function isValidPairingTransition(from, to) {
    if (from === to) {
 ...
 */
export function assertValidPairingTransition(from, to) {
    if (!isValidPairingTransition(from, to)) {
        throw new Error(`[PAIRING_INVALID_TRANSITION] Illegal pairing state transition from '${from}' to '${to}'`);
    }
}
/**
 * Validates whether a trust level transition is permitted.
 */
export function isValidTrustTransition(from, to) {
    if (from === to) {
        return true; // Idempotent no-op
    }
 ...
 */
export function assertValidTrustTransition(from, to) {
    if (!isValidTrustTransition(from, to)) {
        throw new Error(`[PAIRING_INVALID_TRANSITION] Illegal trust level transition from '${from}' to '${to}'`);
    }
}
