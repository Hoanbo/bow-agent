 ...
 */
export function createPairingRequest(params) {
 ...
 */
export function validatePairingRequest(req) {
    ...
    if (req.protocolVersion !== PAIRING_PROTOCOL_VERSION) {
        throw new Error(`[PAIRING_PROTOCOL_MISMATCH] Incompatible protocol version: expected ${PAIRING_PROTOCOL_VERSION}, got ${req.protocolVersion}`);
    }
    ...
    if (req.scope.deviceId !== req.deviceId) {
        throw new Error(`[PAIRING_DEVICE_MISMATCH] Scope deviceId (${req.scope.deviceId}) does not match request deviceId (${req.deviceId})`);
    }
