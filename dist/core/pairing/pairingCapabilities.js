...
import { SAFE_DEVICE_CAPABILITIES, PAIRING_FORBIDDEN_CAPABILITIES, } from './pairingTypes.js';
 ...
 */
export function hasForbiddenCapability(capabilities) {
    return capabilities.some((cap) => PAIRING_FORBIDDEN_CAPABILITIES.includes(cap) ||
 ...
 */
export function validatePairingCapabilities(capabilities) {
    ...
    for (const cap of capabilities) {
        ...
        if (hasForbiddenCapability([cap])) {
            throw new Error(`[PAIRING_CAPABILITY_REJECTED] Forbidden cognitive/execution capability requested: ${cap}`);
        }
        if (!SAFE_DEVICE_CAPABILITIES.includes(cap)) {
            throw new Error(`[PAIRING_CAPABILITY_REJECTED] Unknown or unpermitted device capability: ${cap}`);
        }
    }
 ...
 */
export function filterSafeCapabilities(capabilities) {
    if (!Array.isArray(capabilities)) {
        return Object.freeze([]);
    }
    const filtered = capabilities.filter((cap) => typeof cap === 'string' &&
        !hasForbiddenCapability([cap]) &&
        SAFE_DEVICE_CAPABILITIES.includes(cap));
 ...
 */
export function computeCanonicalCapabilityFingerprint(capabilities) {
