...
export class WorldActionAuthorizationEngine {
     ...
     */
    validateToken(token, action, context) {
        ...
        const normCap = (c) => (c ? (c.toLowerCase().replace(/\./g, '_').startsWith('cap_') ? c.toLowerCase().replace(/\./g, '_') : `cap_${c.toLowerCase().replace(/\./g, '_')}`) : '');
        const capMatch = stored.toolId === action.actionType ||
        ...
        const actionGoalId = action.metadata?.goalId;
        ...
        const actionTaskId = action.metadata?.taskId;
     ...
     */
    consumeToken(tokenId, actionId) {
        const stored = this.tokens.get(tokenId);
        if (!stored) {
            throw new WorldActionError('AUTHORIZATION_FAILURE', `Token "${tokenId}" not found.`, actionId);
        }
        if (stored.consumedAt) {
            throw new WorldActionError('AUTHORIZATION_FAILURE', `Token "${tokenId}" is already consumed and cannot be reused.`, actionId);
        }
