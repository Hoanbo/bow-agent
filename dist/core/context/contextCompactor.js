...
import { DEFAULT_CONTEXT_CONFIG, } from './conversationContext.js';
export class ContextCompactor {
    /**
     * Evaluates whether a set of turns requires compaction against configured thresholds.
     */
    needsCompaction(turns, config = DEFAULT_CONTEXT_CONFIG) {
     ...
     */
    compactTurns(turns, items = [], config = DEFAULT_CONTEXT_CONFIG) {
        ...
        if (turnsToSummarize.length > 0) {
            const topicsCovered = Array.from(new Set(turnsToSummarize.map(t => t.content.slice(0, 30).trim()))).slice(0, 3);
