...
import { globalTopicTracker } from './topicTracker.js';
import { globalReferenceResolver } from './referenceResolver.js';
import { globalContextRanker } from './contextRanker.js';
import { globalContextCompactor } from './contextCompactor.js';
export class ContextManager {
    ...
    config;
    topicStates = new Map();
    constructor(customStore, customConfig) {
     ...
     */
    async ingestUserTurn(userId, sessionId, userText) {
        ...
        if (this.compactor.needsCompaction(allTurns, this.config)) {
            ...
            this.store.replaceCompactedContext(userId, sessionId, compactionResult.compactedTurns, compactionResult.compactionState);
        }
     ...
     */
    async commitAgentResponse(userId, sessionId, responseText, metadata) {
     ...
     */
    getContextSnapshot(userId, sessionId) {
        try {
            const partitionKey = this.store.buildPartitionKey(userId, sessionId);
            const topicState = this.topicStates.get(partitionKey) || {
                activeTopic: 'general',
            ...
            const totalChars = recentTurns.reduce((acc, t) => acc + t.content.length, 0) +
