...
import { GovernedCrossFederationPolicyError, GovernedCrossFederationLeaseError, computePolicyMetaEvaluationHash, } from './GovernedCrossFederationTypes.js';
export class PolicyMetaGovernanceEngine {
    ...
    evaluateStrategy(tenantId, sessionId, proposal, activeLeaseIds) {
        // 1. Lease Invariant Validation: Proposal's lease must be active
        if (!proposal.leaseId || !activeLeaseIds.has(proposal.leaseId)) {
            throw new GovernedCrossFederationLeaseError(`Lease invariant violation: Federation ${proposal.federationId} proposal ${proposal.proposalId} uses expired or invalid lease ${proposal.leaseId}`, tenantId, sessionId);
        }
        ...
        if (lowerGoal.includes('takeover') ||
            lowerGoal.includes('escalate_privilege') ||
            lowerGoal.includes('command_all_federations') ||
            lowerGoal.includes('override_policy')) {
            throw new GovernedCrossFederationPolicyError(`Privilege escalation attempt detected in proposal ${proposal.proposalId}`, tenantId, sessionId);
        }
