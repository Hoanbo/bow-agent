...
import { MAX_CONVERGENCE_ROUNDS, MAX_CONVERGENCE_REASSESSMENTS, MAX_CONSECUTIVE_CONVERGENCE_FAILURES, MAX_CONVERGENCE_DURATION_MS, MAX_ACTIVE_CONVERGENCE_SESSIONS, GovernedCrossFederationLifecycleError, GovernedCrossFederationBudgetError, GovernedCrossFederationConcurrencyError, computeConvergenceStateSnapshotHash, computeSha256, } from './GovernedCrossFederationTypes.js';
...
export class GovernedConvergenceEngine {
    states = new Map();
    activeSessionsByTenant = new Map();
    registry;
    strategyEngine;
    ...
    securityBoundary;
    persistenceBridge;
    constructor(registry, strategyEngine, reconciliationEngine, conflictResolver, policyMetaEngine, securityBoundary, persistenceBridge) {
    ...
    validateTransition(current, target) {
        ...
        if (!allowed.includes(target)) {
            throw new GovernedCrossFederationLifecycleError(`Illegal lifecycle transition: Cannot transition from ${current} to ${target}`);
        }
    }
    ...
    createSession(params) {
        ...
        if (tenantSessions.size >= MAX_ACTIVE_CONVERGENCE_SESSIONS && !tenantSessions.has(params.sessionId)) {
            throw new GovernedCrossFederationBudgetError(`Budget violation: Maximum active convergence sessions exceeded (${MAX_ACTIVE_CONVERGENCE_SESSIONS}) for tenant ${params.tenantId}`, params.tenantId, params.sessionId);
        }
        const now = Date.now();
        const cleanState = {
    ...
    transitionState(sessionId, targetStatus, expectedVersion) {
        ...
        if (current.version !== expectedVersion) {
            throw new GovernedCrossFederationConcurrencyError(`OCC Version conflict: Expected ${expectedVersion} != current ${current.version}`, current.tenantId, current.sessionId);
        }
        // Check expiration
    ...
    executeConvergenceRound(sessionId, expectedVersion, dependencies = []) {
        ...
        const recResult = this.reconciliationEngine.reconcileProposals(state.tenantId, state.sessionId, state.missionId, state.objectiveId, allProposals);
    ...
    reassessConvergence(sessionId, expectedVersion) {
        ...
        if (state.reassessmentsConsumed >= MAX_CONVERGENCE_REASSESSMENTS) {
            throw new GovernedCrossFederationBudgetError(`Budget violation: Maximum convergence reassessments exceeded (${MAX_CONVERGENCE_REASSESSMENTS})`, state.tenantId, state.sessionId);
        }
        if (state.version !== expectedVersion) {
            throw new GovernedCrossFederationConcurrencyError(`OCC Version conflict: Expected ${expectedVersion} != current ${state.version}`, state.tenantId, state.sessionId);
        }
        const cleanReassessed = {
            ...state,
        ...
        const reassessed = Object.freeze({
            ...cleanReassessed,
            provenanceHash,
        });
