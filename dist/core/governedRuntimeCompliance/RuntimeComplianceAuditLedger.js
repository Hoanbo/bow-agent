...
import { RuntimeComplianceAuditLedgerError, TenantAccessForbiddenError, GENESIS_PREV_HASH, computeAuditEventHash, computeSha256, } from './GovernedRuntimeComplianceTypes.js';
...
export class RuntimeComplianceAuditLedger {
    baseStorageDir;
    inMemoryLedger = [];
    constructor(baseStorageDir = path.join(process.cwd(), 'data', 'partitions_governed_runtime_compliance')) {
    ...
    async appendEvent(tenantId, policyDomain, eventType, payload, metadata = {}) {
        this.assertValidTenantId(tenantId);
            ...
            if (content.length > 0) {
                ...
                if (lastLine) {
                    try {
                        ...
                        throw new RuntimeComplianceAuditLedgerError(`Audit ledger file corrupted: unable to parse last entry for tenant '${tenantId}'.`);
                    }
                }
            }
        }
        ...
        const auditRecordId = `aud_rec_${computeSha256(rawId).slice(0, 16)}`;
    ...
    async verifyLedgerChain(tenantId) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (record.prevHash !== expectedPrevHash) {
                throw new RuntimeComplianceAuditLedgerError(`Audit chain verification failed at sequence ${record.sequenceNumber}: expected prevHash '${expectedPrevHash}', found '${record.prevHash}'.`);
            }
