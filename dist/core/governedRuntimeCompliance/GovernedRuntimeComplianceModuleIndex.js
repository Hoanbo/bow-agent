...
import { RuntimeBehaviorObservationCollector, } from './RuntimeBehaviorObservationCollector.js';
import { ActivePolicySnapshotBindingResolver, } from './ActivePolicySnapshotBindingResolver.js';
...
import { GovernedAdaptiveSafetyController, } from './GovernedAdaptiveSafetyController.js';
...
export class GovernedRuntimeComplianceModuleIndex {
    ...
    constructor(config = {}) {
        ...
        this.bindingResolver = new ActivePolicySnapshotBindingResolver(config.snapshotProvider ?? {
        ...
        this.safetyController = new GovernedAdaptiveSafetyController(config.lifecycleBridge, config.incidentBridge, config.stopProvider);
    ...
    async processRuntimeTelemetry(rawTelemetry, policy) {
        // 1. Ingest & Sanitize Observation
        const profile = this.collector.ingestObservation(rawTelemetry);
        // 2. Resolve Active Policy Binding
        const binding = await this.bindingResolver.resolveActiveBinding(profile.tenantId, profile.policyDomain, policy.policyVersion, policy.metadata.canonicalHash);
        ...
        const assurance = this.scorer.computeAssuranceScore(profile.tenantId, profile.policyDomain, allEvals, allViolations);
        // 6. Adaptive Safety Decision
        const safetyDecision = await this.safetyController.evaluateSafetyIntervention(assurance, allViolations);
        // 7. Compile Immutable Evidence Dossier
        const dossier = this.dossierEngine.compileDossier(profile.tenantId, assurance, allViolations, binding.policyVersion, binding.canonicalPolicyHash);
        // 8. Record Audit Events
        await this.auditLedger.appendEvent(profile.tenantId, profile.policyDomain, evaluation.verdict === 'COMPLIANT' ? 'COMPLIANCE_EVALUATED_COMPLIANT' : 'COMPLIANCE_EVALUATED_NON_COMPLIANT', {
