...
import { AutomatedReactivationForbiddenError, EmergencyStopActiveError, computeSha256, } from './GovernedRuntimeComplianceTypes.js';
export class GovernedAdaptiveSafetyController {
    lifecycleBridge;
    incidentBridge;
    stopProvider;
    constructor(lifecycleBridge, incidentBridge, stopProvider) {
    ...
    assertEmergencyStopInactive() {
        ...
        if (active) {
            throw new EmergencyStopActiveError('Adaptive safety control halted: EMERGENCY_STOP is currently active.');
        }
    }
    ...
    async evaluateSafetyIntervention(assurance, violations) {
        ...
        if (hasCritical || isBreached) {
            ...
            if (this.incidentBridge) {
                await this.incidentBridge.openIncident(assurance.tenantId, assurance.policyDomain, 'INC_INTEGRITY_DRIFT', 'SEV_1_CRITICAL', reason, {
            ...
            if (this.lifecycleBridge) {
                await this.lifecycleBridge.transitionState(assurance.tenantId, assurance.policyDomain, 'SUSPENDED', 'AUTOMATIC_SAFETY_INTERLOCK', { reason, scoreValue: assurance.scoreValue });
            }
        }
        else if (isDegraded) {
            ...
            // Open Incident in MS-1.5.21
            if (this.incidentBridge) {
                await this.incidentBridge.openIncident(assurance.tenantId, assurance.policyDomain, 'INC_HEALTH_DEGRADED', 'SEV_2_HIGH', reason, { scoreValue: assurance.scoreValue });
            }
            // Delegate Downward Transition to MS-1.5.21
            if (this.lifecycleBridge) {
                await this.lifecycleBridge.transitionState(assurance.tenantId, assurance.policyDomain, 'DEGRADED', 'HEALTH_DRIFT_THRESHOLD', { reason, scoreValue: assurance.scoreValue });
            }
        }
    ...
    assertNoAutomatedReactivation(targetState, triggerType) {
        if (targetState === 'ACTIVE') {
            ...
            if (FORBIDDEN_AUTO_TRIGGERS.has(triggerType.toUpperCase())) {
                throw new AutomatedReactivationForbiddenError(`AUTOMATION != REACTIVATION: Automated trigger '${triggerType}' cannot reactivate policy to ACTIVE. Sole Human Authority required.`);
            }
        }
    }
}
