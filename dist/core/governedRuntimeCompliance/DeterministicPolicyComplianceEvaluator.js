...
export class DeterministicPolicyComplianceEvaluator {
    ...
    evaluateCompliance(profile, policy, binding, _context) {
        const matchedRules = [];
        const violatedRules = [];
        ...
        if (profile.tenantId !== policy.tenantId ||
        ...
        else if (profile.actionClassification === 'HIGH_IMPACT' &&
                    ...
                    if (rule.parameters && typeof rule.parameters === 'object') {
                        for (const [pKey, pConstraint] of Object.entries(rule.parameters)) {
                            ...
                            if (pConstraint && typeof pConstraint === 'object') {
                                const constraintObj = pConstraint;
                                if (typeof constraintObj['max'] === 'number' &&
        ...
        const evaluationId = `eval_${computeSha256(rawEvalId).slice(0, 16)}`;
