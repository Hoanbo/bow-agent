...
import { computeSha256, } from './GovernedRuntimeComplianceTypes.js';
export class PolicyViolationDriftClassifier {
    violationHistory = [];
    actionViolationCounts = new Map();
    ...
    classifyViolation(profile, evaluation) {
        if (evaluation.verdict === 'COMPLIANT') {
            return null;
        }
        let category = 'POLICY_RULE_VIOLATION';
        let severity = 'HIGH';
        ...
        else if (evaluation.violatedRules.includes('HIGH_IMPACT_APPROVAL_REQUIRED') ||
    ...
    computeCumulativeDrift(tenantId, policyDomain, evaluations) {
        const tenantEvaluations = evaluations.filter((e) => e.tenantId === tenantId && e.policyDomain === policyDomain);
