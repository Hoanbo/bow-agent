...
import { PolicySnapshotUnavailableError, PolicyVersionBindingMismatchError, EmergencyStopActiveError, TenantAccessForbiddenError, } from './GovernedRuntimeComplianceTypes.js';
export class ActivePolicySnapshotBindingResolver {
    storeProvider;
    stopProvider;
    constructor(storeProvider, stopProvider) {
    ...
    assertEmergencyStopInactive() {
        ...
        if (active) {
            throw new EmergencyStopActiveError('Policy binding resolution halted: EMERGENCY_STOP is currently active.');
        }
    }
    ...
    async resolveActiveBinding(tenantId, policyDomain, expectedVersion, expectedPolicyHash) {
        ...
        if (!snapshot) {
            throw new PolicySnapshotUnavailableError(`No active canonical policy found for tenant '${tenantId}' in domain '${policyDomain}'.`);
        }
        ...
        if (expectedVersion !== undefined && snapshot.version !== expectedVersion) {
            throw new PolicyVersionBindingMismatchError(`Policy version mismatch: expected version ${expectedVersion}, but store resolved version ${snapshot.version}.`);
        }
        ...
        if (expectedPolicyHash !== undefined && snapshot.canonicalPolicyHash !== expectedPolicyHash) {
            throw new PolicyVersionBindingMismatchError(`Canonical policy hash mismatch: expected '${expectedPolicyHash}', but store resolved '${snapshot.canonicalPolicyHash}'.`);
        }
