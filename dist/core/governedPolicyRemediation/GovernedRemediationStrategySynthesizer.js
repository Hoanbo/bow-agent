...
import { asRemediationId, computeCandidateRemediationHash, EmergencyStopActiveError, CrossTenantAccessForbiddenError, } from './GovernedPolicyRemediationTypes.js';
export class GovernedRemediationStrategySynthesizer {
    ...
    synthesizeRemediationCandidate(tenantId, policyDomain, context) {
        ...
        if (context.diagnosis.tenantId !== tenantId) {
            throw new CrossTenantAccessForbiddenError(`Cross-tenant diagnosis record mismatch: expected '${tenantId}', got '${context.diagnosis.tenantId}'`);
        }
        if (context.blastRadius.tenantId !== tenantId) {
            throw new CrossTenantAccessForbiddenError(`Cross-tenant blast radius record mismatch: expected '${tenantId}', got '${context.blastRadius.tenantId}'`);
        }
        let proposedAction = 'AMEND_POLICY_RULE';
        let candidateDelta = {};
        let targetRuleId = context.diagnosis.diagnosedRuleId;
        let targetParameterKey = context.diagnosis.diagnosedParameter;
        let clampedLimitValue;
        let recommendedPredecessorVersion;
        let justification = '';
