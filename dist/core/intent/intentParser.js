...
export function parseIntent(input) {
    ...
    if (validation.length)
        return { intentType: 'UNKNOWN', confidence: 0, confidenceLevel: 'LOW', entities: [], parameters: {}, references: [], missingParameters: [], ambiguities: [], clarification: validation, requiresClarification: true, actionability: 'INFORMATIONAL', sourceText, normalizedText, candidateAction: undefined };
    ...
    for (const reference of references) {
        if (reference.resolved && reference.targetText) {
            for (const entity of extractEntities(reference.targetText)) {
                if (!entities.some(existing => existing.type === entity.type && existing.normalizedValue === entity.normalizedValue))
                    entities.push(entity);
            }
        }
    }
    const missingParameters = [];
    const clarifications = references.filter(reference => !reference.resolved).map(reference => ({ reason: 'UNRESOLVED_REFERENCE', reference: reference.phrase }));
    ...
    if (required && !entities.some(entity => entity.type === required)) {
        ...
        clarifications.push({ reason: 'MISSING_PARAMETER', parameter: required });
    }
    if (classified.intentType === 'UNKNOWN')
        clarifications.push({ reason: 'UNSUPPORTED_INTENT' });
    const confidence = Math.max(0, Math.min(1, clarifications.length ? Math.min(classified.confidence, 0.4) : classified.confidence));
    const candidateAction = classified.intentType === 'UNKNOWN' ? undefined : {
