...
export function parseWireEndpoint(uri) {
    ...
    try {
        ...
        if (err instanceof WireTransportError)
            throw err;
        throw new WireTransportError('WIRE_CONNECTION_FAILED', `Malformed wire endpoint URI: ${err.message}`);
    }
}
export function formatWireEndpoint(endpoint) {
    const isDefaultPort = (endpoint.protocol === 'wss' && endpoint.port === 443) ||
...
export function validateWireEndpoint(endpoint) {
...
export function assertWireEndpointKnowledgeDoesNotGrantAccess(_endpoint) {
