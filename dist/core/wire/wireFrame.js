...
export function computeWireFrameChecksum(payload) {
...
export function createWireFrame(options) {
    ...
    if (payloadLength > MAX_WIRE_FRAME_SIZE) {
        throw new WireTransportError('WIRE_FRAME_SIZE_EXCEEDED', `WireFrame payload size ${payloadLength} exceeds maximum limit of ${MAX_WIRE_FRAME_SIZE} bytes.`);
    }
...
export function serializeWireFrame(frame) {
    validateWireFrame(frame);
    return JSON.stringify(frame);
}
export function deserializeWireFrame(raw) {
    ...
    try {
        ...
        throw new WireTransportError('WIRE_MALFORMED_FRAME', `Malformed JSON in wire frame: ${err.message}`);
    }
    if (!parsed || typeof parsed !== 'object') {
        throw new WireTransportError('WIRE_MALFORMED_FRAME', 'Deserialized wire frame is not an object.');
    }
    const candidate = parsed;
    if (typeof candidate.frameId !== 'string' ||
...
export function validateWireFrame(frame) {
    ...
    if (actualLength > MAX_WIRE_FRAME_SIZE) {
        throw new WireTransportError('WIRE_FRAME_SIZE_EXCEEDED', `Wire frame payload length ${actualLength} exceeds ${MAX_WIRE_FRAME_SIZE} bytes.`);
    }
    const calculatedChecksum = computeWireFrameChecksum(frame.payload);
    if (calculatedChecksum !== frame.checksum) {
        throw new WireTransportError('WIRE_CHECKSUM_MISMATCH', `Wire frame checksum mismatch! Expected ${frame.checksum}, got ${calculatedChecksum}. Fail-closed.`);
    }
}
