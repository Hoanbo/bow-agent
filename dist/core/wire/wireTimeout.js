...
export async function withWireTimeout(promise, timeoutMs, operationName) {
    let timer;
    const timeoutPromise = new Promise((_, reject) => {
        timer = setTimeout(() => {
            reject(new WireTransportError('WIRE_TIMEOUT', `Wire operation "${operationName}" timed out after ${timeoutMs}ms. Aborted fail-closed.`));
        }, timeoutMs);
    });
    try {
