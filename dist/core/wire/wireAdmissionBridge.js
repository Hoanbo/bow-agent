...
export class WireAdmissionBridge {
     ...
     */
    generateProofForChallenge(deviceId, challengeToken, keyId) {
        ...
        if (!key) {
            throw new WireTransportError('WIRE_AUTHENTICATION_FAILED', `No enrolled cryptographic key found for device "${deviceId}" in vault. Cannot generate proof.`);
        }
        ...
        const challengeObj = existingChallenge ?? Object.freeze({
     ...
     */
    async evaluateWireAdmission(params) {
        ...
        if (decision.decision !== 'ADMIT' || !decision.admitted) {
            throw new WireTransportError('WIRE_ADMISSION_FAILED', `Admission denied by ZeroTrustAdmissionRuntime: decision=${decision.decision}, reason=${decision.rejectionReason ?? 'DENIED'}`);
        }
        return decision;
    }
}
