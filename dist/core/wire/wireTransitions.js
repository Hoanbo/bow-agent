 ...
 */
export function isValidWireTransition(from, to) {
    if (from === to)
        return true;
 ...
 */
export function assertValidWireTransition(from, to, context) {
    if (!isValidWireTransition(from, to)) {
        throw new WireTransportError('WIRE_ILLEGAL_STATE_TRANSITION', `Illegal wire state transition from "${from}" to "${to}"${context ? ` [${context}]` : ''}. Transition blocked fail-closed.`);
    }
}
