...
export class GovernedReleaseExecutionEngine {
    ...
    executeRelease(input) {
        ...
        if (input.approval.decision !== 'APPROVED') {
            throw new ReleaseExecutionError('UNAPPROVED_EXECUTION', `Cannot execute release without human approval (decision: "${input.approval.decision}").`);
        }
        ...
        for (const item of filesToApply) {
            const { absolutePath: destAbs } = SandboxPathGuard.resolveAndAssertContainedPath(targetRootAbs, item.relativePath);
        ...
        try {
            ...
            throw new ReleaseExecutionError('MUTATION_FAILED', `Release mutation halted due to filesystem error: ${err?.message ?? String(err)}`);
        }
        return {
