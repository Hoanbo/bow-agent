...
export class ReleaseExecutionPolicyEngine {
     ...
     */
    static PROTECTED_WORKSPACE_NORMALIZED = path.normalize('C:/BOW/shopofbow').toLowerCase();
    /**
    ...
    static assertNotProtectedWorkspace(targetPath) {
        ...
        if (normalized === ReleaseExecutionPolicyEngine.PROTECTED_WORKSPACE_NORMALIZED ||
            normalized.startsWith(ReleaseExecutionPolicyEngine.PROTECTED_WORKSPACE_NORMALIZED + path.sep)) {
            throw new ReleaseExecutionError('PROTECTED_WORKSPACE_VIOLATION', `Release target cannot be inside protected workspace "${targetPath}". Isolation invariant strictly enforced.`);
        }
    }
    /**
     ...
     */
    static assertSafePath(targetPath) {
        ...
        if (targetPath.includes('\0') || targetPath.startsWith('\\\\') || targetPath.startsWith('//')) {
            throw new ReleaseExecutionError('PATH_TRAVERSAL_DETECTED', `Unsafe path components detected in "${targetPath}".`);
        }
        ...
        if (targetPath.includes('..')) {
            ...
            if (parts.includes('..')) {
                throw new ReleaseExecutionError('PATH_TRAVERSAL_DETECTED', `Path traversal ".." detected in target "${targetPath}".`);
            }
        }
    }
    /**
     ...
     */
    static assertNotUserStopped(isUserStopActive) {
        if (isUserStopActive) {
            throw new ReleaseExecutionError('USER_STOP_ACTIVE', 'Release execution halted: USER_STOP is currently active. User supremacy enforced.');
        }
    }
    /**
    ...
    static assertNotRevoked(isRevoked) {
        if (isRevoked) {
            throw new ReleaseExecutionError('REVOCATION_ACTIVE', 'Release execution halted: Authority lease or delegation has been revoked.');
        }
    }
    /**
    ...
    static validateCandidate(candidate) {
        ...
        if (now > candidate.expiresAt) {
            throw new ReleaseExecutionError('INVALID_CANDIDATE', `Release candidate "${candidate.candidateId}" has expired at ${new Date(candidate.expiresAt).toISOString()}.`);
        }
        if (candidate.state === 'FAILED' ||
            ...
            throw new ReleaseExecutionError('INVALID_CANDIDATE', `Release candidate "${candidate.candidateId}" is in terminal failure state "${candidate.state}".`);
        }
    }
    /**
     ...
     */
    static validateVerificationRecord(record, candidate) {
        if (record.candidateId !== candidate.candidateId) {
            throw new ReleaseExecutionError('CANDIDATE_NOT_VERIFIED', `Verification record candidateId "${record.candidateId}" does not match candidate "${candidate.candidateId}".`);
        }
        if (record.verificationState !== 'PASS') {
            throw new ReleaseExecutionError('CANDIDATE_NOT_VERIFIED', `Release candidate "${candidate.candidateId}" has not achieved PASS verification (current: "${record.verificationState}").`);
        }
        if (record.contradictions.length > 0) {
            throw new ReleaseExecutionError('CONTRADICTION_DETECTED', `Release verification record contains unresolved multi-agent contradictions.`);
        }
    }
    /**
     ...
     */
    static validateExecutionRequest(request, candidate, verification, options) {
        ...
        if (request.candidateId !== candidate.candidateId) {
            throw new ReleaseExecutionError('INVALID_CANDIDATE', `Request candidateId "${request.candidateId}" does not match provided candidate "${candidate.candidateId}".`);
        }
        if (request.verificationId !== verification.verificationId) {
            throw new ReleaseExecutionError('STALE_VERIFICATION_PACKET', `Request verificationId "${request.verificationId}" does not match provided verification "${verification.verificationId}".`);
        }
        ...
        if (!request.operatorId || !request.sessionId || !request.taskId || !request.delegationId || !request.capabilityLeaseId) {
            throw new ReleaseExecutionError('MISSING_REQUIRED_BINDING', 'Release execution request is missing mandatory governance identity bindings.');
        }
        // Cross-identity check with candidate.
        // Kiểm tra chéo định danh với ứng viên.
        if (request.sessionId !== candidate.sessionId) {
            throw new ReleaseExecutionError('INVALID_CANDIDATE', `Session mismatch: request sessionId "${request.sessionId}" != candidate sessionId "${candidate.sessionId}".`);
        }
        if (request.taskId !== candidate.taskId) {
            throw new ReleaseExecutionError('INVALID_CANDIDATE', `Task mismatch: request taskId "${request.taskId}" != candidate taskId "${candidate.taskId}".`);
        }
