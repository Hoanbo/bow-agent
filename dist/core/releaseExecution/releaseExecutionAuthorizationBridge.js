...
export class ReleaseExecutionAuthorizationBridge {
    ...
    issueReleaseToken(input) {
        ...
        if (input.approval.decision !== 'APPROVED') {
            throw new ReleaseExecutionError('UNAPPROVED_EXECUTION', `Cannot issue execution token for unapproved release "${input.request.executionId}".`);
        }
        if (input.approval.executionId !== input.request.executionId) {
            throw new ReleaseExecutionError('UNAPPROVED_EXECUTION', `Approval executionId "${input.approval.executionId}" does not match request "${input.request.executionId}".`);
        }
     ...
     */
    validateAndConsumeToken(token, request, candidate) {
        // 1. Anti-replay check.
        // 1. Kiểm tra chống phát lại.
        if (this.consumedTokens.has(token.tokenId)) {
            throw new ReleaseExecutionError('TOKEN_REPLAY_REJECTED', `Execution token "${token.tokenId}" has already been consumed. Replay strictly forbidden.`);
        }
        const params = {
        ...
        if (!validation.valid) {
            ...
            throw new ReleaseExecutionError('INVALID_AUTHORIZATION_TOKEN', `Authorization token invalid for release execution: ${validation.reason}`);
        }
        // 3. Mark as consumed.
