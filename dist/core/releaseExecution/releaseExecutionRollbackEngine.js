    ...
    static rollback(input) {
        ...
        try {
            for (const backup of input.backups) {
                const { absolutePath: fileAbs } = SandboxPathGuard.resolveAndAssertContainedPath(targetRootAbs, backup.relativePath);
            ...
            throw new ReleaseExecutionError('ROLLBACK_FAILED', `Rollback failed to complete cleanly: ${err?.message ?? String(err)}`);
        }
