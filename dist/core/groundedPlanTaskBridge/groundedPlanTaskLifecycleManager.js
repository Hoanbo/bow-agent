...
import { GroundedPlanTaskUserStopError, GroundedPlanTaskLifecycleError, GroundedPlanTaskConcurrencyError, computeBindingProvenanceHash, } from './groundedPlanTaskTypes.js';
...
export class GroundedPlanTaskLifecycleManager {
    ...
    isValidTransition(current, next) {
        if (next === 'PREEMPTED')
            return true; // Global USER_STOP can preempt any non-terminal state
     ...
     */
    transitionState(binding, nextState, expectedVersion, reason = 'Governed state transition') {
        ...
        if (!this.isValidTransition(binding.lifecycleState, nextState)) {
            throw new GroundedPlanTaskLifecycleError(`Illegal transition from "${binding.lifecycleState}" to "${nextState}" for binding "${binding.bindingId}"`, { currentState: binding.lifecycleState, nextState });
        }
        ...
        const updatedBinding = Object.freeze({
            ...partialBinding,
            provenanceHash,
        });
