...
import { GROUNDED_PLAN_TASK_SCHEMA_VERSION, GroundedPlanTaskUserStopError, GroundedPlanTaskConcurrencyError, GroundedPlanTaskProvenanceError, GroundedPlanTaskRecoveryError, computeTaskBindingSessionHash, } from './groundedPlanTaskTypes.js';
export class GroundedPlanTaskPersistenceRecoveryEngine {
    baseDirectory;
    userStopProvider;
    constructor(options) {
        this.baseDirectory = path.resolve(options?.baseDirectory ?? path.join(process.cwd(), 'data', 'partitions_grounded_plan_task'));
     ...
     */
    saveBinding(binding, expectedVersion) {
    ...
    loadSessionDocument(tenantId, sessionId) {
        ...
        try {
            ...
            throw new GroundedPlanTaskRecoveryError(`Canonical corrupted and no backup found: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    /**
     ...
     */
    atomicWriteDocument(sessionDir, doc) {
        if (this.userStopProvider()) {
    ...
    recoverFromBackup(backupPath) {
        try {
            ...
            throw new GroundedPlanTaskRecoveryError(`Backup recovery failed: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    /**
    ...
    verifyDocumentIntegrity(doc) {
        ...
        if (doc.documentHash !== expectedHash) {
            throw new GroundedPlanTaskProvenanceError(`Provenance hash mismatch on session document: expected "${expectedHash}", got "${doc.documentHash}"`);
        }
    }
}
