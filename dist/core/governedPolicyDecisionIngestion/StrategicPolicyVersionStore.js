...
import { PolicyTenantIsolationError, PolicyVersionOCCConflictError, computeCanonicalPolicyHash, } from './GovernedPolicyDecisionIngestionTypes.js';
export class StrategicPolicyVersionStore {
    baseDir;
    inMemoryCache = new Map(); // tenant:domain -> policy
    #tokenVerifier;
    #isEmergencyStopActive;
    #isUserStopActive;
    constructor(customBaseDir, security) {
    ...
    savePolicyVersion(policy) {
        ...
        if (fs.existsSync(versionPath)) {
            throw new PolicyVersionOCCConflictError(`DUPLICATE_VERSION_WRITE: Version ${policy.policyVersion} already exists in tenant '${policy.tenantId}' domain '${policy.policyDomain}'.`);
        }
     ...
     */
    activatePolicy(policy, activationAuthorization) {
        ...
        if (token.policyDomain !== policy.policyDomain ||
        ...
        try {
            const ratRecord = JSON.parse(fs.readFileSync(ratificationPath, 'utf8'));
            if (ratRecord.ratificationId !== policy.metadata.ratificationId ||
            ...
            if (e.message && e.message.startsWith('UNAUTHORIZED_POLICY_ACTIVATION:'))
                throw e;
        ...
        if (currentActive && policy.policyVersion <= currentActive.policyVersion) {
            throw new PolicyVersionOCCConflictError(`VERSION_REGRESSION_BLOCKED: Cannot activate version ${policy.policyVersion} when current active is ${currentActive.policyVersion}.`);
        }
        // Invoke runtime-private atomic activation
        this.#activateVerifiedPolicy(policy);
    }
    #activateVerifiedPolicy(policy) {
        const domainDir = this.ensureDomainDirectory(policy.tenantId, policy.policyDomain);
     ...
     */
    getActivePolicy(tenantId, policyDomain) {
     ...
     */
    restoreFromBackup(tenantId, policyDomain, rollbackAuthorization) {
        this.assertValidTenantAndDomain(tenantId, policyDomain);
        if (!rollbackAuthorization || !this.#tokenVerifier)
            throw new Error('UNAUTHORIZED_ROLLBACK: cryptographic Human Authority evidence is required.');
        if (!this.#isEmergencyStopActive)
            throw new Error('EMERGENCY_STOP_PROVIDER_UNAVAILABLE: rollback fails closed.');
        let emergencyStopActive;
        try {
            emergencyStopActive = this.#isEmergencyStopActive(policyDomain);
        }
        catch {
            throw new Error('EMERGENCY_STOP_PROVIDER_UNAVAILABLE: rollback fails closed.');
        }
        if (typeof emergencyStopActive !== 'boolean')
            throw new Error('EMERGENCY_STOP_PROVIDER_INVALID: rollback fails closed.');
        if (emergencyStopActive)
            throw new Error(`EMERGENCY_STOP_ACTIVE: rollback blocked for domain '${policyDomain}'.`);
        const currentActive = this.getActivePolicy(tenantId, policyDomain);
        if (!currentActive)
            throw new Error(`NO_ACTIVE_POLICY_TO_ROLLBACK: No active policy found for tenant '${tenantId}' domain '${policyDomain}'.`);
    ...
    #restoreVerifiedBackup(tenantId, policyDomain) {
        const domainDir = path.join(this.baseDir, tenantId, policyDomain);
        const activePath = path.join(domainDir, 'active_policy.json');
        const backupPath = path.join(domainDir, 'active_policy.json.bak');
        ...
        if (!fs.existsSync(backupIntegrityPath))
            throw new Error('BACKUP_INTEGRITY_EVIDENCE_MISSING');
        ...
        if (!/^[a-f0-9]{64}$/i.test(committedHash) || committedHash !== actualHash)
            throw new Error('BACKUP_INTEGRITY_MISMATCH');
        let backupPolicy;
        try {
            backupPolicy = JSON.parse(backupRaw);
        }
        catch {
            throw new Error('BACKUP_MALFORMED');
        }
        if (computeCanonicalPolicyHash(backupPolicy) !== backupPolicy.metadata.canonicalHash)
            throw new Error('BACKUP_POLICY_COMMITMENT_MISMATCH');
    ...
    // --- Private Utilities ---
    ensureDomainDirectory(tenantId, policyDomain) {
    ...
    assertValidTenantAndDomain(tenantId, policyDomain) {
    ...
    atomicWriteJsonWithBackup(targetPath, backupPath, data) {
        const parentDir = path.dirname(targetPath);
        if (!fs.existsSync(parentDir)) {
            fs.mkdirSync(parentDir, { recursive: true });
        }
    ...
    recoverActiveFromBackup(tenantId, policyDomain) {
        const domainDir = path.join(this.baseDir, tenantId, policyDomain);
        const backupPath = path.join(domainDir, 'active_policy.json.bak');
        const backupIntegrityPath = `${backupPath}.sha256`;
        if (fs.existsSync(backupPath)) {
            try {
                ...
                if (createHash('sha256').update(raw, 'utf8').digest('hex') !== fs.readFileSync(backupIntegrityPath, 'utf8').trim())
                    return undefined;
                const policy = JSON.parse(raw);
                if (computeCanonicalPolicyHash(policy) !== policy.metadata.canonicalHash)
                    return undefined;
