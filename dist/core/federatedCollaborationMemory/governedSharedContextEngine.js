...
import { MAX_CONTEXT_SIZE, FederatedCollaborationMemoryValidationError, FederatedCollaborationMemoryConcurrencyError, FederatedCollaborationMemoryBudgetError, computeCollaborationContextHash, } from './federatedCollaborationMemoryTypes.js';
...
export class GovernedSharedContextEngine {
     ...
     */
    getSharedValue(contextId, key, tenantId, sessionId, agentId) {
        ...
        if (!context.participatingAgentIds.includes(agentId)) {
            throw new FederatedCollaborationMemoryValidationError(`Agent '${agentId}' is not an authorized participant in context '${contextId}'`);
        }
    ...
    updateSharedValue(params) {
        ...
        if (!context.participatingAgentIds.includes(params.agentId)) {
            throw new FederatedCollaborationMemoryValidationError(`Agent '${params.agentId}' is not authorized to update context '${params.contextId}'`);
        }
        // 4. Validate context status
        if (context.status !== 'ACTIVE') {
            throw new FederatedCollaborationMemoryValidationError(`Cannot update context in status '${context.status}'`);
        }
        // 5. OCC version check
        if (params.expectedVersion !== context.version) {
            throw new FederatedCollaborationMemoryConcurrencyError(`OCC version conflict: expected ${params.expectedVersion}, current version is ${context.version}`, params.tenantId, params.contextId);
        }
        ...
        if (!state.has(params.key) && state.size >= MAX_CONTEXT_SIZE) {
            throw new FederatedCollaborationMemoryBudgetError(`Context '${params.contextId}' exceeded MAX_CONTEXT_SIZE limit (${MAX_CONTEXT_SIZE})`, params.tenantId, params.contextId);
        }
        ...
        this.contextRegistry.contexts.set(params.contextId, updated);
        return updated;
    }
    /**
     ...
     */
    getAllSharedValues(contextId, tenantId, sessionId, agentId) {
        this.securityBoundary.assertStopInactive('PRE_AGENT_CONTEXT_ACCESS', tenantId, contextId);
        const context = this.contextRegistry.assertContextBoundaries(contextId, tenantId, sessionId);
        if (!context.participatingAgentIds.includes(agentId)) {
            throw new FederatedCollaborationMemoryValidationError(`Agent '${agentId}' is not authorized to read context '${contextId}'`);
        }
        const state = this.sharedStates.get(contextId);
        const result = {};
        if (state) {
