...
import { FederatedCollaborationMemoryValidationError, FederatedCollaborationMemoryTenantIsolationError, FederatedCollaborationMemorySessionIsolationError, computeObservationHash, computeObservationReconciliationHash, } from './federatedCollaborationMemoryTypes.js';
...
export class AgentObservationReconciliationEngine {
    ...
    submitObservation(params) {
        ...
        if (this.observations.has(params.observationId)) {
            throw new FederatedCollaborationMemoryValidationError(`Observation '${params.observationId}' already exists`);
        }
        ...
        if (params.observedValue.includes('<thought>') ||
     ...
     */
    reconcileObservations(reconciliationId, observationIds, tenantId, sessionId, federationId, target, minConfidenceThreshold = 0.5) {
        ...
        for (const id of observationIds) {
            ...
            if (obs.tenantId !== tenantId) {
                throw new FederatedCollaborationMemoryTenantIsolationError(`Observation tenant mismatch: '${obs.tenantId}' != '${tenantId}'`, tenantId);
            }
            if (obs.sessionId !== sessionId) {
                throw new FederatedCollaborationMemorySessionIsolationError(`Observation session mismatch: '${obs.sessionId}' != '${sessionId}'`, tenantId);
            }
            if (obs.federationId !== federationId) {
                throw new FederatedCollaborationMemoryValidationError(`Observation federation mismatch: '${obs.federationId}' != '${federationId}'`);
            }
            if (obs.target !== target) {
                throw new FederatedCollaborationMemoryValidationError(`Observation target mismatch: expected '${target}', got '${obs.target}'`);
            }
        ...
        const avgConfidence = matchedObservations.reduce((acc, o) => acc + o.confidence, 0) / matchedObservations.length;
        const recBase = {
            reconciliationId,
            tenantId,
            sessionId,
