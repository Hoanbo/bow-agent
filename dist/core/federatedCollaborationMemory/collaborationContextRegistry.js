...
import { FederatedCollaborationMemoryValidationError, FederatedCollaborationMemoryTenantIsolationError, FederatedCollaborationMemorySessionIsolationError, FederatedCollaborationMemoryAuthorizationError, computeCollaborationContextHash, } from './federatedCollaborationMemoryTypes.js';
export class CollaborationContextRegistry {
     ...
     */
    validateUntrustedMetadata(metadata) {
        if (!metadata)
            return;
        ...
        const inspectRecursive = (val, depth = 0) => {
            if (depth > 5 || val === null || val === undefined)
                return;
            if (typeof val === 'string') {
                for (const pattern of injectionPatterns) {
                    if (pattern.test(val)) {
                        throw new FederatedCollaborationMemoryValidationError(`Prompt injection pattern detected in metadata: "${pattern.source}"`);
                    }
                }
                if (val.includes('<thought>') ||
    ...
    registerContext(params) {
        ...
        if (!params.participatingAgentIds.includes(params.leaderAgentId)) {
            throw new FederatedCollaborationMemoryValidationError(`Leader agent '${params.leaderAgentId}' must be among participating agents`);
        }
     ...
     */
    assertContextBoundaries(contextId, tenantId, sessionId) {
        const context = this.contexts.get(contextId);
        if (!context) {
            throw new FederatedCollaborationMemoryValidationError(`Collaboration context '${contextId}' not found`);
        }
        if (context.tenantId !== tenantId) {
            throw new FederatedCollaborationMemoryTenantIsolationError(`Cross-tenant access rejected: context tenant '${context.tenantId}' != caller tenant '${tenantId}'`, tenantId, contextId);
        }
        if (context.sessionId !== sessionId) {
            throw new FederatedCollaborationMemorySessionIsolationError(`Cross-session access rejected: context session '${context.sessionId}' != caller session '${sessionId}'`, tenantId, contextId);
        }
        return context;
    }
    /**
     ...
     */
    updateContextStatus(contextId, newStatus, tenantId, sessionId) {
        const existing = this.assertContextBoundaries(contextId, tenantId, sessionId);
        // Terminal state invariant
        if (existing.status === 'HALTED_BY_USER_STOP' ||
            existing.status === 'HALTED_BY_EMERGENCY_STOP' ||
            existing.status === 'INVALIDATED' ||
            existing.status === 'COMPLETED' ||
            existing.status === 'FAILED') {
            throw new FederatedCollaborationMemoryValidationError(`Cannot transition from terminal status '${existing.status}' to '${newStatus}'`);
        }
        const now = Date.now();
     ...
     */
    listContextsForSession(tenantId, sessionId) {
        return Array.from(this.contexts.values()).filter((c) => c.tenantId === tenantId && c.sessionId === sessionId);
    }
    /**
