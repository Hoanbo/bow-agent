...
import { computeSha256, deterministicJsonStringify, } from './federatedCollaborationMemoryTypes.js';
export class ConsensusConflictResolver {
     ...
     */
    resolveVoteConflict(proposal, votes) {
        ...
        if (rejectVotes.length === 0 && approveVotes.length > 0) {
            const base = {
                ...
                resolvable: true,
                resolutionStrategy: 'AUTOMATIC_RECONCILED',
     ...
     */
    resolveObservationConflict(target, observations) {
     ...
     */
    detectMemoryConflict(entryA, entryB) {
        if (entryA.memoryId === entryB.memoryId)
            return undefined;
        if (entryA.tenantId !== entryB.tenantId)
            return 'AUTHORIZATION_CONFLICT';
        if (entryA.sessionId !== entryB.sessionId)
            return 'AUTHORIZATION_CONFLICT';
        if (entryA.generation !== entryB.generation)
