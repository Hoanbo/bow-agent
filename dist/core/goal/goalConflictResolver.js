...
export class GoalConflictResolver {
     ...
     */
    resolveConflict(conflictOrGoalA, graphOrGoalB) {
     ...
     */
    applyResolution(resolution, graph) {
        if (this.userStopProvider()) {
            throw new GoalUserStopError('apply_conflict_resolution');
        }
        const toBlock = new Set();
        if (resolution.loserGoalId)
            toBlock.add(resolution.loserGoalId);
    ...
    checkResourceConflict(a, b) {
        const extractResources = (goal) => {
            const set = new Set();
    ...
    checkPolicyConflict(a, b) {
        // Check if Goal A requires what Goal B explicitly forbids
        for (const cA of a.constraints) {
            for (const cB of b.constraints) {
                if ((cA.toLowerCase().includes('prohibit:') && cB.toLowerCase().includes(cA.toLowerCase().replace('prohibit:', '').trim())) ||
