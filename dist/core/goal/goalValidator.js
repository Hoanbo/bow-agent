...
// COGNITION != AUTHORITY
import { GoalValidationError, GoalSecurityError, MAX_TITLE_LENGTH, MAX_DESCRIPTION_LENGTH, assertNoChainOfThought, } from './goalTypes.js';
...
export class GoalValidator {
    ...
    static validateProposal(proposal) {
        ...
        const isValidIntent = (typeof p.sourceIntent === 'string' && p.sourceIntent.trim().length > 0) ||
            (typeof p.sourceIntent === 'object' &&
                p.sourceIntent !== null &&
                typeof p.sourceIntent.intentId === 'string' &&
                p.sourceIntent.intentId.trim().length > 0);
        if (!isValidIntent) {
        ...
        if (!Array.isArray(p.successCriteria) || p.successCriteria.length === 0) {
            ...
            const validCriteria = p.successCriteria.filter((c) => typeof c === 'string' && c.trim().length > 0);
     ...
     */
    static sanitizePriorityVector(input) {
        const sanitizeDim = (val, def = 0.5) => {
