...
import { DeliberationUserStopError, DeliberationConcurrencyError, CrossTenantDeliberationError, DeliberationIntegrityError, DeliberationValidationError, computeSessionProvenanceHash, } from './deliberationTypes.js';
...
export class DeliberationPersistenceRecoveryEngine {
     ...
     */
    saveSession(doc, expectedVersion, activeTenantId) {
        ...
        if (doc.provenanceHash !== expectedHash) {
            throw new DeliberationIntegrityError(`Session provenance mismatch: document hash '${doc.provenanceHash}' does not match computed hash '${expectedHash}'`, { sessionId: doc.sessionId });
        }
     ...
     */
    loadSession(sessionId, tenantId) {
        if (this.userStopProvider()) {
        ...
        if (fs.existsSync(backupFile)) {
            try {
                ...
                const bakDoc = JSON.parse(rawBak);
     ...
     */
    deleteSession(sessionId, tenantId) {
        if (this.userStopProvider()) {
