...
// COGNITION != AUTHORITY
import { DELIBERATION_BOUNDS, DeliberationValidationError, DeliberationSecurityError, assertNoDeliberationCoT, } from './deliberationTypes.js';
...
export class DeliberationValidator {
     ...
     */
    static validateHypothesis(hypo) {
        ...
        const errors = [];
        if (!hypo.hypothesisId || typeof hypo.hypothesisId !== 'string')
            errors.push('Missing hypothesisId');
        if (!hypo.sessionId || typeof hypo.sessionId !== 'string')
            errors.push('Missing sessionId');
        if (!hypo.tenantId || typeof hypo.tenantId !== 'string')
            errors.push('Missing tenantId');
        if (!hypo.title || typeof hypo.title !== 'string')
            errors.push('Missing title');
        if (!hypo.premise || typeof hypo.premise !== 'string')
            errors.push('Missing premise');
        if (!hypo.provenanceHash || hypo.provenanceHash.length !== 64)
            errors.push('Invalid provenanceHash length');
    ...
    static validateEvidence(evidence) {
        ...
        if (!evidence.evidenceId || typeof evidence.evidenceId !== 'string')
            errors.push('Missing evidenceId');
        if (!evidence.tenantId || typeof evidence.tenantId !== 'string')
            errors.push('Missing tenantId');
    ...
    static validateConstraint(constraint) {
        ...
        if (!constraint.constraintId || typeof constraint.constraintId !== 'string')
            errors.push('Missing constraintId');
    ...
    static validateSession(session) {
        ...
        if (!s.tenantId || typeof s.tenantId !== 'string')
            errors.push('Missing tenantId');
        if (typeof s.sessionVersion !== 'number' || s.sessionVersion < 1)
