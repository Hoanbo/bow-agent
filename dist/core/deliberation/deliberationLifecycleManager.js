...
import { DELIBERATION_SCHEMA_VERSION, DELIBERATION_BOUNDS, DeliberationUserStopError, DeliberationTransitionError, DeliberationConcurrencyError, DeliberationCapacityError, DeliberationValidationError, computeSessionProvenanceHash, computeResultHash, } from './deliberationTypes.js';
...
import { globalEvidenceBindingEngine } from './evidenceBindingEngine.js';
import { globalSymbolicConstraintEngine } from './symbolicConstraintEngine.js';
import { globalDeliberationSearchEngine } from './deliberationSearchEngine.js';
import { globalContradictionResolver } from './contradictionResolver.js';
export const LEGAL_DELIBERATION_TRANSITIONS = Object.freeze({
    INITIALIZING: Object.freeze(['DELIBERATING', 'ABORTED']),
    DELIBERATING: Object.freeze(['CONVERGED', 'INCONCLUSIVE', 'CONTRADICTED', 'ABORTED']),
    CONVERGED: Object.freeze(['RESOLVED', 'CONTRADICTED', 'ABORTED']),
    RESOLVED: Object.freeze(['ABORTED']),
    INCONCLUSIVE: Object.freeze(['DELIBERATING', 'ABORTED']),
    CONTRADICTED: Object.freeze(['ABORTED']),
...
export class DeliberationLifecycleManager {
     ...
     */
    createSession(tenantId, origin, targetGoalId, customCreatedAt) {
     ...
     */
    addHypothesis(doc, proposal, expectedVersion) {
        ...
        if (doc.hypotheses.length >= DELIBERATION_BOUNDS.MAX_HYPOTHESES_PER_SESSION) {
            throw new DeliberationCapacityError(doc.hypotheses.length, DELIBERATION_BOUNDS.MAX_HYPOTHESES_PER_SESSION, 'hypotheses per session');
        }
     ...
     */
    addEvidence(doc, evidenceInput, expectedVersion) {
        ...
        const bound = this.evidenceEngine.bindEvidence({ ...evidenceInput, tenantId: doc.tenantId, sessionId: doc.sessionId }, doc.tenantId);
     ...
     */
    addConstraint(doc, constraintInput, expectedVersion) {
        ...
        if (doc.constraints.length >= DELIBERATION_BOUNDS.MAX_CONSTRAINTS_PER_SESSION) {
            throw new DeliberationCapacityError(doc.constraints.length, DELIBERATION_BOUNDS.MAX_CONSTRAINTS_PER_SESSION, 'constraints per session');
        }
     ...
     */
    startDeliberation(doc, expectedVersion) {
     ...
     */
    evaluateAndAdvance(doc, expectedVersion) {
        ...
        const contradictionAnalysis = this.contradictionResolver.detectAndAnalyze(doc.hypotheses, doc.constraints, doc.evidenceBindings, doc.targetGoalId);
        ...
        const searchOutcome = this.searchEngine.searchAndRank(doc.hypotheses, doc.constraints, doc.evidenceBindings);
        let nextStatus = 'DELIBERATING';
     ...
     */
    resolveSession(doc, expectedVersion, recommendedAction) {
     ...
     */
    abortSession(doc, expectedVersion, reason) {
    ...
    assertMutationPreconditions(doc, expectedVersion, checkpoint) {
        if (this.userStopProvider()) {
     ...
     */
    projectTransientRegisters(doc) {
        if (!this.workingRegisterStore)
            return;
        try {
