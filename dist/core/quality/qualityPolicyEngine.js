...
export class QualityPolicyEngine {
    ...
    static assertNotProtectedWorkspace(targetPath) {
        ...
        if (normalized === 'c:\\bow\\shopofbow' || normalized.startsWith('c:\\bow\\shopofbow\\')) {
            throw new QualityError(QualityErrorCode.PROTECTED_WORKSPACE_VIOLATION, `Operation targets protected workspace C:\\BOW\\shopofbow. Execution denied.`);
        }
    }
    /**
     ...
     */
    static validateCommandExecution(command, context, sandbox, options) {
        ...
        if (options?.isUserStopped || sandbox.isStopped) {
            throw new QualityError(QualityErrorCode.USER_STOP_ACTIVE, `Command "${command.commandId}" rejected: USER_STOP is currently active.`);
        }
        // 2. Revocation Supremacy
        // 2. Quyền tối cao của việc thu hồi (REVOCATION)
        if (options?.isRevoked || sandbox.state === 'REVOKED') {
            throw new QualityError(QualityErrorCode.REVOCATION_ACTIVE, `Command "${command.commandId}" rejected: Execution context or sandbox has been revoked.`);
        }
        ...
        if (context.sessionId !== sandbox.binding.sessionId) {
            throw new QualityError(QualityErrorCode.SESSION_MISMATCH, `Context sessionId "${context.sessionId}" does not match sandbox sessionId "${sandbox.binding.sessionId}".`);
        }
        // 6. Task Binding Validation
        // 6. Xác thực liên kết tác vụ
        if (context.taskId !== sandbox.binding.taskId) {
            throw new QualityError(QualityErrorCode.TASK_MISMATCH, `Context taskId "${context.taskId}" does not match sandbox taskId "${sandbox.binding.taskId}".`);
        }
        ...
        if (options?.delegation) {
            ...
            if (options.delegation.revocationState?.isRevoked || options.delegation.status === 'REVOKED') {
                throw new QualityError(QualityErrorCode.REVOCATION_ACTIVE, `Delegation "${options.delegation.delegationId}" has been revoked.`);
            }
            if (options.delegation.expiresAt <= now) {
                throw new QualityError(QualityErrorCode.DELEGATION_INVALID, `Delegation "${options.delegation.delegationId}" has expired.`);
            }
        }
        ...
        if (options?.capabilityLease) {
            ...
            if (options.capabilityLease.revokedAt || options.capabilityLease.status === 'REVOKED') {
                throw new QualityError(QualityErrorCode.REVOCATION_ACTIVE, `Capability lease "${options.capabilityLease.leaseId}" has been revoked.`);
            }
            if (options.capabilityLease.expiresAt <= now) {
                throw new QualityError(QualityErrorCode.CAPABILITY_LEASE_INVALID, `Capability lease "${options.capabilityLease.leaseId}" has expired.`);
            }
        }
    }
}
