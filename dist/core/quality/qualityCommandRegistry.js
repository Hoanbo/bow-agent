...
export class QualityCommandRegistry {
    ...
    registerCommand(command) {
        if (!command.commandId || command.commandId.trim() === '') {
            throw new QualityError(QualityErrorCode.COMMAND_NOT_ALLOWLISTED, 'Cannot register command with empty commandId.');
        }
    ...
    getCommand(commandId) {
        ...
        if (!cmd) {
            throw new QualityError(QualityErrorCode.COMMAND_NOT_ALLOWLISTED, `Command "${commandId}" is not in the governed quality allowlist.`);
        }
        return cmd;
    }
    /**
