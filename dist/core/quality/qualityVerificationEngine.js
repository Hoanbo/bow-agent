...
export class QualityVerificationEngine {
     ...
     */
    verifyEvidenceBundle(bundle, sandbox, currentManifest) {
        ...
        const recomputedHash = QualityEvidenceEngine.hashEvidenceBundle(bundle.evidenceId, bundle.context, bundle.manifestHash, buildHashes, testHashes, bundle.securityScanResult?.scanHash);
        if (recomputedHash !== bundle.evidenceHash) {
            throw new QualityError(QualityErrorCode.EVIDENCE_CORRUPTED, `Evidence bundle "${bundle.evidenceId}" hash mismatch: expected "${bundle.evidenceHash}", recomputed "${recomputedHash}".`);
        }
        // 2. Validate sandbox ID binding.
        // 2. Xác thực liên kết Sandbox ID.
        if (bundle.context.sandboxId !== sandbox.id) {
            throw new QualityError(QualityErrorCode.SANDBOX_NOT_FOUND, `Evidence sandboxId "${bundle.context.sandboxId}" does not match live sandbox "${sandbox.id}".`);
        }
        ...
        if (bundle.context.sessionId !== sandbox.binding.sessionId) {
            throw new QualityError(QualityErrorCode.SESSION_MISMATCH, `Evidence sessionId "${bundle.context.sessionId}" does not match live sandbox session "${sandbox.binding.sessionId}".`);
        }
        ...
        if (bundle.manifestHash !== currentManifest.manifestHash) {
            throw new QualityError(QualityErrorCode.MANIFEST_HASH_MISMATCH, `Evidence manifest hash "${bundle.manifestHash}" does not match current sandbox manifest "${currentManifest.manifestHash}". Worktree is stale.`);
        }
