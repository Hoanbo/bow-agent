...
import { createTestExecutionId, } from './qualityTypes.js';
...
export class TestExecutionEngine {
     ...
     */
    static hashTestResult(executionId, commandId, exitCode, stdoutHash, stderrHash, summary) {
     ...
     */
    static parseTestSummary(stdout, exitCode) {
        let suiteCount = 1;
        let passedSuites = exitCode === 0 ? 1 : 0;
        let failedSuites = exitCode !== 0 ? 1 : 0;
        let skippedSuites = 0;
        let assertionCount = undefined;
     ...
     */
    async executeTest(commandId, context, sandbox, options) {
        ...
        const testEvidenceHash = TestExecutionEngine.hashTestResult(executionId, commandId, executed.exitCode, executed.stdoutHash, executed.stderrHash, summary);
