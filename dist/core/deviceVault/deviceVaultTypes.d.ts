...
export declare const VAULT_PROTOCOL_VERSION = "4.0.0";
export declare const CURRENT_VAULT_SCHEMA_VERSION = 2;
/**
 * 10 discrete canonical vault lifecycle states.
 */
export type VaultState = 'UNINITIALIZED' | 'INITIALIZING' | 'READY' | 'LOCKED' | 'DEGRADED' | 'CORRUPTED' | 'RECOVERING' | 'MIGRATING' | 'REVOKED' | 'DESTROYED';
export declare const ALL_VAULT_STATES: readonly VaultState[];
/**
 * Lifecycle status of individual credential entries within the vault.
 */
export type VaultEntryStatus = 'ACTIVE' | 'ROTATED' | 'REVOKED' | 'EXPIRED' | 'PENDING_ROTATION';
export declare const ALL_VAULT_ENTRY_STATUSES: readonly VaultEntryStatus[];
/**
 * Security reasons for locking the vault (internal security state, NOT a user password).
 */
export type VaultLockReason = 'ADMIN_LOCK' | 'INTEGRITY_SUSPICION' | 'SESSION_TERMINATED' | 'MANUAL_LOCK' | 'TAMPER_DETECTED' | 'MIGRATION_IN_PROGRESS';
/**
 ...
 */
export type DeviceVaultErrorCode = 'VAULT_UNINITIALIZED' | 'VAULT_LOCKED' | 'VAULT_CORRUPTED' | 'VAULT_DEGRADED' | 'VAULT_ENTRY_NOT_FOUND' | 'VAULT_DUPLICATE_ENTRY' | 'VAULT_INTEGRITY_MISMATCH' | 'VAULT_FINGERPRINT_MISMATCH' | 'VAULT_SCOPE_MISMATCH' | 'VAULT_VERSION_MISMATCH' | 'VAULT_SCHEMA_INVALID' | 'VAULT_TRANSACTION_FAILED' | 'VAULT_RECOVERY_FAILED' | 'VAULT_MIGRATION_FAILED' | 'VAULT_ROTATION_FAILED' | 'VAULT_DEVICE_REVOKED' | 'VAULT_KEY_REVOKED' | 'VAULT_KEY_EXPIRED' | 'VAULT_SECRET_LEAKAGE_PREVENTED' | 'VAULT_INVALID_TRANSITION' | 'VAULT_IO_ERROR';
export declare const ALL_VAULT_ERROR_CODES: readonly DeviceVaultErrorCode[];
/**
