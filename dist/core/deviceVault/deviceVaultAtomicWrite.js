 ...
 */
export function atomicWriteFileSync(destPath, payload) {
    ...
    try {
        ...
        throw new Error(`[VAULT_IO_ERROR] Atomic write failed for ${destPath}: ${err.message}`);
    }
}
/**
 * Checks for and parses an existing recovery marker for a target file.
 */
export function readRecoveryMarkerSync(destPath) {
    const markerPath = `${destPath}.marker`;
    if (!fs.existsSync(markerPath)) {
        return null;
    }
 ...
 */
export function cleanupAtomicArtifactsSync(destPath) {
    const markerPath = `${destPath}.marker`;
    if (fs.existsSync(markerPath)) {
        try {
            fs.unlinkSync(markerPath);
