 ...
 */
export function formatPrivateKeyRef(vaultId, entryId) {
    return `ref://vault/${vaultId}/${entryId}`;
}
/**
 * Parses an opaque privateKeyRef string.
 */
export function parsePrivateKeyRef(ref) {
 ...
 */
export function containsRawPrivateKey(value) {
 ...
 */
export function assertNoRawPrivateKey(value, context = 'Vault') {
    if (containsRawPrivateKey(value)) {
        throw new Error(`[VAULT_SECRET_LEAKAGE_PREVENTED] Raw private key material detected in ${context}. Exposure strictly prohibited.`);
    }
}
