...
function resolveAndValidatePath(userPath) {
    ...
    if (!resolved.startsWith(EXECUTION_WORKSPACE)) {
        throw new BrainError('BRAIN_TOOL_EXECUTION_FAILED', `Path "${resolved}" is outside the execution workspace. Access denied.`);
    }
    ...
    if (resolved.toLowerCase().startsWith(PROTECTED_WORKSPACE.toLowerCase())) {
        throw new BrainError('BRAIN_TOOL_EXECUTION_FAILED', 'Access to the protected workspace (C:\\BOW\\shopofbow) is absolutely forbidden.');
    }
    return resolved;
}
/**
...
export const brainFsAppendTool = {
...
/** Delete a file */
export const brainFsDeleteTool = {
...
export const ALL_BRAIN_TOOLS = Object.freeze([
    brainFsWriteTool,
    brainFsReadTool,
    brainFsAppendTool,
    brainFsListTool,
...
export function readFileContent(filePath) {
    try {
        const resolved = resolveAndValidatePath(filePath);
        if (!fs.existsSync(resolved))
            return { error: 'File not found', resolvedPath: resolved };
