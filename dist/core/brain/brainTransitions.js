...
export function isValidBrainTransition(from, to) {
    if (from === to)
        return true; // self-transition always idempotent
    const allowed = BRAIN_TRANSITIONS[from];
    return allowed ? allowed.includes(to) : false;
}
export function assertValidBrainTransition(from, to) {
    if (!isValidBrainTransition(from, to)) {
        throw new BrainError('BRAIN_ILLEGAL_TRANSITION', `Brain state transition from "${from}" to "${to}" is not allowed.`);
    }
}
