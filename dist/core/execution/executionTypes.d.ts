 ...
 */
export type ExecutionStatus = 'SUCCESS' | 'FAILED' | 'BLOCKED' | 'READY' | 'AUTHORIZED' | 'EXECUTING' | 'SUCCEEDED' | 'REPLAY_REJECTED' | 'UNAUTHORIZED' | 'INVALID';
/**
 * EN: High-level execution outcome classification.
 * VI: Phân loại kết quả thực thi cấp cao.
 */
export type ExecutionOutcome = 'COMPLETED' | 'AUTHORIZATION_DENIED' | 'REPLAY_REJECTED' | 'CAPABILITY_NOT_FOUND' | 'VALIDATION_FAILED' | 'HANDLER_FAILED' | 'SUCCESS' | 'FAILURE' | 'BLOCKED' | 'REPLAY';
/**
 * EN: Replay and idempotency status.
 * VI: Trạng thái lặp lại (replay) và bất biến lặp (idempotency).
 */
export type ReplayStatus = 'NOT_REPLAY' | 'IDEMPOTENT_HIT' | 'REPLAY_REJECTED';
/**
...
export declare class ExecutionError extends Error {
    ...
    constructor(code: ExecutionStatus, message: string, sanitized?: boolean);
}
