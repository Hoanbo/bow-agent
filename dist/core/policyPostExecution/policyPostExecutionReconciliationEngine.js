...
import { createReconciliationId, } from './policyPostExecutionTypes.js';
...
export class PolicyPostExecutionReconciliationEngine {
     ...
     */
    reconcile(receipt, verification) {
        ...
        if (receipt.tenantPartition !== verification.tenantPartition) {
            throw new Error(`TENANT_MISMATCH_RECONCILIATION_FAILED: Receipt tenant '${receipt.tenantPartition}' does not match verification tenant '${verification.tenantPartition}'`);
        }
        if (receipt.executionId !== verification.executionId) {
            throw new Error(`EXECUTION_ID_MISMATCH: Receipt executionId '${receipt.executionId}' does not match verification executionId '${verification.executionId}'`);
        }
        const discrepancies = [];
        let status = 'VERIFIED_UNKNOWN';
        ...
        if (receipt.status === 'SUCCEEDED' && verification.status !== 'SUCCESS') {
            discrepancies.push(`CONTRADICTORY_OUTCOME: Receipt claimed SUCCEEDED but outcome verifier determined '${verification.status}'`);
            status = 'RECONCILIATION_INVALID';
        }
        else if (receipt.status === 'FAILED' && verification.status === 'SUCCESS') {
            discrepancies.push(`CONTRADICTORY_OUTCOME: Receipt claimed FAILED but outcome verifier claimed SUCCESS`);
            status = 'RECONCILIATION_INVALID';
        }
        else if (receipt.status === 'SUCCEEDED' && receipt.error) {
            discrepancies.push(`CONTRADICTORY_RECEIPT_ERROR: Receipt status is SUCCEEDED but error field is populated (${receipt.error})`);
        ...
        const reconciliationId = createReconciliationId(`rec_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`);
