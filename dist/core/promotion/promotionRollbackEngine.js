...
export class PromotionRollbackEngine {
    promotionEngine;
    auditLedger;
    completedRollbacks = new Map();
    constructor(promotionEngine, auditLedger = globalAuditLedger) {
    ...
    executeRollback(input) {
        ...
        if (input.isUserStopActive) {
            throw new PromotionError('USER_STOP_ACTIVE', `Rollback rejected: USER_STOP is active.`);
        }
        ...
        if (!completedPromotion) {
            throw new PromotionError('PROMOTION_NOT_FOUND', `Cannot rollback: completed promotion "${input.promotionId}" not found.`);
        }
        ...
        if (this.completedRollbacks.has(input.promotionId)) {
            throw new PromotionError('DUPLICATE_ROLLBACK', `Promotion "${input.promotionId}" has already been rolled back.`);
        }
        const backups = this.promotionEngine.getBackups(input.promotionId);
        if (!backups) {
            throw new PromotionError('ROLLBACK_BACKUP_MISSING', `Cannot rollback: backup state for promotion "${input.promotionId}" is unavailable.`);
        }
        ...
        for (const backup of backups) {
            const { absolutePath: targetAbs } = SandboxPathGuard.resolveAndAssertContainedPath(targetRootAbs, backup.relativePath);
        ...
        const auditRecordId = this.logAudit(input.rolledBackBy, 'PROMOTION_ROLLED_BACK', completedPromotion.targetProjectRoot, 'PERMIT', 'SUCCESS', {
