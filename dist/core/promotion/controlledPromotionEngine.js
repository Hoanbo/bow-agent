...
export class ControlledPromotionEngine {
    ...
    rollbackBackups = new Map();
    completedPromotions = new Map();
    constructor(authEngine, auditLedger = globalAuditLedger) {
    ...
    executePromotion(input) {
        ...
        if (input.isUserStopActive) {
            throw new PromotionError('USER_STOP_ACTIVE', `Promotion execution rejected: USER_STOP is active.`);
        }
        ...
        if (this.completedPromotions.has(input.proposal.promotionId)) {
            throw new PromotionError('DUPLICATE_PROMOTION', `Promotion "${input.proposal.promotionId}" has already been executed.`);
        }
        // 4. Verify approval record is APPROVED.
        // 4. Xác minh bản ghi phê duyệt phải là APPROVED.
        if (input.approval.decision !== 'APPROVED') {
            throw new PromotionError('UNAPPROVED_PROMOTION', `Cannot execute unapproved promotion "${input.proposal.promotionId}".`);
        }
        // 5. Verify canonical authorization token.
        // 5. Xác minh mã ủy quyền chuẩn tắc.
        const isTokenValid = this.authEngine.validatePromotionToken(input.authorizationToken, input.proposal);
        if (!isTokenValid) {
            throw new PromotionError('INVALID_AUTHORIZATION_TOKEN', `Authorization token invalid or expired for promotion "${input.proposal.promotionId}".`);
        }
        ...
        if (input.proposal.baseManifestHash &&
            preManifestHash &&
            input.proposal.baseManifestHash !== preManifestHash) {
            throw new PromotionError('STALE_TARGET_STATE', `Target state changed concurrently. Current hash "${preManifestHash}" != expected base "${input.proposal.baseManifestHash}".`);
        }
        ...
        for (const change of input.proposal.proposedChanges) {
            const { absolutePath: targetAbs } = SandboxPathGuard.resolveAndAssertContainedPath(targetRootAbs, change.relativePath);
        ...
        for (const change of input.proposal.proposedChanges) {
            ...
            if (change.changeType === 'ADDED' || change.changeType === 'MODIFIED') {
                ...
                const { absolutePath: sandboxSourceAbs } = SandboxPathGuard.resolveAndAssertContainedPath(input.sandbox.rootPath, change.relativePath);
                if (!fs.existsSync(sandboxSourceAbs)) {
                    throw new PromotionError('SANDBOX_SOURCE_MISSING', `Source file "${change.relativePath}" missing in sandbox "${input.sandbox.id}".`);
                }
                ...
                const { absolutePath: oldTargetAbs } = SandboxPathGuard.resolveAndAssertContainedPath(targetRootAbs, change.previousPath);
        ...
        const auditRecordId = this.logAudit(input.executedBy, 'CONTROLLED_PROMOTION_EXECUTED', input.proposal.targetProjectRoot, 'PERMIT', 'SUCCESS', {
     ...
     */
    getCompletedPromotion(promotionId) {
