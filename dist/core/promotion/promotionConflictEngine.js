...
export class PromotionConflictEngine {
    ...
    detectConflicts(input) {
        ...
        if (input.targetBaseManifestHash &&
        ...
        if (input.concurrentActiveProposals) {
            ...
            for (const other of input.concurrentActiveProposals) {
                if (other.promotionId === input.proposal.promotionId)
                    continue;
                if (path.resolve(other.targetProjectRoot) !== path.resolve(input.proposal.targetProjectRoot))
                    continue;
