...
import { createReleaseAcceptanceCriteriaId, } from './releaseTypes.js';
...
export class ReleaseAcceptanceCriteriaEngine {
     ...
     */
    static hashCriteriaResults(results) {
     ...
     */
    evaluateAll(candidate, qualityReport, evidenceBundle) {
        const now = Date.now();
        const results = [];
        ...
        // Trạng thái cổng tổng thể của báo cáo chất lượng phải là PASS.
        {
            ...
            if (!passed)
                failureReasons.push(`[QUALITY_REPORT_PASS] ${details}`);
        }
        // ── Criterion 2: GATE_PASS ────────────────────────────────────────────────
        // Ensure the report has no blocking reasons.
        // Đảm bảo báo cáo không có lý do chặn.
        {
            const passed = qualityReport.overallState === 'PASS' && qualityReport.blockingReasons.length === 0;
            const details = passed
            ...
            if (!passed)
                failureReasons.push(`[GATE_PASS] ${details}`);
        }
        ...
        // Trạng thái mâu thuẫn của báo cáo chất lượng phải là CONSISTENT.
        {
            ...
            if (!passed)
                failureReasons.push(`[CONTRADICTION_FREE] ${details}`);
        }
        ...
        // Tính toán lại mã băm bằng chứng và xác minh nó khớp với giá trị đã lưu.
        {
            ...
            try {
                ...
                const recomputed = QualityEvidenceEngine.hashEvidenceBundle(evidenceBundle.evidenceId, evidenceBundle.context, evidenceBundle.manifestHash, buildHashes, testHashes, evidenceBundle.securityScanResult?.scanHash);
                ...
                details = `Evidence hash recomputation failed: ${err.message}`;
            }
            results.push({ criterion: 'EVIDENCE_VERIFIED', passed, details, evidenceHash, evaluatedAt: now });
            if (!passed)
                failureReasons.push(`[EVIDENCE_VERIFIED] ${details}`);
        }
        ...
        // sourceManifestHash của ứng viên phải bằng manifestHash của evidenceBundle.
        {
            ...
            if (!passed)
                failureReasons.push(`[MANIFEST_HASH_BOUND] ${details}`);
        }
        ...
        // Không có lần dựng thất bại trong báo cáo chất lượng.
        {
            ...
            if (!passed)
                failureReasons.push(`[NO_BUILD_FAILURES] ${details}`);
        }
        ...
        // Không có bộ kiểm thử thất bại trong báo cáo chất lượng.
        {
            ...
            if (!passed)
                failureReasons.push(`[NO_TEST_FAILURES] ${details}`);
        }
        ...
        // Tính toán lại và xác minh mã băm nguồn gốc của ứng viên.
        {
            ...
            if (!passed)
                failureReasons.push(`[PROVENANCE_HASH_VALID] ${details}`);
        }
        // ── Final assembly ────────────────────────────────────────────────────────
        ...
        const acceptanceCriteriaId = createReleaseAcceptanceCriteriaId(`ac_${Date.now()}_${evaluationHash.slice(0, 8)}`);
