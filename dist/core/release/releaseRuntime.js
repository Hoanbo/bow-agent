...
import { ReleaseVerificationPipeline } from './releaseVerificationPipeline.js';
...
export class ReleaseRuntime {
    ...
    _userStopReason = '';
    constructor(qualityRuntime = globalQualityRuntime, auditLedger = globalAuditLedger) {
     ...
     */
    proposeReleaseCandidate(params, options) {
        // USER_STOP check before any candidate creation.
        // Kiểm tra USER_STOP trước khi tạo ứng viên.
        if (this._isUserStopped) {
            throw new ReleaseError(ReleaseErrorCode.USER_STOP_ACTIVE, `Release candidate proposal rejected: USER_STOP is active (${this._userStopReason}).`);
        }
        ...
        this.logAudit(params.agentId, 'release_candidate_proposed', params.sandboxId, 'PERMIT', 'SUCCESS', {
     ...
     */
    async runReleaseVerification(candidateId, context, sandbox, currentManifest, options) {
        // USER_STOP check first.
        // Kiểm tra USER_STOP trước tiên.
        if (this._isUserStopped) {
            throw new ReleaseError(ReleaseErrorCode.USER_STOP_ACTIVE, `Release verification rejected: USER_STOP is active (${this._userStopReason}).`);
        }
        // Retrieve candidate from store.
        // Lấy ứng viên từ kho lưu trữ.
        const candidate = this.candidates.get(candidateId);
        if (!candidate) {
            throw new ReleaseError(ReleaseErrorCode.CANDIDATE_NOT_FOUND, `Release candidate "${candidateId}" not found in ReleaseRuntime.`);
        }
        ...
        // Thực thi đường ống.
        const record = await this.pipeline.run(candidate, pipelineContext, sandbox, currentManifest, pipelineOptions);
        ...
        const updatedCandidate = ReleaseCandidateEngine.transitionState(candidate, newCandidateState, `Verification pipeline completed with state: ${record.verificationState}`);
        this.candidates.set(candidateId, updatedCandidate);
        this.logAudit(context.agentId, 'release_verification_completed', context.projectRoot, record.verificationState === 'PASS' ? 'PERMIT' : 'DENY', record.verificationState === 'PASS' ? 'SUCCESS' : 'FAILURE', {
     ...
     */
    getReleaseCandidate(candidateId) {
