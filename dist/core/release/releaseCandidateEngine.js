...
import { RELEASE_SCHEMA_VERSION, createReleaseCandidateId, ReleaseError, ReleaseErrorCode, } from './releaseTypes.js';
export class ReleaseCandidateEngine {
     ...
     */
    static hashCandidateProvenance(candidateId, milestoneTag, sourceManifestHash, qualityReportId, evidenceId, agentId, sessionId, taskId, delegationId, capabilityLeaseId, sandboxId, proposedAt) {
     ...
     */
    static validateMilestoneTag(tag) {
        if (!tag || tag.trim().length === 0) {
            throw new ReleaseError(ReleaseErrorCode.INVALID_MILESTONE_TAG, `Milestone tag must not be empty. / Thẻ mốc không được rỗng.`);
        }
    }
    /**
     ...
     */
    static validateRequiredBindings(params) {
        const required = [
            'milestoneTag',
            'sourceManifestHash',
            'qualityReportId',
        ...
        for (const field of required) {
            ...
            if (!value || (typeof value === 'string' && value.trim().length === 0)) {
                throw new ReleaseError(ReleaseErrorCode.MISSING_REQUIRED_BINDING, `Missing required release candidate binding: "${field}". ` +
    ...
    createReleaseCandidate(params) {
        ...
        const candidateId = createReleaseCandidateId(`rc_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`);
        const proposedAt = Date.now();
        const ttlMs = params.ttlMs ?? 30 * 60 * 1000; // 30-minute default TTL
        const expiresAt = proposedAt + ttlMs;
        const provenanceHash = ReleaseCandidateEngine.hashCandidateProvenance(candidateId, params.milestoneTag, params.sourceManifestHash, params.qualityReportId, params.evidenceId, params.agentId, params.sessionId, params.taskId, params.delegationId, params.capabilityLeaseId, params.sandboxId, proposedAt);
     ...
     */
    static verifyProvenanceHash(candidate) {
        const recomputed = ReleaseCandidateEngine.hashCandidateProvenance(candidate.candidateId, candidate.milestoneTag, candidate.sourceManifestHash, candidate.qualityReportId, candidate.evidenceId, candidate.agentId, candidate.sessionId, candidate.taskId, candidate.delegationId, candidate.capabilityLeaseId, candidate.sandboxId, candidate.proposedAt);
        if (recomputed !== candidate.provenanceHash) {
            throw new ReleaseError(ReleaseErrorCode.PROVENANCE_HASH_MISMATCH, `Release candidate "${candidate.candidateId}" provenance hash mismatch: ` +
     ...
     */
    static transitionState(candidate, newState, reason) {
