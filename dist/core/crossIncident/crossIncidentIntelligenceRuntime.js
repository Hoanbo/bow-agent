...
import { IncidentHistoryArchiveStore } from './incidentHistoryArchiveStore.js';
...
export class CrossIncidentIntelligenceRuntime {
    static AUDIT_DOMAIN = 'CROSS_INCIDENT_INTELLIGENCE';
     ...
     */
    ingestIncident(input) {
        this.assertOperationalSafety(`${input.targetId} ${JSON.stringify(input.closureRecord)} ${JSON.stringify(input.postMortemReport)}`);
     ...
     */
    getHypothesisReliability(hypothesisType) {
     ...
     */
    getStratifiedRemediationReliability(key) {
     ...
     */
    generateGovernanceAdvisory(targetScope, actionClass) {
