...
import { createIncidentArchiveId, } from './crossIncidentTypes.js';
/**
 ...
 */
function validateArchivedIncidentRecords(data) {
    ...
    const isValid = data.every((item) => item &&
        ...
        typeof item.deterministicIngestionId === 'string' &&
        typeof item.closureCertificateHash === 'string' &&
        typeof item.postMortemSha256 === 'string' &&
        typeof item.ingestedAt === 'number');
    if (!isValid) {
...
export class IncidentHistoryArchiveStore {
     ...
     */
    computeIngestionIdentity(incidentId, closureCertificateHash, postMortemSha256) {
        return crypto
            .createHash('sha256')
    ...
    archiveIncident(input) {
        ...
        const deterministicId = this.computeIngestionIdentity(input.incidentId, input.closureRecord.closureCertificateHash, input.postMortemReport.postMortemSha256);
        ...
        const sanitizedPostMortem = this.sanitizer.sanitize(input.postMortemReport);
        const sanitizedClosure = this.sanitizer.sanitize(input.closureRecord);
        const sanitizedMetrics = this.sanitizer.sanitize(input.effectivenessMetrics);
     ...
     */
    getIncidentById(incidentId, userId) {
    ...
    queryIncidents(filter, userId) {
        ...
        let matched = allRecords.filter((record) => {
            if (filter.targetId && record.targetId !== filter.targetId)
                return false;
            if (filter.category && record.failureCategory !== filter.category)
                return false;
            if (filter.actionClass && record.actionClass !== filter.actionClass)
                return false;
