...
export class ApprovalService {
    ...
    singleFileOverride;
    stores = new Map();
    tokenToUserIndex = new Map();
    approvalToUserIndex = new Map();
    ...
    requestApproval(params) {
        const isAnon = (id) => !id || ['anonymous', 'anon', 'unknown', 'unauthenticated'].includes(id.toLowerCase());
    ...
    grantApproval(approvalId, approver, userId) {
        ...
        if (new Date(record.expiresAt).getTime() < Date.now()) {
            store.update(list => list.map(r => (r.id === approvalId ? { ...r, status: 'EXPIRED' } : r)));
            return { success: false, error: 'APPROVAL_EXPIRED' };
        }
        const token = 'tok_' + crypto.randomBytes(24).toString('hex');
        store.update(list => list.map(r => r.id === approvalId
            ? {
                ...r,
                status: 'APPROVED',
                approver,
     ...
     */
    validateToken(token, actionName, args, userId) {
        if (!token) {
        ...
        if (new Date(record.expiresAt).getTime() < Date.now()) {
            store.update(list => list.map(r => (r.id === record.id ? { ...r, status: 'EXPIRED' } : r)));
     ...
     */
    validateAndConsumeToken(token, actionName, args, userId) {
    ...
    listPending(userId) {
        const now = Date.now();
        return this.getAllApprovals(userId).filter(r => r.status === 'PENDING' && new Date(r.expiresAt).getTime() > now);
    }
}
