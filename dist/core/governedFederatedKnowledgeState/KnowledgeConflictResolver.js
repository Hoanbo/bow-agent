...
export class KnowledgeConflictResolver {
    ...
    detectConflict(entryA, entryB) {
        ...
        if (entryA.sessionId !== entryB.sessionId)
            return 'AUTHORIZATION_CONFLICT';
        if (entryA.missionId !== entryB.missionId || entryA.objectiveId !== entryB.objectiveId)
            return 'POLICY_CONFLICT';
        if (entryA.leaseId !== entryB.leaseId && entryA.leaseId && entryB.leaseId)
     ...
     */
    resolveConflict(entryA, entryB, category) {
