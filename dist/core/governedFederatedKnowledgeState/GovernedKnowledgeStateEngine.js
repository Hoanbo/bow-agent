...
import { MAX_KNOWLEDGE_ENTRIES_PER_FEDERATION, MAX_KNOWLEDGE_ENTRIES_PER_AGENT, MAX_KNOWLEDGE_STATE_SIZE, MAX_ACTIVE_KNOWLEDGE_STATES, MAX_KNOWLEDGE_STATE_DURATION_MS, GovernedFederatedKnowledgeStateValidationError, GovernedFederatedKnowledgeStateTenantIsolationError, GovernedFederatedKnowledgeStateSessionIsolationError, GovernedFederatedKnowledgeStateBudgetError, GovernedFederatedKnowledgeStateConcurrencyError, computeKnowledgeStateSnapshotHash, } from './GovernedFederatedKnowledgeStateTypes.js';
...
export class GovernedKnowledgeStateEngine {
     ...
     */
    createKnowledgeState(params) {
        this.securityBoundary.assertStopInactive('KNOWLEDGE_ENTRY', params.tenantId, params.stateId);
        // Active states budget check
        const activeStates = Array.from(this.states.values()).filter((s) => s.sessionId === params.sessionId && s.status !== 'COMPLETED' && s.status !== 'FAILED');
        if (activeStates.length >= MAX_ACTIVE_KNOWLEDGE_STATES) {
            throw new GovernedFederatedKnowledgeStateBudgetError(`Session reached MAX_ACTIVE_KNOWLEDGE_STATES limit (${MAX_ACTIVE_KNOWLEDGE_STATES})`, params.tenantId, params.stateId);
        }
        if (this.states.has(params.stateId)) {
            throw new GovernedFederatedKnowledgeStateValidationError(`Knowledge state '${params.stateId}' already exists`);
        }
        const now = Date.now();
     ...
     */
    addKnowledgeEntry(stateId, entry, expectedVersion) {
        ...
        if (state.tenantId !== entry.tenantId) {
            throw new GovernedFederatedKnowledgeStateTenantIsolationError(`Cross-tenant entry addition: state tenant '${state.tenantId}' != entry tenant '${entry.tenantId}'`, entry.tenantId);
        }
        if (state.sessionId !== entry.sessionId) {
            throw new GovernedFederatedKnowledgeStateSessionIsolationError(`Cross-session entry addition: state session '${state.sessionId}' != entry session '${entry.sessionId}'`, entry.tenantId);
        }
        // OCC version check
        if (expectedVersion !== state.version) {
            throw new GovernedFederatedKnowledgeStateConcurrencyError(`OCC version mismatch: expected ${expectedVersion}, current version is ${state.version}`, state.tenantId, stateId);
        }
        // Capacity checks
        const entryCount = Object.keys(state.entries).length;
        if (!state.entries[entry.knowledgeId] && entryCount >= MAX_KNOWLEDGE_STATE_SIZE) {
            throw new GovernedFederatedKnowledgeStateBudgetError(`Knowledge state reached MAX_KNOWLEDGE_STATE_SIZE limit (${MAX_KNOWLEDGE_STATE_SIZE})`, state.tenantId, stateId);
        }
        ...
        if (!state.entries[entry.knowledgeId] && fedEntries.length >= MAX_KNOWLEDGE_ENTRIES_PER_FEDERATION) {
            throw new GovernedFederatedKnowledgeStateBudgetError(`Federation reached MAX_KNOWLEDGE_ENTRIES_PER_FEDERATION limit (${MAX_KNOWLEDGE_ENTRIES_PER_FEDERATION})`, state.tenantId, stateId);
        }
        ...
        if (!state.entries[entry.knowledgeId] && agentEntries.length >= MAX_KNOWLEDGE_ENTRIES_PER_AGENT) {
            throw new GovernedFederatedKnowledgeStateBudgetError(`Agent reached MAX_KNOWLEDGE_ENTRIES_PER_AGENT limit (${MAX_KNOWLEDGE_ENTRIES_PER_AGENT})`, state.tenantId, stateId);
        }
        const now = Date.now();
     ...
     */
    transitionStateStatus(stateId, newStatus, tenantId, sessionId) {
        const state = this.states.get(stateId);
        if (!state) {
            throw new GovernedFederatedKnowledgeStateValidationError(`Knowledge state '${stateId}' not found`);
        }
        if (state.tenantId !== tenantId) {
            throw new GovernedFederatedKnowledgeStateTenantIsolationError(`Tenant mismatch: '${state.tenantId}' != '${tenantId}'`, tenantId);
        }
        if (state.sessionId !== sessionId) {
            throw new GovernedFederatedKnowledgeStateSessionIsolationError(`Session mismatch: '${state.sessionId}' != '${sessionId}'`, tenantId);
        }
        // Terminal state invariant
        if (state.status === 'HALTED_BY_USER_STOP' ||
            state.status === 'HALTED_BY_EMERGENCY_STOP' ||
            state.status === 'INVALIDATED' ||
            state.status === 'COMPLETED' ||
            state.status === 'FAILED') {
            throw new GovernedFederatedKnowledgeStateValidationError(`Cannot transition out of terminal status '${state.status}'`);
        }
        const now = Date.now();
        const nextVersion = state.version + 1;
        const base = {
