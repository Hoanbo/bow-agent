...
import { GovernedFederatedKnowledgeStateUserStopError, GovernedFederatedKnowledgeStateEmergencyStopError, GovernedFederatedKnowledgeStateTenantIsolationError, GovernedFederatedKnowledgeStateSessionIsolationError, } from './GovernedFederatedKnowledgeStateTypes.js';
export class FederatedKnowledgeSecurityBoundary {
     ...
     */
    assertStopInactive(checkpoint, tenantId, stateId) {
        // 1. EMERGENCY_STOP has absolute priority
        if (this.emergencyStopActive) {
            throw new GovernedFederatedKnowledgeStateEmergencyStopError(`EMERGENCY_STOP active at checkpoint '${checkpoint}'`, tenantId, stateId);
        }
        // 2. USER_STOP has immediate priority
        if (this.userStopActive) {
            throw new GovernedFederatedKnowledgeStateUserStopError(`USER_STOP active at checkpoint '${checkpoint}'`, tenantId, stateId);
        }
    }
    /**
    ...
    assertTenantSafe(tenantId) {
        ...
        if (tenantId.includes('..') || tenantId.includes('/') || tenantId.includes('\\') || tenantId.includes('\0')) {
            throw new GovernedFederatedKnowledgeStateTenantIsolationError(`Path traversal or invalid characters in tenant ID: '${tenantId}'`, tenantId);
        }
        ...
        if (reservedWindowsNames.includes(tenantId.toUpperCase())) {
            throw new GovernedFederatedKnowledgeStateTenantIsolationError(`Reserved Windows device name rejected as tenant ID: '${tenantId}'`, tenantId);
        }
    }
    /**
    ...
    assertSessionSafe(sessionId) {
        ...
        if (sessionId.includes('..') || sessionId.includes('\0')) {
            throw new GovernedFederatedKnowledgeStateSessionIsolationError(`Invalid characters in session ID: '${sessionId}'`);
        }
    }
    /**
    ...
    assertTenantIsolation(primaryTenantId, ...otherTenantIds) {
        ...
        for (const tid of otherTenantIds) {
            ...
            if (tid !== primaryTenantId) {
                throw new GovernedFederatedKnowledgeStateTenantIsolationError(`Tenant isolation violation: '${tid}' does not match primary '${primaryTenantId}'`, primaryTenantId);
            }
        }
    }
    /**
    ...
    assertSessionIsolation(primarySessionId, ...otherSessionIds) {
        ...
        for (const sid of otherSessionIds) {
            ...
            if (sid !== primarySessionId) {
                throw new GovernedFederatedKnowledgeStateSessionIsolationError(`Session isolation violation: '${sid}' does not match primary '${primarySessionId}'`);
            }
        }
    }
}
