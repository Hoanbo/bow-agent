...
import { MAX_AUDIT_LOG_RECORDS_PER_SESSION, StrategicPolicyPersistenceError, StrategicPolicyEvolutionOCCConflictError, TenantIsolationViolationError, computeStrategicPolicyAuditHash, } from './GovernedStrategicPolicyEvolutionTypes';
...
export class StrategicPolicyDeliberationContinuityPersistenceBridge {
    ...
    validateIdentifier(id, label = 'Identifier') {
        ...
        if (id.includes('..') || id.includes('/') || id.includes('\\') || id.includes('\0')) {
            throw new TenantIsolationViolationError(`Path traversal or illegal character detected in ${label}: '${id}'`);
        }
        ...
        if (WINDOWS_RESERVED_NAMES.has(upper)) {
            throw new TenantIsolationViolationError(`Windows reserved name detected in ${label}: '${id}'`);
        }
        // Only allow alphanumeric, hyphens, underscores, and colons
        if (!/^[a-zA-Z0-9_\-:]+$/.test(id)) {
            throw new TenantIsolationViolationError(`Invalid character set in ${label}: '${id}'. Allowed: alphanumeric, underscore, hyphen, colon.`);
        }
    }
    toSafeFilename(id) {
    ...
    persistProposalAtomically(tenantId, proposal, expectedVersion) {
        ...
        if (proposal.tenantId !== tenantId) {
            throw new TenantIsolationViolationError(`Tenant mismatch: proposal belongs to tenant '${proposal.tenantId}', attempted persistence under '${tenantId}'`);
        }
        ...
        if (fs.existsSync(targetPath)) {
            try {
                ...
                if (existingData.version !== undefined && existingData.version !== expectedVersion) {
                    throw new StrategicPolicyEvolutionOCCConflictError(`OCC version conflict for proposal '${proposal.proposalId}': expected version ${expectedVersion}, found ${existingData.version}`);
                }
            }
            catch (err) {
        ...
        if (readbackChecksum !== checksum) {
            ...
            throw new StrategicPolicyPersistenceError(`Readback checksum mismatch for proposal '${proposal.proposalId}': expected ${checksum}, got ${readbackChecksum}`);
        }
    ...
    loadProposal(tenantId, proposalId) {
        this.validateIdentifier(tenantId, 'TenantId');
        ...
        try {
            ...
            if (parsed.tenantId !== tenantId) {
                throw new TenantIsolationViolationError(`Tenant boundary violation: loaded proposal tenant '${parsed.tenantId}' does not match requested tenant '${tenantId}'`);
            }
            return parsed;
        }
        catch (err) {
            if (err instanceof TenantIsolationViolationError)
                throw err;
            throw new StrategicPolicyPersistenceError(`Failed to parse proposal file '${filePath}': ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    ...
    persistDossierAtomically(tenantId, dossier, expectedVersion) {
        this.validateIdentifier(tenantId, 'TenantId');
        this.validateIdentifier(dossier.dossierId, 'DossierId');
        if (dossier.tenantId !== tenantId) {
            throw new TenantIsolationViolationError(`Tenant mismatch: dossier belongs to tenant '${dossier.tenantId}', attempted persistence under '${tenantId}'`);
        }
        ...
        if (fs.existsSync(targetPath)) {
            try {
                ...
                const existingData = JSON.parse(rawExisting);
                if (existingData.version !== undefined && existingData.version !== expectedVersion) {
                    throw new StrategicPolicyEvolutionOCCConflictError(`OCC version conflict for dossier '${dossier.dossierId}': expected version ${expectedVersion}, found ${existingData.version}`);
                }
            }
            catch (err) {
                if (err instanceof StrategicPolicyEvolutionOCCConflictError) {
        ...
        if (readbackChecksum !== checksum) {
            if (fs.existsSync(tempPath))
                fs.unlinkSync(tempPath);
            throw new StrategicPolicyPersistenceError(`Readback checksum mismatch for dossier '${dossier.dossierId}': expected ${checksum}, got ${readbackChecksum}`);
        }
        const bakPath = `${targetPath}.bak`;
        if (fs.existsSync(targetPath)) {
            fs.copyFileSync(targetPath, bakPath);
    ...
    loadDossier(tenantId, dossierId) {
        this.validateIdentifier(tenantId, 'TenantId');
        ...
        try {
            ...
            const parsed = JSON.parse(content);
            if (parsed.tenantId !== tenantId) {
                throw new TenantIsolationViolationError(`Tenant boundary violation: loaded dossier tenant '${parsed.tenantId}' does not match requested tenant '${tenantId}'`);
            }
            return parsed;
        }
        catch (err) {
            if (err instanceof TenantIsolationViolationError)
                throw err;
            throw new StrategicPolicyPersistenceError(`Failed to parse dossier file '${filePath}': ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    ...
    emitAuditEvent(eventType, tenantId, sessionId, details = {}, proposalId, dossierId) {
        this.validateIdentifier(tenantId, 'TenantId');
        ...
        if (chain.length >= MAX_AUDIT_LOG_RECORDS_PER_SESSION) {
            throw new StrategicPolicyPersistenceError(`Audit log record ceiling reached: max ${MAX_AUDIT_LOG_RECORDS_PER_SESSION} records per session.`);
        }
    ...
    persistAuditLedger(tenantId, sessionId) {
        this.validateIdentifier(tenantId, 'TenantId');
