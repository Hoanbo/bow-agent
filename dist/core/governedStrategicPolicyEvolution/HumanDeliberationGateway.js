...
export class HumanDeliberationGateway {
    ...
    compileDossier(proposal, impact, simulation, compliance) {
        if (proposal.status !== 'DOSSIER_COMPILING' && proposal.status !== 'INVARIANT_REVIEW') {
            throw new StrategicPolicySecurityCheckpointError(`Precondition failed: Proposal in '${proposal.status}' cannot be compiled into dossier`);
        }
        if (!compliance.passed) {
            throw new StrategicPolicySecurityCheckpointError(`Constitutional violation: Cannot compile dossier for proposal with unresolved violations: [${compliance.violations.join(', ')}]`);
        }
        ...
        if (Buffer.byteLength(serialized, 'utf8') > MAX_DELIBERATION_DOSSIER_SIZE_BYTES) {
            throw new StrategicPolicySecurityCheckpointError(`Dossier size ceiling exceeded: ${Buffer.byteLength(serialized, 'utf8')} bytes > ${MAX_DELIBERATION_DOSSIER_SIZE_BYTES}`);
        }
    ...
    getOrCreateTenantDossiers(tenantId) {
