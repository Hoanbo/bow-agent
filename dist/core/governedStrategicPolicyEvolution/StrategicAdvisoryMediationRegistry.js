...
import { MAX_POLICY_PROPOSALS_PER_TENANT, computePolicyEvolutionProposalHash, computeAdvisoryMediationRecordHash, StrategicPolicyEvolutionBaseError, } from './GovernedStrategicPolicyEvolutionTypes.js';
export class StrategicAdvisoryMediationRegistry {
    proposalsByTenant = new Map();
    mediationRecordsByTenant = new Map();
    ...
    assertPrototypeSafety(obj) {
        if (obj === null || typeof obj !== 'object') {
            return;
        }
        for (const key of Object.keys(obj)) {
            if (key === '__proto__' || key === 'constructor' || key === 'prototype') {
                throw new StrategicPolicyEvolutionBaseError(`Security violation: Prototype pollution attempt detected key '${key}'`);
            }
            this.assertPrototypeSafety(obj[key]);
        }
    }
    ...
    sanitizeText(text) {
        ...
        for (const reg of this.injectionRegexes) {
            if (reg.test(current)) {
                throw new StrategicPolicyEvolutionBaseError(`Adversarial prompt-injection pattern quarantined: '${current.substring(0, 60)}...'`);
            }
        }
    ...
    ingestAndAdmit(input) {
        this.assertPrototypeSafety(input);
        if (!input.tenantId || !input.sessionId || !input.missionId || !input.sourceRecommendationId) {
            throw new StrategicPolicyEvolutionBaseError('Malformed advisory input: tenantId, sessionId, missionId, and sourceRecommendationId are required');
        }
        ...
        if (input.advisoryOnly === false) {
            throw new StrategicPolicyEvolutionBaseError('Constitutional violation: Incoming recommendation claims non-advisory authority');
        }
        const tenantMap = this.getOrCreateTenantMap(input.tenantId);
        if (tenantMap.size >= MAX_POLICY_PROPOSALS_PER_TENANT) {
            throw new StrategicPolicyEvolutionBaseError(`Tenant quota exceeded: maximum ${MAX_POLICY_PROPOSALS_PER_TENANT} proposals per tenant reached`);
        }
        const { sanitized: sanitizedJustification, scrubbed: justScrubbed } = this.sanitizeText(input.justification);
        const sanitizedChanges = (input.proposedChanges || []).map((delta) => {
    ...
    getOrCreateMediationMap(tenantId) {
