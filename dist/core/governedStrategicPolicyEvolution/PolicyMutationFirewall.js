...
// ============================================================================
import { PolicyMutationViolationError, computeDeliberationDossierHash, } from './GovernedStrategicPolicyEvolutionTypes.js';
export class PolicyMutationFirewall {
    ...
    verifyPdpHandoff(handoffPackage, dossier) {
        ...
        if (handoffPackage.isAuthoritativePolicy !== false) {
            throw new PolicyMutationViolationError('Firewall violation: MS-1.5.19 handoff package cannot claim authoritative policy status');
        }
        ...
        if (!handoffPackage.humanApprovalCertified) {
            throw new PolicyMutationViolationError('Firewall violation: Cannot hand off to PDP without explicit human approval certification');
        }
        ...
        if (handoffPackage.dossierProvenanceHash !== recomputedHash) {
            throw new PolicyMutationViolationError('Firewall violation: Dossier provenance hash mismatch between handoff and deliberation record');
        }
        ...
        if (!dossier.humanDecision || dossier.humanDecision.decision !== 'APPROVE') {
            throw new PolicyMutationViolationError('Firewall violation: Cannot construct PDP handoff for unapproved or rejected proposal');
        }
    }
    ...
    assertNoDirectPolicyMutation(targetPath) {
        const normalized = targetPath.replace(/\\/g, '/').toLowerCase();
        if (!normalized.includes('data/partitions_governed_policy_deliberation')) {
            throw new PolicyMutationViolationError(`Firewall violation: MS-1.5.19 is strictly prohibited from writing to production policy path '${targetPath}'`);
        }
    }
    ...
    assertNoLeaseCreationOrExpansion() {
        throw new PolicyMutationViolationError('Firewall violation: MS-1.5.19 does not possess AutonomyLeaseEngine privileges to create or expand leases');
    }
}
