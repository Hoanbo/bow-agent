...
import { MAX_SIMULATION_SCENARIOS_PER_ROUND, MAX_SIMULATION_DEPTH, MAX_CONCURRENT_SIMULATIONS, SimulationCeilingExceededError, computeCounterfactualSimulationHash, } from './GovernedStrategicPolicyEvolutionTypes.js';
export class CounterfactualPolicySimulationEngine {
    ...
    simulate(proposal, requestedScenarios = 3, recursionDepth = 2) {
        ...
        if (this.activeSimulationCount >= MAX_CONCURRENT_SIMULATIONS) {
            throw new SimulationCeilingExceededError(`Simulation concurrency limit reached: Maximum ${MAX_CONCURRENT_SIMULATIONS} concurrent simulations permitted`);
        }
        ...
        if (requestedScenarios > MAX_SIMULATION_SCENARIOS_PER_ROUND) {
            throw new SimulationCeilingExceededError(`Scenario limit exceeded: Requested ${requestedScenarios}, maximum permitted is ${MAX_SIMULATION_SCENARIOS_PER_ROUND}`);
        }
        ...
        if (recursionDepth > MAX_SIMULATION_DEPTH) {
            throw new SimulationCeilingExceededError(`Simulation depth exceeded: Requested depth ${recursionDepth}, maximum permitted is ${MAX_SIMULATION_DEPTH}`);
        }
