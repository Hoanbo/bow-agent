...
// ============================================================================
import { MAX_ACTIVE_DELIBERATION_SESSIONS, InvalidProposalLifecycleTransitionError, StrategicPolicyEvolutionOCCConflictError, computePolicyEvolutionProposalHash, StrategicPolicySecurityCheckpointError, } from './GovernedStrategicPolicyEvolutionTypes.js';
export class StrategicPolicyEvolutionDeliberationEngine {
    ...
    openSession(tenantId, sessionId) {
        const tenantSessions = this.getOrCreateTenantSessions(tenantId);
        if (tenantSessions.size >= MAX_ACTIVE_DELIBERATION_SESSIONS) {
            throw new StrategicPolicySecurityCheckpointError(`Session limit reached: Maximum ${MAX_ACTIVE_DELIBERATION_SESSIONS} active deliberation sessions per tenant`);
        }
        const now = Date.now();
        const session = {
            sessionId,
    ...
    transitionState(proposal, targetState, expectedVersion, emergencyStopSignaled, userStopSignaled) {
        ...
        if (!allowed || allowed.length === 0) {
            throw new InvalidProposalLifecycleTransitionError(`Lifecycle violation: Cannot transition from terminal state '${proposal.status}' to '${targetState}'`);
        }
        if (!allowed.includes(targetState)) {
            throw new InvalidProposalLifecycleTransitionError(`Illegal transition from '${proposal.status}' to '${targetState}'. Permitted: [${allowed.join(', ')}]`);
        }
        ...
        if (expectedVersion !== undefined && proposal.version !== expectedVersion) {
            throw new StrategicPolicyEvolutionOCCConflictError(`OCC conflict: Expected version ${expectedVersion}, but current proposal version is ${proposal.version}`);
        }
    ...
    closeSession(tenantId, sessionId) {
    ...
    getOrCreateTenantSessions(tenantId) {
