...
const safeTurns = (turns = []) => turns.map(turn => ({ id: turn.id, sender: turn.sender, content: turn.content }));
...
export function createDecisionContext(input, semanticIntent, currentState = 'MEMORY_LOADED') {
    if (SECRET.test(semanticIntent.sourceText) || typeof input.userId !== 'string' || typeof input.sessionId !== 'string')
        throw new Error('Unsafe decision context');
    const conversationContext = safeTurns(input.context?.recentTurns);
    const relevantMemory = safeTurns(input.workingMemory);
    if ([...conversationContext, ...relevantMemory].some(turn => SECRET.test(turn.content)))
        throw new Error('Unsafe decision context');
