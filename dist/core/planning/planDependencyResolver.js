...
import { PLANNER_LIMITS, PlanBudgetExceededError, PlanDagCycleError, PlanValidationError, } from './governedPlanningTypes.js';
export class PlanDependencyResolver {
     ...
     */
    static resolve(steps, rawEdges, config) {
        ...
        const reverseAdjacency = new Map(); // to -> [from] (dependencies)
        ...
        if (rawEdges && rawEdges.length > 0) {
            if (rawEdges.length > maxEdges) {
                throw new PlanBudgetExceededError(`Plan dependency edge count (${rawEdges.length}) exceeds maximum limit (${maxEdges})`);
            }
            for (const edge of rawEdges) {
                ...
                if (edge.fromStepId === edge.toStepId) {
                    throw new PlanValidationError(`Self-dependency detected: Step '${edge.fromStepId}' cannot depend on itself`);
                }
                // Existence check
                if (!stepMap.has(edge.fromStepId)) {
                    throw new PlanValidationError(`Dependency edge references non-existent prerequisite stepId: '${edge.fromStepId}'`);
                }
                if (!stepMap.has(edge.toStepId)) {
                    throw new PlanValidationError(`Dependency edge references non-existent dependent stepId: '${edge.toStepId}'`);
                }
                // Duplicate edge check
                const edgeKey = `${edge.fromStepId}->${edge.toStepId}`;
                if (edgeSet.has(edgeKey)) {
                    throw new PlanValidationError(`Duplicate dependency edge detected: '${edge.fromStepId}' -> '${edge.toStepId}'`);
                }
                edgeSet.add(edgeKey);
        ...
        while (zeroInDegreeQueue.length > 0) {
            ...
            const sortedNeighbors = [...neighbors].sort((a, b) => (stepMap.get(a)?.sequence ?? 0) - (stepMap.get(b)?.sequence ?? 0));
        ...
        if (topologicalOrder.length !== nodeIds.length) {
            ...
            throw new PlanDagCycleError(`Dependency cycle detected in plan graph involving steps: [${cycleNodes.join(', ')}]`, cycleNodes);
        }
