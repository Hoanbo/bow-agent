...
import { createRemediationExecutionId, } from './remediationTypes.js';
...
export class RemediationExecutionEngine {
        ...
        try {
            ...
            if (customAdapters && customAdapters[plan.actionClass]) {
                ...
                switch (plan.actionClass) {
                    ...
                    default:
                        throw new RemediationExecutionError('UNSUPPORTED_ACTION_CLASS', `Remediation action class "${plan.actionClass}" is not supported.`);
                }
            }
    ...
    // ---------------------------------------------------------------------------
    async handleConfigSync(plan, snapshot, baseDirectory, isUserStopActive) {
        if (isUserStopActive())
            throw new RemediationExecutionError('USER_STOP', 'Halted by USER_STOP');
    ...
    async handleRollbackAction(plan, snapshot, baseDirectory, isUserStopActive) {
        if (isUserStopActive())
            throw new RemediationExecutionError('USER_STOP', 'Halted by USER_STOP');
        const appliedChanges = [];
    ...
    async handleProcessRestart(plan, isUserStopActive) {
        if (isUserStopActive())
            throw new RemediationExecutionError('USER_STOP', 'Halted by USER_STOP');
        // Emulated typed capability adapter for process restart (zero child_process primitives)
        const serviceName = plan.parameters.serviceName || plan.targetId;
        return {
    ...
    async handleTrafficDrain(plan, isUserStopActive) {
        if (isUserStopActive())
            throw new RemediationExecutionError('USER_STOP', 'Halted by USER_STOP');
        const drainPercentage = plan.parameters.drainPercentage ?? 100;
        return {
    ...
    computeExecutionSha256(executionId, plan, appliedChanges, state) {
