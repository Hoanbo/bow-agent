...
export class PostMitigationVerificationEngine {
    static DEFAULT_VERIFICATION_WINDOW_MS = 30000;
    /**
     ...
     */
    async verifyMitigation(options) {
        const { plan, observabilityRuntime, sessionId, verificationWindowMs = PostMitigationVerificationEngine.DEFAULT_VERIFICATION_WINDOW_MS, baselineLatencyP95 = 200, } = options;
        ...
        const healthResult = observabilityRuntime.evaluateHealth(sessionId, latestSample ? { ...latestSample.metrics, targetId: plan.targetId, sourceId: latestSample.sourceId } : undefined);
        ...
        if (!errorRatePassed) {
            violationDetails.push(`Error rate ${(observedErrorRate * 100).toFixed(2)}% exceeds mandatory threshold of 1.0%`);
        }
        ...
        if (!latencyPassed) {
            violationDetails.push(`P95 latency ${observedP95.toFixed(1)}ms exceeds baseline + 10% threshold (${maxAllowedLatency.toFixed(1)}ms)`);
        }
        ...
        if (!healthPassed) {
            violationDetails.push(`Health state "${healthResult.healthState}" is unacceptable (score: ${healthResult.healthScore})`);
        }
        ...
        const verificationSha256 = this.computeVerificationSha256(plan.planId, passed, observedErrorRate, observedP95, violatedChecks.length, driftPassed ? 'NO_DRIFT' : 'DRIFT_DETECTED');
        return {
            planId: plan.planId,
            verified: passed,
            passed,
            ...
            driftStatus: driftPassed ? 'NO_DRIFT' : activeDrifts[0]?.classification ?? 'CRITICAL_DRIFT',
    ...
    computeVerificationSha256(planId, passed, errorRate, latencyP95, violationsCount, drift) {
