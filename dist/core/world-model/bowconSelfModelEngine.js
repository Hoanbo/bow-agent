...
import { isAuthoritativeFactProvenance, assertValidEpistemicPromotion, } from './worldModelTypes.js';
export class BowconSelfModelEngine {
    _whatIKnow = new Map();
    _whatIObserved = new Map();
    _whatIInferred = new Map();
    _whatIRemember = new Map();
    _whatIExpect = new Map();
    _whatIAssume = new Map();
    _whatIDoNotKnow = new Map();
    _whatICannotMeasure = new Map();
    _whatICannotExecute = new Map();
    _whatIAmNotAuthorizedToExecute = new Map();
    _whatIHaveVerified = new Map();
    _whatIHaveNotVerified = new Map();
    // ---------------------------------------------------------------------------
    // Observation & Telemetry (Authoritative Empirical Input)
    // ---------------------------------------------------------------------------
    recordObservation(key, value, source = 'host_observation') {
        const item = {
    ...
    recordTelemetry(key, value, source = 'host_telemetry') {
        const item = {
    ...
    recordOwnerConfirmed(key, value) {
        const item = {
    ...
    // ---------------------------------------------------------------------------
    recordInference(key, value, premises, confidence = 0.75) {
        const item = {
    ...
    recordAssumption(key, value, rationale) {
        const item = {
    ...
    recordExpectation(key, value, context = '') {
        const item = {
    ...
    recordMemory(key, value, memoryId) {
        const item = {
    ...
    recordUnmeasurable(metricName, reason) {
        const item = {
    ...
    recordUnexecutable(capabilityId, reason) {
        const item = {
    ...
    recordUnauthorized(capabilityId, requiredPermission) {
        const item = {
    ...
    // ---------------------------------------------------------------------------
    recordVerifiedExecution(key, value, verificationHash) {
        const item = {
    ...
    recordUnverifiedOutput(key, value, actionId) {
        const item = {
    ...
    // ---------------------------------------------------------------------------
    attemptPromoteFact(key, targetProvenance) {
        const candidate = this._whatIInferred.get(key) ||
    ...
    getProvenance(key) {
        return (this._whatIKnow.get(key)?.provenance ||
