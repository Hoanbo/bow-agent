...
import { EPISODIC_MEMORY_AUDIT_DOMAIN, DuplicateMemoryError, } from './episodicMemoryTypes.js';
import { globalEpisodicMemoryValidator, } from './episodicMemoryValidator.js';
import { globalEpisodicMemoryExecutionGate, } from './episodicMemoryExecutionGate.js';
import { globalEpisodicMemoryStore, } from './episodicMemoryStore.js';
import { globalEpisodicMemorySynthesizer, } from './episodicMemorySynthesizer.js';
...
export class EpisodicMemoryRuntime {
    ...
    recordAudit(eventType, tenantId, metadata) {
        ...
        this.auditLedger.record({
            ...
            resultHash: metadata.memoryProvenanceHash ?? '',
            actor: {
    ...
    async recordMemory(request) {
        ...
        this.validator.validateBindings(commit, task, request.context, request.expectedTaskVersion);
        ...
        if (this.store.hasMemory(task.tenantId, memoryId)) {
            ...
            throw new DuplicateMemoryError(`Memory record '${memoryId}' has already been persisted for tenant '${task.tenantId}'. Replay rejected.`, { memoryId, commitId: commit.commitId, taskId: task.taskId, tenantId: task.tenantId });
        }
        // 7. Prepare State Payload & Memory Provenance
        const recordedAt = new Date().toISOString();
        const stateDelta = commit.committedState ?? {};
        ...
        const finalStatus = synthesis ? 'SYNTHESIZED' : 'RECORDED';
        const result = {
            memoryId,
            commitId: commit.commitId,
            taskId: task.taskId,
     ...
     */
    synthesizeFromHistory(tenantId, commitRecord) {
