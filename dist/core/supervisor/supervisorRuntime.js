...
export class SupervisorRuntime {
    ...
    _safeStopReason;
    _activeAnomalies = [];
    _activeDiagnoses = new Map();
    _activePlans = new Map();
    _totalRecoveriesAttempted = 0;
    _totalRecoveriesSucceeded = 0;
    _totalRecoveriesFailed = 0;
    _startTime = Date.now();
    constructor() {
    ...
    triggerSafeStop(reason) {
        this.activateSafeStop(reason);
    }
    resetSafeStop(operatorToken) {
        if (!operatorToken || typeof operatorToken !== 'string') {
            throw new SupervisorError('AUTHORIZATION_REQUIRED', 'Valid operator token required to reset supervisor SAFE_STOP.');
        }
    ...
    observe() {
        if (this._safeStop) {
            throw new SupervisorError('SAFE_STOP_TRIGGERED', `Operation blocked: SAFE_STOP is active (${this._safeStopReason}).`);
        }
    ...
    async executeRecovery(plan, diagnosis, options) {
        ...
        if (this._safeStop) {
            throw new SupervisorError('SAFE_STOP_TRIGGERED', `Autonomous execution prohibited: SAFE_STOP is active (${this._safeStopReason}).`);
        }
        // 2. Policy Check
        ...
        if (policyDecision.requiresHumanGate && !options?.authorizationToken && !options?.isDryRun) {
            throw new SupervisorError('AUTHORIZATION_REQUIRED', `Recovery plan "${plan.planId}" requires explicit human authorization token.`);
        }
        if (options?.authorizationToken) {
            ...
            if (!valResult.valid) {
                throw new SupervisorError('AUTHORIZATION_REQUIRED', `Authorization token invalid: ${valResult.reason}`);
            }
        }
        // 3. Retry Bounding & Escalation Check
        if (globalSupervisorEscalation.shouldEscalate(plan)) {
            const esc = globalSupervisorEscalation.escalate(plan, diagnosis, `Exceeded maximum recovery retry limit (${plan.maxAttempts}).`);
