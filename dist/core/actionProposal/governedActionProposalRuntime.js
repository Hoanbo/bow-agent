...
import { ACTION_PROPOSAL_AUDIT_DOMAIN, ProposalUserStopError, StaleProposalError, CrossTenantProposalError, ProposalValidationError, ProposalDeniedError, } from './actionProposalTypes.js';
export class GovernedActionProposalRuntime {
     ...
     */
    assertUserStopNotActive(checkpointName, context) {
        const isStopped = (this.isUserStopActiveFn && this.isUserStopActiveFn()) ||
            globalMasterHumanAuthority.isUserStopActive;
        if (isStopped) {
            this.recordAudit({
                eventType: 'ACTION_USER_STOP_ABORTED',
                toolName: context?.actionType || 'UNKNOWN_ACTION',
                policyDecision: 'DENY',
                details: { checkpoint: checkpointName, reason: 'USER_STOP_ACTIVE', ...context },
            });
            throw new ProposalUserStopError(`EMERGENCY_USER_STOP_ACTIVE: Processing aborted at ${checkpointName} due to active Master Human USER_STOP.`, { checkpoint: checkpointName, ...context });
        }
    }
    /**
     ...
     */
    async governProposal(request, injectedTask) {
        ...
        if (!pdpDecision.allowed || pdpDecision.action === 'DENY') {
            ...
            throw new ProposalDeniedError(`PDP_DENIED: PolicyDecisionPoint rejected action "${proposal.actionType}". Reason: ${pdpDecision.reason}`, { proposalId: proposal.proposalId, reason: pdpDecision.reason });
        }
        // ------------------------------------------------------------------------
     ...
     */
    async resolveAuthoritativeTask(taskId, tenantId, injectedTask) {
        if (injectedTask) {
            if (injectedTask.taskId !== taskId) {
                throw new ProposalValidationError(`TASK_ID_MISMATCH: Injected task "${injectedTask.taskId}" does not match requested taskId "${taskId}".`);
            }
            if (injectedTask.tenantId !== tenantId) {
                throw new CrossTenantProposalError(`CROSS_TENANT_TASK: Injected task tenant "${injectedTask.tenantId}" does not match plan tenant "${tenantId}".`);
            }
            return injectedTask;
        }
        if (this.taskStore) {
            ...
            if (task.tenantId !== tenantId) {
                throw new CrossTenantProposalError(`CROSS_TENANT_TASK: Stored task tenant "${task.tenantId}" does not match plan tenant "${tenantId}".`);
            }
            return task;
        }
        throw new ProposalValidationError('TASK_RESOLUTION_FAILED: Neither an injected task nor an AgentTaskStore was provided to the runtime.');
    }
    /**
     * Verifies that the task version has not changed.
     */
    async assertTaskVersionFresh(task, expectedVersion) {
        if (this.taskStore) {
            ...
            if (current && current.version !== expectedVersion) {
                ...
                throw new StaleProposalError(`STALE_TASK_VERSION: Task version has changed from ${expectedVersion} to ${current.version}. Proposal rejected.`);
            }
        }
    }
    /**
    ...
    recordAudit(params) {
        try {
            ...
            this.auditLedger.record({
                timestamp,
                actor: {
                    userId: sanitizedDetails.userId || 'action_proposal_runtime',
                    role: 'agent',
                    channel: 'governance',
                },
