...
import { PolicyCandidateSynthesisEngine } from './policyCandidateSynthesisEngine.js';
...
import { globalPolicyEvolutionPlanningAuditEngine } from './policyEvolutionPlanningAuditEngine.js';
...
export class PolicyEvolutionPlanningRuntime {
     ...
     * 6. Audit logging
     */
    planAndSynthesize(intake, options) {
        this.assertUserStopInactive();
        this.validateTenant(intake.tenantPartition);
        // 1. Plan creation
        const plan = this.planEngine.createEvolutionPlan(intake);
        this.provenanceEngine.appendEvent(intake.tenantPartition, intake.intakeId, 'EVOLUTION_PLAN_CREATED', { planId: plan.planId, details: { planType: plan.planType } });
        ...
        this.provenanceEngine.appendEvent(intake.tenantPartition, intake.intakeId, 'CANDIDATE_DRAFT_SYNTHESIZED', { planId: plan.planId, candidateDraftId: candidateDraft.candidateDraftId });
        this.auditEngine.recordEvent({
        ...
        this.provenanceEngine.appendEvent(intake.tenantPartition, intake.intakeId, 'HUMAN_REVIEW_REQUIREMENT_FORMULATED', {
            planId: plan.planId,
            candidateDraftId: candidateDraft.candidateDraftId,
     ...
     */
    verifyProvenanceIntegrity(tenantPartition, intakeId) {
        this.assertUserStopInactive();
