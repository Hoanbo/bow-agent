 ...
 */
export function extractGenericDuration(text) {
    if (!text)
        return undefined;
 ...
 */
export function resolveStructuredIntent(text) {
    const norm = normalizeText(text);
...
let activeDomainIntentExtension = null;
/**
 ...
 */
export function registerDomainIntentExtension(ext) {
    activeDomainIntentExtension = ext;
}
export function getDomainIntentExtension() {
    return activeDomainIntentExtension;
}
export function matchPlanByDuration(plans, durationQuery, fullQuery) {
...
export function extractDeferredBuyContext(text) {
...
export function extractShopDuration(text) {
 ...
 */
export function detectAdminIntent(text) {
    ...
    const norm = lower
        ...
        .replace(/\s+/g, ' ')
        .trim();
    // 1. Pending Fulfillment Queue (Đơn chờ bàn giao - Ưu tiên cao nhất)
    ...
    // 2. Daily Summary (Báo cáo tổng quan vận hành hôm nay)
    if (norm.includes('hom nay shop co gi can toi xu ly') ||
    ...
    // 3. Task Prioritization (Thứ tự ưu tiên xử lý trong ngày)
    if (norm.includes('xu ly gi truoc') ||
    ...
    if (norm.includes('kiem tra don') ||
        ...
        ((norm.includes('bow-ord') || /#bow-ord/i.test(text) || norm.includes('ord-')) &&
    ...
    if (norm.includes('kiem tra khach hang') ||
    ...
    if (norm.includes('ban chay nhat') ||
 ...
 */
export function resolveMultiIntent(text, agentContext) {
    const lower = text.toLowerCase().trim();
    ...
    // --------------------------------------------------------------------------
    const isGreetingWord = cleanLower === 'chào' ||
        cleanLower === 'chào bạn' ||
    ...
    // --------------------------------------------------------------------------
    const isThanks = cleanLower === 'cảm ơn' ||
    ...
    const isConfirmation = cleanLower === 'ok' ||
        cleanLower === 'oke' ||
        cleanLower === 'oki' ||
        cleanLower === 'ok bạn' ||
        cleanLower === 'oke shop' ||
    ...
    const isGoodbye = cleanLower === 'tạm biệt' ||
        cleanLower === 'bye' ||
        cleanLower === 'bye bye' ||
        cleanLower === 'hẹn gặp lại' ||
        cleanLower === 'gặp lại sau' ||
        cleanLower === 'chào tạm biệt';
    const isNegativeOrCancel = cleanLower === 'thôi' ||
    ...
    const isBotIdentity = cleanLower === 'bạn là ai' ||
    ...
    const isAdminRole = agentContext?.role === 'admin' || agentContext?.role === 'owner' || agentContext?.isAdmin === true;
    ...
    // --------------------------------------------------------------------------
    if (cleanLower.includes('bạn có thể làm gì') ||
    ...
    // --------------------------------------------------------------------------
    const hasBuy = lower.startsWith('mua') ||
    ...
    const hasWallet = cleanLower === 'nap' ||
    ...
    const hasOrderQuery = cleanLower.includes('don hang') ||
    ...
    const hasExpiring = cleanLower.includes('sắp hết hạn') ||
    ...
    const hasRenew = lower.includes('gia hạn') ||
    ...
    const hasTicket = cleanLower.includes('ticket') ||
        cleanLower.includes('phiếu hỗ trợ');
    const hasWarranty = lower.includes('bảo hành') ||
        lower.includes('hỏng') ||
    ...
    const hasCoupon = lower.includes('mã giảm giá') ||
    ...
    const hasCatalog = (cleanLower === 'xem danh mục' ||
        cleanLower === 'danh mục' ||
    ...
    const hasViewCategory = cleanLower === 'ai tools' ||
    ...
    // --------------------------------------------------------------------------
    if (hasExpiring)
        return { primaryIntent: 'EXPIRING_SOON', secondaryIntents: [] };
    if (hasRenew)
        return { primaryIntent: 'RENEW', secondaryIntents: [] };
    if (hasTicket)
        return { primaryIntent: 'TICKET', secondaryIntents: [] };
    if (hasWarranty)
        return { primaryIntent: 'WARRANTY', secondaryIntents: [] };
    if (hasOrderQuery)
        return { primaryIntent: 'ORDER_QUERY', secondaryIntents: [] };
    if (hasWallet)
        return { primaryIntent: 'WALLET', secondaryIntents: [] };
    if (hasCoupon)
        return { primaryIntent: 'COUPON', secondaryIntents: [] };
    if (hasBuy) {
        const deferred = extractDeferredBuyContext(text);
        return { primaryIntent: 'BUY', secondaryIntents: [], deferredContext: deferred };
    }
    if (hasCatalog)
        return { primaryIntent: 'CATALOG', secondaryIntents: [] };
    if (hasViewCategory)
        return { primaryIntent: 'VIEW_CATEGORY', secondaryIntents: [] };
    if (hasFaq)
        return { primaryIntent: 'FAQ', secondaryIntents: [] };
    if (hasGeneral)
        return { primaryIntent: 'GENERAL', secondaryIntents: [] };
    return { primaryIntent: 'PRODUCT_SEARCH', secondaryIntents: [] };
}
/**
 * Phân loại ý định từ câu nói của người dùng (tương thích ngược)
 */
export function resolveIntent(text, agentContext) {
 ...
 */
export function detectPluralDiscoveryIntent(rawText) {
    ...
    if (/có những .{1,30}? gì\b/i.test(lower) || /co nhung .{1,30}? gi\b/i.test(noAccent))
        return true;
    if (/có những .{1,30}? nào\b/i.test(lower) || /co nhung .{1,30}? nao\b/i.test(noAccent))
        return true;
    // Pattern 2: "các ... gì" / "các ... nào"
    if (/các .{1,30}? gì\b/i.test(lower) || /cac .{1,30}? gi\b/i.test(noAccent))
        return true;
    if (/các .{1,30}? nào\b/i.test(lower) || /cac .{1,30}? nao\b/i.test(noAccent))
        return true;
    // Pattern 3: "... thì có những app gì", "... thì có app nào"
    if (/thì có những\b/i.test(lower) || /thi co nhung\b/i.test(noAccent))
        return true;
    if (/thì có (app|tool|phần mềm|dịch vụ|sản phẩm)\b/i.test(lower))
        return true;
    if (/thi co (app|tool|phan mem|dich vu|san pham)\b/i.test(noAccent))
        return true;
    // Pattern 4: "có app nào", "có tool nào", "có sản phẩm nào"
    if (/\bcó (app|tool|phần mềm|dịch vụ|sản phẩm|công cụ).{0,20}? nào\b/i.test(lower))
        return true;
    if (/\bco (app|tool|phan mem|dich vu|san pham|cong cu).{0,20}? nao\b/i.test(noAccent))
        return true;
    // Pattern 5: "liệt kê app ...", "danh sách app ..."
    if (/\b(liệt kê|danh sách|list)\s+(app|tool|sản phẩm|công cụ|phần mềm)\b/i.test(lower))
        return true;
    if (/\b(liet ke|danh sach|list)\s+(app|tool|san pham|cong cu|phan mem)\b/i.test(noAccent))
        return true;
    // Pattern 6: "những app ..." / "các app ..."
    if (/\bnhững (app|tool|công cụ|phần mềm|sản phẩm|dịch vụ)\b/i.test(lower))
        return true;
    if (/\bnhung (app|tool|cong cu|phan mem|san pham|dich vu)\b/i.test(noAccent))
        return true;
    if (/\bcác (app|tool|công cụ|phần mềm|sản phẩm|dịch vụ)\b/i.test(lower))
        return true;
    if (/\bcac (app|tool|cong cu|phan mem|san pham|dich vu)\b/i.test(noAccent))
        return true;
    // Pattern 7: "có gì để ...", "có gì cho ..." (khi đi với capability)
    if (/\bcó gì (để|cho|giúp|hỗ trợ)\b/i.test(lower))
        return true;
    if (/\bco gi (de|cho|giup|ho tro)\b/i.test(noAccent))
        return true;
    // Pattern 8: "những lựa chọn nào", "các lựa chọn"
    if (/\b(những|các) lựa chọn\b/i.test(lower))
        return true;
    if (/\b(nhung|cac) lua chon\b/i.test(noAccent))
        return true;
    // Pattern 9: "có bao nhiêu" (khi hỏi số lượng sản phẩm)
    if (/\bcó bao nhiêu (app|tool|sản phẩm|công cụ)\b/i.test(lower))
        return true;
    if (/\bco bao nhieu (app|tool|san pham|cong cu)\b/i.test(noAccent))
        return true;
    // Pattern 10: "... app nào để ...", "... tool nào để ..."
    if (/\b(app|tool|phần mềm).{0,20}? nào (để|cho|giúp)\b/i.test(lower))
        return true;
    if (/\b(app|tool|phan mem).{0,20}? nao (de|cho|giup)\b/i.test(noAccent))
        return true;
    // Pattern 11: "có những công cụ AI nào", "những AI tool nào"
    if (/có những .{0,20}?(ai|tool|app).{0,20}? nào\b/i.test(lower))
        return true;
    // Pattern 12: "có app nào ... không?", "có tool nào ... không?", "có phần mềm nào ... không?"
    if (/\bcó\s+(?:những\s+|các\s+)?(app|tool|phần\s*mềm|dịch\s*vụ|sản\s*phẩm|công\s*cụ)\s+.{1,30}?\s*(?:không|\?|$)/i.test(lower))
        return true;
    if (/\bco\s+(?:nhung\s+|cac\s+)?(app|tool|phan\s*mem|dich\s*vu|san\s*pham|cong\s*cu)\s+.{1,30}?\s*(?:khong|\?|$)/i.test(noAccent))
        return true;
    // Pattern 13: "app ... có gì?", "tool ... có gì?"
    if (/\b(app|tool|phần\s*mềm|sản\s*phẩm)\s+.{1,30}?\s+có\s+(?:gì|những\s*gì)\b/i.test(lower))
        return true;
    if (/\b(app|tool|phan\s*mem|san\s*pham)\s+.{1,30}?\s+co\s+(?:gi|nhung\s*gi)\b/i.test(noAccent))
        return true;
    // Pattern 14: "muốn ... thì dùng app nào?", "cần ... thì dùng app nào?"
    if (/\b(muốn|cần|để)\s+.{1,30}?\s*thì\s+(?:dùng|xài|chọn|có|mua)\s+(?:app|tool|phần\s*mềm|gì|nào)\b/i.test(lower))
        return true;
    if (/\b(muon|can|de)\s+.{1,30}?\s*thi\s+(?:dung|xai|chon|co|mua)\s+(?:app|tool|phan\s*mem|gi|nao)\b/i.test(noAccent))
        return true;
    return false;
}
/**
 ...
 */
export function isAmbiguousDemandQuery(rawText) {
    if (!rawText || rawText.trim().length === 0)
        return false;
    const clean = rawText
        .toLowerCase()
    ...
    // -----------------------------------------------------------------------
    const hasSpecificProduct = /\b(netflix|spotify|youtube|canva|chatgpt|gemini|cursor|notion|adobe|figma|duolingo|memrise|capcut|meitu|xingtu|proton|microsoft|office|word|excel|google|icloud|tv360|youku|wink|locket|autodesk|elevenlabs|veo|grok|kling|perplexity|leonardo|claude|codex|disney|apple tv)\b/i.test(noAccent);
    ...
    // -----------------------------------------------------------------------
    const hasActionKeyword = /\b(bao hanh|don hang|kiem tra don|so du|nap tien|nap vi|voucher|ma giam gia|khuyen mai|gia bao nhieu|bao nhieu tien|bang gia|dang ky|thanh toan)\b/i.test(noAccent);
    if (hasActionKeyword)
        return false;
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    const hasShopCatalogKeyword = /^(?:shop|ben minh|ben shop|cua hang|ban)\s+(?:dang\s+)?(?:co|ban|cung cap)\s+(?:nhung\s+|cac\s+)?(?:gi|san pham gi|san pham nao|mat hang gi|app gi|app nao)\??$/i.test(noAccent);
    if (hasShopCatalogKeyword)
        return false;
    // -----------------------------------------------------------------------
