...
export function extractDuration(text) {
    ...
    // Gói Token
    if (/\b(100m|100\s*trieu)\s*token\b/.test(norm))
        return '100M Token';
    if (/\b(50m|50\s*trieu)\s*token\b/.test(norm))
        return '50M Token';
...
export function matchPlanByDuration(plans, durationOrText, fullQuery) {
