...
export class PolicyIncidentRecoveryVerificationEngine {
    ...
    verifyRecovery(input) {
        ...
        const { tenantPartition, handoffRecord, activePolicyState, runtimeSnapshot, reconciliationResult, safetyBoundary, } = input;
        const checksPassed = [];
        const discrepancies = [];
        // 1. Tenant match
        ...
        if (activePolicyState?.activeModifications) {
            for (const forbidden of ROLLBACK_HARD_FORBIDDEN_ACTIONS) {
                if (activePolicyState.activeModifications[forbidden] !== undefined) {
        ...
        const isSuccess = activePolicyVerified &&
        ...
        return Object.freeze({
            ...
            isAutoRepairAttempted: false,
            isAutoResyncAttempted: false,
        });
    }
}
