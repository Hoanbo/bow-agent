...
import { EmergencyStopActiveError, SimulationCrossTenantAccessForbiddenError, SimulationAuthorityViolationError, SimulationHandoffExpiredError, DuplicateSimulationHandoffError, asSimulationHandoffId, MAX_HANDOFF_TTL_MS, } from './GovernedPolicySimulationTypes.js';
export class PreRatificationSimulationAdvisoryBridge {
    ...
    assertEmergencyStopInactive() {
        ...
        try {
            ...
            throw new EmergencyStopActiveError(`Emergency stop provider threw error during simulation handoff: ${err instanceof Error ? err.message : String(err)}`);
        }
     ...
     */
    packageSimulationAdvisory(dossier, overrideNonce) {
        ...
        if (dossier.expiresAt && dossier.expiresAt <= now) {
            throw new SimulationHandoffExpiredError(`Simulation dossier ${dossier.dossierId} has expired at ${dossier.expiresAt} (current: ${now})`);
        }
        ...
        const advisoryPackage = Object.freeze({
            handoffId,
        ...
        this.deliveryLog.push(Object.freeze({
            handoffId,
            dossierId: dossier.dossierId,
            tenantId: cleanTenant,
            policyDomain: dossier.policyDomain,
