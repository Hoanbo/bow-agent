...
export class DelegationScopeValidator {
    static PROTECTED_PATTERNS = ['shopofbow', 'c:\\bow\\shopofbow'];
    /**
    ...
    static validateScope(scope) {
        ...
        if (scope.maxScopePercentage < 0 || scope.maxScopePercentage > 100) {
            throw new DelegationScopeValidationError('INVALID_SCOPE_PERCENTAGE', `Scope percentage must be between 0 and 100. Received: ${scope.maxScopePercentage}`);
        }
        ...
        for (const cap of scope.allowedCapabilities) {
            if (disallowedSet.has(cap)) {
                throw new DelegationScopeValidationError('CONTRADICTORY_CAPABILITY_SCOPE', `Capability "${cap}" is listed in both allowed and disallowed capabilities.`);
            }
        }
    }
    /**
     ...
     */
    static assertProtectedWorkspaceSafe(paths, fieldName) {
        for (const p of paths) {
            const lower = p.toLowerCase();
            for (const pattern of this.PROTECTED_PATTERNS) {
                if (lower.includes(pattern)) {
                    throw new DelegationScopeValidationError('SECURITY_VIOLATION', `Delegation cannot target protected workspace C:\\BOW\\shopofbow (found in ${fieldName}: "${p}").`);
                }
            }
        }
    }
     ...
     */
    static assertValidChildDelegation(parent, childInput, now = Date.now()) {
        // 1. Parent state must be ACTIVE
        if (parent.status !== 'ACTIVE') {
            throw new DelegationScopeValidationError('PARENT_NOT_ACTIVE', `Cannot delegate from parent delegation with status "${parent.status}". Must be ACTIVE.`);
        }
        if (parent.revocationState.isRevoked) {
            throw new DelegationScopeValidationError('PARENT_REVOKED', 'Cannot delegate from a revoked parent delegation.');
        }
        // 2. Parent expiration check
        if (now >= parent.expiresAt) {
            throw new DelegationScopeValidationError('PARENT_EXPIRED', 'Cannot delegate from an expired parent delegation.');
        }
        // 3. Sub-delegation permission check
        if (!parent.scope.allowSubDelegation) {
            throw new DelegationScopeValidationError('SUBDELEGATION_DISALLOWED', 'Parent delegation explicitly disallows child sub-delegation.');
        }
        // 4. Depth check
        const childDepth = parent.scope.currentDepth + 1;
        if (childDepth > parent.scope.maxChildDelegationDepth) {
            throw new DelegationScopeValidationError('MAX_DELEGATION_DEPTH_EXCEEDED', `Child delegation depth ${childDepth} exceeds parent maximum depth ${parent.scope.maxChildDelegationDepth}.`);
        }
        // 5. Scope percentage check: CHILD <= PARENT
        if (childInput.scope.maxScopePercentage > parent.scope.maxScopePercentage) {
            throw new DelegationScopeValidationError('SCOPE_AMPLIFICATION_FORBIDDEN', `Child scope percentage (${childInput.scope.maxScopePercentage}%) exceeds parent scope (${parent.scope.maxScopePercentage}%).`);
        }
        ...
        for (const cap of childInput.requestedCapabilities) {
            if (!parentAllowedSet.has(cap)) {
                throw new DelegationScopeValidationError('CAPABILITY_WIDENING_FORBIDDEN', `Child requested capability "${cap}" which is not granted in parent delegation.`);
            }
        }
        for (const cap of childInput.scope.allowedCapabilities) {
            if (!parentAllowedSet.has(cap)) {
                throw new DelegationScopeValidationError('CAPABILITY_WIDENING_FORBIDDEN', `Child allowed capability "${cap}" is not in parent granted capabilities.`);
            }
        }
        ...
        if (childExpiresAt > parent.expiresAt) {
            throw new DelegationScopeValidationError('EXPIRATION_EXTENSION_FORBIDDEN', `Child expiration (${childExpiresAt}) cannot extend beyond parent expiration (${parent.expiresAt}).`);
        }
        // 8. Path validation
        ...
        if (childInput.sessionId !== parent.sessionId) {
            throw new DelegationScopeValidationError('CROSS_SESSION_DELEGATION_FORBIDDEN', `Child delegation session "${childInput.sessionId}" does not match parent session "${parent.sessionId}".`);
        }
    }
}
