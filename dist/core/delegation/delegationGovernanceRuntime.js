...
import { DELEGATION_SCHEMA_VERSION, isMasterOwner, } from './delegationTypes.js';
import { DelegationScopeValidator } from './delegationScopeValidator.js';
import { globalAgentIdentityManager } from './agentIdentityManager.js';
import { globalFederatedDeviceRegistry } from './federatedDeviceRegistry.js';
import { globalCapabilityLeaseManager } from './capabilityLeaseManager.js';
...
export class DelegationGovernanceRuntime {
    ...
    leaseManager;
    delegations = new Map();
    completedOrTerminalIds = new Set();
    store;
    constructor(agentIdentityManager = globalAgentIdentityManager, deviceRegistry = globalFederatedDeviceRegistry, leaseManager = globalCapabilityLeaseManager, store) {
    ...
    requestDelegation(input) {
        ...
        if (globalMasterHumanAuthority.isUserStopActive) {
            throw new DelegationGovernanceError('USER_STOP_ACTIVE', 'Cannot request delegation while USER_STOP is active.');
        }
        // 2. Target path & protected workspace check
        if (input.scope.targetPaths) {
            DelegationScopeValidator.assertProtectedWorkspaceSafe(input.scope.targetPaths, 'targetPaths');
        }
        if (input.scope.forbiddenPaths) {
            DelegationScopeValidator.assertProtectedWorkspaceSafe(input.scope.forbiddenPaths, 'forbiddenPaths');
        }
        // 3. Agent identity must exist
        if (!this.agentIdentityManager.hasAgent(input.targetAgentId)) {
            throw new DelegationGovernanceError('TARGET_AGENT_NOT_REGISTERED', `Target agent "${input.targetAgentId}" is not registered.`);
        }
     ...
     */
    authorizeDelegation(delegationId, actorId, grantedCapabilities) {
        if (globalMasterHumanAuthority.isUserStopActive) {
            throw new DelegationGovernanceError('USER_STOP_ACTIVE', 'Cannot authorize delegation while USER_STOP is active.');
        }
        if (!isMasterOwner(actorId)) {
            throw new DelegationGovernanceError('UNAUTHORIZED_DELEGATION_APPROVAL', `Actor "${actorId}" is not the Master Owner. Only Master Owner authority can authorize delegations.`);
        }
        const delegation = this.delegations.get(delegationId);
        if (!delegation) {
            throw new DelegationGovernanceError('DELEGATION_NOT_FOUND', `Delegation "${delegationId}" not found.`);
        }
        if (delegation.status !== 'PENDING_AUTHORIZATION') {
            throw new DelegationGovernanceError('INVALID_STATE_TRANSITION', `Cannot authorize delegation in state "${delegation.status}". Must be PENDING_AUTHORIZATION.`);
        }
        ...
        const granted = grantedCapabilities
            ? [...grantedCapabilities].filter((cap) => delegation.scope.allowedCapabilities.includes(cap))
     ...
     */
    activateDelegation(delegationId, actorId) {
        if (globalMasterHumanAuthority.isUserStopActive) {
            throw new DelegationGovernanceError('USER_STOP_ACTIVE', 'Cannot activate delegation while USER_STOP is active.');
        }
        const delegation = this.delegations.get(delegationId);
        if (!delegation) {
            throw new DelegationGovernanceError('DELEGATION_NOT_FOUND', `Delegation "${delegationId}" not found.`);
        ...
        if (delegation.status !== 'AUTHORIZED') {
            throw new DelegationGovernanceError('INVALID_STATE_TRANSITION', `Cannot activate delegation in state "${delegation.status}". Must be AUTHORIZED.`);
        }
        if (Date.now() >= delegation.expiresAt) {
            const expired = { ...delegation, status: 'EXPIRED' };
            this.delegations.set(delegationId, expired);
            throw new DelegationGovernanceError('DELEGATION_EXPIRED', `Delegation "${delegationId}" has already expired.`);
        }
        const updated = {
            ...delegation,
            status: 'ACTIVE',
     ...
     * - CHILD_EXPIRATION <= PARENT_EXPIRATION
     */
    createChildDelegation(parentDelegationId, childInput, actorId) {
        if (globalMasterHumanAuthority.isUserStopActive) {
            throw new DelegationGovernanceError('USER_STOP_ACTIVE', 'Cannot create child delegation while USER_STOP is active.');
        }
        const parent = this.delegations.get(parentDelegationId);
        if (!parent) {
            throw new DelegationGovernanceError('PARENT_DELEGATION_NOT_FOUND', `Parent delegation "${parentDelegationId}" not found.`);
        }
        ...
        if (!this.agentIdentityManager.hasAgent(childInput.targetAgentId)) {
            throw new DelegationGovernanceError('TARGET_AGENT_NOT_REGISTERED', `Child target agent "${childInput.targetAgentId}" is not registered.`);
        }
     ...
     */
    grantCapabilityLease(delegationId, capabilityId, actorId, ttlMs) {
        if (globalMasterHumanAuthority.isUserStopActive) {
            throw new DelegationGovernanceError('USER_STOP_ACTIVE', 'Cannot grant capability lease while USER_STOP is active.');
        }
        const delegation = this.delegations.get(delegationId);
        if (!delegation) {
            throw new DelegationGovernanceError('DELEGATION_NOT_FOUND', `Delegation "${delegationId}" not found.`);
        ...
        if (delegation.status !== 'ACTIVE' || delegation.revocationState.isRevoked) {
            throw new DelegationGovernanceError('DELEGATION_NOT_ACTIVE', `Cannot issue capability lease for delegation with status "${delegation.status}".`);
        }
        if (Date.now() >= delegation.expiresAt) {
            throw new DelegationGovernanceError('DELEGATION_EXPIRED', 'Cannot issue capability lease for expired delegation.');
        }
        if (!delegation.grantedCapabilities.includes(capabilityId)) {
            throw new DelegationGovernanceError('CAPABILITY_NOT_IN_DELEGATION', `Capability "${capabilityId}" is not granted in delegation "${delegationId}".`);
        }
     ...
     */
    completeDelegation(delegationId, evidence, actorId) {
        const delegation = this.delegations.get(delegationId);
        if (!delegation) {
            throw new DelegationGovernanceError('DELEGATION_NOT_FOUND', `Delegation "${delegationId}" not found.`);
        }
        if (delegation.status !== 'ACTIVE') {
            throw new DelegationGovernanceError('CANNOT_COMPLETE_INACTIVE_DELEGATION', `Cannot complete delegation in state "${delegation.status}". Must be ACTIVE.`);
        }
        const updated = {
            ...delegation,
            status: 'COMPLETED',
        ...
        this.leaseManager.revokeLeasesForDelegation(delegationId, actorId, 'Delegation completed successfully');
        globalAuditLedger.record({
            timestamp: new Date().toISOString(),
            actor: {
                userId: actorId,
    ...
    isDelegationActive(delegationId, now = Date.now()) {
        const d = this.delegations.get(delegationId);
        if (!d)
            return false;
        if (d.status !== 'ACTIVE')
            return false;
        if (d.revocationState.isRevoked)
            return false;
        if (now >= d.expiresAt)
            return false;
        return true;
    }
    assertActiveDelegation(delegationId, sessionId, now = Date.now()) {
        const delegation = this.delegations.get(delegationId);
        if (!delegation) {
            throw new DelegationGovernanceError('DELEGATION_NOT_FOUND', `Delegation "${delegationId}" not found.`);
        }
        if (delegation.sessionId !== sessionId) {
            throw new DelegationGovernanceError('CROSS_SESSION_DELEGATION_REJECTED', `Delegation belongs to session "${delegation.sessionId}", cannot be accessed by session "${sessionId}".`);
        }
        if (delegation.revocationState.isRevoked || delegation.status === 'REVOKED') {
            throw new DelegationGovernanceError('DELEGATION_REVOKED', `Delegation "${delegationId}" is revoked. REVOCATION > AGENT_INTENT.`);
        }
        if (now >= delegation.expiresAt || delegation.status === 'EXPIRED') {
            throw new DelegationGovernanceError('DELEGATION_EXPIRED', `Delegation "${delegationId}" has expired.`);
        }
        if (delegation.status !== 'ACTIVE') {
            throw new DelegationGovernanceError('DELEGATION_NOT_ACTIVE', `Delegation "${delegationId}" is in state "${delegation.status}", not ACTIVE.`);
        }
        return delegation;
    }
    // ---------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------
    saveDurableState(ownerSessionId) {
        const allDelegations = Array.from(this.delegations.values());
        const allLeases = Array.from(new Set(allDelegations.flatMap((d) => this.leaseManager.getLeasesForDelegation(d.delegationId))));
