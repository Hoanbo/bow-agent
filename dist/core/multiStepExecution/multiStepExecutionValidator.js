...
import { MAX_EXECUTION_STEPS, MAX_REPLANNING_GENERATIONS, MultiStepExecutionValidationError, MultiStepExecutionDependencyError, MultiStepExecutionTenantIsolationError, MultiStepExecutionSessionIsolationError, computeGenerationProvenanceHash, computeEnvironmentSnapshotProvenanceHash, computeReplanningRequestProvenanceHash, computeMultiStepSessionProvenanceHash, } from './multiStepExecutionTypes.js';
...
export class MultiStepExecutionValidator {
        ...
        if (typeof obj === 'string') {
            ...
            for (const marker of PROHIBITED_COT_MARKERS) {
                if (obj.includes(marker)) {
                    throw new MultiStepExecutionValidationError(`Prohibited CoT/deliberation marker detected in payload at ${path}: "${marker}"`);
                }
            }
            // Check Prompt Injection patterns
            for (const pattern of SUSPICIOUS_INJECTION_PATTERNS) {
                if (pattern.test(obj)) {
                    throw new MultiStepExecutionValidationError(`Untrusted prompt injection pattern detected in payload at ${path}: "${obj.slice(0, 80)}..."`);
                }
            }
            return;
        }
        ...
        if (typeof obj === 'object') {
            ...
            for (const key of keys) {
                if (DANGEROUS_KEYS.includes(key)) {
                    throw new MultiStepExecutionValidationError(`Prototype pollution attempt detected with key "${key}" at ${path}`);
                }
    ...
    static validateIsolation(expectedTenantId, expectedSessionId, actualTenantId, actualSessionId) {
        if (!expectedTenantId || !actualTenantId || expectedTenantId.trim() !== actualTenantId.trim()) {
            throw new MultiStepExecutionTenantIsolationError(`Tenant isolation breach: expected "${expectedTenantId}", received "${actualTenantId}"`);
        }
        if (!expectedSessionId || !actualSessionId || expectedSessionId.trim() !== actualSessionId.trim()) {
            throw new MultiStepExecutionSessionIsolationError(`Session isolation breach: expected "${expectedSessionId}", received "${actualSessionId}"`);
        }
    }
    /**
    ...
    static validateGeneration(generation) {
        ...
        if (typeof generation.generationIndex !== 'number' || generation.generationIndex < 0 || generation.generationIndex >= MAX_REPLANNING_GENERATIONS) {
            throw new MultiStepExecutionValidationError(`generationIndex must be within [0, ${MAX_REPLANNING_GENERATIONS - 1}], got ${generation.generationIndex}`);
        }
        ...
        if (stepEntries.length > MAX_EXECUTION_STEPS) {
            throw new MultiStepExecutionValidationError(`Generation step count exceeds limit of ${MAX_EXECUTION_STEPS}: got ${stepEntries.length}`);
        }
        ...
        for (const [key, step] of stepEntries) {
            if (key !== step.stepId) {
                throw new MultiStepExecutionValidationError(`Dictionary key "${key}" does not match stepId "${step.stepId}"`);
            }
        ...
        if (generation.provenanceHash !== expectedHash) {
            throw new MultiStepExecutionValidationError(`Generation provenance hash mismatch: expected ${expectedHash}, got ${generation.provenanceHash}`);
        }
    }
    /**
     ...
     */
    static validateStepState(step, totalSteps) {
        ...
        if (typeof step.stepIndex !== 'number' || step.stepIndex < 0 || step.stepIndex >= totalSteps) {
            throw new MultiStepExecutionValidationError(`stepIndex must be within [0, ${totalSteps - 1}], got ${step.stepIndex}`);
        }
     ...
     */
    static validateStepDependencies(steps) {
        const seenIds = new Set();
        for (const [key, step] of Object.entries(steps)) {
            if (key !== step.stepId || seenIds.has(step.stepId)) {
                throw new MultiStepExecutionValidationError(`Duplicate or mismatched stepId detected: "${step.stepId}" (key: "${key}")`);
            }
            seenIds.add(step.stepId);
        }
        ...
        for (const [stepId, step] of Object.entries(steps)) {
            for (const depId of step.dependencies) {
                if (!stepIds.has(depId)) {
                    throw new MultiStepExecutionDependencyError(`Step "${stepId}" depends on unknown step "${depId}"`);
                }
                if (depId === stepId) {
                    throw new MultiStepExecutionDependencyError(`Self-dependency detected for step "${stepId}"`);
                }
            }
        }
        ...
        const dfs = (curr) => {
            ...
            for (const dep of deps) {
                if (!visited.has(dep)) {
                    ...
                    throw new MultiStepExecutionDependencyError(`Cyclic dependency detected in execution graph involving step "${curr}" -> "${dep}"`);
                }
            }
     ...
     */
    static validateEnvironmentSnapshot(snapshot) {
        ...
        if (snapshot.provenanceHash !== expectedHash) {
            throw new MultiStepExecutionValidationError(`Environment snapshot provenance mismatch: expected ${expectedHash}, got ${snapshot.provenanceHash}`);
        }
    }
    /**
     ...
     */
    static validateReplanningRequest(request) {
        ...
        if (request.provenanceHash !== expectedHash) {
            throw new MultiStepExecutionValidationError(`Replanning request provenance hash mismatch: expected ${expectedHash}, got ${request.provenanceHash}`);
        }
    }
    /**
    ...
    static validateSession(session) {
        ...
        if (typeof session.sessionVersion !== 'number' || session.sessionVersion < 1) {
            throw new MultiStepExecutionValidationError(`sessionVersion must be a positive integer >= 1, got ${session.sessionVersion}`);
        }
        ...
        if (session.generations.length > MAX_REPLANNING_GENERATIONS) {
            throw new MultiStepExecutionValidationError(`Generations count exceeds limit ${MAX_REPLANNING_GENERATIONS}: got ${session.generations.length}`);
        }
        ...
        if (session.provenanceRoot !== expectedRoot) {
            throw new MultiStepExecutionValidationError(`Session provenance root mismatch: expected ${expectedRoot}, got ${session.provenanceRoot}`);
        }
    }
    /**
