...
import { MAX_REPLANNING_GENERATIONS, computeGenerationProvenanceHash, computeMultiStepSessionProvenanceHash, MultiStepExecutionGenerationError, MultiStepExecutionConcurrencyError, } from './multiStepExecutionTypes.js';
...
export class ExecutionGenerationManager {
    ...
    createGeneration(params) {
        if (params.generationIndex >= MAX_REPLANNING_GENERATIONS) {
            throw new MultiStepExecutionGenerationError(`Cannot create generation ${params.generationIndex}: exceeds limit of ${MAX_REPLANNING_GENERATIONS}`);
        }
        ...
        const sealedGeneration = Object.freeze({
            ...rawGeneration,
            provenanceHash,
        });
     ...
     */
    transitionToNewGeneration(session, newGeneration, expectedSessionVersion) {
        if (session.sessionVersion !== expectedSessionVersion) {
            throw new MultiStepExecutionConcurrencyError(`OCC conflict in generation transition: expected version ${expectedSessionVersion}, found ${session.sessionVersion}`);
        }
        MultiStepExecutionValidator.validateIsolation(session.tenantId, session.sessionId, newGeneration.tenantId, newGeneration.sessionId);
        if (newGeneration.generationIndex !== session.generations.length) {
            throw new MultiStepExecutionGenerationError(`Invalid generation sequence: expected generationIndex ${session.generations.length}, got ${newGeneration.generationIndex}`);
        }
        ...
        const activeGen = Object.freeze({ ...activeRaw, provenanceHash: activeHash });
