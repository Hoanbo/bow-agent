import { validateAndFinalizeAction } from './actionValidator.js';
/**
 * Lập kế hoạch mở CheckoutModal mua nhanh 1 sản phẩm & plan cụ thể
 */
export function planCheckoutAction(product, plan, context) {
 ...
 */
export function planMultipleCheckoutActions(product, plans, context) {
    const actions = [];
 ...
 */
export function planOrderDetailAction(order, context) {
    return validateAndFinalizeAction({
 ...
 */
export function planRenewalAction(order, context) {
    return validateAndFinalizeAction({
 ...
 */
export function isOrderWarrantyEligible(order) {
 ...
 */
export function planSupportTicketAction(order, issueDescription, context) {
 ...
 */
export function findRelevantWarrantyOrder(orders, queryText, lastMentionedOrder) {
    ...
    const matchedProdInEligible = eligibleOrders.find((o) => {
        ...
        if (cleanLow.includes(pNameLow))
            return true;
        const firstWord = pNameLow.split(/\s+/)[0];
        if (firstWord && firstWord.length >= 3 && cleanLow.includes(firstWord))
            return true;
        return false;
    });
    if (matchedProdInEligible)
        return matchedProdInEligible;
 ...
 */
export function planApplyCouponAction(couponCode, discountLabel, context) {
    return validateAndFinalizeAction({
        type: 'APPLY_COUPON',
 ...
 */
export function planDepositAction(amount, context) {
 ...
 */
export function planTicketDetailAction(ticket, context) {
    return validateAndFinalizeAction({
 ...
 */
export function planCreateTicketAction(context) {
    return validateAndFinalizeAction({
        type: 'NAVIGATE_SUPPORT',
 ...
 */
export function planGenericAction(action, context) {
    return validateAndFinalizeAction({
        type: (action.type || 'COMMERCE_ACTION'),
        label: action.label,
        icon: action.icon || '⚡',
 ...
 */
export function planBodyAction(capabilityName, params, context, targetBodyId) {
