...
export class RelayResumeCoordinator {
     ...
     */
    generateResumeToken(session, ttlMs = 60000, now = Date.now()) {
    ...
    validateResumeRequest(params) {
        ...
        if (params.clientNextSeq <= params.clientLastAckSeq) {
            throw new RelayResumeError(`SEQUENCE_REWIND_DETECTED: Next seq ${params.clientNextSeq} <= ack seq ${params.clientLastAckSeq}. Possible replay.`);
        }
        ...
        if (params.clientNextSeq > entry.expectedSeq + 1) {
            throw new RelayResumeError(`SEQUENCE_GAP_DETECTED: Next seq ${params.clientNextSeq} > expected ${entry.expectedSeq + 1}. Unacknowledged gap.`);
        }
        return { valid: true };
    }
