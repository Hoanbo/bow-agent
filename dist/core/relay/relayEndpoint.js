...
export function validateRelayEndpoint(endpoint) {
    ...
    if (!validProtocols.includes(endpoint.protocol)) {
        throw new RelayEndpointError(`Unsupported relay protocol: ${endpoint.protocol}. Allowed: ${validProtocols.join(', ')}`);
    }
 ...
 */
export function formatRelayEndpointUrl(endpoint) {
 ...
 */
export function areEndpointsEquivalent(a, b) {
    if (!a || !b)
        return false;
 ...
 */
export function assertRelayEndpointKnowledgeDoesNotGrantAccess(hasEndpointInfo, isAdmitted) {
    if (hasEndpointInfo && !isAdmitted) {
        throw new RelayEndpointError('INVARIANT_ENFORCED: ENDPOINT_KNOWLEDGE != ACCESS. Relay connection rejected without Zero-Trust admission.');
    }
}
