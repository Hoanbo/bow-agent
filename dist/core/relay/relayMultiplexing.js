...
export class RelayMultiplexer {
    ...
    routeInbound(message) {
        ...
        if (!session) {
            throw new RelayMultiplexingError(`UNKNOWN_SESSION: Inbound message targeted non-existent session ${message.sessionId}`);
        }
        // Cross-device injection defense
        if (session.deviceId !== message.deviceId) {
            throw new RelayMultiplexingError(`CROSS_DEVICE_INJECTION: Message deviceId ${message.deviceId} does not match session deviceId ${session.deviceId}`);
        }
        // Cross-surface injection defense
        if (session.surfaceType !== message.surfaceType || session.surfaceId !== message.surfaceId) {
            throw new RelayMultiplexingError(`CROSS_SURFACE_INJECTION: Message surface ${message.surfaceType}:${message.surfaceId} does not match session surface ${session.surfaceType}:${session.surfaceId}`);
        }
        // Cross-Brain injection defense
        if (session.brainId !== message.brainId) {
            throw new RelayMultiplexingError(`CROSS_BRAIN_INJECTION: Message brainId ${message.brainId} does not match session brainId ${session.brainId}`);
        }
        return session;
    }
    clear() {
