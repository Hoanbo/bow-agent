...
export class SecureBrainRelayRuntime {
    ...
    async establishSession(params) {
        ...
        if (admissionDecision.decision !== 'ADMIT' || !admissionDecision.admitted) {
            ...
            throw new RelayError('FAKE_DEVICE', `Admission rejected for device ${params.deviceId}: ${admissionDecision.rejectionReason}`);
        }
        this.auditLedger.record({
     ...
     */
    handleNetworkRoaming(sessionId, newNetwork) {
     ...
     */
    recordHeartbeat(relayId, sessionId, rttMs) {
