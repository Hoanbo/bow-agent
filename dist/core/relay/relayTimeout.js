...
export class RelayTimeoutError extends Error {
    ...
    timeoutMs;
    operationName;
    constructor(category, timeoutMs, operationName) {
        super(`RELAY_TIMEOUT [${category}]: Operation '${operationName}' timed out after ${timeoutMs}ms.`);
...
export const RELAY_TIMEOUT_DEFAULTS = Object.freeze({
    CONNECTION: 10000,
    REGISTRATION: 5000,
    ADMISSION: 8000,
...
export function getRelayDefaultTimeout(category) {
    return RELAY_TIMEOUT_DEFAULTS[category] ?? 5000;
}
/**
 * Wraps a promise with a fail-closed typed timeout.
 */
export async function withRelayTimeout(promise, timeoutMs, category, operationName) {
    let timerId;
