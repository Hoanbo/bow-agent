...
import { validateSyncScope, validateEventIdentifier, containsEventSecret, } from './eventValidator.js';
import { computeEventFingerprint, computeEventIdentity, } from './eventFingerprint.js';
/**
 ...
 */
export function createBrainEventEnvelope(params) {
    // 1. Validate scope
    ...
    if (typeof params.sequence !== 'number' ||
        ...
        params.sequence <= 0) {
        throw new Error(`[SYNCHRONIZATION_ENVELOPE_ERROR] Invalid sequence ${params.sequence}. Sequence must be a positive integer.`);
    }
    const previousSequence = params.previousSequence !== undefined
    ...
    if (typeof previousSequence !== 'number' ||
        ...
        previousSequence < 0) {
        throw new Error(`[SYNCHRONIZATION_ENVELOPE_ERROR] Invalid previousSequence ${previousSequence}. Must be non-negative integer.`);
    }
    if (previousSequence >= params.sequence) {
        throw new Error(`[SYNCHRONIZATION_ENVELOPE_ERROR] previousSequence (${previousSequence}) must be strictly less than sequence (${params.sequence}).`);
    }
    ...
    if (containsEventSecret(correlationId) || containsEventSecret(causationId)) {
        throw new Error('[SYNCHRONIZATION_SECURITY_ERROR] Event correlation or causation identifier contains sensitive credentials.');
    }
