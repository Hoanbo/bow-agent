 ...
 */
export function reconcileSynchronizationState(brainState, incomingState) {
    // 1. Scope mismatch check
    if (incomingState.userId !== brainState.userId ||
    ...
    if (incomingState.sequence === brainState.sequence) {
        if (incomingState.latestEventFingerprint === brainState.latestEventFingerprint &&
    ...
    if (incomingState.sequence < brainState.sequence) {
        if (brainState.getEventBySequence) {
            ...
            if (historical) {
                if (historical.eventId === incomingState.latestEventId &&
