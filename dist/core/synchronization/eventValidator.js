 ...
 */
export function validateEventIdentifier(name, value) {
 ...
 */
export function validateSyncScope(userId, sessionId, brainId) {
 ...
 */
export function containsEventSecret(input) {
 ...
 */
export function redactEventSecrets(input) {
    let result = input;
 ...
 */
export function assertEventRiskPreservation(priorRisk, newRisk) {
    const priorRank = RISK_ORDER[priorRisk] ?? 1;
    const newRank = RISK_ORDER[newRisk] ?? 1;
    if (newRank < priorRank) {
        throw new Error(`[SYNCHRONIZATION_RISK_DOWNGRADE_ERROR] Monotonic risk violation: cannot downgrade risk from ${priorRisk} to ${newRisk}.`);
    }
}
/**
 ...
 */
export function assertEventSequenceMonotonicity(priorSequence, newSequence) {
    ...
    if (newSequence < priorSequence) {
        throw new Error(`[SYNCHRONIZATION_SEQUENCE_ERROR] Monotonic sequence violation: new sequence ${newSequence} is less than prior sequence ${priorSequence}.`);
    }
}
