...
export class CapabilityRuntime {
    ...
    makeResourceKey(target) {
        if (!target)
            return '__system_capability_resource__';
    ...
    async executeCapability(request) {
        ...
        if (this._emergencyStop) {
            throw new CapabilityError('FATAL', `Cannot execute capability: Emergency Stop is active (${this._emergencyStopReason}).`, request.capabilityId);
        }
        ...
        if (plan.requiresAuthorization && !request.isDryRun) {
            if (!request.authorizationToken) {
                const err = new CapabilityError('AUTHORIZATION_REQUIRED', `Capability "${descriptor.capabilityId}" requires an explicit authorization token.`, descriptor.capabilityId, request.target);
            ...
            if (!tokenValidation.valid) {
                const err = new CapabilityError('AUTHORIZATION_REQUIRED', `Authorization token invalid: ${tokenValidation.reason}`, descriptor.capabilityId, request.target);
                globalCapabilityAudit.record('CAPABILITY_DENIED', descriptor.capabilityId, { reason: err.message });
                throw err;
            }
        ...
        if (!request.isDryRun && request.target) {
            ...
            if (!lockAcquired) {
                throw new CapabilityError('RECOVERABLE', `Target resource "${request.target}" is currently locked by another concurrent action.`, descriptor.capabilityId, request.target);
            }
        }
        try {
