...
import { createInvariantCheckId, } from './observabilityTypes.js';
...
export class InvariantVerificationEngine {
     ...
     */
    verifyWorkspaceIsolation(targetPath) {
        if (!targetPath) {
            return { valid: true };
        }
        ...
        try {
            ...
            throw new Error(`PROTECTED_WORKSPACE_VIOLATION: ${err.message}`);
        }
    }
    /**
    ...
    evaluateInvariant(input) {
        ...
        const evidenceHash = crypto
            ...
            .digest('hex');
        const invariantId = createInvariantCheckId(`inv_${input.category.toLowerCase()}_${evaluatedAt}_${crypto.randomBytes(4).toString('hex')}`);
        const check = {
            invariantId,
            sessionId: input.sessionId,
            targetId: input.targetId,
