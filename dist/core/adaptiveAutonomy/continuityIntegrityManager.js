...
import { AdaptiveAutonomyProvenanceError, computeContinuitySnapshotHash, } from './adaptiveAutonomyTypes.js';
export class ContinuityIntegrityManager {
    tenantId;
    sessionId;
    snapshots = [];
    lastSnapshotHash = 'GENESIS_CONTINUITY_HASH';
    constructor(tenantId, sessionId, initialSnapshots = []) {
    ...
    verifySnapshotChain() {
        ...
        for (let i = 0; i < this.snapshots.length; i++) {
            ...
            if (snap.tenantId !== this.tenantId || snap.sessionId !== this.sessionId) {
                throw new AdaptiveAutonomyProvenanceError(`Continuity snapshot tenant/session mismatch at index ${i}`, this.tenantId, this.sessionId);
            }
            if (snap.previousSnapshotHash !== prevHash) {
                throw new AdaptiveAutonomyProvenanceError(`Continuity chain broken at index ${i}: expected prev=${prevHash}, actual=${snap.previousSnapshotHash}`, this.tenantId, this.sessionId);
            }
            ...
            if (currentSnapshotHash !== computedHash) {
                throw new AdaptiveAutonomyProvenanceError(`Continuity snapshot tamper detected at index ${i}: expected=${computedHash}, actual=${currentSnapshotHash}`, this.tenantId, this.sessionId);
            }
     ...
     */
    detectDrift(currentEnvFingerprint, currentScope, authorizedScope) {
