...
import { MAX_EVIDENCE_PAYLOAD_BYTES, MAX_EVIDENCE_DEPTH, DEFAULT_MAX_EVIDENCE_AGE_MS, VerificationSecurityViolationError, VerificationValidationError, CrossTenantVerificationError, ContradictoryEvidenceError, } from './realityVerificationTypes.js';
export class RealityEvidenceCollector {
    sanitizer;
    maxEvidenceAgeMs;
    observedEvidenceHashes = new Set();
    constructor(options) {
     ...
     */
    collectEvidence(result, externalEvidence, currentTimeMs) {
        ...
        if (result.sanitizedOutput !== null && result.sanitizedOutput !== undefined) {
            ...
            if (typeof sanitizedPayload === 'object' && !Array.isArray(sanitizedPayload)) {
                for (const [key, val] of Object.entries(sanitizedPayload)) {
                    if (key === 'output')
                        continue;
                    collected.push(this.createEvidenceItem({
                        source: 'ADAPTER_OBSERVATION',
                        path: key,
                        observedValue: val,
                        timestamp: result.executedAt || new Date(now).toISOString(),
        ...
        if (externalEvidence && Array.isArray(externalEvidence)) {
            for (const item of externalEvidence) {
                ...
                if (this.observedEvidenceHashes.has(evidenceHash)) {
                    throw new VerificationValidationError(`REPLAYED_EVIDENCE: Evidence item with hash "${evidenceHash}" has already been processed.`);
                }
     ...
     */
    validateExternalEvidenceItem(item, result, now) {
        ...
        if (item.tenantId && item.tenantId !== result.tenantId) {
            throw new CrossTenantVerificationError(`CROSS_TENANT_EVIDENCE: Evidence tenant "${item.tenantId}" does not match execution tenant "${result.tenantId}".`);
        }
        if (item.taskId && item.taskId !== result.taskId) {
            throw new VerificationValidationError(`TASK_ID_MISMATCH: Evidence taskId "${item.taskId}" does not match execution taskId "${result.taskId}".`);
        }
        if (item.executionId && item.executionId !== result.executionId) {
            throw new VerificationValidationError(`EXECUTION_ID_MISMATCH: Evidence executionId "${item.executionId}" does not match result executionId "${result.executionId}".`);
        }
        if (typeof item.confidence !== 'number' || item.confidence < 0 || item.confidence > 1) {
            throw new VerificationValidationError(`INVALID_CONFIDENCE: Evidence confidence must be a number between 0.0 and 1.0 (received: ${item.confidence}).`);
        }
        // Freshness check
        ...
        if (now - timestampMs > this.maxEvidenceAgeMs) {
            throw new VerificationValidationError(`STALE_EVIDENCE: Evidence is older than maximum age limit of ${this.maxEvidenceAgeMs}ms.`);
        }
    }
    /**
     * Scans for contradictory evidence across multiple sources for the same target path.
     */
    detectContradictions(evidenceList) {
        ...
        for (const ev of evidenceList) {
            ...
            if (existing) {
                ...
                if (valA !== valB) {
                    throw new ContradictoryEvidenceError(`CONTRADICTORY_EVIDENCE: Sources "${existing.source}" and "${ev.source}" report conflicting values for path "${ev.path}".`);
                }
            }
            else {
    ...
    createEvidenceItem(params) {
        ...
        const matched = params.expectedValue !== undefined
     ...
     */
    defensivelyValidatePayload(payload) {
        let serialized;
        try {
        ...
        if (byteLength > MAX_EVIDENCE_PAYLOAD_BYTES) {
            throw new VerificationSecurityViolationError(`PAYLOAD_SIZE_EXCEEDED: Evidence payload size (${byteLength} bytes) exceeds limit of ${MAX_EVIDENCE_PAYLOAD_BYTES} bytes.`);
        }
        if (/(?:__proto__|constructor|prototype)\s*":/i.test(serialized) || serialized.includes('"__proto__"')) {
            throw new VerificationSecurityViolationError('PROTOTYPE_POLLUTION_KEY: Forbidden prototype pollution key detected in evidence payload.');
        }
        this.scanRecursive(payload, 0);
    }
    scanRecursive(val, currentDepth) {
        if (currentDepth > MAX_EVIDENCE_DEPTH) {
            throw new VerificationSecurityViolationError(`MAX_DEPTH_EXCEEDED: Evidence recursion depth exceeds limit of ${MAX_EVIDENCE_DEPTH}.`);
        }
        ...
        if (typeof val === 'object') {
            ...
            if (proto !== null && proto !== Object.prototype && proto !== Array.prototype) {
                throw new VerificationSecurityViolationError('PROTOTYPE_POLLUTION_KEY: Abnormal prototype hierarchy detected in evidence.');
            }
            if (Object.prototype.hasOwnProperty.call(val, '__proto__') ||
                ...
                throw new VerificationSecurityViolationError('PROTOTYPE_POLLUTION_KEY: Forbidden prototype pollution property in evidence.');
            }
            for (const key of Object.keys(val)) {
                if (key === '__proto__' || key === 'constructor' || key === 'prototype') {
                    throw new VerificationSecurityViolationError(`PROTOTYPE_POLLUTION_KEY: Forbidden prototype pollution key "${key}" in evidence.`);
                }
                if (key.includes('\0')) {
