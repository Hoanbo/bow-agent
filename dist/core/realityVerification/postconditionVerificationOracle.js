...
export class PostconditionVerificationOracle {
    ...
    evaluateInvariants(params) {
        ...
        for (const rawPost of postconditions) {
            let rawOp = rawPost.operator || rawPost.predicate || 'EQUALS';
            if (rawOp === 'GREATER_THAN_OR_EQUALS' || rawOp === 'GTE')
                rawOp = 'GREATER_OR_EQUAL';
            if (rawOp === 'LESS_THAN_OR_EQUALS' || rawOp === 'LTE')
                rawOp = 'LESS_OR_EQUAL';
            if (rawOp === 'GT')
                rawOp = 'GREATER_THAN';
            if (rawOp === 'LT')
            ...
            if (rawOp === 'NEQ')
                rawOp = 'NOT_EQUALS';
            const canonicalPostcondition = {
                id: rawPost.id || `post_${results.length + 1}`,
                description: rawPost.description || '',
                targetPath: rawPost.targetPath || rawPost.target || '',
                operator: rawOp,
                expectedValue: rawPost.expectedValue !== undefined ? rawPost.expectedValue : rawPost.expected,
     ...
     */
    buildObservedStateMap(evidenceList) {
        const map = {};
