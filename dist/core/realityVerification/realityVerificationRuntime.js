...
import { REALITY_VERIFICATION_AUDIT_DOMAIN, VerificationValidationError, CrossTenantVerificationError, } from './realityVerificationTypes.js';
import { globalRealityEvidenceCollector } from './realityEvidenceCollector.js';
import { globalPostconditionVerificationOracle } from './postconditionVerificationOracle.js';
import { globalVerificationExecutionGate } from './verificationExecutionGate.js';
export class RealityVerificationRuntime {
     ...
     */
    async verifyReality(request, currentTime) {
        ...
        if (!executionResult.executionId || !executionResult.taskId || !executionResult.tenantId) {
            throw new VerificationValidationError('INVALID_EXECUTION_RESULT: executionResult must contain executionId, taskId, and tenantId.');
        }
        if (!executionResult.executionProvenanceHash ||
            typeof executionResult.executionProvenanceHash !== 'string' ||
            !/^[a-f0-9]{64}$/i.test(executionResult.executionProvenanceHash)) {
            throw new VerificationValidationError('MALFORMED_PROVENANCE_HASH: executionProvenanceHash must be a valid 64-character hex SHA-256 string.');
        }
        if (typeof expectedTaskVersion !== 'number' || expectedTaskVersion < 0 || !Number.isInteger(expectedTaskVersion)) {
            throw new VerificationValidationError(`INVALID_TASK_VERSION: expectedTaskVersion must be a non-negative integer (received: ${expectedTaskVersion}).`);
        }
        this.recordAudit({
        ...
        if (authoritativeTask) {
            if (executionResult.tenantId !== authoritativeTask.tenantId) {
                ...
                throw new CrossTenantVerificationError(`CROSS_TENANT_VERIFICATION: Result tenantId "${executionResult.tenantId}" does not match authoritative task tenantId "${authoritativeTask.tenantId}".`);
            }
            if (executionResult.taskId !== authoritativeTask.taskId) {
                throw new VerificationValidationError(`TASK_ID_MISMATCH: Result taskId "${executionResult.taskId}" does not match authoritative taskId "${authoritativeTask.taskId}".`);
            }
        ...
        let evidenceList;
        try {
            evidenceList = this.evidenceCollector.collectEvidence(executionResult, request.externalEvidence, Date.parse(now));
        }
        catch (err) {
            const eventType = err.code === 'CROSS_TENANT_VERIFICATION_ERROR'
        ...
        if (oracleOutcome.status === 'UNKNOWN' &&
        ...
        const auditEventType = finalResult.status === 'VERIFIED'
     ...
     */
    createVerificationResult(params) {
    ...
    recordAudit(params) {
        try {
            ...
            this.auditLedger.record({
                timestamp,
                actor: {
                    userId: sanitizedDetails.userId || 'reality_verification_runtime',
                    role: 'agent',
                    channel: 'governance',
                },
