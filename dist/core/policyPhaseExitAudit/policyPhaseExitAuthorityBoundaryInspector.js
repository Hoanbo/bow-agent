...
export class PolicyPhaseExitAuthorityBoundaryInspector {
    ...
    inspectBoundaries() {
        ...
        for (const target of targets) {
            ...
            const deficiencies = [];
            if (!rejectsAutonomous)
                deficiencies.push('Missing explicit autonomous actor rejection pattern check');
            if (!rejectsAnonymous)
                deficiencies.push('Missing explicit anonymous actor rejection check');
            if (!enforcesAntiSelfApproval)
                deficiencies.push('Missing anti-self-approval enforcement');
            if (!requiresHumanRole)
                deficiencies.push('Missing authorized human role restriction');
