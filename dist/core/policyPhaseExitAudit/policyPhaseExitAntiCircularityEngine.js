...
export class PolicyPhaseExitAntiCircularityEngine {
     ...
     */
    analyzeEvidence(evidenceItems) {
        this.assertUserStopInactive();
        const graph = new Map();
        const groundedCriteria = new Set();
        const allCriteria = new Set();
        for (const item of evidenceItems) {
            const cId = item.criterionId;
            allCriteria.add(cId);
            if (!graph.has(cId)) {
                graph.set(cId, new Set());
            }
            // Check if physically grounded
            const isPhysical = item.verificationMethod === 'AST_SOURCE_INSPECTION' ||
