...
export class PolicyPhaseExitTenantIsolationInspector {
    ...
    probeIsolation() {
        ...
        if (!traversalBlocked)
            anomalies.push('Path traversal was not rejected');
        // 2. Null byte probe
        ...
        if (!nullByteBlocked)
            anomalies.push('Null byte in tenantId was not rejected');
        ...
        if (!reservedDeviceBlocked)
            anomalies.push('Windows reserved device name CON was not rejected');
        ...
        if (!anonymousBlocked)
            anomalies.push('Whitespace-only tenantId was not rejected');
        ...
        if (!crossTenantSegregationVerified)
            anomalies.push('Cross-tenant partition keys collided');
