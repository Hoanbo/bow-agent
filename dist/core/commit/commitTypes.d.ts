 ...
 */
export type CommitStatus = 'COMMITTED' | 'ALREADY_COMMITTED' | 'REJECTED' | 'FAILED' | 'ROLLBACK_REQUIRED' | 'ROLLED_BACK' | 'INCONSISTENT' | 'UNKNOWN';
/**
 * EN: Deterministic failure categories for commit breakdowns.
 * VI: Các danh mục lỗi tất định cho các trường hợp thất bại commit.
 */
export type CommitFailureCategory = 'COMMIT_VALIDATION_FAILURE' | 'COMMIT_AUTHORIZATION_FAILURE' | 'COMMIT_CONFLICT' | 'COMMIT_PERSISTENCE_FAILURE' | 'COMMIT_CONSISTENCY_FAILURE' | 'COMMIT_PARTIAL_FAILURE' | 'COMMIT_REPLAY' | 'COMMIT_SCOPE_FAILURE' | 'COMMIT_INTEGRITY_FAILURE' | 'COMMIT_UNKNOWN_FAILURE';
/**
 * EN: Authoritative consistency issue classifications.
 * VI: Các phân loại vấn đề nhất quán có thẩm quyền.
 */
export type ConsistencyIssueType = 'STATE_MATCH' | 'EXPECTED_STATE_MISSING' | 'UNEXPECTED_STATE_CHANGE' | 'SEQUENCE_MISMATCH' | 'IDENTITY_MISMATCH' | 'VERIFICATION_MISMATCH' | 'COMMIT_FINGERPRINT_MISMATCH' | 'USER_SCOPE_MISMATCH' | 'SESSION_SCOPE_MISMATCH' | 'GOVERNANCE_MISMATCH' | 'RISK_MISMATCH' | 'APPROVAL_METADATA_MISMATCH' | 'PARTIAL_COMMIT' | 'UNKNOWN_COMMIT_STATE';
/**
 ...
 */
export type CommitOperationType = 'SESSION_TURN_APPEND' | 'DURABLE_RULE_RECORD' | 'DURABLE_HABIT_RECORD' | 'STATE_SNAPSHOT_COMMIT' | 'GENERIC_MUTATION';
/**
