 ...
 */
export function redactCommitSecrets(text) {
 ...
 */
export function containsCommitSecret(val) {
 ...
 */
export function hasCommitPrototypePollution(target) {
    ...
    for (const [k, v] of Object.entries(target)) {
        if (PROTOTYPE_POLLUTION_REGEX.test(k))
            return true;
        if (v && typeof v === 'object' && hasCommitPrototypePollution(v))
            return true;
    }
    return false;
}
 ...
 */
export function validateCommitScope(userId, sessionId) {
 ...
 */
export function assertCommitRiskPreservation(priorRisk, currentRisk) {
    const priorRank = RISK_HIERARCHY[priorRisk] ?? 1;
    const currentRank = RISK_HIERARCHY[currentRisk] ?? 1;
    if (currentRank < priorRank) {
        throw new Error(`RISK_DOWNGRADE_FORBIDDEN: Cannot downgrade commit risk from ${priorRisk} to ${currentRisk}`);
    }
}
