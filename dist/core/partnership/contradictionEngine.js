...
export class ContradictionEngine {
     ...
     */
    recordContradiction(type, previousBelief, currentEvidence, recommendedAction) {
     ...
     */
    detectTelemetryVsMemory(metricKey, observedValue, memoryBeliefValue, memoryItem) {
        ...
        if (obsStr !== memStr) {
            ...
            return this.recordContradiction('TELEMETRY_VS_MEMORY', `Telemetry/Belief '${metricKey}' was assumed to be '${memoryBeliefValue}'${memoryRef}`, `Current observation reports '${metricKey}' is '${observedValue}'`, `Reconcile telemetry '${metricKey}' before proceeding with reliant operations.`);
        }
        return null;
    }
    /**
     ...
     */
    detectObservationVsAssumption(assumption, observedFact, context) {
        if (!assumption || !observedFact)
            return null;
        return this.recordContradiction('OBSERVATION_VS_ASSUMPTION', `Assumed: ${assumption}${context ? ` [Context: ${context}]` : ''}`, `Observed Fact: ${observedFact}`, `Revise assumption in light of observed fact; do not proceed on invalid premise.`);
    }
    /**
     * Check for outcome vs expectation conflict.
     */
    detectOutcomeVsExpectation(actionId, expectedOutcome, actualOutcome) {
        ...
        if (expLower !== actLower && !actLower.includes(expLower)) {
            return this.recordContradiction('OUTCOME_VS_EXPECTATION', `Action ${actionId} expected: ${expectedOutcome}`, `Actual execution result: ${actualOutcome}`, `Conduct failure analysis and update domain model before retrying.`);
        }
        return null;
    }
    /**
     ...
     */
    detectStatementVsFact(statement, verifiedFact, context) {
        if (!statement || !verifiedFact)
            return null;
        return this.recordContradiction('STATEMENT_VS_FACT', `Statement asserted: ${statement}${context ? ` (${context})` : ''}`, `Verified Host Fact: ${verifiedFact}`, `Clarify discrepancy with Master Owner; verified host truth takes operational precedence.`);
    }
    /**
     * Check capability availability against previous belief.
     */
    detectCapabilityContradiction(capabilityId, isAvailableNow, previouslyBelievedAvailable) {
        if (isAvailableNow !== previouslyBelievedAvailable) {
            return this.recordContradiction('TELEMETRY_VS_MEMORY', `Capability '${capabilityId}' was believed to be ${previouslyBelievedAvailable ? 'AVAILABLE' : 'UNAVAILABLE'}`, `Current capability query indicates '${capabilityId}' is ${isAvailableNow ? 'AVAILABLE' : 'UNAVAILABLE'}`, `Refresh capability state and update planned tasks requiring '${capabilityId}'.`);
        }
        return null;
    }
    /**
     ...
     */
    resolveContradiction(contradictionId, resolutionNotes) {
