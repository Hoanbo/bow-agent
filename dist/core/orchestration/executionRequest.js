 ...
 */
export function resolveToolNameForIntent(actionType) {
    switch (actionType) {
        case 'CANCEL_REQUEST':
            return 'cancel_order';
        case 'DELETE_REQUEST':
 ...
 */
export function createExecutionRequest(intent, status) {
    const executionFingerprint = computeDeterministicExecutionFingerprint(intent.userId, intent.sessionId, intent.actionType, intent.sourceDecisionFingerprint, intent.parameters);
