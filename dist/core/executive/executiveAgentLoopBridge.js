...
import { globalAgentLoopRuntime } from '../agent-loop/agentLoopRuntime.js';
...
export class ExecutiveAgentLoopBridge {
     ...
     */
    async stepGoalThroughAgentLoop(goalId, agentLoop = globalAgentLoopRuntime, options) {
        ...
        if (!govDecision.permitted) {
            ...
            globalExecutiveGoalManager.updateGoalStatus(goalId, 'FAILED', `Security violation: ${govDecision.violations.join('; ')}`);
            return {
                success: false,
                goalId,
                taskId: nextTask.taskId,
        ...
        if (govDecision.requiresHumanApproval && !effectiveTokenId) {
            globalExecutiveTaskManager.updateTaskStatus(nextTask.taskId, 'AWAITING_HUMAN', 'Human authorization required');
            globalExecutiveGoalManager.updateGoalStatus(goalId, 'AWAITING_HUMAN', `Task '${nextTask.taskId}' awaiting Master Human approval`);
            return {
                success: false,
                goalId,
                taskId: nextTask.taskId,
        ...
        const execResult = await globalExecutiveExecution.executeTask(nextTask, goal.sessionId, effectiveTokenId, goal.goalId);
        ...
        if (!execResult.success) {
            ...
            if (assessment.suggestedAction === 'RETRY') {
                ...
                globalExecutiveGoalManager.updateGoalStatus(goalId, 'FAILED', `Unrecoverable task '${nextTask.taskId}': ${execResult.error}`);
            }
            return {
                success: false,
                goalId,
        ...
        if (progress.isComplete) {
            const success = globalExecutiveProgress.evaluateGoalSuccess(goal, allTasks);
            globalExecutiveGoalManager.updateGoalStatus(goalId, success ? 'COMPLETED' : 'FAILED', success ? 'All tasks complete' : 'Criteria failed');
        }
