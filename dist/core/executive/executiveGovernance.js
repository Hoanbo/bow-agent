...
export class ExecutiveGovernance {
    _protectedPath = path.normalize('C:/BOW/shopofbow').toLowerCase();
    _forbiddenExecutables = [
        'cmd.exe',
        'powershell.exe',
        'pwsh.exe',
        'bash.exe',
    ...
    evaluate(task) {
        ...
        if (task.plan?.targetPath) {
            ...
            if (normTarget.startsWith(this._protectedPath)) {
                violations.push(`Protected workspace violation: Access to '${task.plan.targetPath}' is strictly forbidden.`);
            }
        }
