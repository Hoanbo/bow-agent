...
export class ExecutiveRuntime {
    ...
    _failedGoalsCount = 0;
    _totalTasksExecutedCount = 0;
    // -------------------------------------------------------------------------
    // 1. Goal Submission & Stepwise Orchestration
    // -------------------------------------------------------------------------
    async submitGoal(opts, customTaskSpecs) {
        // 1. Check USER_STOP
    ...
    async interpretGoal(goalId) {
        ...
        if (interpretation.containsForbiddenPatterns) {
            globalExecutiveGoalManager.updateGoalStatus(goal.goalId, 'FAILED', `Forbidden patterns: ${interpretation.forbiddenReasons.join('; ')}`);
    ...
    async decomposeGoal(goalId, customTaskSpecs) {
        const goal = globalExecutiveGoalManager.getGoal(goalId);
        if (!goal)
            throw new Error(`[GOAL_NOT_FOUND] Goal '${goalId}' not found`);
     ...
     */
    async stepGoal(goalId, humanAuthTokenId) {
        const goal = globalExecutiveGoalManager.getGoal(goalId);
        if (!goal) {
            throw new Error(`[GOAL_NOT_FOUND] Goal '${goalId}' not found`);
        }
        ...
        if (!nextTask) {
            ...
            if (progress.isComplete) {
                ...
                if (success) {
                    ...
                    globalExecutiveGoalManager.updateGoalStatus(goalId, 'FAILED', 'Tasks finished but verification/success criteria failed');
        ...
        if (!govDecision.permitted) {
            globalExecutiveTaskManager.updateTaskStatus(nextTask.taskId, 'FAILED', `Governance blocked: ${govDecision.violations.join('; ')}`);
            globalExecutiveGoalManager.updateGoalStatus(goalId, 'FAILED', `Critical security violation: ${govDecision.violations.join('; ')}`);
            this._failedGoalsCount++;
        ...
        if (govDecision.requiresHumanApproval && !humanAuthTokenId) {
            globalExecutiveTaskManager.updateTaskStatus(nextTask.taskId, 'AWAITING_HUMAN', 'Task requires human authorization token');
            globalExecutiveGoalManager.updateGoalStatus(goalId, 'AWAITING_HUMAN', `Task '${nextTask.taskId}' awaiting human authorization`);
        ...
        try {
            const execResult = await globalExecutiveExecution.executeTask(nextTask, goal.sessionId, humanAuthTokenId, goal.goalId);
            ...
            if (!execResult.success) {
                ...
                if (assessment.suggestedAction === 'RETRY') {
                    globalExecutiveTaskManager.updateTaskStatus(nextTask.taskId, 'RECOVERING', `Attempting recovery: ${assessment.reason}`);
                    ...
                    const escalation = globalExecutiveEscalation.createEscalation(goalId, `Task failed: ${execResult.error}`, assessment.reason, 'Manual operator decision or abort required', nextTask);
                    globalExecutiveGoalManager.updateGoalStatus(goalId, 'FAILED', `Unrecoverable failure on task '${nextTask.taskId}': ${execResult.error}`);
                    this._failedGoalsCount++;
            ...
            if (!verified) {
                ...
                globalExecutiveGoalManager.updateGoalStatus(goalId, 'FAILED', `Independent verification failed for task '${nextTask.taskId}'`);
                this._failedGoalsCount++;
                return goal;
            }
            // 9. Task Completion
     ...
     */
    async runGoalUntilCompletion(goalId, options) {
    ...
    cancelGoal(goalId, reason = 'User cancellation') {
    ...
    pauseGoal(goalId, reason = 'User pause') {
    ...
    restoreFromCheckpoint(goalId) {
        ...
        for (const task of checkpoint.tasks)
            globalExecutiveTaskManager.restoreTask(task);
