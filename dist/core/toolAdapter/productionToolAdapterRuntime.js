...
import { globalGovernedPEP } from '../policyEnforcement/index.js';
import { globalDiagnosisSanitizer } from '../diagnosis/diagnosisSanitizer.js';
import { TOOL_ADAPTER_AUDIT_DOMAIN, DEFAULT_EXECUTION_TIMEOUT_MS, MAX_EXECUTION_TIMEOUT_MS, MAX_RESULT_PAYLOAD_BYTES, ToolAdapterTimeoutError, ToolExecutionFailureError, ToolReplayError, } from './toolAdapterTypes.js';
import { globalToolAdapterRegistry } from './toolAdapterRegistry.js';
import { globalAuthorizedHandoffValidator } from './authorizedHandoffValidator.js';
import { globalToolExecutionGate } from './toolExecutionGate.js';
export class ProductionToolAdapterRuntime {
    ...
    constructor(options) {
        ...
        this.defaultTimeoutMs = Math.min(options?.defaultTimeoutMs ?? DEFAULT_EXECUTION_TIMEOUT_MS, MAX_EXECUTION_TIMEOUT_MS);
    }
    /**
     ...
     */
    async executeHandoff(handoff, options) {
        ...
        try {
            ...
            const eventType = err.code === 'CROSS_TENANT_TOOL_EXECUTION_ERROR'
        ...
        if (this.executedHandoffHashes.has(handoff.handoffProvenanceHash)) {
            ...
            throw new ToolReplayError(`TOOL_REPLAY_FORBIDDEN: Authorized handoff with hash "${handoff.handoffProvenanceHash}" has already been executed.`);
        }
        this.recordAudit({
        ...
        try {
            ...
            const eventType = err.code === 'TOOL_ADAPTER_DISABLED'
        ...
        // ------------------------------------------------------------------------
        const timeoutMs = Math.min(options?.timeoutMs ?? this.defaultTimeoutMs, MAX_EXECUTION_TIMEOUT_MS);
        const context = {
            taskId: handoff.taskId,
        ...
        try {
            ...
            if (handoff.leaseId) {
                try {
                    this.pep.releaseLease(handoff.leaseId);
                }
                catch {
     ...
     */
    async invokeWithTimeout(adapter, args, context, timeoutMs) {
        let timerId;
        const timeoutPromise = new Promise((_, reject) => {
            timerId = setTimeout(() => {
                reject(new ToolAdapterTimeoutError(`EXECUTION_TIMEOUT: Tool adapter "${adapter.toolName}" exceeded maximum timeout of ${timeoutMs}ms.`));
            }, timeoutMs);
        });
        try {
    ...
    recordAudit(params) {
        try {
            ...
            this.auditLedger.record({
                timestamp,
                actor: {
                    userId: sanitizedDetails.userId || 'tool_adapter_runtime',
                    role: 'agent',
                    channel: 'governance',
                },
