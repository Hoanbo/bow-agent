...
import { ToolHandoffValidationError, ToolSecurityViolationError, CrossTenantToolExecutionError, StaleToolExecutionError, MAX_ARG_PAYLOAD_BYTES, MAX_ARG_DEPTH, } from './toolAdapterTypes.js';
export class AuthorizedHandoffValidator {
     ...
     */
    validateHandoff(handoff, authoritativeTask, currentTimeMs) {
        ...
        for (const field of requiredStringFields) {
            ...
            if (typeof val !== 'string' || !val.trim()) {
                throw new ToolHandoffValidationError(`MISSING_FIELD: Handoff field "${field}" must be a non-empty string.`);
            }
        }
        // 3. Authorization Decision MUST be 'PERMIT'
        if (candidate.authorizationDecision !== 'PERMIT') {
            throw new ToolHandoffValidationError(`UNAUTHORIZED_DECISION: Execution requires authorizationDecision === 'PERMIT', received "${String(candidate.authorizationDecision)}".`);
        }
        // 4. Expiration check
        const now = currentTimeMs ?? Date.now();
        const expiresAtMs = Date.parse(candidate.expiresAt);
        if (Number.isNaN(expiresAtMs)) {
            throw new ToolHandoffValidationError(`INVALID_TIMESTAMP: Field "expiresAt" ("${candidate.expiresAt}") is not a valid ISO date.`);
        }
        if (expiresAtMs <= now) {
            throw new ToolHandoffValidationError(`HANDOFF_EXPIRED: Authorized handoff expired at ${candidate.expiresAt} (current time: ${new Date(now).toISOString()}).`);
        }
        // 5. Hard-forbidden action safety floor
        const toolName = candidate.toolName.trim();
        if (CANONICAL_HARD_FORBIDDEN_ACTIONS.includes(toolName)) {
            throw new ToolSecurityViolationError(`HARD_FORBIDDEN_ACTION: Action "${toolName}" is permanently FORBIDDEN by canonical safety floor.`);
        }
        // 6. Provenance hash format check (SHA-256 hex string)
        const provHash = candidate.handoffProvenanceHash;
        if (!/^[a-f0-9]{64}$/i.test(provHash)) {
            throw new ToolHandoffValidationError(`INVALID_PROVENANCE_HASH: "handoffProvenanceHash" must be a 64-character hex SHA-256 digest.`);
        }
        // 7. Authoritative Task & Tenant binding
        if (authoritativeTask) {
            if (candidate.taskId !== authoritativeTask.taskId) {
                throw new ToolHandoffValidationError(`TASK_ID_MISMATCH: Handoff taskId "${candidate.taskId}" does not match authoritative taskId "${authoritativeTask.taskId}".`);
            }
            if (candidate.tenantId !== authoritativeTask.tenantId) {
                throw new CrossTenantToolExecutionError(`CROSS_TENANT_TASK: Handoff tenantId "${candidate.tenantId}" does not match authoritative task tenantId "${authoritativeTask.tenantId}".`);
            }
            ...
            if (typeof candidate.taskVersion === 'number') {
                if (candidate.taskVersion !== authoritativeTask.version) {
                    throw new StaleToolExecutionError(`STALE_TASK_VERSION: Handoff taskVersion (${candidate.taskVersion}) does not match current task version (${authoritativeTask.version}).`);
                }
            }
        }
        ...
        if (args !== undefined && args !== null) {
            if (typeof args !== 'object' || Array.isArray(args)) {
                throw new ToolHandoffValidationError('INVALID_ARGUMENTS: Field "sanitizedArgs" must be a plain key-value object.');
            }
            this.validateArgumentsDefensively(args);
        }
    }
    /**
     * Defensively validates argument payload against size bounds, depth limits, prototype pollution, and null bytes.
     */
    validateArgumentsDefensively(args) {
        let serialized;
        try {
        ...
        if (byteLength > MAX_ARG_PAYLOAD_BYTES) {
            throw new ToolSecurityViolationError(`PAYLOAD_SIZE_EXCEEDED: Arguments payload size (${byteLength} bytes) exceeds maximum limit of ${MAX_ARG_PAYLOAD_BYTES} bytes.`);
        }
        // Direct serialized key inspection
        if (/(?:__proto__|constructor|prototype)\s*":/i.test(serialized) || serialized.includes('"__proto__"')) {
            throw new ToolSecurityViolationError('PROTOTYPE_POLLUTION_KEY: Forbidden prototype pollution key detected in arguments payload.');
        }
        // Deep recursive scan for dangerous keys, abnormal prototype chains, null bytes, and recursion depth
        this.scanRecursive(args, 0);
    }
    scanRecursive(val, currentDepth) {
        if (currentDepth > MAX_ARG_DEPTH) {
            throw new ToolSecurityViolationError(`MAX_DEPTH_EXCEEDED: Arguments recursion depth exceeds limit of ${MAX_ARG_DEPTH}.`);
        }
        ...
        if (typeof val === 'object') {
            ...
            if (proto !== null && proto !== Object.prototype && proto !== Array.prototype) {
                throw new ToolSecurityViolationError('PROTOTYPE_POLLUTION_KEY: Abnormal prototype hierarchy detected in arguments.');
            }
            if (Object.prototype.hasOwnProperty.call(val, '__proto__') ||
                Object.prototype.hasOwnProperty.call(val, 'prototype') ||
                (Object.prototype.hasOwnProperty.call(val, 'constructor') && typeof val.constructor !== 'function')) {
                throw new ToolSecurityViolationError('PROTOTYPE_POLLUTION_KEY: Forbidden prototype pollution property detected in arguments.');
            }
            for (const key of Object.keys(val)) {
                if (key === '__proto__' || key === 'constructor' || key === 'prototype') {
                    throw new ToolSecurityViolationError(`PROTOTYPE_POLLUTION_KEY: Forbidden prototype pollution key "${key}" detected in arguments.`);
                }
                if (key.includes('\0')) {
                    throw new ToolSecurityViolationError('NULL_BYTE_DETECTED: Object key contains forbidden null byte (\\0).');
                }
                this.scanRecursive(val[key], currentDepth + 1);
            }
        }
    }
}
