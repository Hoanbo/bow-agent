...
import { computeBrainIdentity, computeContinuityFingerprint, } from './coordinationFingerprint.js';
import { validateCoordinationScope, hasCoordinationPrototypePollution, containsCoordinationSecret, } from './coordinationValidator.js';
import { createSurface, createActiveSurface, } from './coordinationSurface.js';
import { assertValidSurfaceTransition } from './coordinationTransitions.js';
import { validateHandoff, acceptHandoff, rejectHandoff, } from './coordinationHandoff.js';
import { detectSplitBrainConflict, detectStaleUpdate, } from './coordinationConsistency.js';
import { createCoordinationCheckpoint } from './coordinationCheckpoint.js';
import { createCoordinationRecord, } from './coordinationResult.js';
/**
...
export class CoordinationService {
    ...
    checkpoints = new Map();
    history = new Map();
    surfaceOwnership = new Map();
    getPartitionKey(userId, sessionId) {
        return `${userId}::${sessionId}`;
    }
    getNextSequence(userId, sessionId) {
    ...
    getCurrentSequence(userId, sessionId) {
        const key = this.getPartitionKey(userId, sessionId);
        return this.sequences.get(key) ?? 0;
    }
    recordAction(brainId, userId, sessionId, action, status, metadata) {
        const key = this.getPartitionKey(userId, sessionId);
        const seq = this.getCurrentSequence(userId, sessionId);
        const active = this.getActiveSurfaces(userId, sessionId);
        const record = createCoordinationRecord(brainId, userId, sessionId, seq, action, status, active, metadata);
     ...
     */
    registerBrain(ownerUserId, name = 'BOWCON Central Brain', seed) {
     ...
     */
    registerSurface(surfaceType, name, capabilities) {
     ...
     */
    attachSurface(surfaceId, userId, sessionId, brainId, initialStatus = 'AVAILABLE') {
     ...
     */
    updateSurfaceStatus(surfaceId, newStatus, userId, sessionId) {
     ...
     */
    detachSurface(surfaceId, userId, sessionId, reason = 'USER_DETACHED') {
        validateCoordinationScope(userId, sessionId);
        const key = this.getPartitionKey(userId, sessionId);
        const surfaceMap = this.activeSurfaces.get(key);
        const active = surfaceMap?.get(surfaceId);
     ...
     */
    getActiveSurfaces(userId, sessionId) {
        const key = this.getPartitionKey(userId, sessionId);
        const surfaceMap = this.activeSurfaces.get(key);
     ...
     */
    getContinuityContext(userId, sessionId) {
        validateCoordinationScope(userId, sessionId);
        const key = this.getPartitionKey(userId, sessionId);
        ...
        if (!context) {
            ...
            const fingerprint = computeContinuityFingerprint(brainId, userId, sessionId, sequence, 'LOW', activeIds);
            context = deepFreeze({
     ...
     */
    createCheckpoint(userId, sessionId, metadata) {
        validateCoordinationScope(userId, sessionId);
        const brainId = this.defaultBrain?.brainId || 'brain_default';
        const sequence = this.getCurrentSequence(userId, sessionId);
        const activeSurfaces = this.getActiveSurfaces(userId, sessionId);
        const checkpoint = createCoordinationCheckpoint(brainId, userId, sessionId, sequence, activeSurfaces, metadata);
        const key = this.getPartitionKey(userId, sessionId);
     ...
     */
    detectConflict(incomingContext) {
     ...
     */
    detectStaleUpdate(userId, sessionId, incomingSequence) {
