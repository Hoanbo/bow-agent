...
import { createIncidentClosureId, } from './incidentResilienceTypes.js';
export class IncidentClosureEngine {
    ...
    evaluateClosure(input) {
        const { incidentId, remediationPlanId, executionId, verificationResult, rollbackResult, isUserStopActive = false, forceEscalationReason, } = input;
        ...
        if (isUserStopActive) {
            return this.buildRecord(closureId, incidentId, 'ESCALATED_TO_HUMAN', 'USER_STOP active during post-mitigation evaluation. Fail closed to human authority.', false, false, closedAt, true, remediationPlanId, executionId);
        }
        // Rule 2: Explicit force escalation reason
        // Quy tắc 2: Lý do cưỡng chế leo thang tường minh
        if (forceEscalationReason) {
            return this.buildRecord(closureId, incidentId, 'ESCALATED_TO_HUMAN', `Incident escalated to human: ${forceEscalationReason}`, verificationResult?.verified ?? false, rollbackResult?.success ?? false, closedAt, true, remediationPlanId, executionId);
        }
        // Rule 3: Rollback occurred
        // Quy tắc 3: Khôi phục đã xảy ra
        if (rollbackResult) {
            if (rollbackResult.success) {
                return this.buildRecord(closureId, incidentId, 'CLOSED_ROLLED_BACK', `Remediation failed verification or triggered rollback. State successfully reverted to snapshot: ${rollbackResult.snapshotId}.`, false, true, closedAt, true, // Rolled back incident requires human awareness/follow-up
                remediationPlanId, executionId);
            }
            else {
                ...
                return this.buildRecord(closureId, incidentId, 'ESCALATED_TO_HUMAN', `CRITICAL: Rollback failed (${rollbackResult.reason}). System in unverified ambiguous state.`, false, false, closedAt, true, remediationPlanId, executionId);
            }
        }
        ...
        if (isVerified) {
            return this.buildRecord(closureId, incidentId, 'CLOSED_RESOLVED', `Post-mitigation verification succeeded. Error rate, latency, and invariants within nominal thresholds (${verificationResult?.verificationSummary ?? 'Nominal'}).`, true, false, closedAt, false, remediationPlanId, executionId);
        }
        ...
        return this.buildRecord(closureId, incidentId, 'ESCALATED_TO_HUMAN', 'Unresolved or ambiguous remediation outcome without verifiable recovery evidence. Fail closed to human supervisor.', false, false, closedAt, true, remediationPlanId, executionId);
    }
    buildRecord(closureId, incidentId, status, closureReason, verified, rolledBack, closedAt, requiresHumanFollowUp, remediationPlanId, executionId) {
