 ...
 */
export function createDeviceChallenge(params) {
 ...
 */
export function createDeviceProof(params) {
    const now = params.timestamp ?? Date.now();
    ...
    if (key.deviceId !== params.challenge.deviceId) {
        throw new Error(`[DEVICE_CLONE_DETECTED] Key deviceId (${key.deviceId}) does not match challenge deviceId (${params.challenge.deviceId})`);
    }
 ...
 */
export function generateDeviceProof(challenge, keyStore, keyId, timestamp) {
 ...
 */
export function verifyDeviceProof(challenge, proof, keyStore, currentTime) {
