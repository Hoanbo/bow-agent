...
export const PERSISTENT_DEVICE_TRANSITION_MATRIX = Object.freeze({
...
export const DEVICE_KEY_TRANSITION_MATRIX = Object.freeze({
 ...
 */
export function isValidPersistentDeviceStateTransition(from, to) {
    if (from === to) {
 ...
 */
export function assertValidPersistentDeviceStateTransition(from, to) {
    if (!isValidPersistentDeviceStateTransition(from, to)) {
        throw new Error(`[INVALID_STATE_TRANSITION] Illegal persistent device state transition from '${from}' to '${to}'`);
    }
}
/**
 * Validates whether a device key state transition is permitted.
 */
export function isValidDeviceKeyStateTransition(from, to) {
    if (from === to) {
        return true; // Idempotent no-op
    }
 ...
 */
export function assertValidDeviceKeyStateTransition(from, to) {
    if (!isValidDeviceKeyStateTransition(from, to)) {
        throw new Error(`[DEVICE_INVALID_TRANSITION] Illegal device key state transition from '${from}' to '${to}'`);
    }
}
