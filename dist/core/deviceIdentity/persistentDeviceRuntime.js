...
import { InMemoryPersistentDeviceStore } from './persistentDeviceStorage.js';
import { InMemoryDeviceKeyStore } from './persistentDeviceKey.js';
...
import { createSuccessRecognitionResult, createFailureRecognitionResult, createFailureRehydrationResult, } from './persistentDeviceResult.js';
import { generatePersistentDeviceId } from './persistentDeviceIdentity.js';
...
export class PersistentDeviceIdentityRuntime {
     ...
     */
    createRecognitionChallenge(deviceId, scope, ttlMs) {
     ...
     */
    respondToChallenge(challengeId, deviceId) {
    ...
    verifyProofAndRecognize(proof) {
        ...
        if (!challenge) {
            ...
            return createFailureRecognitionResult('DEVICE_CHALLENGE_MISMATCH', `Challenge ${proof.challengeId} not found or expired`);
        }
        ...
        if (this.registry.hasProofNonce(proof.deviceId, proof.nonce)) {
            ...
            return createFailureRecognitionResult('DEVICE_PROOF_REPLAY', `Proof nonce ${proof.nonce} has already been consumed (replay attempt blocked)`);
        }
        ...
        if (!verification.valid) {
            ...
            return createFailureRecognitionResult(verification.failureCode ?? 'DEVICE_PROOF_INVALID', verification.failureReason ?? 'Cryptographic proof verification failed');
        }
        ...
        if (!trustValidation.trusted) {
            ...
            return createFailureRecognitionResult(trustValidation.failureCode ?? 'DEVICE_NOT_TRUSTED', trustValidation.failureReason ?? 'Device trust validation failed', 'FAILED', record);
        }
     ...
     */
    rehydrateDevice(deviceId, currentScope) {
    ...
    revokeDevice(deviceId, reason) {
        ...
        const revokedResult = revokePersistentDevice(this.store, this.keyStore, deviceId, record.scopeString, reason);
    ...
    rotateDeviceKey(deviceId) {
        ...
        const rotationResult = rotatePersistentDeviceKey(this.store, this.keyStore, deviceId, record.scopeString);
