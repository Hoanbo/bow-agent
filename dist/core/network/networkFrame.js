...
import { computeNetworkFrameFingerprint, computePayloadChecksum, } from './networkFingerprint.js';
import { validateNetworkIdentifier, validateNetworkPayloadBounds, deepFreeze, } from './networkValidator.js';
/**
 * EN: Constructs an authoritative immutable NetworkFrame.
 * VI: Khởi tạo một NetworkFrame bất biến có thẩm quyền.
 */
export function createNetworkFrame(params) {
    const protocolVersion = params.protocolVersion ?? '1.0.0';
    const networkConnectionId = validateNetworkIdentifier('networkConnectionId', params.networkConnectionId);
    ...
    if (typeof params.sequence !== 'number' ||
        ...
        params.sequence < 0) {
        throw new Error(`[NETWORK_FRAME_ERROR] Sequence must be a non-negative integer. Received ${params.sequence}.`);
    }
 ...
 */
export function validateNetworkFrame(frame) {
    if (!frame || typeof frame !== 'object')
        return false;
    const f = frame;
    if (typeof f.frameId !== 'string' ||
 ...
 */
export function assertValidNetworkFrame(frame) {
