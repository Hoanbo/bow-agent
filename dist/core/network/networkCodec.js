...
import { validateNetworkPayloadBounds, validateNetworkIdentifier, redactNetworkSecrets, MAX_NETWORK_PAYLOAD_SIZE_BYTES, deepFreeze, } from './networkValidator.js';
import { computePayloadChecksum, computeNetworkFrameFingerprint, } from './networkFingerprint.js';
    ...
    encode(frame) {
        try {
            ...
            if (encoded.length > this.maxRawSizeBytes) {
                throw new Error(`Encoded frame exceeds max size of ${this.maxRawSizeBytes} bytes (got ${encoded.length}).`);
            }
            return encoded;
        }
        catch (err) {
    ...
    decode(raw) {
        try {
            ...
            if (raw.length > this.maxRawSizeBytes) {
                throw new Error(`Raw input size (${raw.length} bytes) exceeds limit of ${this.maxRawSizeBytes} bytes.`);
            }
            if (raw.includes('\0')) {
            ...
            const networkConnectionId = validateNetworkIdentifier('networkConnectionId', parsed.networkConnectionId);
            ...
            if (parsed.checksum !== actualChecksum) {
                throw new Error(`Frame checksum mismatch: declared "${parsed.checksum}", computed "${actualChecksum}".`);
            }
            ...
            if (parsed.fingerprint !== expectedFingerprint) {
                throw new Error(`Frame fingerprint mismatch: declared "${parsed.fingerprint}", computed "${expectedFingerprint}".`);
            }
            const decodedFrame = {
                frameId,
                protocolVersion,
