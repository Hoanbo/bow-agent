 ...
 */
export function createNetworkOperationResult(params) {
    const ts = params.timestamp ?? 0;
    const fp = fnv1aHex(`NET_RES::${params.status}::${params.networkConnectionId ?? 'NONE'}::${params.frameId ?? 'NONE'}::${params.success}::${ts}`);
    const result = {
