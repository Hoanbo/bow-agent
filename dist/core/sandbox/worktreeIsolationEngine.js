...
import { createWorktreeId, SandboxError, } from './sandboxTypes.js';
...
export class WorktreeIsolationEngine {
     ...
     */
    validateScopeContainment(parentScope, childScope) {
        ...
        for (const op of childScope.allowedOperations) {
            if (!parentScope.allowedOperations.includes(op)) {
                throw new SandboxError('WORKTREE_SCOPE_CANNOT_EXCEED_SANDBOX', `Child worktree requested operation "${op}" not permitted in parent sandbox scope.`);
            }
        }
        ...
        if (parentScope.maxFileCount !== undefined) {
            if (childScope.maxFileCount === undefined || childScope.maxFileCount > parentScope.maxFileCount) {
                throw new SandboxError('WORKTREE_SCOPE_CANNOT_EXCEED_SANDBOX', `Child worktree maxFileCount (${childScope.maxFileCount}) exceeds parent limit (${parentScope.maxFileCount}).`);
            }
        }
        ...
        if (parentScope.maxWorkspaceSizeBytes !== undefined) {
            if (childScope.maxWorkspaceSizeBytes === undefined ||
                childScope.maxWorkspaceSizeBytes > parentScope.maxWorkspaceSizeBytes) {
                throw new SandboxError('WORKTREE_SCOPE_CANNOT_EXCEED_SANDBOX', `Child worktree maxWorkspaceSizeBytes exceeds parent sandbox limit.`);
            }
        }
    }
    /**
    ...
    createWorktree(input) {
        ...
        const { absolutePath: worktreeAbs } = SandboxPathGuard.resolveAndAssertContainedPath(sandbox.rootPath, relativeWorktreeDir);
