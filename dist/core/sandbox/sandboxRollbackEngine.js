...
export class SandboxRollbackEngine {
    fsEngine;
    policyEngine;
    constructor(fsEngine, policyEngine) {
     ...
     */
    rollbackAll(sandbox, context, reason = 'Governed rollback requested') {
        ...
        if (context.sessionId !== sandbox.binding.sessionId) {
            throw new SandboxError('CROSS_SESSION_SANDBOX_REJECTED', `Cannot rollback sandbox across sessions: "${context.sessionId}" != "${sandbox.binding.sessionId}".`);
        }
        ...
        for (const [relPath, entry] of staged.entries()) {
            const { absolutePath: absPath } = SandboxPathGuard.resolveAndAssertContainedPath(sandbox.rootPath, relPath);
     ...
     */
    rollbackFile(sandbox, relativePath, originalContent, context) {
        if (this.policyEngine.isUserStopped() || sandbox.isStopped) {
            throw new SandboxError('USER_STOP_ACTIVE', 'Cannot perform rollback: USER_STOP is active.');
        }
        if (context.sessionId !== sandbox.binding.sessionId) {
        ...
        const { absolutePath: absPath } = SandboxPathGuard.resolveAndAssertContainedPath(sandbox.rootPath, relativePath);
