...
export class SandboxPolicyEngine {
    ...
    _userStopReason = '';
    violations = [];
    constructor(delegationRuntime, leaseManager) {
     ...
     */
    recordViolation(code, message, sessionId, sandboxId, agentId, filePath) {
        const v = {
    ...
    validateOperation(input) {
        ...
        if (this._isUserStopped || sandbox.isStopped) {
            this.recordViolation('USER_STOP_ACTIVE', `Operation rejected: USER_STOP is active (${this._userStopReason || 'Emergency Stop'}).`, sessionId, sandbox.id, agentId, relativePath);
            throw new SandboxError('USER_STOP_ACTIVE', `USER_STOP is active. Operation ${operationType} on "${relativePath}" is rejected.`);
        }
        // 2. Enforce strict session isolation.
        // 2. Thực thi cô lập phiên nghiêm ngặt.
        if (sessionId !== sandbox.binding.sessionId) {
            this.recordViolation('CROSS_SESSION_SANDBOX_REJECTED', `Session mismatch: request session "${sessionId}" does not match sandbox session "${sandbox.binding.sessionId}".`, sessionId, sandbox.id, agentId, relativePath);
            throw new SandboxError('CROSS_SESSION_SANDBOX_REJECTED', `Cross-session sandbox access rejected: "${sessionId}" != "${sandbox.binding.sessionId}".`);
        }
        // 3. Enforce task binding.
        // 3. Thực thi ràng buộc tác vụ.
        if (taskId !== sandbox.binding.taskId) {
            this.recordViolation('POLICY_VIOLATION', `Task mismatch: request task "${taskId}" does not match sandbox task "${sandbox.binding.taskId}".`, sessionId, sandbox.id, agentId, relativePath);
            throw new SandboxError('POLICY_VIOLATION', `Task mismatch: "${taskId}" is not authorized for sandbox "${sandbox.id}".`);
        }
        ...
        if (agentId !== sandbox.binding.agentId) {
            this.recordViolation('POLICY_VIOLATION', `Agent mismatch: request agent "${agentId}" does not match sandbox agent "${sandbox.binding.agentId}".`, sessionId, sandbox.id, agentId, relativePath);
            throw new SandboxError('POLICY_VIOLATION', `Agent "${agentId}" is not authorized to operate sandbox "${sandbox.id}".`);
        }
        // 5. Enforce device binding.
        // 5. Thực thi ràng buộc thiết bị.
        if (deviceId !== sandbox.binding.deviceId) {
            this.recordViolation('POLICY_VIOLATION', `Device mismatch: request device "${deviceId}" does not match sandbox device "${sandbox.binding.deviceId}".`, sessionId, sandbox.id, agentId, relativePath);
            throw new SandboxError('POLICY_VIOLATION', `Device "${deviceId}" is not authorized for sandbox "${sandbox.id}".`);
        }
        ...
        if (sandbox.isRevoked || sandbox.state === 'REVOKED') {
            this.recordViolation('REVOKED_DELEGATION', `Operation rejected: sandbox "${sandbox.id}" has been revoked.`, sessionId, sandbox.id, agentId, relativePath);
            throw new SandboxError('REVOKED_DELEGATION', `Sandbox "${sandbox.id}" is revoked.`);
        }
        const now = Date.now();
        if (now > sandbox.expiresAt || sandbox.state === 'EXPIRED') {
            this.recordViolation('SANDBOX_EXPIRED', `Operation rejected: sandbox "${sandbox.id}" expired at ${new Date(sandbox.expiresAt).toISOString()}.`, sessionId, sandbox.id, agentId, relativePath);
        ...
        if (!validStates.includes(sandbox.state)) {
            this.recordViolation('SANDBOX_NOT_ACTIVE', `Sandbox "${sandbox.id}" is in non-operational state: "${sandbox.state}".`, sessionId, sandbox.id, agentId, relativePath);
            throw new SandboxError('SANDBOX_NOT_ACTIVE', `Cannot execute operation in sandbox state: "${sandbox.state}".`);
        }
        ...
        if (this.leaseManager) {
            try {
                ...
                this.leaseManager.validateLease(sandbox.binding.capabilityLeaseId, {
                    sessionId,
                    capabilityId: lease.capabilityId,
                });
            }
            catch (err) {
                this.recordViolation('INVALID_CAPABILITY_LEASE', `Capability lease validation failed: ${err.message}`, sessionId, sandbox.id, agentId, relativePath);
        ...
        if (!sandbox.scope.allowedOperations.includes(operationType)) {
            this.recordViolation('POLICY_VIOLATION', `Operation "${operationType}" is not permitted in sandbox scope.`, sessionId, sandbox.id, agentId, relativePath);
            throw new SandboxError('POLICY_VIOLATION', `Operation type "${operationType}" is not permitted by sandbox scope.`);
        }
        ...
        if (sandbox.scope.allowedFileExtensions && sandbox.scope.allowedFileExtensions.length > 0) {
            ...
            if (operationType === 'CREATE' || operationType === 'UPDATE') {
                const isAllowedExt = sandbox.scope.allowedFileExtensions
                    ...
                    .includes(ext);
                if (!isAllowedExt) {
                    this.recordViolation('POLICY_VIOLATION', `File extension "${ext}" is not permitted in sandbox scope.`, sessionId, sandbox.id, agentId, relativePath);
                    throw new SandboxError('POLICY_VIOLATION', `File extension "${ext}" is forbidden by sandbox scope.`);
                }
            }
        }
        ...
        if (sandbox.scope.deniedFilePatterns && sandbox.scope.deniedFilePatterns.length > 0) {
            ...
            for (const pattern of sandbox.scope.deniedFilePatterns) {
                if (lowerRel.includes(pattern.toLowerCase())) {
                    this.recordViolation('POLICY_VIOLATION', `Path matches denied file pattern: "${pattern}".`, sessionId, sandbox.id, agentId, relativePath);
                    throw new SandboxError('POLICY_VIOLATION', `File path "${relativePath}" matches denied pattern "${pattern}".`);
                }
            }
        }
        ...
        if (operationType === 'CREATE' && sandbox.scope.maxFileCount !== undefined) {
            if ((input.currentFileCount ?? 0) >= sandbox.scope.maxFileCount) {
                throw new SandboxError('POLICY_VIOLATION', `Sandbox file count limit reached: ${sandbox.scope.maxFileCount} files.`);
            }
        }
        if ((operationType === 'CREATE' || operationType === 'UPDATE') &&
            ...
            if (totalSize > sandbox.scope.maxWorkspaceSizeBytes) {
                throw new SandboxError('POLICY_VIOLATION', `Sandbox workspace size limit exceeded: ${totalSize} > ${sandbox.scope.maxWorkspaceSizeBytes} bytes.`);
            }
        }
    }
    /**
