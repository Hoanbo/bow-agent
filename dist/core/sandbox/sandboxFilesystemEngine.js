...
export class SandboxFilesystemEngine {
     ...
     */
    assertNoForbiddenCredentials(content, filePath) {
        ...
        for (const pattern of forbiddenPatterns) {
            if (lower.includes(pattern)) {
                throw new SandboxError('FORBIDDEN_CREDENTIAL_PERSISTENCE', `Forbidden credential or authorization token pattern detected in file "${filePath}": "${pattern}".`);
            }
        }
    }
    /**
     ...
     */
    createFile(sandbox, relativePath, content, context) {
        // 1. Path containment check.
        // 1. Kiểm tra bao chứa đường dẫn.
        const { relativePath: safeRel, absolutePath: safeAbs } = SandboxPathGuard.resolveAndAssertContainedPath(sandbox.rootPath, relativePath);
        ...
        if (fs.existsSync(safeAbs)) {
            throw new SandboxError('POLICY_VIOLATION', `Cannot create file: "${safeRel}" already exists in sandbox "${sandbox.id}".`);
        }
     ...
     */
    readFile(sandbox, relativePath, context) {
        const { relativePath: safeRel, absolutePath: safeAbs } = SandboxPathGuard.resolveAndAssertContainedPath(sandbox.rootPath, relativePath);
        this.policyEngine.validateOperation({
            sandbox,
            operationType: 'READ',
     ...
     */
    updateFile(sandbox, relativePath, content, context) {
        const { relativePath: safeRel, absolutePath: safeAbs } = SandboxPathGuard.resolveAndAssertContainedPath(sandbox.rootPath, relativePath);
        this.assertNoForbiddenCredentials(content, safeRel);
        const metrics = this.getSandboxMetrics(sandbox.rootPath);
     ...
     */
    renameFile(sandbox, oldRelativePath, newRelativePath, context) {
        const { relativePath: safeOldRel, absolutePath: safeOldAbs } = SandboxPathGuard.resolveAndAssertContainedPath(sandbox.rootPath, oldRelativePath);
        const { relativePath: safeNewRel, absolutePath: safeNewAbs } = SandboxPathGuard.resolveAndAssertContainedPath(sandbox.rootPath, newRelativePath);
        this.policyEngine.validateOperation({
            sandbox,
     ...
     */
    deleteFile(sandbox, relativePath, context) {
        const { relativePath: safeRel, absolutePath: safeAbs } = SandboxPathGuard.resolveAndAssertContainedPath(sandbox.rootPath, relativePath);
        this.policyEngine.validateOperation({
            sandbox,
     ...
     */
    listFiles(sandbox, relativeDir = '', context) {
        const targetDir = relativeDir === '' ? '.' : relativeDir;
        const { relativePath: safeRel, absolutePath: safeAbs } = SandboxPathGuard.resolveAndAssertContainedPath(sandbox.rootPath, targetDir);
        this.policyEngine.validateOperation({
            sandbox,
            operationType: 'LIST',
            relativePath: safeRel,
        ...
        const walk = (dir) => {
            ...
            for (const item of items) {
                ...
                const { relativePath: itemRel } = SandboxPathGuard.resolveAndAssertContainedPath(sandbox.rootPath, path.relative(sandbox.rootPath, itemAbs));
                if (item.isDirectory()) {
                    results.push({
     ...
     */
    statFile(sandbox, relativePath, context) {
        const { relativePath: safeRel, absolutePath: safeAbs } = SandboxPathGuard.resolveAndAssertContainedPath(sandbox.rootPath, relativePath);
        this.policyEngine.validateOperation({
            sandbox,
            operationType: 'STAT',
