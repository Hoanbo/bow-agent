...
export class SandboxPathGuard {
     ...
     */
    static resolveAndAssertContainedPath(sandboxRoot, rawRelativePath) {
        ...
        if (path.isAbsolute(rawRelativePath) || /^[a-zA-Z]:[\\/]/.test(rawRelativePath)) {
            throw new SandboxError('ABSOLUTE_PATH_REJECTED', `Absolute paths outside sandbox root are forbidden: "${rawRelativePath}"`);
        }
        ...
        for (const segment of segments) {
            if (segment === '..' || segment === '.') {
                if (segment === '..') {
                    throw new SandboxError('PATH_TRAVERSAL_REJECTED', `Path traversal segment ".." detected in "${rawRelativePath}"`);
                }
            }
        }
        ...
        const isContained = normalizedAbsolute === normalizedRoot ||
            normalizedAbsolute.startsWith(normalizedRoot + path.sep.toLowerCase());
        if (!isContained) {
            throw new SandboxError('PATH_TRAVERSAL_REJECTED', `Resolved path "${resolvedAbsolute}" escapes sandbox root "${resolvedRoot}".`);
        }
        ...
        if (fs.existsSync(resolvedAbsolute)) {
            try {
                ...
                const isRealContained = normalizedRealTarget === normalizedRoot ||
                    normalizedRealTarget.startsWith(normalizedRoot + path.sep.toLowerCase());
                if (!isRealContained) {
                    throw new SandboxError('SECURITY_VIOLATION', `Symlink or junction target "${realTargetPath}" escapes sandbox root "${resolvedRoot}".`);
                }
            }
            catch (err) {
                ...
                throw new SandboxError('SECURITY_VIOLATION', `Cannot resolve realpath for "${resolvedAbsolute}": ${err.message}`);
            }
        }
