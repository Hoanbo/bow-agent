...
import { INTERNET_MAX_CERT_AGE_DAYS, } from './internetTypes.js';
...
const STRONG_ALGORITHMS = Object.freeze([
    'ECDSA_P256',
    'ECDSA_P384',
    'RSA_2048',
    'RSA_4096',
 ...
 */
export function validateCertificate(cert, opts = {}) {
    ...
    if (!STRONG_ALGORITHMS.includes(cert.keyAlgorithm)) {
        return {
            valid: false,
 ...
 */
export function assertCertificateValid(cert, opts = {}) {
    const result = validateCertificate(cert, opts);
    if (!result.valid) {
        throw new InternetEdgeError('INTERNET_CERT_CHAIN_INVALID', result.reason ?? 'Certificate validation failed.');
    }
}
/**
 ...
 */
export function makeMockCertFingerprint(seed) {
