...
import { INTERNET_HANDSHAKE_TIMEOUT_MS, makeInternetEdgeSessionId, } from './internetTypes.js';
...
export class InternetEdge {
    _state = 'INTERNET_OFFLINE';
    _sessionId;
    _sessionInfo;
    _roamingCount = 0;
    _reconnectCount = 0;
    _totalTlsRejections = 0;
    _totalDowngradeAttempts = 0;
    _establishedAt;
    _lastActivityAt;
    _auditLedger;
    _config;
    constructor(config = {}, auditLedger) {
        this._config = {
    ...
    get isActive() { return isInternetActive(this._state); }
    get isTerminal() { return isInternetTerminal(this._state); }
    // ---------------------------------------------------------------------------
    // Lifecycle steps — called by InternetRuntime orchestrator
    // ---------------------------------------------------------------------------
    /** Step 1: Move from OFFLINE to RESOLVING. */
    beginResolve(endpoint) {
    ...
    completeTlsHandshake(params) {
        ...
        if (result.result !== 'TLS_ACCEPTED') {
            if (result.result === 'TLS_REJECTED_VERSION')
                this._totalDowngradeAttempts++;
            ...
            throw new InternetEdgeError(result.result === 'TLS_REJECTED_VERSION'
                ? 'INTERNET_TLS_VERSION_REJECTED'
                : result.result === 'TLS_REJECTED_CIPHER'
                    ? 'INTERNET_TLS_CIPHER_REJECTED'
                    : 'INTERNET_TLS_HANDSHAKE_FAILED', result.reason ?? 'TLS validation failed.');
        }
        this.transition('INTERNET_CERT_VALIDATING', 'completeTlsHandshake');
        this._sessionId = makeInternetEdgeSessionId(`iedge_${Date.now()}_${randomBytes(4).toString('hex')}`);
        appendInternetAuditEvent(this._auditLedger, {
        ...
        this._sessionInfo = {
            sessionId: this._sessionId,
            tlsVersion: result.acceptedVersion,
            cipherSuite: result.acceptedCipher,
            certFingerprint: '',
