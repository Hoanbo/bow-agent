...
export class OwnerBriefingEngine {
     ...
     */
    createEpistemicItem(value, status, confidence, evidence, source) {
        return {
            value,
            status,
    ...
    compileBriefing(context) {
        ...
        const mostImportant = this.createEpistemicItem(mostImportantGoals, 'CONFIRMED', 0.95, ['Knowledge graph active goals ledger'], 'KnowledgeGraph');
        // 3. What is blocked?
        const blockers = context.activeBlockers ?? [];
        if (context.knowledgeGraph) {
            const unresolved = context.knowledgeGraph.getUnresolvedProblems();
            for (const p of unresolved) {
                if (!blockers.includes(p.label))
                    blockers.push(p.label);
            }
        }
        ...
        const recentChanges = this.createEpistemicItem(recentChangesList, 'OBSERVED', 0.9, ['Recent decisions recorded in durable memory'], 'MemoryStore');
        // 5. What failed recently?
        const recentFailuresList = [];
        ...
        const nextConsiderations = this.createEpistemicItem(nextCons, 'INFERRED', 0.8, ['Recommendation engine next step synthesis'], 'RecommendationEngine');
        ...
        const progressingGoals = this.createEpistemicItem(progressingGoalsList, 'CONFIRMED', 0.95, ['Active projects in Knowledge Graph'], 'KnowledgeGraph');
        return {
            briefingId,
            ownerId,
            currentWorkingOn,
