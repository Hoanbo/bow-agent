...
import { CognitiveStatePromotionError, } from './cognitiveStateTypes.js';
export class CognitiveStatePromotionGate {
     ...
     */
    static evaluatePromotion(request) {
        ...
        if (request.candidateType === 'HYPOTHESIS_TO_FACT') {
            ...
            if (hyp.status !== 'VERIFIED') {
                throw new CognitiveStatePromotionError(`Cannot promote hypothesis '${hyp.hypothesisId}': status is '${hyp.status}', strictly required 'VERIFIED'`, { hypothesisId: hyp.hypothesisId, status: hyp.status });
            }
            if (!request.verificationEvidenceId || !request.verificationOracle) {
                throw new CognitiveStatePromotionError(`Cannot promote hypothesis '${hyp.hypothesisId}' without verified empirical evidence ID and oracle reference`, { hypothesisId: hyp.hypothesisId });
            }
