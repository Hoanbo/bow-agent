 ...
 */
export function validateScopeSegment(segment, segmentName) {
 ...
 */
export function createConnectionScope(identity) {
 ...
 */
export function parseConnectionScope(scopeStr) {
 ...
 */
export function assertConnectionScopeMatch(expected, actual) {
    if (expected.userId !== actual.userId ||
        ...
        throw new Error(`[CONNECTION_SCOPE_MISMATCH] Scope mismatch: expected ${createConnectionScope(expected)} but got ${createConnectionScope(actual)}`);
    }
}
/**
 ...
 */
export function scrubConnectionSecrets(val) {
    ...
    if (Array.isArray(val)) {
        return val.map((item) => scrubConnectionSecrets(item));
    }
