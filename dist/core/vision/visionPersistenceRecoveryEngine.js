...
import { VISION_SCHEMA_VERSION, computeSessionDocumentHash, VisionUserStopError, VisionConcurrencyError, CrossTenantVisionError, VisionIntegrityError, VisionValidationError, } from './visionTypes.js';
...
export class VisionPersistenceRecoveryEngine {
     ...
     */
    createSessionDocument(tenantId, sessionId) {
     ...
     */
    saveSessionDocument(doc, expectedVersion, activeTenantId) {
        if (this.userStopProvider()) {
        ...
        if (doc.provenanceHash !== expectedHash) {
            throw new VisionIntegrityError(`Session document provenance mismatch: given '${doc.provenanceHash}', computed '${expectedHash}'`, { sessionId: doc.sessionId });
        }
        ...
        if (fs.existsSync(targetFile)) {
            try {
                ...
                if (err instanceof VisionConcurrencyError)
                    throw err;
            }
        }
     ...
     */
    loadSessionDocument(sessionId, tenantId) {
        if (this.userStopProvider()) {
        ...
        if (fs.existsSync(targetFile)) {
            try {
                ...
                if (err instanceof CrossTenantVisionError)
                    throw err;
     ...
     */
    deleteSessionDocument(sessionId, tenantId) {
        if (this.userStopProvider()) {
