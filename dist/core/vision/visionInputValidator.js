...
import { VISION_BOUNDS, VisionValidationError, VisionSecurityError, VisionCapacityError, } from './visionTypes.js';
...
export class VisionInputValidator {
    ...
    static validateFrame(frame) {
        ...
        if (!f.metadata || typeof f.metadata !== 'object') {
            ...
            const m = f.metadata;
            if (typeof m.frameId !== 'string' || !m.frameId.trim())
                errors.push('metadata.frameId must be non-empty string');
            if (typeof m.tenantId !== 'string' || !m.tenantId.trim())
                errors.push('metadata.tenantId must be non-empty string');
            if (typeof m.sessionId !== 'string' || !m.sessionId.trim())
                errors.push('metadata.sessionId must be non-empty string');
        ...
        if (!f.viewport || typeof f.viewport !== 'object') {
            ...
            try {
                this.validateViewport(f.viewport);
            }
            catch (err) {
     ...
     */
    static validateBoundingBox(box, viewport) {
        ...
        if (errors.length === 0) {
            if (box.x < 0)
                errors.push(`boundingBox.x (${box.x}) cannot be negative`);
            if (box.y < 0)
                errors.push(`boundingBox.y (${box.y}) cannot be negative`);
            if (box.width <= 0)
                errors.push(`boundingBox.width (${box.width}) must be strictly positive`);
            if (box.height <= 0)
                errors.push(`boundingBox.height (${box.height}) must be strictly positive`);
            ...
            for (const normProp of ['normX', 'normY', 'normWidth', 'normHeight']) {
     ...
     */
    static validateVisualElement(el, viewport) {
        ...
        if (!el.frameId || typeof el.frameId !== 'string')
            errors.push('VisualElement.frameId must be non-empty string');
     ...
     */
    static validateGroundingQuery(query) {
        ...
        const q = query;
        const errors = [];
        if (typeof q.queryId !== 'string' || !q.queryId.trim())
            errors.push('queryId must be non-empty string');
        if (typeof q.tenantId !== 'string' || !q.tenantId.trim())
        ...
        if (typeof q.frameId !== 'string' || !q.frameId.trim())
            errors.push('frameId must be non-empty string');
        if (typeof q.referenceText !== 'string' || !q.referenceText.trim())
            errors.push('referenceText must be non-empty string');
    ...
    static validateSessionDocument(doc) {
        ...
        if (typeof d.tenantId !== 'string' || !d.tenantId.trim())
            errors.push('tenantId must be non-empty string');
        if (typeof d.sessionVersion !== 'number' || d.sessionVersion < 1)
            errors.push('sessionVersion must be >= 1');
        ...
        if (!Array.isArray(d.groundingResults))
            errors.push('groundingResults must be an array');
        if (!Array.isArray(d.securityAlerts))
            errors.push('securityAlerts must be an array');
        if (errors.length > 0) {
     ...
     */
    static assertNoPrototypePollutionOrCoT(val, path = '') {
        ...
        if (typeof val === 'object') {
            for (const key of Object.getOwnPropertyNames(val)) {
                ...
                this.assertNoPrototypePollutionOrCoT(val[key], `${path}.${key}`);
            }
        }
    }
}
