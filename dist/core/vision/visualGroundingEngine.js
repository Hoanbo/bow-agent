...
import { VISION_BOUNDS, computeGroundingHash, VisionUserStopError, } from './visionTypes.js';
...
export class VisualGroundingEngine {
     ...
     */
    groundReference(query, elements, viewport, spatialGraph = []) {
        ...
        for (const el of elements) {
            ...
            let overallConfidence = 0.50 * scores.semanticScore +
    ...
    calculateCandidateScores(el, query, refLower, viewport, spatialGraph = []) {
        let textScore = 0.0;
        ...
        if (query.spatialHints && query.spatialHints.length > 0) {
            ...
            for (const hint of query.spatialHints) {
                const found = spatialGraph.some((r) => r.sourceElementId === el.elementId &&
