...
import { VISION_BOUNDS, VisionUserStopError, VisionValidationError, } from './visionTypes.js';
...
export class VisualRelationshipEngine {
    ...
    evaluatePairwise(a, b) {
        ...
        const hasVerticalOverlap = Math.max(boxA.y, boxB.y) < Math.min(boxA.y + boxA.height, boxB.y + boxB.height);
        // Check horizontal overlap for vertical alignment (Kiểm tra chồng lấn ngang cho căn hàng dọc)
        const hasHorizontalOverlap = Math.max(boxA.x, boxB.x) < Math.min(boxA.x + boxA.width, boxB.x + boxB.width);
    ...
    makeDescriptor(src, tgt, type, dist, conf) {
