...
import { VISION_BOUNDS, computeDeterministicElementId, computeElementHash, VisionUserStopError, VisionCapacityError, } from './visionTypes.js';
...
export class VisualElementDetectionEngine {
     ...
     */
    detectElements(frame, candidates) {
        ...
        if (candidates.length > VISION_BOUNDS.MAX_VISUAL_ELEMENTS) {
            throw new VisionCapacityError(candidates.length, VISION_BOUNDS.MAX_VISUAL_ELEMENTS, 'visual elements per scene');
        }
        const detected = [];
        for (const input of candidates) {
            ...
            const box = VisualFrameNormalizer.normalizeCoordinates(input.x, input.y, input.width, input.height, frame.viewport);
            ...
            const elementId = computeDeterministicElementId(frame.metadata.frameId, box.x, box.y, box.width, box.height);
            ...
            const finalElement = Object.freeze({
                ...draft,
