...
export class DeploymentExecutionEngine {
     ...
     */
    consumeToken(token, request) {
        // 1. Anti-replay check.
        // 1. Kiểm tra chống phát lại.
        if (this.consumedTokenIds.has(token.tokenId)) {
            throw new DeploymentError('TOKEN_REPLAY_REJECTED', `Single-use execution token "${token.tokenId}" has already been consumed.`);
        }
        // 2. Token expiration check.
        // 2. Kiểm tra thời hạn mã token.
        if (Date.now() > token.expiresAt) {
            throw new DeploymentError('EXPIRED_AUTHORIZATION', `Authorization token "${token.tokenId}" expired at ${token.expiresAt}.`);
        }
        // 3. Action and target binding check.
        // 3. Kiểm tra ràng buộc hành động và mục tiêu.
        if (token.target && token.target !== request.targetId) {
            throw new DeploymentError('INVALID_AUTHORIZATION', `Token target "${token.target}" does not match request targetId "${request.targetId}".`);
        }
        ...
        if (token.sessionId && token.sessionId !== request.sessionId) {
            throw new DeploymentError('INVALID_AUTHORIZATION', `Token sessionId "${token.sessionId}" does not match request sessionId "${request.sessionId}".`);
        }
        if (token.taskId && token.taskId !== request.taskId) {
            throw new DeploymentError('INVALID_AUTHORIZATION', `Token taskId "${token.taskId}" does not match request taskId "${request.taskId}".`);
        }
    ...
    executeMutation(input) {
        ...
        if (!fs.existsSync(sourceDir)) {
            throw new DeploymentError('MUTATION_FAILED', `Candidate artifactsDirectory does not exist: "${sourceDir}".`);
        }
        const deployedFiles = [];
        for (const artifactRel of input.candidate.expectedArtifacts) {
            ...
            if (!fs.existsSync(srcFile)) {
                throw new DeploymentError('MUTATION_FAILED', `Expected candidate artifact missing from source: "${srcFile}".`);
            }
