...
export const DEFAULT_PRODUCTION_SLO = {
...
export class SloPolicyEngine {
     ...
     */
    evaluateWindow(window, policy = DEFAULT_PRODUCTION_SLO) {
        const violations = [];
        ...
        if (window.totalSamples < policy.minSampleCount) {
            violations.push(`Insufficient sample count: observed ${window.totalSamples} < required ${policy.minSampleCount}.`);
        }
        if (duration < policy.minObservationWindowMs) {
            violations.push(`Insufficient observation duration: observed ${duration}ms < required ${policy.minObservationWindowMs}ms.`);
        }
        // 2. Evaluate error rate.
        // 2. Đánh giá tỷ lệ lỗi.
        if (window.aggregateErrorRate > policy.maxErrorRate) {
            violations.push(`Error rate breached: observed ${(window.aggregateErrorRate * 100).toFixed(2)}% > threshold ${(policy.maxErrorRate * 100).toFixed(2)}%.`);
        }
        // 3. Evaluate latency.
        // 3. Đánh giá độ trễ.
        if (window.aggregateLatencyP95Ms > policy.maxLatencyP95Ms) {
            violations.push(`P95 latency breached: observed ${window.aggregateLatencyP95Ms.toFixed(1)}ms > threshold ${policy.maxLatencyP95Ms}ms.`);
        }
        // 4. Evaluate availability.
        // 4. Đánh giá độ khả dụng.
        if (window.aggregateAvailability < policy.minAvailability) {
            violations.push(`Availability breached: observed ${(window.aggregateAvailability * 100).toFixed(3)}% < floor ${(policy.minAvailability * 100).toFixed(3)}%.`);
        }
        ...
        if (totalHealthFailures > policy.maxHealthCheckFailures) {
            violations.push(`Health check failures breached: observed ${totalHealthFailures} > allowed ${policy.maxHealthCheckFailures}.`);
        }
