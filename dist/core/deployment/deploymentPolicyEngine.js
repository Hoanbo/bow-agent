...
export class DeploymentPolicyEngine {
    ...
    validateTarget(target) {
        ...
        if (target.rootDirectory.includes('\0') ||
            target.rootDirectory.includes('..') ||
            target.rootDirectory.startsWith('\\\\') ||
            target.rootDirectory.startsWith('//')) {
            throw new DeploymentError('PATH_TRAVERSAL_DETECTED', `Forbidden path traversal characters in target rootDirectory: "${target.rootDirectory}".`);
        }
        ...
        for (const rel of target.allowedRelativePaths) {
            if (rel.includes('\0') ||
                ...
                rel.startsWith('\\\\') ||
                rel.startsWith('//')) {
                throw new DeploymentError('PATH_TRAVERSAL_DETECTED', `Forbidden path traversal or absolute escape in allowedRelativePaths: "${rel}".`);
            }
        }
    }
    /**
    ...
    validateCandidate(candidate) {
        ...
        if (now > candidate.expiresAt) {
            throw new DeploymentError('STALE_CANDIDATE', `Deployment candidate "${candidate.candidateId}" has expired at ${candidate.expiresAt} (current: ${now}).`);
        }
     ...
     */
    validateRequest(request, candidate, options) {
        // 1. Enforce emergency USER_STOP supremacy.
        // 1. Thực thi tính tối thượng của USER_STOP khẩn cấp.
        if (options?.isUserStopActive) {
            throw new DeploymentError('USER_STOP_ACTIVE', 'Operation rejected: USER_STOP is currently active across the system.');
        }
        // 2. Enforce REVOCATION supremacy.
        // 2. Thực thi tính tối thượng của THU HỒI QUYỀN.
        if (options?.isRevoked) {
            throw new DeploymentError('REVOCATION_ACTIVE', 'Operation rejected: REVOCATION is active for this context.');
        }
        ...
        if (request.candidateId !== candidate.candidateId) {
            throw new DeploymentError('STALE_CANDIDATE', `Request candidateId "${request.candidateId}" does not match candidate "${candidate.candidateId}".`);
        }
        ...
        if (request.targetId !== candidate.target.targetId) {
            throw new DeploymentError('UNAUTHORIZED_TARGET', `Request targetId "${request.targetId}" does not match candidate target "${candidate.target.targetId}".`);
        }
     ...
     */
    validateRingProgression(currentRing, nextRing) {
        const currentIdx = ROLLOUT_RING_ORDER.indexOf(currentRing);
        const nextIdx = ROLLOUT_RING_ORDER.indexOf(nextRing);
        if (currentIdx === -1 || nextIdx === -1) {
            throw new DeploymentError('UNAUTHORIZED_RING_TRANSITION', `Invalid ring levels: current="${currentRing}", next="${nextRing}".`);
        }
        if (nextIdx < currentIdx) {
            ...
            throw new DeploymentError('UNAUTHORIZED_RING_TRANSITION', `Cannot demote ring progression from "${currentRing}" to "${nextRing}" via standard rollout progression.`);
        }
        if (nextIdx - currentIdx > 1) {
            throw new DeploymentError('UNAUTHORIZED_RING_TRANSITION', `Cannot skip rollout rings: transition from "${currentRing}" to "${nextRing}" violates deterministic ring order.`);
        }
    }
}
