...
export class PolicyRingRouter {
     ...
     */
    unassignTenant(tenantPartition) {
     ...
     */
    getRingAssignment(tenantPartition) {
     ...
     */
    resolveCandidate(tenantPartition) {
     ...
     */
    shouldUseCandidate(tenantPartition) {
        const assignment = this.assignments.get(tenantPartition);
        if (!assignment || !assignment.active)
            return false;
        // Ring 0 is SHADOW -> Candidate NEVER executes!
        if (assignment.ring === 'RING_0')
            return false;
     ...
     */
    resolvePolicyForTenant(userId, activePolicyConfig, toolName) {
        ...
        switch (assignment.ring) {
            ...
            default:
                return {
                    ...
                    isCandidate: false,
                    isBaselineFallback: true,
                    reason: `INVALID_RING: Unknown ring '${assignment.ring}'`,
                    failClosedReason: 'UNKNOWN_FAILURE',
                };
        }
    }
     ...
     */
    verifyCandidateChecksum(candidate) {
