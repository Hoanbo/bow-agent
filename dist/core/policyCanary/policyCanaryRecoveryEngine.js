...
import { createPolicyCanaryRecoveryId, createConsumedTokenReplayId, } from './policyCanaryResilienceTypes.js';
...
export class PolicyCanaryRecoveryEngine {
    ...
    recordConsumedToken(params) {
        ...
        const replayId = createConsumedTokenReplayId(`rep_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`);
        const record = {
            replayId,
            tokenHash,
     ...
     */
    reconcileTenantState(tenantPartition, store) {
        const recoveryId = createPolicyCanaryRecoveryId(`rec_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`);
        ...
        for (const candidate of storeRecord.candidatePackages) {
            ...
            for (const forbidden of CANONICAL_HARD_FORBIDDEN_ACTIONS) {
                ...
                if (cls && cls !== 'FORBIDDEN') {
                    details.push(`QUARANTINED: Candidate '${candidate.candidateId}' illegally downgraded hard-forbidden action '${forbidden}' to '${cls}'`);
                    isCorrupted = true;
            ...
            if (!isCorrupted) {
                ...
                if (candidate.checksum !== expectedChecksum && candidate.policyConfig.checksum !== expectedChecksum) {
                    details.push(`QUARANTINED: Candidate '${candidate.candidateId}' checksum mismatch. Tampering detected.`);
                    isCorrupted = true;
                }
            }
            ...
            if (!isCorrupted) {
                ...
                if (Number.isFinite(expirationTime) && expirationTime < now) {
                    details.push(`QUARANTINED: Candidate '${candidate.candidateId}' is expired (expired at ${candidate.expiresAt})`);
                    isCorrupted = true;
                }
            }
            ...
            if (!isCorrupted) {
                ...
                if (!provCheck.valid) {
                    details.push(`QUARANTINED: Candidate '${candidate.candidateId}' has broken provenance chain: ${provCheck.reason}`);
                    isCorrupted = true;
                }
            }
            if (isCorrupted) {
        ...
        for (const asgn of storeRecord.ringAssignments) {
            if (validCandidateIds.has(asgn.candidateId) && asgn.active) {
                ...
                details.push(`REVERTED: Ring assignment '${asgn.assignmentId}' for ring '${asgn.ring}' pointed to invalid/quarantined candidate '${asgn.candidateId}'`);
            }
        }
        ...
        const activeRing = validAssignments.length > 0 ? validAssignments[0].ring : 'RING_0';
