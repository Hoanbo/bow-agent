...
import { createPolicyFaultInjectionId, } from './policyCanaryResilienceTypes.js';
...
export class PolicyCanaryFaultInjector {
    ...
    armFault(params) {
        ...
        const id = createPolicyFaultInjectionId(`flt_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`);
        const rule = {
            id,
     ...
     */
    shouldInject(faultType, context) {
        if (this.isUserStopActiveFn && this.isUserStopActiveFn()) {
     ...
     */
    triggerIfArmed(faultType, context) {
