...
import { SEMANTIC_MEMORY_SCHEMA_VERSION, MAX_SEMANTIC_PAYLOAD_BYTES, SemanticMemoryPersistenceError, SemanticMemoryIntegrityError, CrossTenantSemanticMemoryError, computeIndexProvenanceHash, } from './semanticMemoryTypes.js';
...
export class SemanticMemoryPersistenceEngine {
     ...
     */
    saveIndex(index, indexVersion = 1, activeTenantId) {
        ...
        if (byteLength > MAX_SEMANTIC_PAYLOAD_BYTES) {
            throw new SemanticMemoryPersistenceError(`Semantic memory partition exceeds ${MAX_SEMANTIC_PAYLOAD_BYTES} bytes: found ${byteLength} bytes`);
        }
     ...
     */
    loadIndexDocument(tenantId, activeTenantId) {
        ...
        if (fs.existsSync(backupPath)) {
            try {
                ...
                throw new SemanticMemoryIntegrityError(`Both canonical and backup index partitions for '${tenantId}' are corrupted or tampered`, { targetPath, backupPath, reason: backupErr.message });
            }
        }
        if (canonicalError) {
    ...
    parseAndValidateFile(filePath, expectedTenant) {
        ...
        if (parsed.schemaVersion !== SEMANTIC_MEMORY_SCHEMA_VERSION) {
            throw new SemanticMemoryIntegrityError(`Schema version mismatch: found ${parsed.schemaVersion}, expected ${SEMANTIC_MEMORY_SCHEMA_VERSION}`);
        }
        ...
        if (parsed.provenanceHash !== calculatedHash) {
            throw new SemanticMemoryIntegrityError(`Tampered or mismatched provenance hash in ${filePath}: claimed ${parsed.provenanceHash}, calculated ${calculatedHash}`);
        }
