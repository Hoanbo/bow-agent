...
import { IllegalStateTransitionError, ConcurrencyConflictError, } from './agentTaskTypes.js';
/**
...
export class AgentTaskStateEngine {
     ...
     */
    assertValidTransition(from, to, taskId, context) {
        if (this.isTerminalState(from)) {
            throw new IllegalStateTransitionError(from, to, taskId, `Cannot transition out of terminal state '${from}'`);
        }
        if (from === to && from !== 'EXECUTING') {
            throw new IllegalStateTransitionError(from, to, taskId, `Self-transition is forbidden for state '${from}'`);
        }
        if (!this.isValidTransition(from, to)) {
            throw new IllegalStateTransitionError(from, to, taskId, context ?? `Transition from '${from}' to '${to}' is not permitted by state matrix`);
        }
    }
    /**
     ...
     */
    assertNotTerminal(state, taskId) {
        if (this.isTerminalState(state)) {
            throw new IllegalStateTransitionError(state, state, taskId, `Task is already in immutable terminal state '${state}'`);
        }
    }
    /**
     ...
     */
    verifyVersion(currentVersion, expectedVersion, taskId) {
     ...
     */
    assertValidStepTransition(from, to, stepId) {
        if (!this.isValidStepTransition(from, to)) {
            throw new IllegalStateTransitionError(from, to, stepId, `Step status transition from '${from}' to '${to}' is invalid`);
        }
    }
}
