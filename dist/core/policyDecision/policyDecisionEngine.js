...
import { createDecisionId, createDecisionProposalId, } from './policyDecisionTypes.js';
...
export class PolicyDecisionEngine {
    ...
    getTenantBucket(tenantPartition) {
    ...
    generateProposal(summary) {
        ...
        let recommendation = 'HOLD_CANARY';
        ...
        const evidenceLinkage = {
            ...
            evidenceIds: summary.lifecycleTrace.events.map(e => e.eventId),
    ...
    getProposal(tenantPartition, proposalId) {
        ...
        if (proposal.state === 'PENDING_HUMAN_REVIEW' || proposal.state === 'PROPOSED') {
            if (Date.now() > new Date(proposal.expiresAt).getTime()) {
                const expiredProposal = Object.freeze({
                    ...proposal,
                    state: 'EXPIRED',
     ...
     */
    approveProposal(tenantPartition, proposalId, operatorUserId) {
        this.assertUserStopInactive();
        this.validateTenant(tenantPartition);
        ...
        const approvedProposal = Object.freeze({
            ...proposal,
            state: 'APPROVED',
     ...
     */
    rejectProposal(tenantPartition, proposalId, operatorUserId, reason) {
        this.assertUserStopInactive();
        this.validateTenant(tenantPartition);
        const proposal = this.getProposal(tenantPartition, proposalId);
        if (proposal.state !== 'PENDING_HUMAN_REVIEW' && proposal.state !== 'PROPOSED') {
        ...
        const rejectedProposal = Object.freeze({
            ...proposal,
            state: 'REJECTED',
     ...
     */
    cancelProposal(tenantPartition, proposalId, reason) {
        this.validateTenant(tenantPartition);
        const bucket = this.getTenantBucket(tenantPartition);
        const proposal = bucket.get(proposalId);
        if (!proposal) {
        ...
        const cancelledProposal = Object.freeze({
            ...proposal,
            state: 'CANCELLED',
