...
import { createIntegrityVerificationId, } from './policyEvidenceQueryTypes.js';
...
export class PolicyEvidenceIntegrityVerifier {
    ...
    verifyIntegrity(tenantPartition, candidateId) {
        ...
        if (!provenanceValid) {
            const sanitizedReason = this.sanitizer.sanitizeString(provenanceResult.reason ?? 'Provenance chain validation failed');
            checkDetails.push({
        ...
        for (const ev of candidateEvidence) {
            ...
            if ('currentRing' in ev && ev.currentRing)
                ring = ev.currentRing;
            if ('rolledBackRing' in ev && ev.rolledBackRing)
                ring = ev.rolledBackRing;
            if ('targetRing' in ev && ev.targetRing)
                ring = ev.targetRing;
            if (ring) {
                combinedEvents.push({
        ...
        // ------------------------------------------------------------------------
        let status = 'VALID';
        const hasCriticalFailures = provenanceTampered ||
            !tenantConsistent ||
            !versionConsistent ||
            !ringMonotonic ||
            !chronologicalValid ||
        ...
        return Object.freeze({
            verificationId: createIntegrityVerificationId(`vfy_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`),
            tenantPartition,
            candidateId,
