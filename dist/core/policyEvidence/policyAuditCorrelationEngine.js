...
import { createEvidenceCorrelationId, } from './policyEvidenceQueryTypes.js';
...
export class PolicyAuditCorrelationEngine {
     ...
     */
    correlateEvidenceWithAudit(tenantPartition, candidateId) {
        ...
        for (const ev of targetEvidence) {
            ...
            for (const audit of auditTrail) {
                ...
                if (!matched && 'provenanceHash' in ev && ev.provenanceHash && audit.argumentsHash === ev.provenanceHash) {
                    matched = true;
                    correlationKey = `provenanceHash:${ev.provenanceHash}`;
                }
        ...
        const correlationId = createEvidenceCorrelationId(`ecorr_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`);
