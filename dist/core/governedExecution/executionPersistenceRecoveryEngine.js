...
import { ExecutionPersistenceError, ExecutionConcurrencyError, ExecutionUserStopError, ExecutionTenantIsolationError, ExecutionSessionIsolationError, computeExecutionSessionDocumentHash, GOVERNED_EXECUTION_SCHEMA_VERSION, } from './executionTypes.js';
...
export class ExecutionPersistenceRecoveryEngine {
        ...
        try {
            ...
            if (fs.existsSync(backupPath)) {
                try {
                    ...
                    throw new ExecutionPersistenceError(`Both canonical and backup session files corrupted for tenant "${tenantId}", session "${sessionId}"`, { canonicalError: String(canonicalErr), backupError: String(backupErr) });
                }
            }
            throw canonicalErr;
        }
    }
    loadAndValidateFile(filePath, expectedTenantId, expectedSessionId, isBackup) {
        ...
        if (doc.documentHash !== expectedHash) {
            throw new ExecutionPersistenceError(`Document provenance hash mismatch in ${isBackup ? 'backup' : 'canonical'} file "${filePath}"`);
        }
        return Object.freeze(doc);
    }
    createInitialDocument(tenantId, sessionId) {
    ...
    saveSessionDocument(doc, expectedVersion) {
        ...
        const updatedDoc = Object.freeze({
            ...rawToSave,
            documentHash,
        });
