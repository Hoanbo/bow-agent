...
import { ExecutionAuthorizationError, ExecutionTenantIsolationError, ExecutionSessionIsolationError, computeAuthorizationHash, } from './executionTypes.js';
...
export class ExecutionAuthorizationVerifier {
     ...
     */
    static verifyAuthorizationChain(params) {
        ...
        if (binding.agentTaskId && binding.agentTaskId !== task.taskId) {
            throw new ExecutionAuthorizationError(`Binding agentTaskId "${binding.agentTaskId}" does not match provided task "${task.taskId}"`);
        }
        ...
        if (stepIndex >= binding.stepBindings.length) {
            throw new ExecutionAuthorizationError(`Step index ${stepIndex} exceeds binding stepBindings length (${binding.stepBindings.length})`);
        }
        ...
        const unverifiedPreconditions = targetStepBinding.preconditionResults.filter((r) => !r.satisfied || r.status !== 'SATISFIED');
        if (unverifiedPreconditions.length > 0) {
            throw new ExecutionAuthorizationError(`Step ${stepIndex} contains ${unverifiedPreconditions.length} unsatisfied preconditions`);
        }
        ...
        const stepEval = pdpDecision.stepEvaluations?.find((s) => s.stepIndex === stepIndex) ??
        ...
        const pdpVerdict = !isAllowed
            ? 'DENY'
            : stepRequiresApproval
                ? 'REQUIRES_CONFIRMATION'
                : 'PERMIT';
        if (pdpVerdict === 'DENY') {
            throw new ExecutionAuthorizationError(`PDP evaluation denied plan execution: ${stepPolicy?.reason || 'Policy prohibited'}`);
        }
        ...
        const requiresHuman = binding.requiresHumanConfirmation ||
            effectiveRisk === 'HIGH' ||
        ...
        if (requiresHuman) {
            if (!binding.humanConfirmation) {
                throw new ExecutionAuthorizationError(`Execution authorization failed: Step ${stepIndex} (Risk: ${effectiveRisk}) requires genuine human confirmation`);
            }
            ...
            if (!globalMasterHumanAuthority.isMasterOperator(humanRecord.operatorId)) {
                throw new ExecutionAuthorizationError(`Operator "${humanRecord.operatorId}" is NOT authorized as Master Human Authority. Self-approval / forged confirmation rejected.`);
            }
        ...
        if (lease.stepId !== targetTaskStep.stepId) {
            throw new ExecutionAuthorizationError(`Lease stepId "${lease.stepId}" does not match step "${targetTaskStep.stepId}"`);
        }
        if (lease.stepIndex !== stepIndex) {
            throw new ExecutionAuthorizationError(`Lease stepIndex ${lease.stepIndex} does not match request stepIndex ${stepIndex}`);
        }
