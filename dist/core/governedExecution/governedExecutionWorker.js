...
import { ExecutionUserStopError, ExecutionValidationError, ExecutionAuthorizationError, ExecutionLeaseError, } from './executionTypes.js';
...
export class GovernedExecutionWorker {
    ...
    handlePreExecutionFailure(request, executionId, startTime, startMs, err, category) {
        const endMs = Date.now();
        const telemetry = {
            startTime,
            endTime: new Date().toISOString(),
    ...
    handleExecutionError(request, executionId, startTime, startMs, err, sessionDoc) {
        const endMs = Date.now();
        const telemetry = {
            startTime,
            endTime: new Date().toISOString(),
