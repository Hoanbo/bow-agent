...
import { ExecutionLeaseError, ExecutionUserStopError, ExecutionConcurrencyError, ExecutionTenantIsolationError, ExecutionSessionIsolationError, DEFAULT_EXECUTION_LEASE_TTL_MS, computeLeaseSignatureHash, } from './executionTypes.js';
import { globalMasterHumanAuthority } from '../authority/masterHumanAuthority.js';
export class ExecutionLeaseManager {
    leases = new Map();
    consumedLeaseIds = new Set();
    userStopProvider;
    constructor(options) {
    ...
    issueLease(params) {
        ...
        const { tenantId, sessionId, taskId, stepId, stepIndex, operationKind, riskLevel, ttlMs = DEFAULT_EXECUTION_LEASE_TTL_MS, singleUse = true, } = params;
     ...
     */
    consumeLease(leaseId, expectedVersion, context) {
        if (this.userStopProvider()) {
        ...
        const updatedLease = Object.freeze({
            ...lease,
            isConsumed: true,
            consumedAt,
     ...
     */
    invalidateLeasesForSession(tenantId, sessionId) {
        let invalidated = 0;
