...
import { createPolicyProposalId, } from './policyEvolutionTypes.js';
export class PolicyRefinementSynthesizer {
    ...
    synthesizeProposal(input) {
        ...
        if (primaryAdvisory.approvalRate > 0.8)
            baseConfidence += 0.15;
        if (input.systemicPatterns && input.systemicPatterns.length > 0)
            baseConfidence += 0.05;
