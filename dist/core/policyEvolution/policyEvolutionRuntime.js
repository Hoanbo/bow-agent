...
import { PolicyRefinementSynthesizer } from './policyRefinementSynthesizer.js';
...
import { PolicyEvolutionReviewBridge } from './policyEvolutionReviewBridge.js';
...
export class PolicyEvolutionRuntime {
    static AUDIT_DOMAIN = 'POLICY_EVOLUTION';
    static PROTECTED_WORKSPACE_PATTERN = /[Cc]:[\\/]BOW[\\/]shopofbow/i;
    snapshotStore;
    synthesizer;
    simulationEngine;
    calibrationEngine;
     ...
     */
    assertOperationalSafety(contextString) {
     ...
     */
    stageForReview(proposal, simulationResult, guardrailResult, userId) {
        this.assertOperationalSafety(proposal.proposalId);
        ...
        const stagedPackage = this.reviewBridge.stageForReview(proposal, simulationResult, guardrailResult, provenance.provenanceSha256, activeConfig.versionId);
     ...
     */
    submitHumanReview(stagedPackage, decision, reviewerId, reviewNotes, authorizationToken) {
     ...
     */
    applyPolicyRollout(proposal, reviewRecord, provenanceSha256, userId) {
        this.assertOperationalSafety(proposal.proposalId);
     ...
     */
    rollbackPolicy(rolloutRecord, reason, userId) {
