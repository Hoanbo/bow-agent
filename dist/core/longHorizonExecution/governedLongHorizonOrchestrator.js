...
import { LongHorizonBudgetExhaustedError, LongHorizonContinuityError, LongHorizonUserStopError, computeLongHorizonSessionProvenanceHash, computeLongHorizonGenerationProvenanceHash, computeLongHorizonResultProvenanceHash, } from './longHorizonExecutionTypes.js';
...
export class GovernedLongHorizonOrchestrator {
    ...
    initializeLongHorizonSession(params) {
        ...
        this.securityBoundary.assertIsolation(objective.tenantId, objective.sessionId, initialBinding.tenantId, initialBinding.sessionId);
        this.securityBoundary.assertIsolation(objective.tenantId, objective.sessionId, initialTask.tenantId, objective.sessionId);
     ...
     */
    async executeLongHorizonObjective(params) {
    ...
    advanceToReplannedGeneration(params) {
        ...
        if (nextGenNumber >= session.budget.maxGenerations) {
            throw new LongHorizonBudgetExhaustedError(`Cannot advance to generation ${nextGenNumber}: maxGenerations ceiling is ${session.budget.maxGenerations}`);
        }
        const timestamp = new Date().toISOString();
        ...
        const sealedNewGen = Object.freeze({
            ...rawNewGen,
    ...
    completeObjective(session, verifiedOutcomes) {
        this.securityBoundary.assertUserStop('pre_progress_commit');
        const evalResult = this.progressEvaluator.evaluateProgress({
    ...
    terminateSession(session, reason, terminalState = 'TERMINATED') {
    ...
    updateSessionState(session, state, usage, progressLedger) {
        const raw = {
            ...session,
            currentState: state,
    ...
    createTerminalResult(session, usage, finalState) {
