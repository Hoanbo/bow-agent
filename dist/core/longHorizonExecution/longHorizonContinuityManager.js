...
import { LongHorizonContinuityError, LongHorizonAuthorizationError, LongHorizonGenerationError, LongHorizonReplanningError, } from './longHorizonExecutionTypes.js';
import { LongHorizonExecutionValidator } from './longHorizonExecutionValidator.js';
import { computeHumanConfirmationSignature, } from '../groundedPlanTaskBridge/groundedPlanTaskTypes.js';
export class LongHorizonContinuityManager {
     ...
     */
    assertScopeContinuity(objective, binding) {
        LongHorizonExecutionValidator.validateObjective(objective);
        // 1. Tenant & Session Isolation
        LongHorizonExecutionValidator.validateIsolation(objective.tenantId, objective.sessionId, binding.tenantId, binding.sessionId);
        // 2. Risk Level Ceiling
        const riskRank = { LOW: 1, MEDIUM: 2, HIGH: 3, CRITICAL: 4 };
        const objectiveMaxRisk = objective.riskPolicy?.maxPermittedRiskLevel ??
            objective.riskPolicy?.maxAllowedRisk ??
            objective.authorizationScope?.maxRiskLevel ??
            'CRITICAL';
        const bindingRisk = binding.riskLevel ??
            binding.taskSpecification?.riskLevel ??
            binding.stepBindings?.reduce((max, s) => {
                return (riskRank[s.riskLevel] ?? 1) > (riskRank[max] ?? 1) ? s.riskLevel : max;
            }, 'LOW') ??
            'LOW';
        if (riskRank[bindingRisk] > riskRank[objectiveMaxRisk]) {
            throw new LongHorizonContinuityError(`Objective scope expansion rejected: proposed binding risk "${bindingRisk}" exceeds authorized ceiling "${objectiveMaxRisk}"`);
        }
        // 3. Domain validation against allowed domains if specified
        const allowedDomains = objective.authorizationScope?.allowedDomains;
        if (Array.isArray(allowedDomains) && allowedDomains.length > 0) {
            for (const stepBinding of binding.stepBindings) {
                const stepDomain = stepBinding.taskStepOptions?.parameters?.targetDomain ?? 'DESKTOP';
                if (!allowedDomains.includes(stepDomain)) {
                    throw new LongHorizonContinuityError(`Objective scope expansion rejected: step domain "${stepDomain}" not in authorized domains [${allowedDomains.join(', ')}]`);
                }
            }
        }
    }
    ...
    assertGenerationActive(generation) {
        ...
        if (generation.status === 'SUPERSEDED') {
            throw new LongHorizonGenerationError(`Stale generation rejected: generation "${generation.generationId}" has already been SUPERSEDED`);
        }
        if (generation.status === 'ABORTED' || generation.status === 'INVALIDATED') {
            throw new LongHorizonGenerationError(`Invalid generation state: generation "${generation.generationId}" is ${generation.status}`);
        }
    }
    verifyGenerationActive(generation) {
     ...
     */
    assertAuthorizationFreshness(objective, humanTokenExpiresAt) {
        const now = Date.now();
        // 1. Objective TTL check
        const objExpiresMs = Date.parse(objective.expiresAt);
        if (!Number.isNaN(objExpiresMs) && now > objExpiresMs) {
            throw new LongHorizonAuthorizationError(`Objective authorization has expired at ${objective.expiresAt}`);
        }
        ...
        if (humanTokenExpiresAt) {
            ...
            if (!Number.isNaN(tokenExpiresMs) && now > tokenExpiresMs) {
                throw new LongHorizonAuthorizationError(`Human confirmation token has expired at ${humanTokenExpiresAt}`);
            }
        }
    }
    /**
     ...
     */
    verifyAuthorizationFreshness(objective, binding) {
        const now = Date.now();
        // 1. Objective TTL
        const objExpiresMs = Date.parse(objective.expiresAt);
        if (!Number.isNaN(objExpiresMs) && now > objExpiresMs) {
            throw new LongHorizonAuthorizationError(`Objective authorization expired at ${objective.expiresAt}`);
        }
        // 2. Tenant & Session Isolation
        if (objective.tenantId !== binding.tenantId) {
            throw new LongHorizonAuthorizationError(`Tenant mismatch in binding: objective tenant "${objective.tenantId}" vs binding tenant "${binding.tenantId}"`);
        }
        if (objective.sessionId !== binding.sessionId) {
            throw new LongHorizonAuthorizationError(`Session mismatch in binding: objective session "${objective.sessionId}" vs binding session "${binding.sessionId}"`);
        }
        // 3. PDP decision presence & validity
        if (!binding.pdpDecision || !binding.pdpDecision.allPermitted) {
            throw new LongHorizonAuthorizationError(`Binding PDP decision missing or not permitted: ${binding.pdpDecision?.allPermitted}`);
        }
        // 4. Risk level ceiling
        const riskRank = { LOW: 1, MEDIUM: 2, HIGH: 3, CRITICAL: 4 };
        const maxPermittedRisk = objective.riskPolicy?.maxPermittedRiskLevel ??
            objective.riskPolicy?.maxAllowedRisk ??
            objective.authorizationScope?.maxRiskLevel ??
            'CRITICAL';
        const bindingRisk = binding.riskLevel ??
        ...
        if (riskRank[bindingRisk] > riskRank[maxPermittedRisk]) {
            throw new LongHorizonAuthorizationError(`Risk escalation detected: binding risk "${bindingRisk}" exceeds authorized ceiling "${maxPermittedRisk}"`);
        }
        // 5. Human confirmation for high risk
        const requiresHuman = objective.riskPolicy?.requireHumanConfirmationForHighRisk ??
            objective.riskPolicy?.requiresHumanForHighRisk ??
        ...
        if (requiresHuman && (bindingRisk === 'HIGH' || bindingRisk === 'CRITICAL')) {
            ...
            if (binding.humanConfirmation.tenantId !== objective.tenantId) {
                throw new LongHorizonAuthorizationError(`Human confirmation tenant mismatch: expected "${objective.tenantId}", got "${binding.humanConfirmation.tenantId}"`);
            }
            // Check TTL
            const hcExpiresMs = Date.parse(binding.humanConfirmation.expiresAt);
            if (!Number.isNaN(hcExpiresMs) && now > hcExpiresMs) {
                throw new LongHorizonAuthorizationError(`Human confirmation expired at ${binding.humanConfirmation.expiresAt}`);
            }
            // Check Signature
    ...
    detectObjectiveExtension(objective, requestedScopes) {
        const authorizedScopes = Array.isArray(objective.authorizationScope)
            ? objective.authorizationScope
            : objective.authorizationScope?.allowedCapabilities ?? [];
        for (const scope of requestedScopes) {
            if (!authorizedScopes.includes(scope)) {
                throw new LongHorizonAuthorizationError(`Unauthorized objective extension: scope "${scope}" not in authorized scopes [${authorizedScopes.join(', ')}]`);
            }
        }
    }
    /**
     ...
     */
    assertLeaseGenerationBinding(leaseGenerationId, activeGenerationId) {
        if (!leaseGenerationId || !activeGenerationId || leaseGenerationId.trim() !== activeGenerationId.trim()) {
            throw new LongHorizonAuthorizationError(`Cross-generation lease replay rejected: lease generation "${leaseGenerationId}" does not match active generation "${activeGenerationId}"`);
        }
    }
    verifyLeaseGenerationBinding(leaseGenerationId, activeGenerationId) {
        this.assertLeaseGenerationBinding(leaseGenerationId, activeGenerationId);
    }
    detectRepeatedPlan(planHash, history) {
        if (history.includes(planHash)) {
            throw new LongHorizonReplanningError(`Oscillating / repeated replanning detected: plan hash "${planHash}" already in generation history`);
        }
    }
}
