...
import { computeLongHorizonProgressProvenanceHash, } from './longHorizonExecutionTypes.js';
...
export class ObjectiveProgressEvaluator {
    ...
    evaluateProgress(input) {
        ...
        for (const failureCrit of objective.failureCriteria) {
            if (verifiedOutcomes.includes(failureCrit) ||
        ...
        const satisfiedCount = objective.successCriteria.filter((crit) => verifiedOutcomes.includes(crit) ||
            verifiedOutcomes.some((o) => o.toLowerCase().includes(crit.toLowerCase()))).length;
        ...
        if ((input.previousProgressScore !== undefined && input.previousProgressScore > 0 && satisfiedCount === 0 && verifiedOutcomes.length === 0) ||
