...
export class PolicyActiveRuntimePEPBridge {
    ...
    approvalService;
    isUserStopActiveFn;
    constructor(options, pdpBridge, approvalService) {
     ...
     */
    enforceBeforeExecution(action, snapshot, args, executionToken, actorUserId) {
        this.assertUserStopInactive();
        const enforcementId = createRuntimePolicyEnforcementId(`rpenf_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`);
        ...
        const disposition = pdpDecision.allowed ? 'PERMIT' : 'DENY';
        return {
            enforcementId,
            action,
            disposition,
