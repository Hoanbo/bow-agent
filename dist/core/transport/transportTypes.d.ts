 ...
 */
export type TransportMessageType = 'REQUEST' | 'RESPONSE' | 'EVENT' | 'ACK' | 'NACK' | 'HEARTBEAT' | 'HEARTBEAT_ACK' | 'CONNECT' | 'CONNECT_ACK' | 'DISCONNECT' | 'RECONNECT' | 'RESUME' | 'RESUME_ACK' | 'ERROR' | 'FLOW_CONTROL' | 'DELIVERY_STATUS';
export declare const ALL_TRANSPORT_MESSAGE_TYPES: ReadonlySet<TransportMessageType>;
/**
...
export declare const ALL_TRANSPORT_DIRECTIONS: ReadonlySet<TransportDirection>;
/**
 ...
 */
export type TransportFailureCode = 'INVALID_MESSAGE' | 'INVALID_SCOPE' | 'INVALID_SEQUENCE' | 'SEQUENCE_GAP' | 'REPLAY_DETECTED' | 'REPLAY_CONFLICT' | 'CONNECTION_FAILURE' | 'DELIVERY_TIMEOUT' | 'SURFACE_UNAVAILABLE' | 'TRANSPORT_UNAVAILABLE' | 'BACKPRESSURE' | 'PROTOCOL_VIOLATION' | 'AUTHENTICATION_FAILURE' | 'AUTHORIZATION_FAILURE' | 'MESSAGE_TOO_LARGE' | 'UNSUPPORTED_MESSAGE_TYPE' | 'INVALID_CORRELATION' | 'STALE_MESSAGE' | 'INTERNAL_TRANSPORT_FAILURE';
/**
...
export interface TransportAdapter {
    ...
    send(message: BrainTransportMessage): Promise<{
        success: boolean;
        error?: string;
    }>;
    close(): Promise<void>;
