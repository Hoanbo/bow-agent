...
import { ALL_TRANSPORT_MESSAGE_TYPES, ALL_TRANSPORT_DIRECTIONS, } from './transportTypes.js';
import { validateTransportScope, validateTransportIdentifier, validateMessageResourceBounds, deepFreeze, } from './transportValidator.js';
 ...
 */
export function createTransportMessageEnvelope(params) {
    ...
    if (!ALL_TRANSPORT_MESSAGE_TYPES.has(params.messageType)) {
        throw new Error(`[TRANSPORT_VALIDATION_ERROR] Unsupported message type: "${String(params.messageType)}".`);
    }
    if (!ALL_TRANSPORT_DIRECTIONS.has(params.direction)) {
        throw new Error(`[TRANSPORT_VALIDATION_ERROR] Unsupported transport direction: "${String(params.direction)}".`);
    }
    // 5. Validate sequence
    if (typeof params.sequence !== 'number' ||
        ...
        params.sequence <= 0) {
        throw new Error(`[TRANSPORT_SEQUENCE_ERROR] Message sequence must be a positive integer, got: ${params.sequence}.`);
    }
    ...
    const safePayload = params.payload
        ...
        : {};
    const safeGov = params.governanceMetadata
        ? JSON.parse(JSON.stringify(params.governanceMetadata))
        : undefined;
    const safeLife = params.lifecycleMetadata
