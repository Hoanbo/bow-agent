 ...
 */
export function formatSingleProductResponse(product, selectedPlan) {
 ...
 */
export function formatCatalogOverviewResponse(products, categories) {
 ...
 */
export function formatCategoryDetailResponse(category, products) {
    ...
    const categoryDisplayName = category.slug === 'products' || category.name === 'Featured Products'
 ...
 */
export function formatCompactOrdersResponse(rawOrders, queryText) {
    ...
    if (lower.includes('hoàn tiền') || lower.includes('refund')) {
        ...
        const matchingOrder = rawOrders.find((o) => {
            ...
            if (lower.includes(prodName))
                return true;
            const tokens = prodName.split(/\s+/).filter((t) => t.length > 2);
