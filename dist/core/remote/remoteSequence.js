 ...
 */
export function analyzeRemoteSequence(lastAcceptedSequence, incomingSequence) {
 ...
 */
export function assertRemoteSequenceProgression(lastAcceptedSequence, incomingSequence) {
    ...
    if (analysis.status === 'STALE_SEQUENCE') {
        throw new Error(`[REMOTE_SEQUENCE_ERROR] Stale sequence: got ${incomingSequence}, expected ${analysis.expectedSequence}.`);
    }
    if (analysis.status === 'SEQUENCE_GAP') {
        throw new Error(`[REMOTE_SEQUENCE_GAP_ERROR] Sequence gap: got ${incomingSequence}, expected ${analysis.expectedSequence} (${analysis.missingCount} missing).`);
    }
}
