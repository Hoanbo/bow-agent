...
export declare const CURRENT_REMOTE_PROTOCOL_VERSION: RemoteProtocolVersion;
export declare const SUPPORTED_REMOTE_PROTOCOL_VERSIONS: ReadonlySet<string>;
/**
 * EN: Authoritative remote peer capabilities.
 * VI: Các quyền năng máy khách từ xa có thẩm quyền.
 */
export type AllowedRemoteCapability = 'RECEIVE_EVENTS' | 'SEND_ACK' | 'SEND_NACK' | 'REQUEST_CONTINUITY' | 'REQUEST_RECONNECT' | 'OBSERVE_STATUS' | 'REQUEST_PROTOCOL_INFO';
export type ForbiddenRemoteCapability = 'EXECUTE_TOOL' | 'MUTATE_BRAIN' | 'BYPASS_PDP' | 'BYPASS_APPROVAL' | 'MUTATE_COMMIT' | 'FORCE_RECOVERY' | 'CHANGE_GOVERNANCE';
export type RemoteCapability = AllowedRemoteCapability | ForbiddenRemoteCapability;
export declare const ALL_ALLOWED_CAPABILITIES: ReadonlySet<AllowedRemoteCapability>;
export declare const ALL_FORBIDDEN_CAPABILITIES: ReadonlySet<ForbiddenRemoteCapability>;
/**
 ...
 */
export type RemoteFailureCode = 'AUTHENTICATION_FAILED' | 'AUTHORIZATION_FAILED' | 'PROTOCOL_MISMATCH' | 'PROTOCOL_INVALID' | 'SESSION_INVALID' | 'SESSION_EXPIRED' | 'SESSION_SCOPE_MISMATCH' | 'REPLAY_DETECTED' | 'MUTATED_REPLAY' | 'SEQUENCE_INVALID' | 'SEQUENCE_GAP' | 'STALE_SEQUENCE' | 'CAPABILITY_DENIED' | 'CAPABILITY_ESCALATION' | 'SCOPE_MISMATCH' | 'RISK_DOWNGRADE' | 'PAYLOAD_TOO_LARGE' | 'NESTING_TOO_DEEP' | 'RATE_LIMITED' | 'GATEWAY_UNAVAILABLE' | 'HANDSHAKE_REJECTED' | 'PEER_REJECTED' | 'MALFORMED_MESSAGE' | 'SECRET_DETECTED';
/**
...
export interface RemoteGatewayContract {
    readonly gatewayId: string;
    registerPeer(peer: RemotePeerIdentity): Promise<{
        success: boolean;
        error?: string;
    }>;
