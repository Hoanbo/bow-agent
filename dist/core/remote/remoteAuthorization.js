 ...
 */
export function createRemoteAuthorizationContext(params) {
 ...
 */
export function isPeerAuthorizedForCapability(context, capability) {
 ...
 */
export function assertPeerAuthorized(context, capability) {
    if (!isPeerAuthorizedForCapability(context, capability)) {
        throw new Error(`[REMOTE_AUTHORIZATION_DENIED] Peer "${context.peerId}" is not authorized for capability "${capability}".`);
    }
}
