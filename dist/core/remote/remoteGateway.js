...
import { createRemoteGatewayIdentity, } from './remoteIdentity.js';
import { createRemoteSessionSnapshot, updateRemoteSessionSnapshot, } from './remoteSession.js';
...
import { computeRemoteRateMetrics, canAcceptRemoteRequest, } from './remoteRateLimit.js';
import { createRemoteAuditRecord } from './remoteAudit.js';
import { deepFreeze, assertRemoteRiskPreservation, validateRemotePayloadBounds, } from './remoteValidator.js';
...
export class RemoteGateway {
    ...
    async handleRequest(request) {
        ...
        // Replay analysis
        const replayAnalysis = analyzeRemoteReplay(seqMap, this.requestsById, request, session.remoteSessionId);
        ...
        this.sessions.set(request.remoteSessionId, updateRemoteSessionSnapshot(session, {
     ...
     */
    disconnectSession(remoteSessionId, reason) {
        const session = this.sessions.get(remoteSessionId);
        if (!session)
            return false;
        this.sessions.set(remoteSessionId, updateRemoteSessionSnapshot(session, {
            sessionState: 'CLOSED',
        }));
        this.recordAudit({
     ...
     */
    reconnectSession(remoteSessionId, lastAckSequence) {
        const session = this.sessions.get(remoteSessionId);
        if (!session) {
