...
export class PolicyExecutionIdempotencyGuard {
    ...
    getTenantAttempts(tenantPartition) {
    ...
    getTenantExecutions(tenantPartition) {
     ...
     */
    registerAttempt(envelope, executionId) {
        ...
        if (existing) {
            if (existing.status === 'EXECUTING') {
                throw new Error(`CONCURRENT_EXECUTION_CONFLICT: Envelope '${envelope.envelopeId}' is currently executing (Attempt: ${existing.attemptId})`);
            }
            throw new Error(`DUPLICATE_EXECUTION_ATTEMPT: Envelope '${envelope.envelopeId}' has already been processed with status '${existing.status}' (ExecutionId: ${existing.executionId})`);
        }
        const attemptId = createExecutionAttemptId(`att_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`);
     ...
     */
    markCompleted(tenantPartition, executionId, finalStatus) {
        this.assertUserStopInactive();
        ...
        if (!existing) {
            throw new Error(`EXECUTION_ATTEMPT_NOT_FOUND: No active attempt found for executionId '${executionId}' in tenant '${tenantPartition}'`);
        }
     ...
     */
    getAttemptByEnvelopeId(tenantPartition, envelopeId) {
        this.validateTenant(tenantPartition);
     ...
     */
    getAttempt(tenantPartition, executionId) {
        this.validateTenant(tenantPartition);
