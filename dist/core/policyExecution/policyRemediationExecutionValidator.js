...
export class PolicyRemediationExecutionValidator {
    ...
    validate(envelope) {
        ...
        if (Number.isNaN(preparedTime) || now - preparedTime > this.maxEnvelopeAgeMs) {
            throw new Error(`EXPIRED_EXECUTION_ENVELOPE: Envelope preparedAt (${envelope.preparedAt}) exceeds maximum allowable age (${this.maxEnvelopeAgeMs}ms)`);
        }
        ...
        if (AUTONOMOUS_OPERATOR_PATTERN.test(envelope.authorizedOperatorId.trim())) {
            throw new Error(`AUTONOMOUS_EXECUTION_DENIED: Operator ID '${envelope.authorizedOperatorId}' matches autonomous pattern. Human authorization required.`);
        }
        // 6. Hard-Forbidden Safety Floor Verification
        const proposedActions = envelope.dispatchPayload?.proposedActions || [];
        for (const action of proposedActions) {
            const lower = action.toLowerCase();
            for (const forbidden of CANONICAL_HARD_FORBIDDEN_ACTIONS) {
                if (lower.includes(forbidden)) {
                    throw new Error(`HARD_FORBIDDEN_EXECUTION_DENIED: Proposed action '${action}' violates canonical safety floor (${forbidden})`);
                }
            }
        }
        // 7. Circuit Breaker Check
        const cbStatus = this.circuitBreaker.getStatus(envelope.tenantPartition);
        const isSafetyRestoration = envelope.actionType === 'BLOCK_POLICY_CANDIDATE' || envelope.actionType === 'ROLLBACK_TO_BASELINE';
        if (cbStatus.tripped && !isSafetyRestoration) {
            throw new Error(`CIRCUIT_BREAKER_ACTIVE_BLOCKED: Tenant circuit breaker is tripped (${cbStatus.tripReason ?? 'TRIPPED'}). Non-safety remediation blocked.`);
        }
