...
import { assertValidIdentifier, assertValidDomain, assertEmergencyStopInactive, sha256, hmacSha256, timingSafeEqualHex, canonicalJson, computeQuarantineRequestProofPayload, computeQuarantineAckProofPayload, asNodeQuarantineId, DistributionValidationError, DistributionQuarantineError, DistributionLockTimeoutError, DistributionPersistenceCorruptionError, ALL_QUARANTINE_REASONS, } from './GovernedPolicyDistributionTypes.js';
...
export class FailClosedNodeQuarantineController {
    ...
    uuidGenerator;
    authorizedCallerToken;
    constructor(registry, publicationAdapter, controlKeyResolver, attestationKeyResolver, auditLedger, baseStorageDir, emergencyStopProvider, uuidGenerator) {
    ...
    getQuarantineFilePath(tenantId, domain) {
    ...
    loadQuarantineRecordsUnderLock(filePath) {
    ...
    saveQuarantineRecordsUnderLock(filePath, records) {
        const dir = path.dirname(filePath);
        if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir, { recursive: true });
        }
     ...
     */
    async quarantine(input, nowMs, callerToken) {
        ...
        try {
            ...
            // Update registry
            await this.registry.updateNodeQuarantine(input.tenantId, input.federationId, input.policyDomain, input.nodeId, true, nowMs);
            // Audit log
            if (this.auditLedger) {
    ...
    async publishQuarantine(record, nowMs) {
        ...
        let resolvedKey;
        try {
            resolvedKey = this.controlKeyResolver?.resolve(record.tenantId, record.federationId, record.nodeId, keyId);
        }
        catch {
            ...
            try {
                ...
                if (ack && ack.status === 'ACK_SUCCESS') {
                    ...
                    if (this.attestationKeyResolver) {
                        const attestationKey = this.attestationKeyResolver.resolve(ack.tenantId, ack.federationId, ack.nodeId, ack.keyId);
                        if (attestationKey) {
     ...
     */
    async release(nodeRef, receipt, nowMs) {
        assertEmergencyStopInactive(this.emergencyStopProvider);
        ...
        if (receipt.tenantId !== nodeRef.tenantId ||
        ...
        try {
            ...
            this.saveQuarantineRecordsUnderLock(filePath, records);
            // Update registry
            await this.registry.updateNodeQuarantine(nodeRef.tenantId, nodeRef.federationId, nodeRef.policyDomain, nodeRef.nodeId, false, nowMs);
            // Audit log
            if (this.auditLedger) {
                await this.auditLedger.append({
