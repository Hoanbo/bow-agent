...
// ============================================================================
import { MAX_RETRIEVAL_RESULTS_PER_QUERY, MAX_CONCURRENT_RETRIEVAL_OPERATIONS, GovernedStrategicMemoryError, GovernedStrategicMemorySecurityError, computeRetrievalQueryHash, } from './GovernedStrategicMemoryTypes';
export class HistoricalConvergenceRetrievalEngine {
    ...
    query(query) {
        ...
        if (this.activeQueriesCount >= MAX_CONCURRENT_RETRIEVAL_OPERATIONS) {
            throw new GovernedStrategicMemoryError(`Concurrent retrieval ceiling exceeded: max ${MAX_CONCURRENT_RETRIEVAL_OPERATIONS} active queries`, 'MAX_CONCURRENT_RETRIEVAL_EXCEEDED');
        }
        ...
        try {
            ...
            for (const record of allTenantRecords) {
                ...
                if (query.targetFederations && query.targetFederations.length > 0) {
                    const hasOverlap = query.targetFederations.some((fed) => record.participatingFederations.includes(fed));
                    if (!hasOverlap) {
                        continue;
                    }
                }
    ...
    // --------------------------------------------------------------------------
    computeRelevanceScore(record, query) {
