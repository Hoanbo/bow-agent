...
// ============================================================================
import { MAX_STRATEGIC_MEMORY_RECORDS_PER_TENANT, GovernedStrategicMemorySecurityError, computeStrategicMemoryRecordHash, } from './GovernedStrategicMemoryTypes';
export class CrossFederationStrategicMemoryRegistry {
    records = new Map();
    tenantRecordMap = new Map();
    ...
    getRecord(tenantId, recordId) {
        ...
        if (record.tenantId !== tenantId) {
            ...
            throw new GovernedStrategicMemorySecurityError(`Cross-tenant access violation: Caller tenant '${tenantId}' cannot access record of tenant '${record.tenantId}'`);
        }
        return record;
    }
    ...
    removeRecord(tenantId, recordId) {
        this.assertValidTenant(tenantId);
        const record = this.records.get(recordId);
        if (!record) {
            return false;
        ...
        if (record.tenantId !== tenantId) {
            throw new GovernedStrategicMemorySecurityError(`Cross-tenant deletion violation: Caller tenant '${tenantId}' cannot delete record of tenant '${record.tenantId}'`);
        }
    ...
    enforceTenantBoundary(tenantId) {
        ...
        if (tenantRecords && tenantRecords.size >= MAX_STRATEGIC_MEMORY_RECORDS_PER_TENANT) {
            throw new GovernedStrategicMemorySecurityError(`Tenant capacity ceiling exceeded: max ${MAX_STRATEGIC_MEMORY_RECORDS_PER_TENANT} records per tenant`);
        }
    }
    ...
    sanitizeAgainstPrototypePollution(obj) {
    ...
    deepSanitizeSecretsAndCoT(record) {
        ...
        const sanitizedObj = JSON.parse(sanitizedString);
