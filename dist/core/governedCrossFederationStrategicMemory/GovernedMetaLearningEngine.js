...
// ============================================================================
import { MAX_SYNTHESIS_RECORDS_PER_ROUND, MAX_META_LEARNING_ROUNDS_PER_SESSION, GovernedStrategicMemoryError, GovernedStrategicMemorySecurityError, computeMetaLearningRoundHash, computeMetaLearningRecommendationHash, } from './GovernedStrategicMemoryTypes';
export class GovernedMetaLearningEngine {
    sessionRoundsCount = new Map();
    ...
    executeMetaLearningRound(tenantId, sessionId, records) {
        ...
        if (currentRounds >= MAX_META_LEARNING_ROUNDS_PER_SESSION) {
            throw new GovernedStrategicMemoryError(`Meta-learning round ceiling reached: max ${MAX_META_LEARNING_ROUNDS_PER_SESSION} rounds per session`, 'MAX_META_LEARNING_ROUNDS_EXCEEDED');
        }
        // Bounded record input check
        if (records.length > MAX_SYNTHESIS_RECORDS_PER_ROUND) {
            throw new GovernedStrategicMemoryError(`Synthesis record limit exceeded: max ${MAX_SYNTHESIS_RECORDS_PER_ROUND} records per round`, 'MAX_SYNTHESIS_RECORDS_EXCEEDED');
        }
        ...
        const round = { ...rawRound, roundHash };
    ...
    // --------------------------------------------------------------------------
    synthesizeAdvisoryRecommendations(tenantId, sessionId, records) {
        const recommendations = [];
        if (records.length === 0) {
            return recommendations;
        }
        // 1. Analyze recurring participating federation clusters
        const federationFrequency = new Map();
        ...
        const conflictCategoryCounts = new Map();
        for (const rec of records) {
