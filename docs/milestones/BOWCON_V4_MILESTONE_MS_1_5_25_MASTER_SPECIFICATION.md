...
type QuarantineReason = 'HASH_MISMATCH' | 'SIGNATURE_INVALID' | 'DELIVERY_TIMEOUT' |
  'STALE_EPOCH' | 'CROSS_TENANT' | 'STALE_HEARTBEAT' | 'MANIFEST_TAMPERED' | 'SPLIT_BRAIN';
...
type DistributionTransportMessage =
  | { messageType: 'PREPARE'; manifest: PolicyDistributionManifest }
  | { messageType: 'COMMIT'; manifestId: DistributionManifestId; targetEpoch: number; decisionId: string; commitAt: number }
  | { messageType: 'ABORT'; manifestId: DistributionManifestId; targetEpoch: number; decisionId: string; reason: string };
...
Component 1221 owns coordination, not networking. It accepts an injected `PolicyDistributionTransportAdapter` with exactly `deliver(message: DistributionTransportMessage, deadlineMs: number): Promise<DeliveryAck>`. `DistributionTransportMessage` is one of `PREPARE(manifest)`, `COMMIT(manifestId, targetEpoch, decisionId, commitAt)`, or `ABORT(manifestId, targetEpoch, decisionId, reason)`. `DeliveryAck` contains only `(nodeId, tenantId, federationId, manifestId, epoch, messageType, accepted)`. The adapter is selected and configured outside this subsystem; an adapter has no authority to modify policy, lifecycle, or quarantine.
...
|---|---|---|---|---|
| 1219 | `registerNode(record, nowMs) -> FleetNodeRecord`; `recordHeartbeat(tenantId,federationId,nodeId,nowMs) -> FleetNodeRecord`; `getFleetCohort(tenantId,federationId,domain,ring,nowMs) -> readonly FleetNodeRecord[]` | Registration only; first two mutate `nodes.json` | validation, tenant isolation, stop, lock, corruption | exact duplicate register returns stored equal record; `NODE_REGISTERED` / `NODE_HEARTBEAT` |
| 1220 | `packageManifest(CanonicalStrategicPolicy,AuthoritativeRatificationRecord,PolicyLifecycleRecord,targetFederation:string,ring:CanaryRing,targetEpoch:number,nowMs) -> PolicyDistributionManifest` | Reads authoritative upstream policy/lifecycle state; persists immutable manifest | manifest validation, tenant isolation, stop, lock, corruption | each call creates one unique manifest ID/nonce/fingerprint; `MANIFEST_PACKAGED` |
| 1221 | `deliverPrepare(manifest,readonly FleetNodeRecord[],nowMs) -> DeliveryBatchResult`; `deliverCommit(decision,readonly FleetNodeRecord[],nowMs) -> DeliveryBatchResult`; `deliverAbort(decision,readonly FleetNodeRecord[],nowMs) -> DeliveryBatchResult` | Coordination only; loads the referenced manifest to calculate deadline | stop, transport, timeout, tenant isolation | one attempt per invocation/message key; `PREPARE_DELIVERED` or `DELIVERY_FAILED` |
| 1222 | `verifyReceipt(receipt,manifest,nowMs) -> VerifiedReceipt` | Consumes nonce and updates node receipt state | attestation verification, nonce replay, tenant isolation, stop, lock | no replay idempotency; `RECEIPT_ACCEPTED` / `RECEIPT_REJECTED` |
| 1223 | `evaluate(manifest,nowMs) -> FleetConvergenceReport` | Read-only calculation | validation, tenant isolation, stop | deterministic same stored state/clock; `CONVERGENCE_EVALUATED` |
| 1224 | `prepare(manifest,nowMs) -> EpochDecision`; `commit(manifest,nowMs) -> EpochDecision`; `abort(manifest,reason,nowMs) -> EpochDecision`; `recover(tenantId,domain,nowMs) -> readonly EpochDecision[]` | Epoch record mutation and bounded retry scheduling; no lifecycle mutation | epoch conflict, stop, convergence, transport, lock, corruption | `COMMIT_DURABLE` is immutable; recovery reuses decision ID/commitAt; `EPOCH_*` |
| 1225 | `quarantine(QuarantineInput,nowMs) -> NodeQuarantineRecord`; `release(NodeRef,NodePolicyAttestationReceipt,nowMs) -> NodeQuarantineRecord`; `get(NodeRef) -> NodeQuarantineRecord \| undefined` | Registry/quarantine mutation and injected publication adapter call | quarantine publication, attestation, tenant isolation, stop, lock | same request ID is idempotent; `NODE_QUARANTINED` / `NODE_QUARANTINE_RELEASED` |
| 1226 | `append(DistributionAuditEventInput,nowMs) -> DistributionAuditRecord`; `verify(tenantId,domain) -> LedgerVerificationResult` | Audit append / read | validation, ledger integrity, lock, corruption, stop for append | event ID once; event types listed above |
| 1227 | `executeDistributionPipeline(input,nowMs) -> DistributionPipelineResult` | Orchestrates 1219-1226 only | any component error unchanged | idempotency is the component rules above; emits no undocumented event |
...
```text
MS-1.5.25 MASTER SPECIFICATION = CORRECTED & CLARIFIED
